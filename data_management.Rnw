...
%%%%%%%%%%%%%%%
\subsection{Reading and Writing Data Using Package \protect\emph{data.table}}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The easiest way to share data between \texttt{R} and \emph{Excel} is through writing and reading \emph{csv()} files.
      \vskip1ex
      The function \texttt{data.table::fread()} reads from \emph{csv} files over \texttt{6} times faster than the function \texttt{read.csv()}!
      \vskip1ex
      The function \texttt{data.table::fwrite()} writes to \emph{csv} files over \texttt{12} times faster than the function \texttt{write.csv()}, and \texttt{278} times faster than function \texttt{cat()}!
    \column{0.5\textwidth}
      \vspace{-1em}
      <<echo=TRUE,eval=FALSE>>=
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The package \emph{data.table} implements an extension of the data frame class, with functions designed for convenience and high performance.
      \vskip1ex
      ...
      \begin{itemize}
        ...
        \item Type \texttt{character} columns are never converted to factors,
      \end{itemize}
    \column{0.5\textwidth}
      \vspace{-1em}
      <<eval=FALSE>>=
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The function \texttt{data.table::data.table()} creates a \emph{data table} object of class \texttt{data.table}, which is derived from the class \texttt{data.frame}.
      \vskip1ex
      The package \emph{data.table} is optimized for high performance (speed), and \emph{data tables} are fast and convenient to work with.
      \vskip1ex
      For example, \emph{data table} columns can be referenced directly by their names (not by strings), and \emph{data table} rows can be referenced without a following comma.
      \vskip1ex
      The function \texttt{data.table::setDT()} coerces data frames to \emph{data tables} by reference (in place without copying data), which is very fast.
      \vskip1ex
      An \texttt{xts} time series can be coerced into a \emph{data table} by first coercing it into a data frame and then into a \emph{data table} using the function \texttt{setDT()}.
      \vskip1ex
      An \texttt{xts} time series can also be coerced directly into a \emph{data table} using the function \texttt{as.data.table()}.
    \column{0.5\textwidth}
      \vspace{-1em}
      <<echo=TRUE,eval=FALSE>>=
# Create a data table
...
%%%%%%%%%%%%%%%
\subsection{Coercing \protect\emph{Data Table} Objects Into Data Frames}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The function \texttt{data.table::setDF()} coerces a \emph{data table} object into a data frame using a \emph{side effect}, without making copies of data.
      \vskip1ex
      \emph{Data table} objects can also be coerced into data frames using the function \texttt{as.data.frame()}, but it's much slower because it makes copies of data.
      \vskip1ex
    \column{0.5\textwidth}
      \vspace{-1em}
      <<echo=TRUE,eval=FALSE>>=
...
it_er <- 0
while (((sum(!down_loaded)) > 0) & (it_er<NROW(down_loaded))) {
