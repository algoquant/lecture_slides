...
for (symboln in ls(sp500env)) {
  zoo::write.zoo(sp500env$symbol, file=paste0(dir_name, symboln, ".csv"))
}  # end for
# Or using lapply()
file_names <- lapply(ls(sp500env), function(symboln) {
  xtsv <- get(symboln, envir=sp500env)
  zoo::write.zoo(xtsv, file=paste0(dir_name, symboln, ".csv"))
  symboln
})  # end lapply
unlist(file_names)
