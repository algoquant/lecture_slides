...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      contains functions designed for high performance data management.
      \vskip1ex
      ...
      \vskip1ex
      The package \emph{data.table} is optimized for high performance (speed), and \emph{data tables} are faster and more convenient to work with than \emph{data frames}.
      \vskip1ex
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      \emph{Data table} columns can be referenced directly by their names (not by strings), and their rows can be referenced without a following comma.
      \vskip1ex
      The function \texttt{data.table::setDT()} coerces \emph{data frames} to \emph{data tables} by reference (in place without copying data), which is very fast.
      \vskip1ex
      An \texttt{xts} time series can be coerced into a \emph{data table} by first coercing it into a \emph{data frame} and then into a \emph{data table} using the function \texttt{setDT()}.
      \vskip1ex
...
data_table <- data.table::fread(file.path(dir_name, "weather_delays14.csv"))
class(data_table)
dim(data_table)
...
%%%%%%%%%%%%%%%
\subsection{Manipulating \protect\emph{Data Table} Objects}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      \emph{Data table} columns can be referenced directly by their names, as if they were variables.
      \vskip1ex
...
data_table <- data.table::fread(file.path(dir_name, "flights14.csv"))
...
setwd("C:/Develop/R/lecture_slides/data")
data_frame <- data.table::fread("weather_delays14.csv")
...
summary(microbenchmark(
  ...
  read_csv=read.csv("weather_delays14.csv"),
  times=10))[, c(1, 4, 5)]  # end microbenchmark summary
