...
dates <- seq(from=as.Date("2013-06-15"), by="day",
              length.out=100)
pricets <- zoo(rnorm(NROW(dates)), order.by=dates)
head(pricets, 3)
# Write zoo series to text file, and then read it back
write.zoo(pricets, file="pricets.txt")
pricezoo <- read.zoo("pricets.txt")  # Read it back
all.equal(pricezoo, pricets)
# Perform the same using write.table() and read.table()
# First coerce pricets into data frame
dframe <- as.data.frame(pricets)
dframe <- cbind(dates, dframe)
# Write pricets to text file using write.table
write.table(dframe, file="pricets.txt",
            row.names=FALSE, col.names=FALSE)
# Read data frame from file
pricezoo <- read.table(file="pricets.txt")
sapply(pricezoo, class)  # A data frame
# Coerce data frame into pricets
pricezoo <- zoo::zoo(
...
write.zoo(pricets, file="pricets.csv",
          sep=",", col.names=TRUE)
pricezoo <- read.zoo(file="pricets.csv",
  header=TRUE, sep=",", drop=FALSE)
all.equal(pricets, drop(pricezoo))
      @
  \end{columns}
\end{block}
...
pricezoo <- read.csv.zoo(file="pricets.csv")
all.equal(pricets, pricezoo)
...
write.table(dframe, file="pricets.csv",
...
pricezoo <- read.csv.zoo(file="pricets.csv", header=FALSE,
  format="%m-%d-%Y %H:%M:%S", tz="America/New_York")
head(pricezoo, 3)
all.equal(pricets, pricezoo)
      @
...
pricets <- rutils::etfenv$VTI
class(pricets); head(pricets)
pricets <- as.data.frame(pricets)
class(pricets); head(pricets)
# Coerce data frame to a data table
data.table::setDT(pricets, keep.rownames=TRUE)
class(pricets); head(pricets)
# Dates are coerced to strings
sapply(pricets, class)
...
sapply(dtable, class)
all.equal(pricets, dtable, check.attributes=FALSE)
      @
  \end{columns}
\end{block}
...
names(pricets) <- as.numeric(sapply(symbolv,
    paste, c("Close", "Volume"), sep="."))
# Save pricets to a comma-separated CSV file
write.zoo(pricets, file="pricets.csv", sep=",")
# Save pricets to a binary .RData file
save(pricets, file="pricets.RData")
      @
  \end{columns}
\end{block}
...
for (symbol in ls(sp500env)) {
  ...
  zoo::index(xtsv) <- as.Date(zoo::index(xtsv))
...
colnames(pricets[, 1])
rutils::get_name(colnames(pricets[, 1]))
# Modify column names
colnames(pricets) <- rutils::get_name(colnames(pricets))
# Or
# colnames(pricets) <- do.call(rbind,
...
\begin{block}{}
  \begin{columns}[T]
    ...
    \column{0.5\textwidth}
      ...
      <<echo=(-(1:1)),eval=FALSE>>=
# Extract Close prices
pricets <- eapply(etfenv, quantmod::Cl)
pricets <- do.call(cbind, pricets)
# Drop ".Close" from colnames
colnames(pricets) <- do.call(rbind, strsplit(colnames(pricets), split="[.]"))[, 1]
...
riskstats <- PerformanceAnalytics::table.Stats(retsp)
...
capmstats <- PerformanceAnalytics::table.CAPM(Ra=retsp[, symbolv], 
...
pricets <- Quandl(code="WIKI/AAPL",
                  type="xts", startd="1990-01-01")
x11(width=14, height=7)
chart_Series(pricets["2016", 1:4], name="AAPL OHLC prices")
# Add trade volume in extra panel
add_TA(pricets["2016", 5])
# Download euro currency rates
pricets <- Quandl(code="BNP/USDEUR",
    startd="2013-01-01",
    endd="2013-12-01", type="xts")
# Download multiple time series
pricets <- Quandl(code=c("NSE/OIL", "WIKI/AAPL"),
    startd="2013-01-01", type="xts")
# Download AAPL gross profits
prof_it <- Quandl("RAYMOND/AAPL_GROSS_PROFIT_Q", type="xts")
chart_Series(prof_it, name="AAPL gross profits")
# Download Hurst time series
pricets <- Quandl(code="PE/AAPL_HURST",
    startd="2013-01-01", type="xts")
chart_Series(pricets["2016/", 1], name="AAPL Hurst")
      @
  \end{columns}
\end{block}
...
pricets <- Quandl(code="CHRIS/CME_ES1",
  type="xts", startd="1990-01-01")
pricets <- pricets[, c("Open", "High", "Low", "Last", "Volume")]
colnames(pricets)[4] <- "Close"
# Plot the prices
x11(width=5, height=4)  # Open x11 for plotting
chart_Series(x=pricets["2008-06/2009-06"],
             TA="add_Vo()", name="S&P500 Futures")
# Plot dygraph
dygraphs::dygraph(pricets["2008-06/2009-06", -5],
  main="S&P500 Futures") %>%
  dyCandlestick()
      @
      \vspace{-1em}
