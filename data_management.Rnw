...
%%%%%%%%%%%%%%%
\subsection{draft: Reading \protect\emph{Data Frames} From Very Large \texttt{.csv} Files}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
...
co_n = file("C:/Develop/lecture_slides/data/spy.csv", "r")
foo <- read.csv(co_n, nrows=1e1, sep=" ")
