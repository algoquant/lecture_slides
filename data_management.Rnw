...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      For example, \emph{ESZ8} is the symbol for the \emph{S\&P500} index \emph{E-mini} futures expiring in December 2018. 
    \column{0.5\textwidth}
    \vspace{-1em}
    \begin{table}[htb]
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The function \texttt{reqAccountUpdates()} returns a list with Interactive Brokers account information (requires an account code). 
    \column{0.5\textwidth}
      \vspace{-1em}
