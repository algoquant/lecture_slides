...
in_dex <- seq(from=as.Date("2013-06-15"), by="day",
              length.out=100)
...
data_frame <- as.data.frame(zoo_series)
data_frame <- cbind(format(in_dex, "%m-%d-%Y %H:%M:%S"),
                    data_frame)
head(data_frame, 3)
...
name_s <- sapply(strsplit(colnames(sp500_env$LOW), split="[.]"),
         function(col_name) col_name[2])
# Or
name_s <- do.call(cbind, strsplit(colnames(sp500_env$LOW),
                                  split="[.]"))[2, ]
