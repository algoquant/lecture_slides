...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      For example, \emph{ESU8} is the symbol for the \emph{S\&P500} index emini futures expiring in September 2018. 
    \column{0.5\textwidth}
    \vspace{-1em}
    \begin{table}[htb]
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      A continuous futures contract is a time series of prices obtained by chaining futures prices.
      \vskip1ex
...
load(file="C:/Develop/data/ib_data/vix_ibrokers.RData")
ls(vix_env)
vix_env$vix_index <- vix_index
ls(vix_env)
save(vix_env, file="C:/Develop/data/ib_data/vix_ibrokers.RData")
# plot OHLC data in x11 window
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The most liquid \emph{VIX} futures are with monthly expiration dates, but weekly \emph{VIX} futures are also traded.
      \vskip1ex
    ...
    \vspace{-1em}
      <<echo=TRUE,eval=FALSE>>=
# Load VIX data from binary file
load(file="C:/Develop/data/ib_data/vix_ibrokers.RData")
...
colnames(curve_s) <- c("Contango 01/11/2018", "Backwardation 02/05/2018")
      @
  \end{columns}
\end{block}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \hskip1em\href{http://www.macroption.com/vix-expiration-calendar/}{VIX Futures Expiration Calendar}\\
...
%%%%%%%%%%%%%%%
\subsection{draft: Futures Prices at Constant Maturity}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      Futures prices at a constant maturity (expiry) date are obtained by interpolating the prices of contracts with neighboring expiration dates.
      \vskip1ex
      ...
      A continuous futures contract is a synthetic
time series of prices, created by splicing together
prices from several futures contracts with
...
# Get symbols starting with VX*, for VIX futures
sym_bols <- ls(vix_env)
sym_bols <- sym_bols[grep("VX*", sym_bols)]
# Get end dates for VIX futures
date_s <- as.Date(sapply(as.list(vix_env)[sym_bols], end))
# Order the end dates
date_s <- sort(date_s)
# Order the end dates
...
# Load the VIX data from binary file
load(file="C:/Develop/data/ib_data/vix_ibrokers.RData")
...
%%%%%%%%%%%%%%%
\subsection{Downloading \protect\emph{VIX} Futures Data from CBOE}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      <<echo=TRUE,eval=FALSE>>=
# read cboe futures expiry calendar
date_s <- read.csv(file="C:/Develop/R/lecture_slides/data/cboe_calendar.csv", 
                   stringsAsFactors=FALSE)
...
# Or download in loop to environment
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The functions \texttt{twsConnect()} and \texttt{ibgConnect()} open a connection to the Interactive Brokers API, either via the \emph{TWS} or the \emph{IBG}.
      \vskip1ex
...
%%%%%%%%%%%%%%%
\subsection{Defining \emph{VIX} Futures Contracts}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      For example, \emph{VXQ8} is the local security name (symbol) for the monthly \emph{VIX} futures contract expiring in August 2018. 
      \vskip1ex
      \emph{VX32Q8} is the local security name (symbol) for the weekly \emph{VIX} futures contract expiring on August 8th 2018. 
      \vskip1ex
      The function \texttt{reqContractDetails()} returns a list with information on the IB instrument. 
      \vskip1ex
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      <<echo=TRUE,eval=FALSE>>=
# define VIX monthly and weekly futures August 2018 contract
con_tract <- twsFuture(symbol=sym_bol, 
  exch="CFE", expiry="201808")
# define VIX monthly futures August 2018 contract
con_tract <- twsFuture(symbol=sym_bol, local="VXQ8", 
  exch="CFE", expiry="201808")
# define VIX weekly futures August 8th 2018 contract
con_tract <- twsFuture(symbol=sym_bol, local="VX32Q8", 
  exch="CFE", expiry="201808")
# download list with instrument information
reqContractDetails(conn=ib_connect, Contract=con_tract)
      @
...
%%%%%%%%%%%%%%%
\subsection{Downloading Historical Data for Expired Futures}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      Historical data for expired futures can be downloaded using \texttt{reqHistoricalData()} with the appropriate \texttt{expiry} date, and the parameter \texttt{include\_expired="1"}.
      \vskip1ex
