...
%%%%%%%%%%%%%%%
\subsection{draft: Downloading Stock Prices Using Package \protect\emph{quantmod}}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      download using while loop
      requires 1 minute pacing
      \vskip1ex
...
\begin{block}{}
  \begin{columns}[T]
    ...
    \column{0.5\textwidth}
      ...
      <<echo=(-(1:1)),eval=FALSE>>=
library(rutils)  # Load package rutils
# Extract and cbind all Close prices for sym_bols
price_s <- do.call(cbind,
  lapply(as.list(etf_env)[sym_bols], quantmod::Cl))
# wippp
...
sp500_env$LO_WES <- sp500_env$LOW[, unique(colnames(sp500_env$LOW))]
rm(LOW, envir=sp500_env)
chart_Series(x=sp500_env$LO_WES["2017-06/"],
  TA="add_Vo()", name="LOWES stock")
# Download "BRK.B" separately with auto.assign=FALSE
BRK_B <- quantmod::getSymbols("BRK-B", auto.assign=FALSE, src="tiingo", adjust=TRUE, from="1990-01-01", api.key="j84ac2b9c5bde2d68e33034f65d838092c6c9f10")
colnames(BRK_B) <- paste("BRK_B", 
  sapply(strsplit(colnames(BRK_B), split="[.]"), 
         function(col_name) col_name[2]), sep=".")
sp500_env$BRK_B <- BRK_B
      @
    \column{0.5\textwidth}
...
%%%%%%%%%%%%%%%
\subsection{draft: Scrubbing Bad Data Time Series}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
...
chart_Series(x=sp500_env$BRK_B["2016/"], TA="add_Vo()",
             name="BRK-B stock")
      @
  \end{columns}
\end{block}
