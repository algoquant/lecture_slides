...
endd <- end(ESM8)
startd <- (endd - 30)
volum <- cbind(Vo(ESU8), Vo(ESM8))[paste0(startd, "/", endd)]
colnames(volum) <- c("ESU8", "ESM8")
colorv <- c("blue", "green")
plot(volum, col=colorv, lwd=3, major.ticks="days",
     format.labels="%b-%d", observation.based=TRUE,
     main="Volumes of ESU8 and ESM8 futures")
legend("topleft", legend=colnames(volum), col=colorv, y.intersp=0.5,
...
volumd <- xts::apply.daily(volum, FUN=sum)
...
retsc <- ifelse(volum > 1e4, retp/sqrt(volum), 0)
...
dates <- format(zoo::index(retp), "%H:%M")
# Aggregate the mean volume
volumagg <- tapply(X=volum, INDEX=dates, FUN=mean)
volumagg <- drop(volumagg)
# Aggregate the mean volatility
volagg <- tapply(X=retp^2, INDEX=dates, FUN=mean)
