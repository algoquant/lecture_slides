...
etflist <- read.csv(file="/Users/jerzy/Develop/lecture_slides/data/etflist.csv")
...
ycnow <- eapply(rates_env, xts::last)
class(ycnow)
...
yc2020 <- eapply(rates_env, function(x) x[date2020])
...
rates <- eapply(rates_env, function(ratev) ratev[dates])
...
pricets <- data.table::fread(file_name)
class(pricets)
# Coerce first column from string to date-time
unlist(sapply(pricets, class))
tail(pricets)
pricets$Index <- as.POSIXct(pricets$Index,
  tz="America/New_York", origin="1970-01-01")
# Coerce prices into xts series
pricets <- data.table::as.xts.data.table(pricets)
class(pricets)
tail(pricets)
colnames(pricets)[1:5] <- c("Open", "High", "Low", "Close", "Volume")
tail(pricets)
      @
    \column{0.5\textwidth}
      \vspace{-1em}
...
chart_Series(x=pricets, TA="add_Vo()",
  name="S&P500 futures")
# Plot dygraph
dygraphs::dygraph(pricets[, 1:4], main="OHLC prices") %>%
  dyCandlestick()
      @
  \end{columns}
\end{block}
  ...
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \begin{itemize}
        ...
        \item three time series called \texttt{SPY}, \texttt{TLT}, and \texttt{VXX}, containing intraday 1-minute \emph{OHLC} price bars for the \emph{SPY}, \emph{TLT}, and \emph{VXX} ETFs.
      \end{itemize}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      <<echo=TRUE,eval=FALSE>>=
pricets <- read.zoo(file="/Users/jerzy/Develop/lecture_slides/data/bid_ask_bounce.csv",
  header=TRUE, sep=",")
pricets <- as.xts(pricets)
x11(width=6, height=4)
par(mar=c(2, 2, 0, 0), oma=c(1, 1, 0, 0))
chart_Series(x=pricets, name="S&P500 Futures Bid-Ask Bounce")
      @
    \column{0.5\textwidth}
...
pricets <- data.table::fread(file_name)
data.table::setDF(pricets)
pricets <- xts::xts(pricets[, 2:6],
  order.by=as.Date(as.POSIXct.numeric(pricets[, 1],
    tz="America/New_York", origin="1970-01-01")))
colnames(pricets) <- c("Open", "High", "Low", "Close", "Volume")
# Plot OHLC data in x11 window
chart_Series(x=pricets, TA="add_Vo()",
  name="S&P500 ESM8 futures")
# Plot dygraph
dygraphs::dygraph(pricets[, 1:4], main="S&P500 ESM8 futures") %>%
  dyCandlestick()
      @
  \end{columns}
\end{block}
...
library(data.table)
pricets <- data.table::fread(file_name)
pricets <- xts::xts(pricets[, paste0("V", 2:6)],
  as.POSIXct.numeric(as.numeric(pricets[, V1]), tz="America/New_York", origin="1970-01-01"))
colnames(pricets) <- c("Open", "High", "Low", "Close", "Volume")
# Plot OHLC data in x11 window
x11()
chart_Series(x=pricets, TA="add_Vo()",
...
# Plot dygraph
library(dygraphs)
dygraphs::dygraph(pricets[, 1:4], main="S&P500 ESM9 futures") %>%
  dyCandlestick()
      @
  \end{columns}
\end{block}
...
pricets <- data.table::fread(file_names[1])
pricets <- xts::xts(pricets[, paste0("V", 2:6)],
  as.POSIXct.numeric(as.numeric(pricets[, V1]), tz="America/New_York", origin="1970-01-01"))
colnames(pricets) <- c("Open", "High", "Low", "Close", "Volume")
# Plot dygraph
...
pricets <- data.table::fread(file_names[2])
pricets <- xts::xts(pricets[, paste0("V", 2:6)],
  as.POSIXct.numeric(as.numeric(pricets[, V1]), tz="America/New_York", origin="1970-01-01"))
colnames(pricets) <- c("Open", "High", "Low", "Close", "Volume")
# Plot dygraph
dygraphs::dygraph(pricets[, 1:4], main="ZN 10yr Treasury futures") %>%
  dyCandlestick()
      @
  \end{columns}
\end{block}
