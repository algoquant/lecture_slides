...
%%%%%%%%%%%%%%%
\subsection{draft: The Bid-ask Bounce in High Frequency Data}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The \emph{bid-ask bounce} produces very high realized volatility and the appearance of mean reversion, that isn't tradeable.
      \vskip1ex
      The \emph{bid-ask bounce} is prominent at very high frequency time scales or in periods of low volatility.
      <<echo=TRUE,eval=FALSE>>=
price_s <- read.zoo(file="C:/Develop/R/lecture_slides/data/bid_ask_bounce.csv",
  header=TRUE, sep=",")
...
chart_Series(x=price_s, name="S&P500 Futures Bid-Ask Bounce")
      @
    \column{0.5\textwidth}
      \includegraphics[width=0.5\paperwidth]{figure/sp500_futures_bounce.png}
  \end{columns}
\end{block}
...
%%%%%%%%%%%%%%%
\subsection{draft: High Frequency \protect\emph{OHLC} Data}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
