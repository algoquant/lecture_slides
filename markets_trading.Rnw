...
library(HighFreq)  # Load package HighFreq
oh_lc <- HighFreq::SPY
# Minutely SPY percentage returns
re_turns <- c(0, HighFreq::diff_vec(log(drop(Cl(oh_lc)))))
re_turns <- re_turns/sd(re_turns)
# Hourly SPY percentage returns
returns_hourly <- c(0, HighFreq::diff_vec(log(drop(Cl(
  xts::to.period(x=oh_lc, period="hours"))))))
returns_hourly <- returns_hourly/sd(returns_hourly)
# Daily SPY percentage returns
returns_daily <- c(0, HighFreq::diff_vec(log(drop(Cl(
  xts::to.period(x=oh_lc, period="days"))))))
returns_daily <- returns_daily/sd(returns_daily)
# Calculate moments
sapply(list(minutely=re_turns, hourly=returns_hourly, daily=returns_daily), 
       function(re_turns) {
         sapply(c(skew=3, kurtosis=4), moments::moment, x=re_turns)
})  # end sapply
      @
    \column{0.5\textwidth}
      \vspace{-1em}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The \emph{partial correlation} between two variables is the correlation after the indirect correlations from other common variables are removed.
      \vskip1ex
      The function \texttt{pacf()} calculates and plots the \emph{partial autocorrelations} of a time series.
      \vskip1ex
      The \emph{Ljung-Box} test, tests if the autocorrelations are \emph{statistically significant}, with the \emph{null hypothesis} that they are equal to zero.
      \vskip1ex
...
pacf(re_turns, lag=10, mgp=c(2, 1, 0), 
...
# Calculate Ljung-Box test statistics
sapply(list(minutely=re_turns, hourly=returns_hourly, daily=returns_daily), 
       function(re_turns) {
         Box.test(re_turns, lag=10, type="Ljung")$statistic
       })  # end sapply
      @
  \end{columns}
\end{block}
...
%%%%%%%%%%%%%%%
\subsection{High Frequency Returns And Trading Volume}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
        As a general empirical rule, the \emph{trading volume} in a given time period is roughly proportional to the \emph{variance} of the returns.
      \vskip1ex
      The \emph{High} minus \emph{Low} price range in a given bar of \emph{OHLC} data is a good proxy for the level of \emph{volatility} (standard deviation).
      \vskip1ex
      It follows that the price range is roughly proportional to the \emph{square root} of the trading volume.
      <<echo=(-(1:2)),eval=FALSE>>=
x11(width=6, height=5)
par(mar=c(3, 3, 2, 1), oma=c(1, 1, 1, 1))
...
da_ta <- cbind(range=log(rang_e), volume=log(vol_ume))
# Remove bad data
da_ta <- na.omit(is.finite(da_ta)*da_ta)
# Regress range vs volume
l_m <- lm(range ~ volume, as.data.frame(da_ta))
coef(l_m)
sampl_e <- sample(NROW(da_ta), 1e3)
plot(range ~ volume, as.data.frame(da_ta[sampl_e, ]), 
     mgp=c(2, 1, 0), 
     main="SPY Price Range vs Trading Volume")
abline(l_m, lwd=3, col="red")
      @
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.5\paperwidth]{figure/hf_range_volume.png}
  \end{columns}
\end{block}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The returns can be divided by the square root of the \emph{trading volume}, to obtain scaled returns over equal trading volumes.
      \vskip1ex
...
returns_list <- list(re_turns=re_turns, returns_scaled=returns_scaled)
# Calculate moments
sapply(returns_list, function(re_turns) {
  sapply(c(skew=3, kurtosis=4), moments::moment, x=re_turns)
})  # end sapply
# Calculate Ljung-Box test statistics
sapply(returns_list,  function(re_turns) {
...
lines(density(returns_scaled, bw=0.4), lwd=3, col="red")
# Add legend
legend("topright", inset=0.05, bty="n",
  leg=c("minutely", "scaled"),
  lwd=6, lty=1, col=c("blue", "red"))
# Plot the partial autocorrelations
x11(width=6, height=5)
par(mar=c(4, 4, 2, 1), oma=c(1, 1, 1, 1))
pacf(returns_scaled, lag=10, mgp=c(2, 1, 0), 
  xlab="lag", ylab="partial autocorrelation", main="")
...
%%%%%%%%%%%%%%%
\subsection{The Bid-ask Bounce in High Frequency Data}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The \emph{bid-ask bounce} produces very high realized volatility and the appearance of mean reversion (negative autocorrelation), that isn't tradeable.
      \vskip1ex
...
%%%%%%%%%%%%%%%
\subsection{draft: Estimating Volatility From \protect\emph{OHLC} Data}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
...
\vspace{-2em}
      \begin{displaymath}
        \sigma^2 = \frac{1}{n} \sum_{i=1}^{n} (0.5 (H_{i}-L_{i})^2 - (2 \log{2}-1)(C_{i}-O_{i})^2)
      \end{displaymath}
\vspace{-1em}
      \vskip1ex
...
chart_Series(var_iance[inter_val],
      ...
      @
    \column{0.5\textwidth}
      \includegraphics[width=0.5\paperwidth]{figure/earl_ttr_runvol-1}
  \end{columns}
\end{block}
...
%%%%%%%%%%%%%%%
\subsection{draft: Daily Volume and Volatility From \protect\emph{OHLC} Data}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The function \texttt{vol\_ohlc()} from package \texttt{HighFreq} calculates volatility estimators over an \emph{OHLC} time series.
      \vskip1ex
...
chart_Series(daily_volume,
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.5\paperwidth]{figure/daily_volume_volat.png}
  \end{columns}
\end{block}
...
%%%%%%%%%%%%%%%
\subsection{draft: Measuring Market Liquidity Using High Frequency Data}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
...
@
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.5\paperwidth]{figure/earl_crash-1}
  \end{columns}
\end{block}
...
\begin{block}{}
  \begin{columns}[T]
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.5\paperwidth]{figure/daily_hurst.png}\\
      \vspace{-1em}
...
abline(h=0.5, col="blue", lwd=2)
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.5\paperwidth]{figure/daily_volat_hurst.png}
  \end{columns}
\end{block}
...
%%%%%%%%%%%%%%%
\subsection{draft: Daily Seasonality of Volume and Volatility}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The function \texttt{season\_ality()} calculates the seasonality aggregations of a statistical estimator over an \emph{OHLC} time series.
      <<daily_season_volume_volat,echo=(-(1:2)),eval=FALSE,fig.width=7,fig.height=8,fig.show='hide'>>=
par(mfrow=c(2,1))  # set plot panels
library(HighFreq)  # Load package HighFreq
...
season_var <- season_ality(vol_ohlc(ohlc=SPY))
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.5\paperwidth]{figure/intraday_season_volume_volat.png}
  \end{columns}
\end{block}
...
%%%%%%%%%%%%%%%
\subsection{draft: Daily Seasonality of Liquidity and Volatility}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      <<daily_season_illiquid_volat,echo=(-(1:2)),eval=FALSE,fig.width=7,fig.height=8,fig.show='hide'>>=
par(mfrow=c(2,1))  # set plot panels
library(HighFreq)  # Load package HighFreq
# Daily seasonality of liquidity
season_illiquid <- season_ality(
  sqrt(HighFreq::run_variance(oh_lc=SPY)/Vo(SPY)))
season_illiquid[!is.finite(season_illiquid)] <- NA
season_illiquid <- na.locf(season_illiquid)
season_illiquid[1] <- coredata(season_illiquid[2])
colnames(season_illiquid) <-
  paste0(sym_bol, ".season_illiquid")
...
season_var <- season_ality(vol_ohlc(ohlc=SPY))
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.5\paperwidth]{figure/intraday_season_illiquid_volat.png}
  \end{columns}
\end{block}
...
season_var <- season_ality(vol_ohlc(ohlc=SPY))
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.5\paperwidth]{figure/intraday_season_hurst_volat.png}
  \end{columns}
\end{block}
...
chart_Series(sk_ew[inter_val],
      ...
      @
    \column{0.5\textwidth}
      \includegraphics[width=0.5\paperwidth]{figure/earl_highfreq_runskew-1}
  \end{columns}
\end{block}
...
chart_Series(daily_skew[inter_val],
      ...
      @
    \column{0.5\textwidth}
      \includegraphics[width=0.5\paperwidth]{figure/earl_dailyskew-1}
  \end{columns}
\end{block}
...
summary(lm(for_mula, data=da_ta["2011-01-01/"]))$coef
      @
    \column{0.5\textwidth}
      \includegraphics[width=0.5\paperwidth]{figure/earl_skew_scatter-1}
...
@
    \column{0.5\textwidth}
      \includegraphics[width=0.5\paperwidth]{figure/earl_skew_strat-1}
      \vspace{-1em}
...
add_TA(vwap_diff[inter_val] < 0, on=-1,
      ...
      @
    \column{0.5\textwidth}
      \includegraphics[width=0.5\paperwidth]{figure/earl_vwap_plot-1}
  \end{columns}
\end{block}
...
summary(lm(for_mula, data=da_ta["2011-01-01/"]))$coef
    ...
    \column{0.5\textwidth}
    \vspace{-1em}
      \includegraphics[width=0.5\paperwidth]{figure/earl_vwap_scatter-1}
...
%%%%%%%%%%%%%%%
\subsection{draft: Momentum Strategy Using \protect\emph{VWAP}}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
...
@
    \column{0.5\textwidth}
      \includegraphics[width=0.5\paperwidth]{figure/earl_vwap_strat-1}
      \vspace{-1em}
