...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The \emph{S\&P500} E-mini futures prices in February and March \texttt{2025} followed a typical market pattern: the trading volumes rose as the prices were dropping.
      \vskip1ex
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      \emph{ESZ4} is the \texttt{December 2024} E-mini futures contract, and \emph{ESH5} is the \texttt{March 2025} E-mini futures contract.
      <<echo=TRUE,eval=FALSE>>=
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      Isolated price spikes can be identified using a \emph{three-point filter} (tri-filter).
      \vskip1ex
      The \emph{z-scores} are equal to the prices minus the mean of the neighboring prices, divided by their differences:
      \begin{displaymath}
      ...
      \vskip1ex
      The \emph{tri-filter} can only remove isolated price spikes, but not consecutive price spikes.
      <<echo=TRUE,eval=FALSE>>=
