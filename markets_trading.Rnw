...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The autocorrelations depend on the length of the time lag and the aggregation interval.
      \vskip1ex
      ...
      \vskip1ex
      The autocorrelations are often negative in the first \textit{10} minutes of the trading session, then they become positive, and after that they become negative again.
      <<echo=TRUE,eval=FALSE>>=
# Calculate the lagged returns
retp <- rutils::diffit(closep, lagg=3)
retl <- rutils::lagit(retp, lagg=3)
# Calculate the instantaneous autocorrelations of returns
covv <- sign(retp*retl)
covagg <- covagg[-(1:3)] ##  Remove first three points biased by overnight returns
...
dygraph(covagg, main="Seasonality of SPY Intraday Autocorrelations") %>%
  dyOptions(colors="blue", strokeWidth=1)
      @
  \end{columns}
\end{block}
