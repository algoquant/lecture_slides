...
%%%%%%%%%%%%%%%
\subsection{Vasicek Process Increments Correlation}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      Under the \emph{Vasicek} process, the rate increments are negatively correlated to the lagged rates.
      <<echo=TRUE,eval=FALSE>>=
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The price of a discount (zero-coupon) bond with maturity $T$ under the \emph{Vasicek} model is given by:
      \begin{displaymath}
        P_T = \exp(A_T - B_T \, r_t)
      \end{displaymath}
      Where $r_t$ is the current short rate, and $A_T$ and $B_T$ are functions of the model parameters $\theta$, $\mu$, and $\sigma$:
      \begin{flalign*}
      ...
      \vskip1ex
      The Vasicek model is a single-factor model because the only source of market risk is the short rate $r_t$. 
      The level of the short rate $r_t$ determines the whole term structure of interest rates.
    \column{0.5\textwidth}
      \vspace{-1em}
...
%%%%%%%%%%%%%%%
\subsection{draft: Multiple Yield Curves Under the Vasicek Model}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The \emph{Vasicek} yield curve formula can be rewritten in terms of the long term yield $y_{inf}$, which is the yield as maturity $T$ goes to infinity:
       \begin{flalign*}
        & y_{inf} = \mu - \frac{\sigma^2}{2 \theta^2} \\
        & B_T = \frac{1 - \exp(-\theta T)}{\theta} \\
        & y_T = y_{inf} + (r_t - y_{inf}) B_T / T + \frac{\sigma^2 B_T^2}{4 \theta T} \\
      \end{flalign*}
      The yield curve gradually converges to the long term yield $y_{inf}$ as the maturity $T$ increases, and it is influenced by the current short rate $r_t$ and the model parameters $\theta$, $\mu$, and $\sigma$.
      \vskip1ex
