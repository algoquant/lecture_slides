...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The \emph{FRED} database provides historical constant maturity Treasury yields: \\
...
colnames(ycurves) <- substr(colnames(ycurves), start=1, stop=4)
    ...
    \column{0.5\textwidth}
    \vspace{-1em}
      \includegraphics[width=0.45\paperwidth]{figure/yield_curve2.png}
      <<echo=TRUE,eval=FALSE>>=
# Plot using matplot()
colorv <- c("blue", "red")
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The \emph{yield curve} changes shape dramatically depending on the economic conditions: in recessions rates drop and the curve flattens, while in expansions rates rise and the curve steepens.
      <<echo=TRUE,eval=FALSE>>=
# Load constant maturity Treasury rates
load(file="/Users/jerzy/Develop/lecture_slides/data/rates_data.RData")
...
%%%%%%%%%%%%%%%
\subsection{Nelson-Siegel Yield Curve Model}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The Nelson-Siegel model of the \emph{yield curve} is given by:
      \begin{displaymath}
        y(\tau) = \beta_0 + \beta_1 (\frac{1 - e^{-\lambda \tau}} {\lambda \tau}) + \beta_2 (\frac{1 - e^{-\lambda \tau}} {\lambda \tau} - e^{-\lambda \tau})
      \end{displaymath}
      Where $y(\tau)$ is the yield at the maturity $\tau$ and $\lambda$ is the time decay factor. 
      \vskip1ex
      The first term $\beta_0$ represents the long-term level of interest rates, the second term $\beta_1$ represents the slope, and the third term $\beta_2$ represents the curvature of the yield curve.
      <<echo=TRUE,eval=FALSE>>=
...
beta0 <- 0.1; beta1 <- -1.5; beta2 <- 3.0
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.45\paperwidth]{figure/nelson_siegel_components.png}
      <<echo=TRUE,eval=FALSE>>=
...
%%%%%%%%%%%%%%%
\subsection{Nelson-Siegel Yield Curve}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      For very short maturities ($\tau \to 0$), the Nelson-Siegel yield is equal to $y(0) = \beta_0 + \beta_1 + \beta_2$.
      \vskip1ex
      For very long maturities ($\tau \to \infty$), the yield is equal to $y(\infty) = \beta_0$
      <<echo=TRUE,eval=FALSE>>=
...
plot(tau, ycurve, type="l", lwd=3, col="blue",
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.45\paperwidth]{figure/nelson_siegel_curve.png}
      <<echo=TRUE,eval=FALSE>>=
...
%%%%%%%%%%%%%%%
\subsection{Calibrating the Nelson-Siegel Model}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The Nelson-Siegel model can be calibrated to the actual yields using the function \texttt{optim()}. 
      \vskip1ex
      <<echo=TRUE,eval=FALSE>>=
...
beta0 <- paroptim[1]; beta1 <- paroptim[2]; beta2 <- paroptim[3]; lambda <- paroptim[4]
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.45\paperwidth]{figure/nelson_siegel_fitted.png}
      <<echo=TRUE,eval=FALSE>>=
# Plot the Nelson-Siegel yield curve
ycurve <- nsfun(tau, beta0, beta1, beta2, lambda)
...
%%%%%%%%%%%%%%%
\subsection{Calibrating the Nelson-Siegel Model Using Differential Evolution}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The Nelson-Siegel model can be calibrated to the actual yields using \emph{global} optimization, to avoid local minima.
      \vskip1ex
      ...
      \vskip1ex
      The function \texttt{DEoptim()} is much slower than \texttt{optim()}, but it is more likely to find the global minimum of the objective function.
      <<echo=TRUE,eval=FALSE>>=
# Extract numeric maturities
ycurve <- ycurves[, "2025"]
tau <- as.numeric(sub("yr", "", names(ycurve)))
...
summary(microbenchmark(
  ...
  times=10))[, c(1, 4, 5)]
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.45\paperwidth]{figure/nelson_siegel_fitted_DEoptim.png}
      <<echo=TRUE,eval=FALSE>>=
# Plot the Nelson-Siegel yield curve
ycurve <- nsfun(tau, beta0, beta1, beta2, lambda)
matplot(tau, cbind(ycurve, ycurves[, "2025"]), 
...
%%%%%%%%%%%%%%%
\subsection{The Vasicek Interest Rate Model}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The \emph{Vasicek} model is a stochastic process for the short-term interest rate $r_t$:
      \begin{displaymath}
        \mathrm{d} r_t = \theta \, (\mu - r_t) + \sigma \, \mathrm{d} B_t
      \end{displaymath}
      Where the parameter $\sigma$ is the volatility, $\mu$ is the equilibrium rate, and $\theta$ is the mean reversion parameter.
      \vskip1ex
      $B_t$ is a \emph{Brownian Motion}, with its increment $\mathrm{d} B_t$ following the normal distribution with the volatility $\sqrt{\mathrm{d}t}$, equal to the square root of the time increment $\mathrm{d}t$.
      \vskip1ex
      The Vasicek model is a single-factor model because it's driven by a single source of randomness, the Brownian motion $B_t$. 
      \vskip1ex
      The \emph{Vasicek} process is path dependent, so it must be simulated using explicit loops, either in \texttt{R} or in \texttt{C++}.
      \vskip1ex
      The \emph{Vasicek} process is also known as the \emph{Ornstein-Uhlenbeck} process.
      \vskip1ex
      The compiled \emph{Rcpp} \texttt{C++} code can be over \texttt{100} times faster than loops in \texttt{R}!
      \vskip1ex
      The disadvantage of the \emph{Vasicek} process is that it can produce negative interest rates, which is not realistic.
      \vskip1ex
...
r0 <- 2.0; muv <- 4.0; nrows <- 200
stepv <- 30/nrows
sigmav <- 0.5*stepv; thetav <- 0.5*stepv
# Initialize the data
...
pricecpp <- HighFreq::sim_ou(prici=r0, priceq=muv, 
...
summary(microbenchmark(
  Rcode={for (i in 2:nrows) {
    rated[i] <- thetav*(muv - ratev[i-1]) + sigmav*innov[i]
    ratev[i] <- ratev[i-1] + rated[i]}},
  Rcpp=HighFreq::sim_ou(prici=r0, priceq=muv, 
    theta=thetav, innov=matrix(sigmav*innov)),
...
%%%%%%%%%%%%%%%
\subsection{The Solution of the Vasicek Process}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The solution of the \emph{Vasicek} process is given by:
      \begin{displaymath}
        r_t = r_0 e^{-\theta t} + \mu (1 - e^{-\theta t}) + \sigma \int_{0}^{t} {e^{\theta (s - t)} \mathrm{d} B_s}
      \end{displaymath}
      \vskip1ex
      The mean and variance are given by:
      \begin{flalign*}
        & \mathbb{E}[r_t] = r_0 e^{-\theta t} + \mu (1 - e^{-\theta t}) \xrightarrow[t \to \infty]{} \mu \\
        & \mathbb{E}[(r_t - \mathbb{E}[r_t])^2] = \frac{\sigma^2}{2 \theta} (1 - e^{-\theta t}) \xrightarrow[t \to \infty]{} \frac{\sigma^2}{2 \theta}
      \end{flalign*}
      The \emph{Vasicek} process is mean reverting to the equilibrium rate $\mu$.
      \vskip1ex
      The \emph{Vasicek} process needs a \emph{warmup period} before it reaches equilibrium.
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.45\paperwidth]{figure/vasicek_rate_process.png}
      \vspace{-1em}
      <<echo=TRUE,eval=FALSE>>=
plot(x=stepv*seq_along(ratev), y=ratev, 
     main="Vasicek Interest Rate Process", 
     type="l", xlab="years", ylab="rate")
...
text(x=5, y=muv+0.15, col="blue", cex=1.0, labels="equilibrium rate")
      @
  \end{columns}
\end{block}
...
%%%%%%%%%%%%%%%
\subsection{Vasicek Rate Increments}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      Under the \emph{Vasicek} process, the rate increments (differences) are negatively correlated to the lagged rates.
      <<echo=TRUE,eval=FALSE>>=
...
text(x=1.0, y=0.3, cex=1.0, col="blue",
 labels=paste("t-value =", round(summary(regmod)$coefficients[2, 3], 2)))
      @
    \column{0.5\textwidth}
      \includegraphics[width=0.45\paperwidth]{figure/vasicek_rate_scatter.png}
  \end{columns}
\end{block}
...
%%%%%%%%%%%%%%%
\subsection{Calibrating the Vasicek Parameters}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The $\theta$ and $\mu$ parameters can be estimated from the linear regression of the rate increments versus the lagged prices.
      \vskip1ex
...
alphasd <- sqrt(sum(resids^2)/(nrows-2)*(1:nrows + mean(retlag)^2/price2))
...
%%%%%%%%%%%%%%%
\subsection{The Yield Curve Under the Vasicek Model}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The price of a discount (zero-coupon) bond is equal to the expected value of the bond's payoff at maturity $T$, under all possible paths of the short rate $r_t$:
      \begin{displaymath}
        P_T = \mathbb{E}[\exp( \sum_{i=0}^T r_t )] = \exp(A_T - B_T \, r_0)
      \end{displaymath}
      Where $r_0$ is the current short rate, and $A_T$ and $B_T$ are functions of the model parameters $\theta$, $\mu$, and $\sigma$:
      \begin{flalign*}
        & B_T = \frac{1 - \exp(-\theta T)}{\theta} \xrightarrow[T \to 0]{} T \\
        & A_T = (\mu - \frac{\sigma^2}{2 \theta^2})(B_T - T) - \frac{\sigma^2 B_T^2}{4 \theta}
      \end{flalign*}
      The discount bond price can be expressed as $P_T = \exp(-y_T T)$, where the yield to maturity $y_T$ is given by:
      \begin{displaymath}
        y_T = \frac{-A_T + B_T \, r_0}{T}
      \end{displaymath}
      With $y_T \xrightarrow[T \to 0]{} r_0$ and $y_T \xrightarrow[T \to \infty]{} \mu - \frac{\sigma^2}{2 \theta^2}$ 
      <<echo=TRUE,eval=FALSE>>=
      @
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.45\paperwidth]{figure/vasicek_rate_curve.png}
      <<echo=TRUE,eval=FALSE>>=
# Calculate the yield curve under the Vasicek model
tauv <- seq(0.1, 30, by=1) # Maturities in years
B <- (1 - exp(-thetav*tauv))/thetav
A <- (muv - sigmav^2/(2*thetav^2))
A <- A*(B - tauv) - (sigmav^2*B^2)/(4*thetav)
r0 <- 2.0 # Current short rate
ycurve <- (-A + B * r0)/tauv # Vasicek yield curve
# Plot the Vasicek yield curve
...
%%%%%%%%%%%%%%%
\subsection{Yield Curves Under the Vasicek Model}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The \emph{Vasicek} yield can be expressed in terms of the long term yield at infinite maturity $y_{inf}$:
       \begin{flalign*}
        & y_T = y_{inf} + (r_0 - y_{inf}) \frac{B_T}{T} + \frac{\sigma^2 B_T^2}{4 \theta T} \\
        & y_{inf} = \mu - \frac{\sigma^2}{2 \theta^2}
      \end{flalign*}
      The yield curve gradually converges to the yield $y_{inf}$ as the maturity $T$ increases: $y_T \xrightarrow[T \to \infty]{} y_{inf}$ 
      \vskip1ex
      The yield $y_{inf}$ is less than the equilibrium rate $\mu$ by the amount $\frac{\sigma^2}{2 \theta^2}$, which is the risk premium for holding long-term bonds.  \\
      The risk premium is higher when the volatility $\sigma$ is higher, and when the mean reversion $\theta$ is weaker.  \\
      Mathematically, it follows from \emph{Ito's lemma}.
      \vskip1ex
      The yield curve is upward sloping when the current short rate $r_0$ is below the long term yield $y_{inf}$, and it is downward sloping when $r_0$ is above $y_{inf}$.
      <<echo=TRUE,eval=FALSE>>=
# Calculate the yield curve using the yield at infinity
yinf <- muv - (sigmav^2 / (2 * thetav^2))
C <- (sigmav^2 * B^2) / (4 * thetav * tauv)
r0 <- 2.0 # Current short rate
ycurve1 <- yinf + (r0 - yinf)*B/tauv + C
r0 <- 5.0 # Current short rate
ycurve2 <- yinf + (r0 - yinf)*B/tauv + C
ycurves <- cbind(ycurve1, ycurve2)
      @
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.45\paperwidth]{figure/vasicek_rate_curves.png}
      <<echo=TRUE,eval=FALSE>>=
# Plot the yield curves
matplot(tauv, ycurves, type="l", lwd=3, 
        main="Yield Curves Under the Vasicek Model",
        col=c("green", "blue"), lty=1,
        xlab="Maturity (years)", ylab="Yield (%)")
legend("topright", legend=c("r0 = 2.0", "r0 = 5.0"),
       bty="n", col=c("green", "blue"), lty=1, lwd=6, inset=0.0, cex=1.0)
abline(h=yinf, col="red", lwd=2)
text(x=5, y=yinf-0.2, cex=1.0, col="blue", 
     labels=paste("yield at infinity =", round(yinf, 2)))
      @
  \end{columns}
\end{block}
...
%%%%%%%%%%%%%%%
\subsection{Advanced Interest Rate Models}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The advantage of the \emph{Vasicek} interest rate model is that it has a closed-form solution for the yield curve, which makes it easy to calibrate.
      \vskip1ex
      The \emph{Vasicek} model has many disadvantages: it can produce negative interest rates, it can't fit the yield curve exactly, it can't fit interest rate options prices, and it allows for arbitrage opportunities between bonds of different maturities.
      \vskip1ex
      The \emph{Hull-White} model is an extension of the \emph{Vasicek} model that allows for time-varying parameters $\theta_t, \mu_t, \sigma_t$, so it can better fit the yield curve and interest rate options prices:
      \begin{displaymath}
        \mathrm{d} r_t = \theta_t \, (\mu_t - r_t) + \sigma_t \, \mathrm{d} B_t
      \end{displaymath}
      The \emph{Black-Karasinski} model simulates the logarithm of the short rate, to ensure that the short rate $r_t$ is always positive:
      \begin{displaymath}
        \mathrm{d} \log(r_t) = \theta_t \, (\mu_t - \log(r_t)) + \sigma_t \, \mathrm{d} B_t
      \end{displaymath}
      This implies a log-normal distribution of rates, not a normal distribution.
    \column{0.5\textwidth}
      The disadvantage of the advanced interest rate models is that they have to be calibrated using numerical methods, which can be slow and numerically unstable.
  \end{columns}
\end{block}
...
%%%%%%%%%%%%%%%
\subsection{Covariance Matrix of the Yield Curve}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
\fbox{\parbox{\columnwidth-1.5em}{
      \emph{Principal Component Analysis} (\emph{PCA}) can be used for \emph{dimension reduction}, to explain a large number of correlated time series as linear combinations of a smaller number of principal component time series.
}}
      \vskip1ex
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      \emph{Principal component} loadings are the weights of portfolios which have mutually orthogonal rate increments.
      \vskip1ex
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The \emph{principal component} time series have mutually orthogonal rate increments.
      \vskip1ex
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The original time series can be calculated \emph{approximately} from just the first few \emph{principal components}, which demonstrates how \emph{PCA} can be used for \emph{dimension reduction}.
      \vskip1ex
      A popular rule of thumb is to use the \emph{principal components} with the largest variances, which sum up to \texttt{80\%} of the total variance of rate increments.
      \vskip1ex
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The matrix of \emph{principal component} rate increments $\mathbf{pc}$ is equal to the matrix $\mathbf{w}$ of the PCA weights (loadings) times the matrix of the return vectors $\mathbf{r}$:
      \begin{displaymath}
...
%%%%%%%%%%%%%%%
\subsection{\protect\emph{S\&P500} Futures Prices}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The \emph{S\&P500} E-mini futures prices in February and March \texttt{2026} followed a typical market pattern: the trading volumes rose as the prices were dropping.
      \vskip1ex
...
quantmod::chart_Series(x=ESH5["2025-01/"], TA="add_Vo()",
  theme=themev, name="March 2025 E-mini Futures Prices")
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.45\paperwidth]{figure/sp500_ESH5_futures.png}
      % \includegraphics[width=0.45\paperwidth]{figure/sp500_ESH5_futures_volumes.png}
      <<echo=TRUE,eval=FALSE>>=
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      \emph{ESZ4} is the \texttt{December 2024} E-mini futures contract, and \emph{ESH5} is the \texttt{March 2026} E-mini futures contract.
      <<echo=TRUE,eval=FALSE>>=
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The \emph{CBOE} (Chicago Board Options Exchange) calculates the \emph{VIX} index and provides free daily historical data for the
      \href{https://www.cboe.com/tradable_products/vix/vix_historical_data}{VIX and other volatility indices}. 
      \vskip1ex
      The \emph{CBOE} also provides free daily historical 
      \href{https://www.cboe.com/us/futures/market_statistics/historical_data/}{VIX futures prices}. 
      \vskip1ex
      The \emph{FRED} database also provides historical data for the
      \href{https://fred.stlouisfed.org/series/VIXCLS}{VIX index}
      and for other 
      \href{https://fred.stlouisfed.org/categories/32425}{volatility indices}. 
      <<echo=TRUE,eval=FALSE>>=
# Download VIX index data from CBOE
vixohlc <- data.table::fread("https://cdn.cboe.com/api/global/us_indices/daily_prices/VIX_History.csv", skip=1)
class(vixohlc)
dim(vixohlc)
tail(vixohlc)
sapply(vixohlc, class)
vixohlc <- xts(vixohlc[, -1],
  order.by=as.Date(vixohlc$V1, format="%m/%d/%Y"))
colnames(vixohlc) <- c("Open", "High", "Low", "Close")
# Save the VIX data to binary file
load(file="/Users/jerzy/Develop/data/vix/vix_data.RData")
# vixenv <- new.env()
ls(vixenv)
vixenv$vixohlc <- vixohlc
save(vixenv, file="/Users/jerzy/Develop/data/vix/vix_data.RData")
    ...
    \column{0.5\textwidth}
      ...
      <<echo=TRUE,eval=FALSE>>=
# Plot dygraph candlestick of VIX OHLC data
dygraphs::dygraph(vixohlc["2019/"], main="VIX Index") %>%
  dyCandlestick()
# Plot VIX OHLC data in x11 window
chart_Series(x=vixohlc["2018"], name="VIX Index")
      @
  \end{columns}
\end{block}
...
chart_Series(x=Cl(vixenv$vixohlc["2007/"]),
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      <<echo=TRUE,eval=FALSE>>=
chart_Series(x=Cl(vixenv$vixohlc["2017/2018"]),
             theme=themev, name="VIX Index")
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      As of July \texttt{2020}, the \emph{S\&P500} stock index constituents data was removed from 
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      <<echo=TRUE,eval=FALSE>>=
# Add date-time index to the TAQ data.table
taqts <- cbind(index=datev, taq)
# Round the time index to seconds
taqts[, index := as.POSIXct(round.POSIXt(index, "secs"))]
# Aggregate to OHLC by seconds
ohlc <- taqts[, .(open=first(price), high=max(price), low=min(price), close=last(price), volume=sum(volume)), by=index]
# Round the time index to minutes
taqts[, index := as.POSIXct(round.POSIXt(index, "mins"))]
...
ohlc <- taqts[, .(open=first(price), high=max(price), low=min(price), close=last(price), volume=sum(volume)), by=index]
    ...
    \column{0.5\textwidth}
      ...
      <<echo=TRUE,eval=FALSE>>=
# Coerce the data.table of OHLC prices to xts time series
ohlc <- xts::xts(ohlc[, -"index"], ohlc$index)
# Plot dygraph of the OHLC prices
dygraphs::dygraph(ohlc[, -5], main="XLK TAQ Aggregated to OHLC") %>%
  dyCandlestick()
      @
  \end{columns}
\end{block}
...
\begin{block}{}
  \begin{columns}[T]
    ...
    \column{0.5\textwidth}
      ...
      <<echo=TRUE,eval=FALSE>>=
# Coerce the TAQ data.table to xts time series
...
%%%%%%%%%%%%%%%
\subsection{Scrubbing Isolated Spikes In Stock Prices}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      Isolated price spikes in historical (offline) prices can be identified using a \emph{three-point filter} (tri-filter).
      \vskip1ex
      The centered \emph{z-score} is equal to the price minus the mean of the neighboring prices, divided by their differences:
      \begin{displaymath}
        z_i = \frac{p_i - 0.5 (p_{i-1} + p_{i+1})}{p_{i-1} - p_{i+1}}
      \end{displaymath}
      If the absolute value of the \emph{z-score} exceeds the \emph{threshold value} then it's classified as \emph{bad data}, and it can be removed or replaced with the previous price.
      \vskip1ex
      The \emph{tri-filter} can remove isolated price spikes, but not consecutive price spikes.
      <<echo=TRUE,eval=FALSE>>=
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The price spikes in prices can be identified more effectively using a \emph{Hampel filter}.
      \vskip1ex
      The \emph{z-score} is equal to the price minus the median of the neighboring prices, divided by the median absolute deviation (\emph{MAD}) of prices:
      \begin{displaymath}
        z_i = \frac{p_i - \operatorname{median}(\mathbf{p})}{\operatorname{MAD}}
      \end{displaymath}
      The \emph{median} and \emph{MAD} values are more robust to price spikes than the mean and standard deviation, so the \emph{Hampel filter} can remove both isolated and consecutive price spikes.
      <<echo=TRUE,eval=FALSE>>=
...
range(zscores); mad(zscores)
madz <- mad(zscores[abs(zscores) > 0])
hist(zscores, breaks=1000, xlim=c(-4*madz, 4*madz))
# Identify good prices with small z-scores
threshv <- 3*madz
isgood <- (abs(zscores) < threshv)
# Calculate the number of bad prices
...
%%%%%%%%%%%%%%%
\subsection{Confusion Matrix of the Classification Model}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The area under the \emph{ROC curve} (AUC) measures the classification ability of the binary classifier.
      \vskip1ex
      The \emph{false positive} and \emph{true positive} rates are higher for lower discrimination threshold values, and lower for higher threshold values.
      \vskip1ex
      <<echo=TRUE,eval=FALSE>>=
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      Bad stock prices can be scrubbed (replaced) with the previous good price using the function \texttt{zoo::na.locf()} from the package \texttt{zoo}.
      \vskip1ex
      ...
      \vskip1ex
      Centered \emph{z-scores} are calculated using past and future prices, so they can only be used to scrub historical (offline) prices. 
      \vskip1ex
...
%%%%%%%%%%%%%%%
\subsection{draft: Plotting High Frequency \protect\emph{OHLC} Data}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
...
datav <- datav["2026-01-06 09:30:00/2026-01-06 16:00:00", 1:4]
...
%%%%%%%%%%%%%%%
\subsection{Hourly Trading Volume vs Volatility}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
...
%%%%%%%%%%%%%%%
\subsection{Intraday Seasonality of Autocorrelations}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The autocorrelations are typically higher at the start and end of the trading sessions.
      Besides that, they are negative for most of the trading session.
      \vskip1ex
      The distribution of autocorrelations depends on the stock and the level of volatility.
      \vskip1ex
      The autocorrelations are often more positive at the start of the trading session and decline during the day.
      <<echo=TRUE,eval=FALSE>>=
# Calculate the lagged returns
retp <- rutils::diffit(closep)
retl <- rutils::lagit(retp, lagg=1)
# Calculate the instantaneous autocorrelation of returns
covv <- (retp*retl)/retp^2
covv[is.na(covv)] <- 0
sum(is.na(covv))
mean(covv) # Autocorrelations slightly negative
madv <- mad(covv)
plot(density(covv), xlim=c(-10*madv, 10*madv), 
     xlab="Autocorrelation", ylab="frequency",
     main="Density of Autocorrelations of SPY Returns")
# Aggregate the autocorrelations by time of day
dated <- format(zoo::index(closep), "%H:%M")
covagg <- tapply(X=covv, INDEX=dated, FUN=mean)
# Coerce to xts
datev <- as.POSIXct(paste(Sys.Date(), names(covagg)))
covagg <- xts::xts(covagg, datev)
covagg <- covagg[-(1:2)] ##  Remove first two points biased by overnight returns
      @
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.45\paperwidth]{figure/hf_intraday_autocorrelations.png}
      <<echo=TRUE,eval=FALSE>>=
# Plot the seasonality of autocorrelations
dygraph(covagg, main="Seasonality of Intraday Autocorrelations") %>%
  dyOptions(colors="blue", strokeWidth=1)
# Aggregate covagg by 10 minutes
endd <- rutils::calc_endpoints(covagg, interval=10)
covagg10 <- sapply(seq_along(endd)[-1], function(it) {
  mean(covagg[(endd[it-1]+1):endd[it]])
})  ##  end sapply
datev <- index(covagg)[endd]
covagg10 <- xts::xts(covagg10, datev)
# Dygraph of autocorrelations aggregated by 10 minutes
dygraph(covagg10, main="Seasonality of Intraday Autocorrelations") %>%
  dyOptions(colors="blue", strokeWidth=2, drawPoints=TRUE, pointSize=4) %>%
  dyLimit(0, color="red", strokePattern="dashed")
      @
  \end{columns}
\end{block}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      For example, \emph{VXV8} is the local security name (symboln) for the monthly \emph{VIX} futures contract expiring on February 17th, 2018.
      \vskip1ex
      \emph{VX40V8} is the local security name (symboln) for the weekly \emph{VIX} futures contract expiring on February 3rd, 2018.
      \vskip1ex
      The function \texttt{reqContractDetails()} returns a list with information on the IB instrument.
      \vskip1ex
