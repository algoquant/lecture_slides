...
dataf <-
  ...
  \end{columns}
      <<echo=FALSE,eval=TRUE,results='asis'>>=
print(xtable(dataf), comment=FALSE, size="scriptsize", include.rownames=FALSE)
      @
\end{block}
...
symbol <- load(file.path(dirout, paste0(symbol, ".RData")))
...
symbol <- load(file.path(dirin, paste0(symbol, "/2014.05.02.", symbol, ".RData")))
...
HighFreq::save_scrub_agg(symbol,
               dirin=dirin,
               dirout=dirout,
               period="minutes")
      @
...
colnames(skewn) <- paste0(symbol, ".skew")
...
# vwap plot
vwapf <- vwapv(xtsv=SPY, look_back=70)
vwaps <- vwapv(xtsv=SPY, look_back=225)
vwapd <- vwapf - vwaps
colnames(vwapd) <- paste0(symbol, ".vwap")
interval <- "2010-05-05/2010-05-07"
invisible(chart_Series(x=Cl(SPY[interval]), name=paste(symbol, "plus VWAP")))
