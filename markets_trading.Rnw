...
cor_mat <- cor_mat[or_der, or_der]
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.4\paperwidth]{figure/cor_int_rates.png}
      <<echo=TRUE,eval=FALSE>>=
...
n_secs <- as.numeric(last(in_dex)) - as.numeric(first(in_dex))
NROW(ta_q)/(6.5*3600)
# Select TAQ data columns
ta_q <- ta_q[, .(price=PRICE, volume=SIZE)]
# Add date-time index
ta_q <- cbind(index=in_dex, ta_q)
      @
  \end{columns}
\end{block}
...
re_turns <- rutils::diff_it(clos_e)/rutils::lag_it(clos_e, pad_zeros=FALSE)
...
ret_scaled <- ifelse(vol_ume>0, re_turns/vol_ume^0.5, 0)
ret_scaled <- ret_scaled/sd(ret_scaled)
# Calculate moments of scaled returns
n_rows <- NROW(re_turns)
sapply(list(re_turns=re_turns, ret_scaled=ret_scaled), 
...
pacf_scaled <- pacf(as.numeric(ret_scaled), lag=10,
     xlab="lag", ylab="partial autocorrelation", main="")
