...
# Test function
vectorv <- runif(1e3)
all.equal(mean(vectorv), mymean_comp(vectorv), my_mean(vectorv))
...
densityv <- densityv$y
# Importance sample weights
lambda <- (-1.5*sd(datav))  # Tilt parameter
data_tilt <- datav + lambda  # Tilt the data
shiftv <- (-findInterval(lambda, datav))
# data_tilt <- rutils::lagit(datav, shiftv, pad_zeros=FALSE)  # Tilt the data
density_tilt <- rutils::lagit(densityv, shiftv, pad_zeros=FALSE)  # Tilt the density
# Interpolate the densityv vector into returns
density_base <- approx(datav, densityv, xout=data_tilt)
all.equal(density_base$x, data_tilt)
density_base$y <- na.locf(density_base$y, fromLast=TRUE)
weightv <- density_base$y/density_tilt
weightv <- weightv/sum(weightv)
# Calculate time series of VTI returns
# Estimate of VaR (quantile - extreme loss)
confl <- 0.02; cutoff <- confl*nrows
# datav <- sort(datav)
va_r <- datav[cutoff]  # Naive Monte Carlo value
sd(datav)*qnorm(confl)  # Assuming normal distribution
# Cumulative probabilities using importance sample
cum_prob <- cumsum(weightv)
# Quantile from importance sample
va_r <- data_tilt[findInterval(confl, cum_prob)]
# Estimate of CVaR (expected loss)
cva_r <- sum((datav < va_r)*datav)/sum((datav < va_r))
...
boot_data <- sapply(1:nboot, function(x) {
  ...
  weightv <- exp(-lambda*data_tilt + lambda^2/2)
  cum_prob <- cumsum(weightv)/nrows
  im_port <- data_tilt[nrows -
    findInterval(1-confl, cum_prob)]
  c(naive_mc=naivemc, importance=im_port)
}) # end sapply
apply(boot_data, MARGIN=1,
  function(x) c(mean=mean(x), sd=sd(x)))
...
object <- function(pa_r, datav) {
  sum(2*log(pa_r[2]) +
    ((datav - pa_r[1])/pa_r[2])^2)
}  # end object
...
optim_fit$sd
# Plot histogram
histo_gram <- hist(datav, plot=FALSE)
...
object <- function(pa_r, datav) {
  ...
  dnorm((datav-pa_r[2])/pa_r[3]) +
  (1-pa_r[1])/pa_r[5]*dnorm((datav-pa_r[4])/pa_r[5])
