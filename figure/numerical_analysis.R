...
##   boot_sample <- r_norm[sample.int(n_rows,
##                               replace=TRUE)]
...
##   function(x, r_norm, n_rows) {
##   boot_sample <- r_norm[sample.int(n_rows, replace=TRUE)]
##   c(mean=mean(boot_sample), median=median(boot_sample))
##   }, r_norm=r_norm, n_rows=n_rows)  # end parLapply
...
## set.seed(1121)  # reset random number generator
## boot_strap <- parLapply(clus_ter, 1:n_boot,
##   function(x, re_turns, n_rows, bar_rier) {
##     boot_sample <- re_turns[sample.int(n_rows, replace=TRUE)]
##     # calculate prices from percentage returns
##     boot_sample <- exp(cumsum(boot_sample))
##     # calculate payout
##     sum(boot_sample > bar_rier) > 0
##   }, re_turns=re_turns, n_rows=n_rows, bar_rier=bar_rier)  # end parLapply
## stopCluster(clus_ter)  # stop R processes over cluster under Windows
