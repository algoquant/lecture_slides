...
Dirk is a $Top Answerer$ for $R$ questions on Stackoverflow, the author of the \verb|Rcpp| package, and the CRAN Finance View\\
...
Romain is an $R$ Enthusiast and \verb|Rcpp| Hero\\
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.3\textwidth}
      ...
      \vskip1ex
      The function \verb|cat()| concatenates strings and writes to output,\\
      \vskip1ex
      The function \verb|grep()| searches for matches of a pattern in a string,\\
      \vskip1ex
      The function \verb|gsub()| replaces all matches of a pattern in a string,\\
    \column{0.7\textwidth}
      \vspace{-1em}
      <<>>=
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.3\textwidth}
      ...
      Examples of $R$ \verb|types| are \verb|integer|, \verb|double|, \verb|character|, \verb|list|, etc.\\
      \vskip1ex
      The \verb|class| of an  $R$ object is an attribute used for function dispatching in S3 classes,\\
      Examples of $R$ \verb|classes| are \verb|character|, \verb|numeric|, \verb|matrix|, \verb|factor|, \verb|data.frame|, etc.\\
    \column{0.7\textwidth}
      \vspace{-1em}
      <<>>=
v.var <- c(pi=pi, euler=exp(1), gamma=-digamma(1))
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.3\textwidth}
      ...
      \vskip1ex
      The function \verb|factor()| converts a character vector into a factor,\\
      \vskip1ex
      Factors have two attributes: \verb|class| ("\verb|factor|") and \verb|levels| (the allowed values),\\
    \column{0.7\textwidth}
      \vspace{-1em}
      <<tidy=FALSE>>=
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The function \verb|ts()| creates time-series objects,\\
      \vskip1ex
      \verb|stats| is a base package and contains functions for statistical calculations and random number generation,\\
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The \verb|EuStockMarkets| dataset contains daily closing prices of european stock indices,\\
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The function \verb|hist()| calculates and plots a histogram,\\
      The parameter \verb|breaks| is the number of cells of the histogram,\\
      \vskip1ex
      The function \verb|density()| calculates a kernel estimate of the probability density,\\
      \vskip1ex
      The function \verb|lines()| draws a line through specified points,\\
