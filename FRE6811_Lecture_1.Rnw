...
\begin{columns}[T]
  \column{0.7\textwidth}
    Download $R$ Interpreter from \texttt{CRAN} (Comprehensive $R$ Archive Network)\\
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.3\textwidth}
      The \emph{type} of an  $R$ object represents how it's stored in memory,\\
      Examples of $R$ \emph{types} are \texttt{integer}, \texttt{double}, \texttt{character}, \texttt{list}, etc.\\
      \vskip1ex
      The \emph{class} of an  $R$ object is an attribute used for function dispatching in S3 classes,\\
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.3\textwidth}
      ...
      \vskip1ex
      Factors have two attributes: \emph{class} ("\texttt{factor}") and \texttt{levels} (the allowed values),\\
    \column{0.7\textwidth}
      \vspace{-1em}
      <<tidy=FALSE>>=
