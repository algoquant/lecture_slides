...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      \emph{Principal components} are linear combinations of the $k$ return vectors $\mathbf{r}_i$:
      \begin{displaymath}
        \mathbf{pc}_j = \sum_{i=1}^k {w_{ij} \, \mathbf{r}_i}
      \end{displaymath}
      Where $\mathbf{w}_j$ is a vector of weights (loadings) of the \emph{principal component} $j$, with $\mathbf{w}_j^T \mathbf{w}_j = 1$,
      \vskip1ex
...
summary(microbenchmark(
  ...
  times=10))[, c(1, 4, 5)]
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.45\paperwidth]{figure/pca_stock_load1.png}\\
      \vspace{-3em}
      <<echo=TRUE,eval=FALSE>>=
...
optim_run <- optim(par=weight_s,
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.45\paperwidth]{figure/pca_stock_load2.png}\\
      \vspace{-3em}
      <<echo=TRUE,eval=FALSE>>=
...
%%%%%%%%%%%%%%%
\subsection{Principal Component Analysis of Stock Returns}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
...
for (or_der in 1:n_weights) {
  ...
  title(paste0("PC", or_der), line=-2.0, 
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.5\paperwidth]{figure/pca_stock_loadings.png}
  \end{columns}
\end{block}
...
for (or_der in 1:n_weights) {
  ...
  title(paste0("PC", or_der), line=-2.0)
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.5\paperwidth]{figure/pca_stock_series.png}
  \end{columns}
\end{block}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The original time series data can be calculated exactly from the time series of all the \emph{principal components}, by inverting the loadings matrix, 
      \vskip1ex
      ...
      \vskip1ex
      The \emph{Kaiser-Guttman} rule retains only \emph{principal components} with \emph{eigenvalues} greater than \texttt{1}, 
      \vskip1ex
      Another rule is to retain the \emph{principal components} with the largest \emph{eigenvalues} which sum up to \texttt{80\%} of total variance explained,
      \vskip1ex
...
for (sym_bol in sym_bols) {
  ...
  legend(x="topright", 
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.5\paperwidth]{figure/pca_stock_series_solved.png}
  \end{columns}
\end{block}
