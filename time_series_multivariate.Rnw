...
retspca <- xts(retp %*% pcad$rotation[, 1:ncomps], order.by=datev)
round(cov(retspca), 3)
...
retspca <- xts(retp %*% pcad$rotation, order.by=datev)
pcacum <- cumsum(retspca)
...
retspca <- xts(pcad$x/100, order.by=zoo::index(retp))
      @
\vspace{-1em}
...
cormat[1:3, 1:3]
table.CAPM(Ra=retspca[, 1:3], Rb=retp$VTI, scale=252)
      @
    \column{0.5\textwidth}
  \end{columns}
\end{block}
