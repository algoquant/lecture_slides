...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The \emph{Security Market Line} (SML) represents the linear relationship between \emph{systematic} risk ($\beta$) and return, 
      \vskip1ex
...
text(x=etf_betas[label_names], 
    ...
    \vspace{-2em}
      ...
      If an asset's returns lie on the \emph{SML}, then all its returns are \emph{systematic}, and its $\alpha$ is equal to zero,
      \vskip1ex
      Assets above the \emph{SML} earn positive \emph{idiosyncratic} returns (positive $\alpha$), and vice versa,
  \end{columns}
\end{block}
