...
sapply(symbolv, function(symbol) {
  c(series=tseries::adf.test(retscum[, symbol])$p.value,
    resid=tseries::adf.test(retscum[, symbol] - solved[, symbol])$p.value)
})  # end sapply
# Plot the residuals
for (symbol in symbolv) {
  plot.zoo(retscum[, symbol] - solved[, symbol], 
...
retspca <- xts(pcad$x/100, order.by=zoo::index(retsp))
      @
\vspace{-1em}
      <<pca_retscum,echo=(-1),eval=FALSE,fig.width=7,fig.height=6,fig.show='hide'>>=
library(PerformanceAnalytics)  # Load package "PerformanceAnalytics"
chart.CumReturns(
