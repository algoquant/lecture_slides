...
plot(XLP ~ VTI, data=rutils::etf_env$returns,
...
returns <- na.omit(rutils::etf_env$returns[, c("XLP", "VTI")])
...
prices <- rutils::etf_env$prices$VTI[dates]
...
prices <- na.omit(rutils::etf_env$prices[, c("XLB", "XLE")])
cor(rutils::diff_it(log(prices)))
...
pcarets <- xts(pcad$x/100, 
    order.by=index(returns))
      @
\vspace{-1em}
      <<pca_cumrets,echo=(-1),eval=FALSE,fig.width=7,fig.height=6,fig.show='hide'>>=
library(PerformanceAnalytics)  # Load package "PerformanceAnalytics"
chart.CumReturns(
...
factor_pca <- fitSfm(rutils::etf_env$returns, k=3)
