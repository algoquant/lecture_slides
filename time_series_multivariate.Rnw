...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The time series of stock returns $r_i - r_f$ in excess of the risk-free rate $r_f$, can be decomposed into \emph{systematic} returns $\beta (r_m - r_f)$ (where $r_m - r_f$ is the time series of the excess market returns) plus \emph{idiosyncratic} returns $\alpha + \varepsilon_i$ (which are uncorrelated to the market returns):
      \begin{displaymath}
      ...
      \vskip1ex
      The regression residuals $\varepsilon_i$ are uncorrelated to the market returns, and can be reduced through portfolio diversification.
      \vskip1ex
      The stock $\beta$ is equal to the ratio of the covariance of the stock returns with the market returns $\sigma_{i,m}$ divided by the variance of the market returns $\sigma^2_m$: 
      \begin{displaymath}
        \beta = \frac{\sigma_{i,m}}{\sigma^2_m} = \rho_{i,m} \frac{\sigma_i}{\sigma_m}
      \end{displaymath}
      Where $\rho_{i,m}$ is the correlation of the stock returns with the market returns, and $\sigma_i$ and $\sigma_m$ are the standard deviations of the stock and market returns, respectively.
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.4\paperwidth]{figure/capm_reg.png}
      \vspace{-1em}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The \emph{CAPM} model states that the expected return of the stock $n$: $\mathbbm{E}[R_n]$ should be proportional to its beta $\beta_n$ times the expected excess return of the market $\mathbbm{E}[R_m] - r_f$:
      \begin{displaymath}
        \mathbbm{E}[R_n] = r_f + \beta_n (\mathbbm{E}[R_m] - r_f)
      \end{displaymath}
\fbox{\parbox{\columnwidth-1.5em}{
      The \emph{CAPM} model states that stocks should only produce a \emph{systematic} return proportional to their \emph{beta} $\beta$ values.  
      If a stock has a higher beta then it should also produce higher returns.
}}
      \vskip1ex
      The \emph{CAPM} model is not the same as the regression model used to determine the \emph{alpha} $\alpha$ and \emph{beta} $\beta$ values of stock returns.
      \vskip1ex
      In reality, stocks also have a non-zero mean idiosyncratic return \emph{alpha} $\alpha_n$, in addition to the \emph{systematic} return proportional to their $\beta_n$:
      \begin{displaymath}
        \mathbbm{E}[R_n] = r_f + \alpha_n + \beta_n (\mathbbm{E}[R_m] - r_f)
      \end{displaymath}
    \column{0.5\textwidth}
      \vspace{-1em}
      <<echo=TRUE,eval=FALSE>>=
...
%%%%%%%%%%%%%%%
\subsection{The \protect\emph{Capital Market Line} For a Single Stock}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The \emph{CAPM} equation can be expressed in terms of the standard deviation $\sigma_n$ of the stock returns:
      \begin{displaymath}
        \mathbbm{E}[R_n] = r_f + \alpha_n + \rho_{n,m} \frac{\sigma_n}{\sigma_m} (\mathbbm{E}[R_m] - r_f)
      \end{displaymath}
\fbox{\parbox{\columnwidth-1.5em}{
      This is the \emph{Capital Market Line} (CML), which states that the expected return of the portfolio of the stock and the risk-free asset is proportional to its standard deviation $\sigma_n$.
}}
      \vskip1ex
      ...
      \vskip1ex
      The sum of the portfolio weights is equal to \emph{one}:
      \begin{displaymath}
        w_s + w_{rf} = 1
      \end{displaymath}
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.45\paperwidth]{figure/capm_cml_stock.png}
      <<echo=TRUE,eval=FALSE>>=
# Plot the CML
...
%%%%%%%%%%%%%%%
\subsection{The Capital Market Line And The Security Market Line}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      First, as a regression of the time series of stock returns versus the time series of market returns, to determine the alpha $\alpha$ and beta $\beta$ coefficients:
      \begin{displaymath}
        r_i = r_f + \alpha + \beta (r_m - r_f) + \varepsilon_i
      \end{displaymath}
      Second, as the Security Market Line (SML), which relates the expected stock returns to their betas $\beta_n$:
      \begin{displaymath}
        \mathbbm{E}[R_n] = r_f + \alpha_n + \beta_n (\mathbbm{E}[R_m] - r_f)
      \end{displaymath}
      Third, as the Capital Market Line (CML), which relates the return of the portfolio of a stock and the risk-free asset to its standard deviation $\sigma_n$.
      \begin{displaymath}
        \mathbbm{E}[R_n] = r_f + \alpha_n + \rho_{n,m} \frac{\sigma_n}{\sigma_m} (\mathbbm{E}[R_m] - r_f)
      \end{displaymath}
      \vskip1ex
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.28\paperwidth]{figure/capm_reg.png}
      \vspace{-2em}
      \includegraphics[width=0.28\paperwidth]{figure/capm_sml_etf.png}
      \vspace{-2em}
      \includegraphics[width=0.28\paperwidth]{figure/capm_cml_stock.png}
  \end{columns}
\end{block}
...
%%%%%%%%%%%%%%%
\subsection{The Treynor and Information Ratios}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The function \texttt{roll\_reg()} from package \emph{HighFreq} performs trailing regressions in \texttt{C++}, so it's much faster than equivalent \texttt{R} code.
      <<echo=TRUE,eval=FALSE>>=
...
@
    \column{0.5\textwidth}
      \includegraphics[width=0.45\paperwidth]{figure/beta_trailing.png}
      \vspace{-1em}
      <<echo=TRUE,eval=FALSE>>=
...
%%%%%%%%%%%%%%%
\subsection{Trailing EMA Stock Beta}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The trailing beta $\beta$ of a stock with returns $r_t$ with respect to a stock index with returns $R_t$ can be updated using these recursive EMA formulas with the decay factor $\lambda$:
      \begin{flalign*}
...
betac <- covarv[, 1]/covarv[, 3]
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.45\paperwidth]{figure/beta_ema.png}
      <<echo=TRUE,eval=FALSE>>=
# dygraph plot of trailing XLP beta and VTI prices
