...
# 5. (5pts) create a zoo object called "zoo_series" from Garch columns "dm" and "cd" and from "date_index", using zoo(),
zoo_series <- zoo(Garch[, c("dm", "cd")], order.by=date_index)


# 6. (5pts) change the column names of zoo_series to "DMark" and "CAD"
colnames(zoo_series) <- c("DMark", "CAD")


# 7. (10pts) plot zoo_series with two "y" axes, and add a legend containing the column names of zoo_series,
...
# 8. (10pts) calculate the running mean of the "DMark" column of zoo_series, using the function rollmean(), 
...
# 9. (5pts) merge the "DMark" column of zoo_series with zoo_mean, using the function merge(), and copy it back to zoo_series,
zoo_series <- merge(zoo_series[, "DMark"], zoo_mean)


# 10. (5pts) change the column names of zoo_series to "DMark" and "mean_DMark"
colnames(zoo_series) <- c("DMark", "mean_DMark")


# 11. (5pts) calculate the number of rows with bad data in zoo_series, and then scrub the bad data using the function na.locf(),
sum(!complete.cases(zoo_series))
zoo_series <- na.locf(zoo_series, fromLast=TRUE)


# 12. (5pts) plot both columns of zoo_series, with the second series in red, and add a legend
# plot the first column
...
# 13. (5pts) save zoo_series to a comma-delimited text file called "Garch.csv", using write.zoo()
