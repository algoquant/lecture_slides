...
# retp <- retp[, numna==0]
# # Select 12 months of recent returns and from the past
# nrows <- NROW(retp)
# retpast <- retp[(nrows-12*25-1):(nrows-1), ]
# retrecent <- retp[(nrows-12*25):nrows, ]
# # Calculate the past covariance matrices and their inverse
# covpast <- cov(retpast, use="pairwise.complete.obs")
# covpastinv <- MASS::ginv(covpast)
# # Check the generalized inverse property
# all.equal(covpast %*% covpastinv %*% covpast, covpast)
# all.equal(covpastinv %*% covpast %*% covpastinv, covpastinv)
# # Calculate the recent covariance matrix and its inverse
# covrecent <- cov(retrecent, use="pairwise.complete.obs")
# covrecinv <- MASS::ginv(covrecent)

# # Calculate the recursive inverse of covrecent in a loop
# iterv <- numeric(5)
# uppert <- upper.tri(covrecent)
# # Unit matrix for initial inverse
# unitm <- diag(ncol(covrecent))
# for (x in 1:5) {
#   # Calculate the recursive inverse of covrecent
#   # covpastinv <- covpastinv %*% (2*unitm - covrecent %*% covpastinv)
#   # covpastinv <- (2*covpastinv - covpastinv %*% covrecent %*% covpastinv)
#   # covpastinv <- covpastinv - covpastinv %*% (covpastinv - covpastinv %*% covrecent %*% covpastinv)
#   covprod <- covrecent %*% covpastinv
#   covpastinv <- covpastinv %*% (3*unitm - covprod %*% (3*unitm - covprod))
#   # Calculate the sum of the off-diagonal elements
#   iterv[x] <- sum(covpastinv %*% covrecent %*% covpastinv - covrecinv)
# }   end for
# # Plot the iterations
...
# # Calculate the portfolio weights with regularization
# lambdav <- seq(1.905e-7, 1.90525e-7, length=5)
# lambdav <- seq(0, 1e-6, length=7)
# ncores <- detectCores() - 1
# weightm <- mclapply(lambdav, function(lambdaf) {
#   optiml <- optim(par=weightv, fn=objfun,
#             retp=retx,
#             lambdaf=lambdaf, alphaf=0,
#             method="L-BFGS-B",
#             control=list(factr=1e5),
#             upper=c(10, 10, 10),
#             lower=c(-10, -10, -10))
#   optiml$par
# })  end mclapply
# weightm <- do.call(cbind, weightm)
# colnames(weightm) <- paste0("lambda=", lambdav)
# colorv <- colorRampPalette(c("blue", "red"))(NCOL(weightm))
# plot(sort(weightm[, 1]), xlab=NA, ylab="weight", col=colorv[1], lwd=2)
# for (i in 2:NCOL(weightm)) {
#   points(sort(weightm[, i]), col=colorv[i], lwd=2)
# }  end for
# apply(weightm, 2, function(x) sum(abs(x)))
# 
# 
# lambdav <- seq(0, 1e-6, length=7)
# ncores <- detectCores() - 1
...
#                        lambdaf=lambdaf,
#                        alphaf=0,
#                        control=list(trace=FALSE, itermax=100))
#   optiml$optim$bestmem
# })  end mclapply
# weightm <- do.call(cbind, weightm)
# colnames(weightm) <- paste0("lambda=", lambdav)
...
# 
# 
# foo <- apply(weightm, 2, function(x) x/sqrt(sum(x^2)))
# plot(sort(foo[, 1]), xlab=NA, ylab="weight", col=colorv[1], lwd=2)
# for (i in 2:NCOL(foo)) {
#   points(sort(foo[, i]), col=colorv[i], lwd=2)
# }  end for
# apply(foo, 2, function(x) sum(abs(x)))
# 
# 
