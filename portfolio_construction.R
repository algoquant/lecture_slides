...
sub_portfolios <- sapply(1:n_portf, function(x) {
  price_s <- price_s[, sample.int(n=n_cols, size=n_stocks, replace=FALSE)]
  rowSums(price_s)/n_stocks
})  # end sapply
sub_portfolios <- xts::xts(sub_portfolios, order.by=date_s)
...
end_p <- xts::endpoints(date_s, on="years")
# end_p are the row numbers for the end of year dates:
date_s[end_p]
end_p <- xts::endpoints(date_s, on="years")
above_index <- (sub_portfolios[end_p, ] > in_dex[end_p])
above_index <- rowSums(above_index)/n_portf
above_index <- xts::xts(above_index, order.by=date_s[end_p])
