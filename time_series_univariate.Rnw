...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The mean squared error (\emph{MSE}) of a forecasting model is the average of the squared forecasting residuals $\varepsilon_i$, equal to the differences between the actual values $r_t$ minus the \emph{forecasts} $f_t$: $\varepsilon_i = r_t - f_t$:
      \begin{displaymath}
        \operatorname{MSE} = \frac{1}{n} \sum_{i=1}^n (r_t - f_t)^2
      \end{displaymath}
      The forecasts have a lower volatility than the empirical returns because the convolution procedure averages out the noise.
      <<echo=TRUE,eval=FALSE>>=
# Calculate the volatilities
sd(retp); sd(fcast)
# Mean squared error
...
legend(x="top", legend=c("returns", "forecasts"),
       y.intersp=0.3, col=c("blue", "red"), lty=1, lwd=6,
