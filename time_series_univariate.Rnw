...
%%%%%%%%%%%%%%%
\subsection{Package \texttt{quantmod}}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.4\textwidth}
      The package \texttt{quantmod} contains \texttt{functions}:
      \begin{itemize}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.4\textwidth}
      ...
      \vskip1ex
      It then assigns the data to those objects, without returning them as a function value,
      \vskip1ex
...
is.OHLC(data_env$VTI)
# plot OHLC candlechart with volume
chartSeries(data_env$VTI, 
            name="VTI", 
