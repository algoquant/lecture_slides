...
\begin{block}{}
  \begin{columns}[T]
    ...
    \column{0.5\textwidth}
      ...
      \vskip1ex
      Common practice is to perform the \emph{ADF} test with a small number of lags, and if the residuals are autocorrelated, then to increase the number of lags until the correlations are no longer significant.
      <<echo=TRUE,eval=FALSE>>=
n_rows <- 1e3
