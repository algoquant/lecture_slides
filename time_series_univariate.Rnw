...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      <<chart_Series,echo=(-(1:10)),eval=FALSE,fig.width=7,fig.height=6,fig.show='hide'>>=
library(quantmod)
library(TTR)
load(file="C:/Develop/data/etf_data.Rdata")
da_ta <- env_data$VTI
...
abline(h=min(v_wap), col='red')
    ...
    \column{0.5\textwidth}
    \vspace{-1em}
      \includegraphics[width=0.5\paperwidth,valign=t]{figure/chart_Series-4}\\
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      A plot object is a list containing all the information needed to render a plot, 
      \vskip1ex
      \texttt{chart\_Series()} returns the plot object \emph{invisibly}, 
      \vskip1ex
