...
pricets <- eapply(sp500env, function(ohlc) {
...
volat <- sapply(pricets, function(closep) {
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The relationship between stock trading volumes and Hurst exponents is not significant.
      \vskip1ex
...
retsp <- lapply(pricets, rutils::diffit)
...
pricets <- lapply(mget(symbolv, rutils::etfenv), function(x) {
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      <<ar_pacf,echo=(-(1:3)),eval=FALSE,fig.height=8,fig.show='hide'>>=
