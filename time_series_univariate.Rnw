...
title(main="MSFT OHLC Prices")
      @
    \column{0.5\textwidth}
      \includegraphics[width=0.4\paperwidth]{figure/tseries_OHLC-1}
  \end{columns}
\end{block}
...
title(main="MSFT Open and Close Prices", line=-1)
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.4\paperwidth]{figure/tseries_seqplot-1}\\
      \vspace{-3em}
      The function \texttt{zoo::coredata()} extracts the underlying numeric data from a complex data object.
  \end{columns}
\end{block}
...
chartSeries(etfenv$VTI["2014-11"],
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.4\paperwidth]{figure/chartSeries_basic-1}\\
...
reChart(subset="2009-02", theme=chartTheme("white"))
    ...
    \column{0.5\textwidth}
    \vspace{-1em}
      \includegraphics[width=0.4\paperwidth]{figure/chartSeries_black.png}\\
      \includegraphics[width=0.4\paperwidth]{figure/chartSeries_white.png}
  \end{columns}
\end{block}
...
chartSeries(etfenv$VTI["2014"],
    ...
    \column{0.5\textwidth}
    \vspace{-1em}
      \includegraphics[width=0.4\paperwidth]{figure/chartSeries_TA-1}
  \end{columns}
\end{block}
...
addTA(ta=(VTI_close-vwapv), col='red')
    ...
    \column{0.5\textwidth}
    \vspace{-1em}
      \includegraphics[width=0.4\paperwidth]{figure/chartSeries_addTA-3}\\
...
addLines(h=min(vwapv), col='red')
    ...
    \column{0.5\textwidth}
    \vspace{-1em}
      \includegraphics[width=0.4\paperwidth]{figure/chartSeries_addTA_shade-7}
  \end{columns}
\end{block}
...
abline(h=min(vwapv), col='red')
    ...
    \column{0.5\textwidth}
    \vspace{-1em}
      \includegraphics[width=0.4\paperwidth]{figure/chart_Series_shaded}\\
...
plot(chobj)
      @
    \column{0.5\textwidth}
      \includegraphics[width=0.4\paperwidth]{figure/chart_Series_custom_axis-1}
  \end{columns}
\end{block}
...
add_TA(XLF_vwap["2009-02/2009-04"], lwd=2, on=1, col='blue')
    ...
    \column{0.5\textwidth}
    \vspace{-1em}
      \includegraphics[width=0.4\paperwidth]{figure/chart_Series_panels.png}
  \end{columns}
\end{block}
...
legend("top", legend=colnamev, cex=1.0, bg="white",
      ...
      @
    \column{0.5\textwidth}
      \includegraphics[width=0.4\paperwidth, height=0.35\paperwidth]{figure/zoo_2yaxis.png}
  \end{columns}
\end{block}
...
dygraphs::dyCandlestick(dygraphs::dyOptions(dygraphs::dygraph(datav), 
  colors="red", strokeWidth=3))
      @
    \column{0.5\textwidth}
      \includegraphics[width=0.4\paperwidth, height=0.35\paperwidth]{figure/dygraphs_candlestick.png}
      Each \emph{candlestick} displays one period of data, and consists of a box representing the \emph{Open} and \emph{Close} prices, and a vertical line representing the \emph{High} and \emph{Low} prices.
      \vskip1ex
      The color of the box signifies whether the \emph{Close} price was higher or lower than the \emph{Open},
  \end{columns}
...
dyplot
      @
    \column{0.5\textwidth}
      \includegraphics[width=0.4\paperwidth, height=0.35\paperwidth]{figure/dygraphs_candlestick_shaded.png}
  \end{columns}
\end{block}
...
dygraphs::dygraph(prices, main=paste(colnamev, collapse=" and ")) %>%
  ...
  dySeries(name=colnamev[2], axis="y2", strokeWidth=2, col="blue")
      @
    \column{0.5\textwidth}
      \includegraphics[width=0.4\paperwidth, height=0.35\paperwidth]{figure/dygraphs_2yaxis.png}
  \end{columns}
\end{block}
...
title(main="Brownian Motion Crossing a Barrier Level", line=0.5)
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.4\paperwidth]{figure/simu_brown_barrier.png}
  \end{columns}
\end{block}
...
symbolv <- symbolv[seq.int(from=1, to=NROW(symbolv), length.out=20)]
    ...
    \column{0.5\textwidth}
    \vspace{-1em}
      \includegraphics[width=0.4\paperwidth]{figure/sp500_price_paths.png}
      <<echo=TRUE,eval=FALSE>>=
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The distribution of final stock prices is extremely skewed, with over \texttt{80\%} of the \emph{S\&P500} constituent stocks from \texttt{1990} now below the average price of that portfolio.
      <<echo=TRUE,eval=FALSE>>=
# Calculate the final stock prices at present
...
sum(pricesf < mean(pricesf))/NROW(pricesf)
    ...
    \column{0.5\textwidth}
    \vspace{-1em}
      \includegraphics[width=0.4\paperwidth]{figure/sp500_price_hist.png}
      <<echo=TRUE,eval=FALSE>>=
# Plot a histogram of final stock prices
hist(pricesf, breaks=1e3, xlim=c(0, 500), 
...
indeks <- xts(indeks, order.by=zoo::index(pricesn))
    ...
    % \vspace{-1em}
      \includegraphics[width=0.4\paperwidth]{figure/sp500_price_percent.png}
      <<echo=TRUE,eval=FALSE>>=
...
drop(retsp[, "VTI"] %*% retsp[, "XLF"])/(nrows-1)
corv <- cor(retsp[, "VTI"], retsp[, "XLF"])
# Test statistical significance of correlation
cortest <- cor.test(retsp[, "VTI"], retsp[, "XLF"])
...
Box.test(macrodiff[, "unemprate"], lag=10, type="Ljung")
      @
      The \emph{n}-value for \emph{VTI} returns is large, and we conclude that the \emph{null hypothesis} is \texttt{TRUE}, and that \emph{VTI} returns are \emph{not} autocorrelated.
      \vskip1ex
...
\begin{block}{}
  \begin{columns}[T]
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      <<echo=TRUE,eval=FALSE>>=
# Compute pacf recursively from acf using Durbin-Levinson algorithm
acfv <- rutils::plot_acf(arimav, lag=10, plotobj=FALSE)
acfv <- as.numeric(acfv$acf)
pacfvl <- numeric(3)
pacfvl[1] <- acfv[1]
pacfvl[2] <- acfv[2] - acfv[1]^2
pacfvl[3] <- acfv[3] - pacfvl[2]*acfv[1] - acfv[2]*pacfvl[1]
# Compute pacf recursively in a loop using Durbin-Levinson algorithm
pacfvl <- numeric(NROW(acfv))
pacfvl[1] <- acfv[1]
for (it in 2:NROW(pacfvl)) {
  pacfvl[it] <- acfv[it] - pacfvl[1:(it-1)] %*% acfv[(it-1):1]
}  # end for
# Compare pacf using Durbin-Levinson algorithm
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \begin{displaymath}
        r_i = \theta p_{i-1} + \varphi_1 r_{i-1} + \ldots + \varphi_n r_{i-n} + \xi_i
      \end{displaymath}
      where $p_i = p_{i-1} + r_i$, so that:
      \begin{displaymath}
        p_i = (1 + \theta) p_{i-1} + \varphi_1 r_{i-1} + \ldots + \varphi_n r_{i-n} + \xi_i
      \end{displaymath}
      If the mean reversion parameter $\theta$ is positive: $\theta > 0$, then the time series $p_i$ exhibits mean reversion and has no \emph{unit root}.
      \vskip1ex
    ...
    \column{0.5\textwidth}
      ...
      \vskip1ex
      If the number of lags in the regression is zero: $n = 0$ then the \emph{ADF} test becomes the standard \emph{Dickey-Fuller} test: $r_i = \theta p_{i-1} + \xi_i$.
      \vskip1ex
      The function \texttt{tseries::adf.test()} performs the \emph{ADF} test.
      <<echo=TRUE,eval=FALSE>>=
nrows <- 1e3
  ...
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \begin{multline*}
        ...
        \sum_{j=1}^n {\varphi_j r_{i-j}} + \xi_i
      \end{multline*}
      Can be expressed as a \emph{multivariate} linear regression model, with the \emph{response} equal to $r_i$, and the columns of the \emph{predictor matrix} equal to the lags of $r_i$.
      \vskip1ex
  ...
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \begin{multline*}
        ...
        \sum_{j=1}^n {\varphi_j r_{i-j}} + \xi_i
      \end{multline*}
      Can be solved as a \emph{multivariate} linear regression model, with the \emph{response} equal to $r_i$, and the \emph{predictor matrix} columns equal to the lags of $r_i$.
    \column{0.5\textwidth}
      \vspace{-1em}
      <<echo=TRUE,eval=FALSE>>=
...
all.equal(forecastv[-(1:ncoeff)], filter_fast[-(1:(ncoeff-1))],
          check.attributes=FALSE)
...
all.equal(forecastv, filter_fast, check.attributes=FALSE)
      @
  \end{columns}
\end{block}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The one step ahead \emph{forecast} $f_i$ of the time series $r_i$ using the process \emph{AR(n)} is defined as:
      \begin{displaymath}
        f_i = \varphi_1 r_{i-1} + \varphi_2 r_{i-2} + \ldots + \varphi_n r_{i-n}
      \end{displaymath}
...
forecastv <- sapply(31:nrows, function(x) {
  cat("len = ", x, "\n")
