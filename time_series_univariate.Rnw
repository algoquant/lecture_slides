...
%%%%%%%%%%%%%%%
\section{Package \texttt{quantmod} for Quantitative Financial Modelling}
...
%%%%%%%%%%%%%%%
\subsection{Downloading Index Time Series Using Package \texttt{quantmod}}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      Some data providers use special characters in their tickers, like \texttt{"-"} or \texttt{"\textasciicircum{}"}, which aren't allowed in  \texttt{R} names, 
      \vskip1ex
      For example \emph{Yahoo} uses the ticker \texttt{"\textasciicircum{}GSPC"} for the \texttt{S\&P500} stock market index, which isn't a valid name in \texttt{R}, 
      \vskip1ex
      The function \texttt{setSymbolLookup()} creates names corresponding to tickers, which are then used by the function \texttt{getSymbols()} to create objects with the valid names, 
    \column{0.5\textwidth}
      \vspace{-1em}
      <<echo=(-(1:1)),eval=FALSE>>=
library(quantmod)  # load package quantmod
...
%%%%%%%%%%%%%%%
\subsection{Scraping S\&P500 Stock Index Constituents From Websites}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The function \texttt{getURL()} from package \texttt{RCurl} downloads the \texttt{HTML} text data from a \texttt{URL}, 
      \vskip1ex
      The function \texttt{readHTMLTable()} from package \texttt{XML} extracts tables from \texttt{HTML} text data or from a remote \texttt{URL}, and returns them as a list of data frames or matrices, 
      \vskip1ex
...
%%%%%%%%%%%%%%%
\subsection{Downloading S\&P500 Time Series Data From \protect\emph{Yahoo}}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.4\textwidth}
      Before time series data for S\&P500 constituents can be downloaded from \emph{Yahoo}, it's necessary to create valid names corresponding to tickers containing special characters like \texttt{"-"}, 
      \vskip1ex
      The function \texttt{setSymbolLookup()} creates a lookup table for \emph{Yahoo} tickers, using valid names in \texttt{R}, 
      \vskip1ex
      For example \emph{Yahoo} uses the ticker \texttt{"BRK-B"}, which isn't a valid name in \texttt{R}, but can be mapped to \texttt{"BRK\_B"}, using the function \texttt{setSymbolLookup()}, 
    \column{0.6\textwidth}
      \vspace{-1em}
...
%%%%%%%%%%%%%%%
\section{Downloading Data from \protect\emph{Quandl}}
...
detach("package:Quandl")
      @
      \emph{Quandl} has developed an \texttt{R} package called \texttt{Quandl} that allows downloading data from \emph{Quandl} directly into \texttt{R}, 
      \vskip1ex
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      \texttt{WIKI} is a community-maintained free database of daily prices for 3,000 U.S. stocks,\\
      \hskip1em\url{https://www.quandl.com/data/WIKI}
      \vskip1ex
      \texttt{SEC} is a free database of stock fundamentals extracted from SEC \texttt{10K} and \texttt{10Q} filings (but not harmonized),\\
      \hskip1em\url{https://www.quandl.com/data/SEC}
      \vskip1ex
      \texttt{RAYMOND} is a free and harmonized database of stock fundamentals, based on the \texttt{SEC} database,\\
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      \emph{Quandl} provides stock index constituents and instrument metadata:\\
      ...
      \vskip1ex
      The \texttt{S\&P500} stock market index is a cap-weighted average of stocks traded on U.S. exchanges, from 500 large-cap companies, and covers about 75\% of the U.S. equity market capitalization, 
      \vskip1ex
      The index is cap-weighted, so a company's weight in the index is proportional to its market capitalization, 
      \vskip1ex
      The \texttt{S\&P500} index is not static, and its constituents and the constituent weights are updated regularly by Dow Jones Indices, 
      \vskip1ex
      The index currently contains 505 stocks because it includes two classes of stock from 5 of its constituent companies, 
      \vskip1ex
      \emph{Quandl} also provides index constituents for other indexes, for example the Dow Jones Industrial Average, NASDAQ Composite, FTSE 100, etc.
    \column{0.5\textwidth}
      \vspace{-1em}
      <<echo=(-(1:1)),eval=FALSE>>=
library(quantmod)  # load package quantmod
...
%%%%%%%%%%%%%%%
\subsection{Downloading Stock Index Time Series Data from \protect\emph{Quandl}}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.4\textwidth}
      Time series data for a portfolio of stocks can be downloaded by performing a loop over the function \texttt{Quandl()} from package \texttt{Quandl}, 
      \vskip1ex
      The \texttt{assign()} function assigns a value to an object in a specified \emph{environment}, by referencing it using a character string (name),
    \column{0.6\textwidth}
      \vspace{-1em}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      Each row of \texttt{TAQ} data contains both the quote or trade prices, and the corresponding quote size or trade volume: 
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \end{itemize}
      The package \texttt{HighFreq} uses time series objects of class \texttt{"xts"}, 
    \column{0.5\textwidth}
      \vspace{-1em}
      <<eval=FALSE>>=
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The package \texttt{HighFreq} contains several datasets, stored in a file called \texttt{hf\_data}, 
      \vskip1ex
      But even after the \texttt{HighFreq} package is loaded, its datasets aren't loaded into the workspace, so they aren't listed in the workspace, 
      \vskip1ex
      ...
      \vskip1ex
      All the datasets in package \texttt{HighFreq} can be loaded into the workspace using the function \texttt{data()}, 
      \vskip1ex
      The package \texttt{HighFreq} contains an \texttt{OHLC} time series in \texttt{xts} format called "\texttt{SPY}", with prices for the \texttt{SPY} etf, 
      \vskip1ex
      Each row of SPY contains a single bar of \texttt{OHLC} prices at 1-minute intervals, 
    \column{0.5\textwidth}
      \vspace{-1em}
      <<eval=FALSE>>=
# load package HighFreq
