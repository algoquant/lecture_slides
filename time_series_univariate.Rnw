...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      Under the \emph{Ornstein-Uhlenbeck} process, the percentage returns $\mathrm{d} \ln{S}$ are proportional to the difference between the equilibrium price $\mu$ minus the current price $S_t$:
      \begin{displaymath}
        \mathrm{d} \ln{S_t} = \theta ( \mu - S_t ) \mathrm{d} t + \sigma \mathrm{d} W_t
      \end{displaymath}
