...
rutils::plot_acf(rnorm(NROW(retsp))^2, lag=10, 
 main="ACF of Squared Random Returns")
# Plot ACF of squared VTI returns
rutils::plot_acf(retsp^2, lag=10, 
...
%%%%%%%%%%%%%%%
\subsection{Autocorrelations in Intervals of High and Low Volatility}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      Stock returns have significant negative autocorrelations in time intervals with high volatility, but not in time intervals with low volatility.
      <<echo=TRUE,eval=FALSE>>=
...
stdev <- sapply(2:npts, function(endp) {
  sd(retsp[endd[endp-1]:endd[endp]])
})  # end sapply
...
for (it in 4:nrows) {
  retsp[it] <- thetav*(priceq - pricev[it-1]) + retsp[(it-1):(it-3)] %*% coeff + innov[it]
...
predv <- sapply(1:3, rutils::lagit, input=arimav)
# Calculate de-meaned returns matrix
predv <- t(t(predv) - colMeans(predv))
predinv <- MASS::ginv(predv)
...
residvar <- sum(residuals^2)/(nrows-NROW(coeff))
covar <- residvar*MASS::ginv(crossprod(predv))
coeffsd <- sqrt(diag(covar))
coefftv <- drop(coeff)/coeffsd
...
predv <- sapply(1:5, rutils::lagit, input=retsp)
predinv <- MASS::ginv(predv)
coeff <- drop(predinv %*% retsp)
# Calculate t-values of AR(5) coefficients
residuals <- drop(retsp - drop(predv %*% coeff))
residvar <- sum(residuals^2)/(nrows-NROW(coeff))
covar <- residvar*MASS::ginv(crossprod(predv))
coeffsd <- sqrt(diag(covar))
coefftv <- drop(coeff)/coeffsd
...
fcast <- sapply(31:nrows, function(x) {
  ...
  predv <- predict(arima_fit, n.ahead=1)
  fcast <- drop(arimav[x:(x-2)] %*% arima_fit$coef)
  c(actual=arimav[x+1], forecast=fcast, predict=as.numeric(predv$pred))
})  # end sapply
foo <- t(foo)
...
nagg <- 5
predv <- rutils::roll_sum(retsp, look_back=nagg)
# Define predictor matrix for forecasting
maxorder <- 5
predv <- sapply(1+nagg*(0:maxorder), rutils::lagit,
                     input=predv)
...
fcast <- sapply(look_backs, sim_fcasts, respv=retsp, 
