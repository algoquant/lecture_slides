...
returns_cpp <- HighFreq::sim_ou(eq_price=eq_price, volat=sig_ma, 
  theta=the_ta, innov=matrix(in_nov))
all.equal(re_turns, drop(returns_cpp))
...
returns_cpp <- HighFreq::sim_ou(eq_price=eq_price, volat=sig_ma, 
    theta=the_ta, innov=matrix(in_nov))
all.equal(price_s, drop(returns_cpp))
# Compare the speed of R code with Rcpp
library(microbenchmark)
summary(microbenchmark(
  Rcode={for (i in 2:n_rows) {
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \begin{displaymath}
        r_i = \theta p_{i-1} + \varphi_1 r_{i-1} + \ldots + \varphi_p r_{i-p} + \xi_i
      \end{displaymath}
      where $p_i = p_{i-1} + r_i$, so that:
      \begin{displaymath}
        p_i = (1 + \theta) p_{i-1} + \varphi_1 r_{i-1} + \ldots + \varphi_p r_{i-p} + \xi_i
      \end{displaymath}
      If the mean reversion parameter $\theta$ is positive: $\theta > 0$, then the time series $p_i$ exhibits mean reversion and has no \emph{unit root}.
      \vskip1ex
