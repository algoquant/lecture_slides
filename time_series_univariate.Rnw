...
\begin{block}{}
  \begin{columns}[T]
    ...
    \column{0.5\textwidth}
      ...
      \vskip1ex
      Examples of interval aggregations are monthly stock returns, or trailing 12-month stock returns calculated every month.
  \end{columns}
    \vspace{-1em}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      An example of an overlapping interval aggregation are trailing 12-month stock returns calculated every month.
      <<echo=TRUE,eval=FALSE>>=
# Number of data points
...
%%%%%%%%%%%%%%%
\section{Time Series of Stock Prices}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      If the percentage stock returns $r_t \mathrm{d} t = \mathrm{d} \log{p_t}$ follow \emph{Brownian motion}:
      \begin{displaymath}
      ...
      \end{displaymath}
      Where $\sigma$ is the volatility of stock returns, and $B_t$ is a \emph{Brownian Motion}, with $\mathrm{d} B_t$ following the normal distribution $\phi(0, \sqrt{\mathrm{d}t})$, with the volatility $\sqrt{\mathrm{d}t}$, equal to the square root of the time increment $\mathrm{d}t$.
      \vskip1ex
...
%%%%%%%%%%%%%%%
\subsection{The Skewness of \protect\emph{Log-normal} Prices}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      \vskip1ex
      If percentage stock returns are \emph{normally} distributed and follow \emph{Brownian motion}, then asset prices follow \emph{Geometric Brownian motion}, and they are \emph{Log-normally} distributed at every point in time.
      \vskip1ex
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      Just performing as well as the index requires \emph{significant} investment skill, while outperforming the index requires \emph{exceptional} investment skill.
    \column{0.5\textwidth}
      \vspace{-1em}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The highly skewed distribution of stock prices makes it difficult for portfolio managers to outperform the stock index.  Because to outperform the index, a portfolio manager needs to select the very few stocks that outperform the index, which is difficult to do because there are very few of them among all the stocks.
      <<echo=TRUE,eval=FALSE>>=
...
%%%%%%%%%%%%%%%
\subsection{depr: U.S. Macroeconomic Data}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
...
%%%%%%%%%%%%%%%
\subsection{depr: Autocorrelations of Econometric Data}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      But the time series of stock returns display very low autocorrelations.
      \vskip1ex
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The autocorrelations of squared stock returns are a very important feature.
      <<echo=TRUE,eval=FALSE>>=
# Open plot window under MS Windows
...
%%%%%%%%%%%%%%%
\subsection{Exponential Moving Average Autocorrelation}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The exponential moving average \emph{EMA} autocorrelation of lag $k$, of the returns $r_t$ is defined as:
      \begin{displaymath}
        \rho^k_t = \lambda \rho^k_{t-1} + (1 - \lambda) \frac{(r_t-\bar{r})(r_{t-k}-\bar{r})}{\sigma^2}
      \end{displaymath}
      Where $\bar{r}$ are the \emph{EMA} of the returns, $\sigma^2$ is the \emph{EMA} variance of the returns, and $\lambda$ is the \emph{EMA} decay factor. 
      \vskip1ex
...
abline(regmod)
    ...
    \column{0.5\textwidth}
    \vspace{-1em}
      \includegraphics[width=0.45\paperwidth]{figure/acf_ema.png}
      <<echo=TRUE,eval=FALSE>>=
...
volatos <- sapply(pricev, function(closep) {
    sqrt(HighFreq::calc_var(HighFreq::diffit(closep["2011/"])))
})  ##  end sapply
# Remove NA values
...
hurstos <- sapply(pricev, function(closep) {
  HighFreq::calc_hurst(closep["2011/"], aggv=aggv)
})  ##  end sapply
# Remove NA values
...
retpca <- retp["2011/"] %*% eigend$vectors
pricepca <- xts::xts(matrixStats::colCumsums(retpca),
                       order.by=index(retp["2011/"]))
colnames(pricepca) <- paste0("PC", 1:NCOL(retp))
# Calculate the Hurst exponents of PCAs
hurstv <- sapply(pricepca, HighFreq::calc_hurst, aggv=aggv)
# Dygraph of PCA with largest Hurst exponent
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The optimized portfolio exhibits very strong trending of returns over longer time intervals, especially in periods of high volatility.
      \vskip1ex
      But it also has strong mean-reverting over shorter time intervals, which can be exploited in trading strategies.
      <<echo=TRUE,eval=FALSE>>=
...
optiml <- optim(par=weightv, fn=calc_phurst, retp=retp,
                method="L-BFGS-B",
                upper=rep(20.0, nweights),
                lower=rep(-20.0, nweights))
# Optimal weights and maximum Hurst
weightv <- optiml$par
names(weightv) <- colnames(retp)
-calc_phurst(weightv, retp=retp)
# PACF of ETF portfolio with largest Hurst exponent
retport <- (retp %*% weightv)
pacf(retport, lag.max = 25)
      @
    \column{0.5\textwidth}
      \vspace{-1em}
...
retport <- xts::xts(retport, zoo::index(retp))
dygraphs::dygraph(cumsum(retport), main="ETF Portfolio With Largest Hurst Exponent")
# PnL of contrarian strategy for ETF portfolio
pnls <- -retport*rutils::lagit(sign(retport))
dygraph(cumsum(pnls))
      @
  \end{columns}
\end{block}
...
weightv <- optiml$par
names(weightv) <- colnames(retp)
# PACF of in-sample portfolio with largest Hurst exponent
-calc_phurst(weightv, retp=retp["/2010"])
retport <- (retp["/2010"] %*% weightv)
pacf(retport, lag.max = 25, main="PACF of In-Sample Portfolio")
# PACF of out-of-sample portfolio with largest Hurst exponent
-calc_phurst(weightv, retp=retp["2011/"])
retport <- (retp["2011/"] %*% weightv)
pacf(retport, lag.max = 25, main="PACF of Out-of-Sample Portfolio")
      @
  \end{columns}
\end{block}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      For example, asset prices follow an \emph{integrated} process with respect to stock returns: $p_t = {\sum_{i=1}^t r_i}$.
      \vskip1ex
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \end{displaymath}
      Where $B_t$ is a \emph{Brownian Motion}, with its increment $\mathrm{d} B_t$ following the normal distribution with the volatility $\sqrt{\mathrm{d}t}$, equal to the square root of the time increment $\mathrm{d}t$.
      \vskip1ex
      The solution of the \emph{Ornstein-Uhlenbeck} process is given by:
      \begin{displaymath}
        p_t = p_0 e^{-\theta t} + \mu (1 - e^{-\theta t}) + \sigma \int_{0}^{t} {e^{\theta (s - t)} \mathrm{d} B_s}
      \end{displaymath}
      \vskip1ex
      ...
      \end{flalign*}
      The \emph{Ornstein-Uhlenbeck} process is mean reverting to the equilibrium price $\mu$.
      \vskip1ex
...
legend("bottomright", title=paste(c(
  ...
  collapse="\n"),
  legend="", cex=1.1, inset=0.1, bg="white", bty="n")
