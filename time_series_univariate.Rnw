...
library(quantmod)
library(TTR)
ohlc <- rutils::etf_env$VTI["2009-02/2009-03"]
...
library(quantmod)
ohlc <- rutils::etf_env$VTI["2010-04/2010-05"]
...
vwapv <- TTR::VWAP(price=quantmod::Cl(rutils::etf_env$VTI),
...
chobj <- chart_Series(x=etf_env$XLF["2009-02/2009-04"],
  name="XLF", plot=FALSE)
...
whichv <- which(rutils::diff_it(indic) != 0)
...
prices <- cbind(Cl(rutils::etf_env$VTI), Cl(rutils::etf_env$IEF))
...
sigmav <- sd(rutils::diff_it(log(rutils::etf_env$VTI[, 4])))
sigma2 <- sigmav^2
nrows <- NROW(rutils::etf_env$VTI)
...
returns <- rutils::etf_env$returns[, c("VTI", "XLF")]
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The speed of mean reversion of an \emph{AR(1)} process depends on the \emph{AR(n)} coefficient $\varphi1$, with a negative coefficient producing faster mean reversion, and a positive coefficient producing stronger diversion.
      \vskip1ex
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      $1 - \varphi1 z - \varphi2 z^2 - \ldots - \varphi_n z^n = 0$
      \vskip1ex
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      $p_i = (1 + \varphi1) p_{i-1} + (\varphi2 - \varphi1) p_{i-2} + \ldots + (\varphi_n - \varphi_{n-1}) p_{i-n} - \varphi_n p_{i-n-1} + \xi_i$
      \vskip1ex
...
%%%%%%%%%%%%%%%
\subsection{draft: The Dickey-Fuller Process}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \begin{displaymath}
        r_i = \theta p_{i-1} + \varphi1 r_{i-1} + \ldots + \varphi_n r_{i-n} + \xi_i
      \end{displaymath}
      where $p_i = p_{i-1} + r_i$, so that:
      \begin{displaymath}
        p_i = (1 + \theta) p_{i-1} + \varphi1 r_{i-1} + \ldots + \varphi_n r_{i-n} + \xi_i
      \end{displaymath}
      If the mean reversion parameter $\theta$ is positive: $\theta > 0$, then the time series $p_i$ exhibits mean reversion and has no \emph{unit root}.
      \vskip1ex
...
plot(x=coeffs, y=adf_test["adf_stat", ], main="ADF Stat Versus AR Coefficient",
