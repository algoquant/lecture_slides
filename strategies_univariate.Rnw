...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The portfolio with proportional wealth allocations has a slightly higher Sharpe ratio than the portfolio with a fixed number of shares.
      <<echo=TRUE,eval=FALSE>>=
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      In the \emph{proportional wealth allocation} strategy (\emph{PWA}), the total wealth $\omega_t$ is allocated to the assets $\omega_i$ proportional to the portfolio weights $w_i$: $\omega_i = w_i \omega_t$.
      \vskip1ex
      The total wealth $\omega_t$ is not fixed and is equal to the portfolio market value $\omega_t = \sum \omega_i$, so there's no margin account.
      \vskip1ex
      The portfolio is rebalanced daily to maintain the dollar allocations $\omega_i$ equal to the total wealth $\omega_t = \sum \omega_i$ times the portfolio weights: $w_i$: $\omega_i = w_i \omega_t$.
      \vskip1ex
      ...
      \vskip1ex
      The total portfolio wealth at time $t$ is equal to the wealth at time $t-1$ multiplied by the weighted returns: $\omega_t = \omega_{t-1} (1 + \bar{r}_t)$.
      \vskip1ex
      The dollar amount of stock $i$ at time $t$ increases by $w_i r_t$ so it's equal to $w_i \omega_{t-1} (1 + r_t)$, while the target amount is $w_i \omega_t = w_i \omega_{t-1} (1 + \bar{r}_t)$
      \vskip1ex
      The dollar amount of stock $i$ needed to trade to rebalance back to the target weight is equal to:
      \begin{flalign*}
        \varepsilon_i &= \left| w_i \omega_{t-1} (1 + \bar{r}_t) - w_i \omega_{t-1} (1 + r_t) \right|\\
        &= w_i \omega_{t-1} \left| \bar{r}_t - r_t \right|
      \end{flalign*}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The total dollar amount of stocks that need to be traded to rebalance back to the target weight is equal to: $\sum_{i=1}^n \varepsilon_i = \omega_{t-1} \sum_{i=1}^n w_i \left| \bar{r}_t - r_t \right|$
      \vskip1ex
      ...
      \vskip1ex
      The \emph{cumulative transaction costs} $\sum_{i=1}^t c^r_t$ must be subtracted from the \emph{wealth} $\omega_t$: $\omega_t - \sum_{i=1}^t c^r_t$.
      <<echo=TRUE,eval=FALSE>>=
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      In the \emph{proportional wealth allocation} strategy (\emph{PWA}), the portfolio is rebalanced daily to maintain the dollar allocations $\omega_i$ equal to the total wealth $\omega_t = \sum \omega_i$ times the portfolio weights: $w_i$: $\omega_i = w_i \omega_t$.
      \vskip1ex
      In the \emph{proportional target allocation} strategy (\emph{PTA}), the portfolio is rebalanced only if the dollar allocations $\omega_i$ differ from their targets $w_i \omega_t$ more than the threshold value $\tau$: $\tau > \frac{\sum \left| \omega_i - w_i \omega_t \right|}{\omega_t}$.
      \vskip1ex
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      In the \emph{Risk Parity} strategy the dollar allocations are rebalanced daily so that their dollar volatilities remain equal.
      \vskip1ex
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The risk parity strategy for stocks and bonds has a higher Sharpe ratio, but lower absolute returns than the proportional wealth strategy. 
      \vskip1ex
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      This allows the risk parity strategy to time the markets better - selling when prices are about to drop and buying when prices are rising.
      \vskip1ex
      The t-value of the equal wealth strategy is negative, because it buys the stock as its prices drop, the opposite of risk parity.
      <<echo=TRUE,eval=FALSE>>=
