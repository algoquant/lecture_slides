...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The risk-adjusted wealth may also be higher for very short holding periods because the risk is low - there's not enough time for the wealth to significantly drop below par (\texttt{1}).
      \vskip1ex
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The \emph{standardized price} is the dollar amount of stock with unit dollar volatility.  
      The \emph{standardized price} is equal to the stock price divided by its dollar volatility: $\frac{p}{\sigma_d}$
      \vskip1ex
      ...
      \vskip1ex
      The equal volatility portfolio has lower absolute returns because it's overweight bonds.
      <<echo=TRUE,eval=FALSE>>=
...
retp <- retd/rutils::lagit(pricev, lagg=1, pad_zeros=FALSE)
# Wealth of fixed shares (without rebalancing)
weightv <- c(0.5, 0.5) # Buy 0.5 shares of each stock
wealthfs <- drop(pricev %*% weightv)
# retd[1, ] <- pricev[1, ]
# wealthfs <- cumsum(retd %*% weightv)
# Calculate the standardized stock prices (fixed shares)
pricesd <- lapply(pricev, function(x) x/sd(rutils::diffit(x)))
pricesd <- do.call(cbind, pricesd)
retsd <- rutils::diffit(pricesd) # Dollar returns
sapply(retsd, sd)
sapply(retd, sd)
# Wealth with equal dollar volatilities
wealthev <- drop(pricesd %*% weightv)
wealthev <- wealthev*wealthfs[1]/wealthev[1]
      @
    \column{0.5\textwidth}
      \vspace{-1em}
      ...
      <<echo=TRUE,eval=FALSE>>=
# Calculate the Sharpe and Sortino ratios
wealthv <- xts::xts(cbind(wealthfs, wealthev), datev)
...
%%%%%%%%%%%%%%%
\subsection{The Rolling Standardized Stock Prices}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The \emph{rolling standardized price} is equal to the stock price divided by its trailing dollar volatility: $\frac{p_t}{\sigma_t}$
      \vskip1ex
      When the volatility changes over time, then the rolling standardized prices also change over time.
      \vskip1ex
      The rolling standardized prices can become very large and very volatile when the volatility is very low.
      \vskip1ex
      The trailing volatility is calculated from past returns using the recursive formula: $\sigma_t^2 = \lambda \sigma_{t-1}^2 + (1 - \lambda) r_t^2$.
      \vskip1ex
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.45\paperwidth]{figure/portf_risk_parity_prices.png}
      <<echo=TRUE,eval=FALSE>>=
# Calculate the trailing dollar volatilities
vold <- HighFreq::run_var(retd, lambda=0.99)
vold <- sqrt(vold)
# Calculate the standardized prices with unit dollar volatilities
pricerp <- pricev/vold
pricerp <- pricerp*sum(pricev[1, ])/sum(pricerp[1, ])
# Plot the rolling standardized prices
pricec <- cbind(pricerp$VTI, pricev$VTI)
colnames(pricec) <- c("Standardized", "VTI")
dygraph(pricec[endd], main="Rolling Standardized Prices of VTI") %>%
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      In the \emph{Risk Parity} strategy the dollar allocations are rebalanced daily so that their dollar volatilities remain proportional to the weights.
      \vskip1ex
      The dollar returns of risk parity are equal to the standardized prices times their percentage returns.
      \vskip1ex
      The risk parity strategy for stocks and bonds has a higher Sharpe ratio than the proportional wealth strategy. 
      \vskip1ex
      The absolute returns can also be higher if the standardized prices become very large when the volatility is very low.
      \vskip1ex
      The risk parity strategy is also called the equal risk contributions (ERC) strategy.
      \vskip1ex
      Risk parity works better for assets with negative or low correlations, like stocks and bonds.
      \vskip1ex
...
wealthrp <- cumsum(retrp %*% weightv)
    ...
    \column{0.5\textwidth}
      ...
      <<echo=TRUE,eval=FALSE>>=
# Wealth of proportional allocation (with rebalancing)
weightv <- c(0.5, 0.5) # Keep 0.5 of wealth in each stock
wealthpr <- cumprod(1 + retp %*% weightv)
wealthpr <- wealthpr*sum(pricev[1, ]*weightv)
...
endd <- rutils::calc_endpoints(wealthv, interval="weeks")
dygraphs::dygraph(log(wealthv[endd]), 
  main="Log Wealth of Risk Parity vs Proportional Allocation") %>%
  dyOptions(colors=c("blue", "red"), strokeWidth=2) %>%
  dyLegend(show="always", width=300)
      @
  \end{columns}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      This allows the risk parity strategy to time the markets better - selling when prices are about to drop and buying when prices are rising.  But only when the changes of the volatility are significant.
      \vskip1ex
      But the t-value of the risk parity strategy is negative, because it's contrarian when the volatility is stable.
      \vskip1ex
      The t-value of the proportional wealth allocation strategy is negative, because it's contrarian, i.e. it buys stocks when their prices drop.
      <<echo=TRUE,eval=FALSE>>=
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      In each period the stocks must be rebalanced to maintain the equal wealths.
      \vskip1ex
      The total dollar amount of stocks that need to be traded to rebalance back to the target weight is equal to: $\sum_{i=1}^n \varepsilon_i = \Pi_{t-1} \sum_{i=1}^n w_i \left| \bar{r}_t - r_t \right|$
      \vskip1ex
      The \emph{transaction costs} $c^r_t$ are equal to half the \emph{bid-ask spread} $\delta$ times the dollar amount of the traded stock: $c^r_t = \frac{\delta}{2} \sum_{i=1}^n \varepsilon_i$.
