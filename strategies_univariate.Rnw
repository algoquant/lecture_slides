...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      For sampling from rows of data, it's better to convert the time series to a matrix, using the function \texttt{zoo::coredata()}.
      \vskip1ex
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The CPPI strategy has a fixed maturity date, and it's designed to gaurantee the payback of the initial principal investment at maturity, while also providing upside participation in any stock gains.
      \vskip1ex
      ...
      \vskip1ex
      The stock investment is levered by the \emph{CPPI multiplier} $C$ through borrowing from a margin account.
      \vskip1ex
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The function \texttt{HighFreq::run\_var()} calculates the trailing variance of returns $r_t$, by recursively weighting the past variance estimates $\sigma^2_{t-1}$, with the squared differences of the returns minus the \emph{EMA} prices $(r_t - \bar{r}_t)^2$, using the decay factor $\lambda$:
      \begin{flalign*}
...
colnames(reton) <- "overnight" # Overnight close-to-open returns
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.45\paperwidth]{figure/strat_overnight_vti.png}
      <<echo=TRUE,eval=FALSE>>=
# Calculate the Sharpe and Sortino ratios
wealthv <- cbind(retp, reton, retd)
sqrt(252)*sapply(wealthv, function(x) 
  c(Sharpe=mean(x)/sd(x), Sortino=mean(x)/sd(x[x<0])))
# Plot dygraph of the Daytime and Overnight returns
endw <- rutils::calc_endpoints(wealthv, interval="weeks")
...
%%%%%%%%%%%%%%%
\subsection{Overnight Market Anomaly Excess Returns}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      Before the \texttt{2008} financial crisis, the overnight returns were higher than the daily \emph{close-to-close} returns, but they were hard to monetize because of the high transaction costs at the time.
      \vskip1ex
      After the \texttt{2008} financial crisis, the trading volumes increased significantly, and the bid-ask spreads dropped, making it easier to monetize the excess overnight returns.
      \vskip1ex
...
sapply(wealthv["2009/"], function(x) {
  pacfl <- pacf(x, lag.max=10, plot=FALSE)
  sum(pacfl$acf)
}) #  end sapply
pacf(reton["2009/"], lag.max=10)
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.45\paperwidth]{figure/strat_overnight_returns.png}
      <<echo=TRUE,eval=FALSE>>=
...
%%%%%%%%%%%%%%%
\subsection{Overnight Market Anomaly for Stocks}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The \emph{Overnight Market Anomaly} is more pronounced in more volatile stocks and also in "meme" stocks which are actively traded by retail investors.
      \vskip1ex
      ...
      \vskip1ex
      Some of the stocks with large overnight returns compared to the daytime returns are \emph{BRKB}, \emph{MGM}, \emph{MU}, \emph{KMI}, etc.
      \vskip1ex
...
colnames(reton) <- "overnight"
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.45\paperwidth]{figure/strat_overnight_mgm.png}
      <<echo=TRUE,eval=FALSE>>=
# Calculate the Sharpe and Sortino ratios
wealthv <- cbind(retp, reton, retd)
sqrt(252)*sapply(wealthv, function(x) 
...
%%%%%%%%%%%%%%%
\subsection{Trend-Flip Strategy}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      After the start-gain level is crossed, the strategy buys back the stock which was sold under the stop-loss.
      \vskip1ex
      The trend-flip strategy trades a single \texttt{\$1} of stock, and is either long \texttt{\$1} of stock or it's short (\texttt{-\$1} of stock).
      \vskip1ex
...
%%%%%%%%%%%%%%%
\subsection{Trend-Flip Strategy Performance}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      The start-gain rule tends to quickly override the stop-loss rule, so that the strategy is long the stock after it recovers after a stop-loss.
      \vskip1ex
      The trend-flip strategy suffers losses when prices are range-bound without a trend, because whenever it switches position the prices soon change direction.  (This is called a \emph{"whipsaw"}.)
      <<echo=TRUE,eval=FALSE>>=
...
dygraphs::dygraph(cumsum(wealthv)[endw], 
  ...
  dyLegend(show="always", width=300)
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.45\paperwidth]{figure/trend_flip_performance.png}
      <<echo=TRUE,eval=FALSE>>=
...
%%%%%%%%%%%%%%%
\subsection{Optimal Trend-Flip Strategy}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
...
stopl <- stopv[which.max(pnlc)]
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.4\paperwidth]{figure/trend_flip_profile.png}
      <<echo=TRUE,eval=FALSE>>=
...
%%%%%%%%%%%%%%%
\subsection{Trend-Flip Strategy for Other ETFs}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The trend-flip strategy can prevent losses for stocks with significant negative returns, like the \emph{USO} ETF (oil fund).
      \vskip1ex
      <<echo=TRUE,eval=FALSE>>=
# Calculate the USO prices and returns
pricev <- log(na.omit(rutils::etfenv$prices$USO))
...
pnlc <- unlist(mclapply(stopv, function(stopl) {
  sum(trend_flip(stopl)[, "pnlv"])
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.4\paperwidth]{figure/trend_flip_uso_profile.png}
      <<echo=TRUE,eval=FALSE>>=
# Plot cumulative PnLs of trend-flip strategies
plot(x=stopv, y=pnlc, 
...
%%%%%%%%%%%%%%%
\subsection{Optimal Trend-Flip Strategy For \protect\emph{USO}}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The trend-flip strategy for the \emph{USO} ETF has performed well because \emph{USO} has had very negative returns in the last \texttt{20} years.
      \vskip1ex
...
dygraphs::dygraph(cumsum(wealthv)[endw], 
  ...
  dyLegend(show="always", width=300)
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.45\paperwidth]{figure/trend_flip_uso_performance.png}
      <<echo=TRUE,eval=FALSE>>=
# Plot the trend-flip strategy with shading
posv <- pnls[, "posv"]
indic <- (rutils::diffit(posv) != 0) ##  Indices of crosses
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The \emph{Exponential Moving Average Price} (\emph{EMA}) is defined as the weighted average of prices over a rolling interval:
      \begin{displaymath}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The compiled \texttt{C++} function \texttt{stats:::C\_rfilter()} calculates the exponential moving average prices recursively.
      \vskip1ex
      The function \texttt{HighFreq::run\_mean()} calculates the exponential moving average prices recursively.
      <<echo=TRUE,eval=FALSE>>=
...
endw <- rutils::calc_endpoints(wealthv, interval="weeks")
captiont <- paste("EMA Crossover Strategy", paste(paste(names(sharper), sharper, sep="="), collapse=", "))
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      If the $\lambda$ parameter is too small, the trading frequency is too high, causing high transaction costs.
      \vskip1ex
      If the $\lambda$ parameter is too large, the trading frequency is too low, causing the strategy to miss profitable trades.
    \column{0.5\textwidth}
      \vspace{-1em}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The Bollinger Bands are three time series, with the middle band equal to the \emph{EMA} prices, the upper band equal to the mean prices plus the trailing standard deviation, and the lower band equal to the mean prices minus the standard deviation.
      \vskip1ex
      ...
      \vskip1ex
      The function \texttt{HighFreq::run\_var()} calculates the \emph{EMA} price and variance of the prices $p_t$, by recursively weighting the past variance estimates $\sigma^2_{t-1}$, with the squared differences of the prices minus the \emph{EMA} prices $(p_t - \bar{p}_t)^2$, using the decay factor $\lambda$:
      \begin{flalign*}
        & \bar{p}_t = \lambda \bar{p}_{t-1} + (1 - \lambda) p_t \\
        & \sigma^2_t = \lambda^2 \sigma^2_{t-1} + (1 - \lambda^2) (p_t - \bar{p}_t)^2
      \end{flalign*}
      Where $\bar{p}_t$ and $\sigma^2_t$ are the \emph{EMA} price and variance at time $t$.
      \vskip1ex
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The \emph{price z-score} is equal to the ratio of the deviation from the \emph{EMA} price divided by the price volatility:
      \begin{displaymath}
        z_i = \frac{p_t - \bar{p}_t}{\sigma_t}
      \end{displaymath}
      The \emph{z-score} is the standardized deviation of the current price away from the \emph{EMA} price.
      <<echo=TRUE,eval=FALSE>>=
...
volp <- HighFreq::roll_var_ohlc(ohlc=ohlc, lookb=lookb, scalit=FALSE)
...
%%%%%%%%%%%%%%%
\subsection{The Informedness}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The \emph{sensitivity} is the \emph{true positive} rate, the fraction of \texttt{FALSE} \emph{null hypothesis} cases that are correctly classified as \texttt{FALSE}.
      \vskip1ex
      The \emph{specificity} is the \emph{true negative} rate, the fraction of \texttt{TRUE} \emph{null hypothesis} cases that are correctly classified as \texttt{TRUE}.
      \vskip1ex
      The \emph{informedness} is equal to the sum of the sensitivity plus the specificity, and measures the performance of the binary classification model. 
      \vskip1ex
      The threshold value can be chosen by maximizing the \emph{informedness} of the classification model.
      <<echo=TRUE,eval=FALSE>>=
...
errorr <- rbind(errorr, c(0, 1))
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.45\paperwidth]{figure/logistic_stocktops_informedness.png}
      \vspace{-1em}
      <<echo=TRUE,eval=FALSE>>=
