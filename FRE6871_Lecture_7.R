...
dtable[, max_delay := max(aircraft_delay),
     by=.(origin, dest)]
...
# Add several columns
dtable[, c("max_carrier", "max_aircraft") :=
       lapply(.SD, max),
     by=.(origin, dest),
...
summary(microbenchmark(
  as.data.frame=data.table:::as.data.frame.data.table(dtable),
...
endp <- xts::endpoints(pricev, on="hours")
head(endp)
