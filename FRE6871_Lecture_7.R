...
sim_our <- function(priceq=5.0,
              volat=0.01, theta=0.01, innov) {
  nrows <- NROW(innov)
  retp <- numeric(nrows)
  pricev <- numeric(nrows)
  pricev[1] <- priceq
  for (i in 2:nrows) {
    retp[i] <- theta*(priceq - pricev[i-1]) + volat*innov[i-1]
...
nrows <- 1e3
innov <- rnorm(nrows)
ousim <- sim_our(priceq=priceq, volat=sigmav, theta=thetav, innov=innov)
...
oucpp <- sim_oucpp(priceq=priceq,
  volat=sigmav, theta=thetav, innov=innov)
all.equal(ousim, oucpp)
# Compare speed of Rcpp and R
library(microbenchmark)
summary(microbenchmark(
  rcode=sim_our(priceq=priceq, volat=sigmav, theta=thetav, innov=innov),
  Rcpp=sim_oucpp(priceq=priceq, volat=sigmav, theta=thetav, innov=innov),
  times=10))[, c(1, 4, 5)]
...
objfun <- function(weightv, retp, covmat, lambdar=0, lambdap=1e3) {
  retportf <- retp %*% weightv
  covridge <- covmat + lambdar*diag(ncol(retx))
  stdev <- sqrt(weightv %*% covridge %*% weightv)
  if (stdev == 0)
    return(0)
  else
...
lambdar <- 300.0
objfun(weightv, retp=retx, lambdar=lambdar)
optiml <- optim(par=weightv,
          fn=objfun,
          retp=retx,
          covmat=covmat, # Covariance matrix of returns
          lambdar=lambdar, # Ridge regularization intensity
