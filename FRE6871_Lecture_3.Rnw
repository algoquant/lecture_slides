...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      For very short maturities ($\tau \to 0$), the yield is equal to $y(0) = \beta_0 + \beta_1 + \beta_2$.
      \vskip1ex
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      A limitation of \emph{Differential Evolution} is that it doesn't estimate the standard errors of the parameters.
    \column{0.5\textwidth}
        <<echo=TRUE,eval=FALSE>>=
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
\fbox{\parbox{\columnwidth-1.5em}{
      The \emph{R-squared} measures how well the regression fits the data, but it doesn't measure the statistical significance of the regression.  
}}
      \vskip1ex
