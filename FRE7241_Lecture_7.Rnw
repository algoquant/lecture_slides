...
arma::vec calc_weights(const arma::mat& re_turns,
  ...
  if (typ_e == "max_sharpe") {
    ...
    // Apply regularized inverse to unit vector
    weight_s = calc_inv(re_turns, eigen_max)*arma::ones(re_turns.n_cols);
...
arma::mat back_test(const arma::mat& ex_cess, // Portfolio excess returns
  ...
  for (arma::uword it=1; it < end_p.size(); it++) {
    // cout << "it: " << it << endl;
    // Calculate portfolio weights
    weight_s = co_eff*calc_weights(ex_cess.rows(start_p(it-1), end_p(it-1)), typ_e, eigen_max, pro_b, al_pha, scal_e);
    // Calculate out-of-sample returns
