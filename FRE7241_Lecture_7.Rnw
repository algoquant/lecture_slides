...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      For example, \emph{ESZ8} is the symbol for the \emph{S\&P500} index \emph{E-mini} futures expiring in December 2019. 
    \column{0.5\textwidth}
    \vspace{-1em}
    \begin{table}[htb]
...
month_codes <- cbind(c("January", "February", "March", "April", "May", "June", "July", "August", "April", "May", "November", "December"), 
...
# Load data for S&P Emini futures December 2019 contract
sym_bol <- "ES"
...
chart_Series(x=chain_ed["2019"], TA="add_Vo()", 
...
# plot OHLC data in x11 window
chart_Series(x=vix_index["2019"], name="VIX Index")
# plot dygraph
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The most liquid \emph{VIX} futures are with monthly expiration dates (\href{http://www.cboe.com/framed/pdfframed?content=/aboutcboe/xcal2019.pdf&section=SEC_RESOURCES&title=2019+Cboe+Expiration+Calendar}{CBOE Expiration Calendar}), but weekly \emph{VIX} futures are also traded.
      \vskip1ex
...
# Get all VIX futures for 2019 except January
sym_bols <- ls(vix_env)
sym_bols <- sym_bols[grep("*8", sym_bols)]
sym_bols <- sym_bols[2:9]
# Specify dates for curves
low_vol <- as.Date("2019-01-11")
hi_vol <- as.Date("2019-02-05")
...
colnames(curve_s) <- c("Contango 01/11/2019", 
                       "Backwardation 02/05/2019")
      @
  \end{columns}
\end{block}
...
%%%%%%%%%%%%%%%
\subsection{\protect\emph{VIX} Crash on February 5th 2019}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The \emph{SVXY} and \emph{XIV} ETFs had \$3.6 billion of assets at the beginning of 2019.
      \vskip1ex
      On February 5th 2019 the U.S. stock markets experienced a mini-crash, which was exacerbated by \emph{VIX} futures short sellers.
      \vskip1ex
      As a result, the \emph{XIV} ETF hit its termination event and its value dropped to zero:\\
      \hskip1em\href{https://www.bloomberg.com/news/articles/2019-02-07/how-two-tiny-volatility-products-helped-fuel-sudden-stock-slump}{Volatility Caused Stock Market Crash}\\
      \hskip1em\href{https://riskreversal.com/2019/02/06/volatility-etn-terminated-xiv/
}{XIV ETF Termination Event}
      <<echo=TRUE,eval=FALSE>>=
chart_Series(x=Cl(vix_env$vix_index["2017/2019"]), 
             theme=plot_theme, name="VIX Index")
chart_Series(x=Cl(rutils::etf_env$SVXY["2017/2019"]), 
...
# Define S&P Emini future December 2019 contract
con_tract <- IBrokers::twsFuture(symbol="ES", 
  exch="GLOBEX", expiry="201912")
# Define 10yr Treasury future December 2019 contract
con_tract <- IBrokers::twsFuture(symbol="ZN", 
  exch="ECBOT", expiry="201912")
# Define euro currency future May 2019 contract
con_tract <- IBrokers::twsFuture(symbol="EUR", 
  exch="GLOBEX", expiry="201910")
# Define Gold future July 2019 contract
con_tract <- IBrokers::twsFuture(symbol="GC", 
  exch="NYMEX", expiry="201907")
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      For example, \emph{VXV8} is the local security name (symbol) for the monthly \emph{VIX} futures contract expiring on May 17th, 2019. 
      \vskip1ex
      \emph{VX40V8} is the local security name (symbol) for the weekly \emph{VIX} futures contract expiring on May 3rd, 2019. 
      \vskip1ex
      The function \texttt{reqContractDetails()} returns a list with information on the IB instrument. 
      \vskip1ex
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      <<echo=TRUE,eval=FALSE>>=
# Define VIX monthly and weekly futures May 2019 contract
sym_bol <- "VIX"
con_tract <- IBrokers::twsFuture(symbol=sym_bol, 
  exch="CFE", expiry="201910")
# Define VIX monthly futures May 2019 contract
con_tract <- IBrokers::twsFuture(symbol=sym_bol, 
  local="VXV8", exch="CFE", expiry="201910")
# Define VIX weekly futures May 3rd 2019 contract
con_tract <- IBrokers::twsFuture(symbol=sym_bol, 
  local="VX40V8", exch="CFE", expiry="201910")
# Download list with instrument information
...
\begin{block}{}
  \begin{columns}[T]
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      <<echo=TRUE,eval=FALSE>>=
# Define S&P Emini futures December 2019 contract
sym_bol <- "ES"
con_tract <- IBrokers::twsFuture(symbol=sym_bol, 
  exch="GLOBEX", expiry="201912")
...
data_dir <- "C:/Develop/data/ib_data"
dir.create(data_dir)
file_name <- file.path(data_dir, paste0(sym_bol, "_201912.csv"))
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      For example, \emph{ESM8} is the symbol for the \emph{S\&P500} emini futures expiring in June 2019. 
      <<echo=TRUE,eval=FALSE>>=
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The function \texttt{eWrapper()} creates an \emph{eWrapper} object, consisting of a data environment and methods (functions) for formatting adding new data to it.
      \vskip1ex
...
# Define S&P Emini future December 2019 contract
con_tract <- IBrokers::twsFuture(symbol="ES", exch="GLOBEX", expiry="201912")
# Define euro currency contract EUR.USD
con_tract <- IBrokers::twsCurrency("EUR", currency="USD")
# Define euro currency E-mini futures December 2019 contract E7Z8
con_tract <- IBrokers::twsFuture(symbol="E7", exch="GLOBEX", expiry="201912")
# Define Japanese yen currency contract JPY.USD
con_tract <- IBrokers::twsCurrency("JPY", currency="USD")
# Define Japanese yen currency E-mini futures December 2019 contract J7Z8
con_tract <- IBrokers::twsFuture(symbol="J7", exch="GLOBEX", expiry="201912")
# Define Japanese yen currency futures December 2019 contract 6JZ8
con_tract <- IBrokers::twsFuture(symbol="JPY", exch="GLOBEX", expiry="201912")
# Connect to Interactive Brokers TWS
ib_connect <- IBrokers::twsConnect(port=7497)
reqContractDetails(conn=ib_connect, Contract=con_tract)
...
vix_env <- new.env()
# Download VIX data for the months 6, 7, and 8 in 2019
library(qmao)
quantmod::getSymbols("VX", Months=1:12, 
  Years=2019, src="cfe", env=vix_env)
# Or
qmao::getSymbols.cfe(Symbols="VX", 
  Months=6:8, Years=2019, env=vix_env, 
