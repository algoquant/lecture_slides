...
fmat <- matrix(c(
  ...
  t(unitv) %*% covinv %*% retm,
  t(retm) %*% covinv %*% unitv,
  t(retm) %*% covinv %*% retm), nc=2)
# Solve for the Lagrange multipliers
lagm <- solve(a=fmat, b=c(2, 2*rett))
# Calculate weights
weightv <- drop(0.5*covinv %*% cbind(unitv, retm) %*% lagm)
# Calculate constraints
all.equal(1, sum(weights))
all.equal(rett, sum(retm*weightv))
      @
  \end{columns}
\end{block}
...
retp <- drop(retp %*% weightv)
retsv <- drop(t(unitv) %*% covinv %*% retm /
  t(unitv) %*% covinv %*% unitv)
