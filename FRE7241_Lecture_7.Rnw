...
arma::vec calc_weights(const arma::mat& returns, // Portfolio returns
  ...
  case method::ranksharpe: {
    ...
    weights = conv_to<vec>::from(arma::sort_zoo::index(arma::sort_zoo::index(meancols)));
  ...
  case method::quantile: {
    ...
    weights = conv_to<vec>::from(arma::sort_zoo::index(arma::sort_zoo::index(weights)));
    weights = (weights - arma::mean(weights));
    break;
  }  // end quantile
  default : {
...
indeks <- xts::xts(rowSums(returns)/sqrt(nassets), zoo::index(returns))
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.45\paperwidth]{figure/portf_stocks_outsample_shrink.png}
      <<echo=TRUE,eval=FALSE>>=
# Plot cumulative portfolio returns
pnls <- rbind(portf_is, portf_os)
pnls <- pnls*sd(indeks)/sd(pnls)
...
rets_mean <- (1 - alpha)*rets_mean + alpha*mean(rets_mean)
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.45\paperwidth]{figure/portf_stocks_outsample_rets_shrink.png}
      <<echo=TRUE,eval=FALSE>>=
# Calculate portfolio weights
...
arma::mat calc_inv(const arma::mat& matrixv, const arma::uword& eigen_max) {
  ...
  arma::eig_sym(eigenval, eigenvec, cov(matrixv));
  eigenvec = eigenvec.cols(eigenvec.n_cols-eigen_max, eigenvec.n_cols-1);
  eigenval = 1/eigenval.subvec(eigenval.n_elem-eigen_max, eigenval.n_elem-1);

  return eigenvec*diagmat(eigenval)*eigenvec.t();
...
arma::vec calc_weights(const arma::mat& returns,
  ...
  if (typev == "max_sharpe") {
    ...
    arma::vec eigenval;
    arma::mat eigenvec;
    arma::eig_sym(eigenval, eigenvec, cov(returns));
    weights = eigenvec.col(0);
  } else if (typev == "rank") {
    // Mean returns by columns
    arma::vec meancols = arma::trans(arma::mean(returns, 0));
