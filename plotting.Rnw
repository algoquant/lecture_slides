...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      To create a shiny plot, you must first create an \texttt{.Rmd} file, embed the \emph{shiny} code in an \texttt{R} chunk, and then compile the \texttt{.Rmd} file into an \emph{HTML} document, using the \emph{knitr} package,
      \vspace{-1em}
      <<echo=TRUE,eval=FALSE>>=
inputPanel(
