...
my_var <- seq(-2*pi, 2*pi, len=100)  # x values

# open Windows graphics device
x11(width=11, height=7, title="simple plot")

# plot a sine function
plot(x=my_var, y=sin(my_var),  # create basic line plot
   xlab="x-val", ylab="y-val", type='l', lwd=2, col="red")
# add a cosine function
lines(x=my_var, y=cos(my_var), lwd=2, col="blue")
# add title
...
# add title
title(main="Normal probability distribution functions", cex=1.5, line=0.1)
# add legend
...
graphics.off()  # close all graphics devices
    ...
    \column{0.5\textwidth}
    \vspace{-2em}
    \includegraphics[width=0.5\paperwidth,valign=t]{figure/sincosfunplot}
  \end{columns}
\end{block}
...
# add legend
legend(x="topright", legend="Poisson density", title="", inset=0.1, 
       cex=1.0, bg="white", lwd=4, lty=1, col="red")
graphics.off()  # close all graphics devices
      @
    \column{0.5\textwidth}
    \vspace{-2em}
