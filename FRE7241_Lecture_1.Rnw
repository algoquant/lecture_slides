...
\subtitle{Lecture\#1, Spring 2021}
...
The graduate assistant (GA) will be Sumit Sethi \href{mailto:sumit.sethi@nyu.edu}{sumit.sethi@nyu.edu}.\\
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The extractor (accessor) functions \texttt{Cl()}, \texttt{Vo()}, etc., from package \emph{quantmod}, extract columns from \emph{OHLC} data.
      \vskip1ex
...
summary(microbenchmark(
  ...
  p_min =do.call(pmin.int,
            lapply(seq_along(mat_rix[1, ]),
                   function(in_dex) mat_rix[, in_dex])),
  as_data_frame =do.call(pmin.int, as.data.frame.matrix(mat_rix)),
  times=10))[, c(1, 4, 5)]  # end microbenchmark summary
      @
  \end{columns}
\end{block}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \end{displaymath}
      Where $\sigma$ is the volatility of asset returns, and $W_t$ is a \emph{Wiener process}, with $\mathrm{d} W_t$ following the standard normal distribution $\phi(0, \sqrt{\mathrm{d}t})$.
      \vskip1ex
...
do.call(rbind, strsplit(colnames(price_s[, 1:4]), 
                        split="[.]"))[, 1]
