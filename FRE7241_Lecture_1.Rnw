...
\subtitle{Lecture\#1, Spring 2026}
...
%%%%%%%%%%%%%%%
\subsection{Algorithmic Portfolio Management}
\begin{frame}[t]{\subsecname}
\vspace{-1em}
\begin{block}{What is Algorithmic Portfolio Management?}
\begin{itemize}
  \item \emph{Algorithmic portfolio management} applies rules (algorithms) to manage investment portfolios to achieve its investment objectives.
  \item The portfolios are usually rebalanced less frequently (daily, weekly, or monthly.)
  \item The trades are usually executed automatically using computer programs.
  \item \emph{Systematic trading} also uses algorithms to trade automatically, but at a higher (intraday) frequency than portfolio management.
  \item \emph{Algorithmic trading} applies algorithms to lower the transaction costs of trade execution.    The algorithms often break large orders into smaller lots, and submit them incrementally.
  \item \emph{Discretionary trading} is when a manager makes buy and sell decisions based on their own subjective analysis, rather than following predefined rules. 
\end{itemize}
\end{block}

\begin{block}{Benefits of Algorithmic Portfolio Management}
\begin{itemize}
  \item Follows emprirical studies and simulations using historical data.
  \item Maintains risk discipline.
  \item Avoids human cognitive biases and emotions.
  \item Automates data analysis and decision making.
  \item Provides speed by processing large amounts of data and executing trades automatically.
  \item Diversifies risk across multiple assets and strategies.
\end{itemize}
\end{block}
...
%%%%%%%%%%%%%%%
\subsection{Types of Systematic (Algorithmic) Strategies}
\begin{frame}[t]{\subsecname}
\vspace{-1em}
\begin{block}{Examples of Systematic (Algorithmic) Strategies}
\begin{itemize}
  \item \emph{Trend following}: Buy stocks with rising prices and short stocks with falling prices.
  \item \emph{Stock momentum}: Buy stocks with higher relative past returns and short stocks with lower returns.
  \item \emph{Portfolio momentum}: Buy portfolios with high past returns and short portfolios with low returns.
  \item \emph{Statistical arbitrage}: Buy cheap stocks and sell expensive stocks.
  \item \emph{Volatility arbitrage}: Sell high implied volatility and buy low realized volatility.
\end{itemize}
\end{block}

\begin{block}{Risk Characteristics of Systematic Strategies}
    Systematic strategies can be viewed as being either \emph{long volatility} or \emph{short volatility}.
    \vskip1ex
    Strategies which are long volatility, profit when prices continue moving in the same direction (when prices trend).\\
    Strategies which are short volatility, profit when prices are range-bound (when prices mean-revert).
    \vskip1ex
    Buying options is an example of a \emph{long volatility} strategy, while selling options is an example of a \emph{short volatility} strategy.
    \vskip1ex
    Trend following and momentum are examples of long volatility strategies.  
    Because they profit when prices trend in the same direction. 
    \vskip1ex
    Statistical arbitrage and volatility arbitrage are examples of short volatility strategies.
    Because they profit when prices mean-revert and stay within a price range.
\end{block}
...
The graduate assistant (GA) will be Chintan Ruparel \href{mailto:cr3745@nyu.edu}{cr3745@nyu.edu}.\\
...
\begin{block}{Introductory Slides}
...
\texttt{R\_environment.pdf}, \texttt{data\_management.pdf}, \texttt{data\_structures.pdf}, \texttt{expressions.pdf}, \texttt{packages.pdf}, \texttt{functions.pdf}, \texttt{plotting.pdf}\\
From the 
...
\begin{block}{Required}
  \begin{itemize}[]
    ...
    \item Download the introductory slides from the 
\href{https://drive.google.com/drive/folders/1abep6n9Jgx4IPhMKXbpYZHAySgLXq1SS}{\color{blue}{Share Drive}},    and use them as references:\\
\texttt{R\_environment.pdf}, \texttt{data\_management.pdf}, \texttt{data\_structures.pdf}, \texttt{expressions.pdf}, \texttt{packages.pdf}, \texttt{functions.pdf}, \texttt{plotting.pdf}\\
    \item Read about \emph{bootstrap simulation} from the files \texttt{bootstrap\_technique.pdf} and \texttt{doBootstrap\_primer.pdf},
    \item Study the 
...
\begin{block}{Recommended}
  \begin{itemize}[]
    \item Download and read the \texttt{R} Cheat Sheets
    \href{https://posit.co/resources/cheatsheets/}{from here.} 
