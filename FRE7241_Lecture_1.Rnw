...
parSapply(compclust, c("VTI", "IEF", "DBC"),
          function(symbol)
...
# Perform parallel loop under Windows
output <- parLapply(compclust, 1:70, rnorm, n=100)
sum(unlist(output))
# Stop R processes over cluster under Windows
stopCluster(compclust)
# Perform parallel loop under Mac-OSX or Linux
...
nboot <- 10000
bootd <- parLapply(compclust, 1:nboot,
...
bootd <- parLapply(compclust, 1:1000,
  function(x, desm) {
    samplev <- sample.int(nsimu, replace=TRUE)
    desm <- desm[samplev, ]
    cov(desm[, 1], desm[, 2])/var(desm[, 2])
