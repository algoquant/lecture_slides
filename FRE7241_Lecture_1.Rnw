...
\subtitle{Lecture\#1, Spring 2022}
...
# Extract symbolv from rutils::etf_env
prices <- mget(symbolv, envir=rutils::etf_env)
...
# Copy prices into etf_env
etf_env$etf_list <- etf_list
# Or
assign("prices", prices, envir=etf_env)
# Save to .RData file
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      Loading a package means attaching the package \emph{namesvpace} to the \emph{search path}, which allows \texttt{R} to call the package functions and data.
      \vskip1ex
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \end{itemize}
      When a package is loaded using \texttt{library()} or \texttt{require()}, its \emph{namesvpace} is attached to the search path.
    \column{0.5\textwidth}
      \vspace{-1em}
        <<echo=(-1)>>=
rm(list=ls())
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      Packages may be loaded without their \emph{namesvpace} being attached to the search path.
      \vskip1ex
      When packages are loaded, then packages they depend on are also loaded, but their \emph{namesvpaces} aren't necessarily attached to the search path.
      \vskip1ex
      The function \texttt{loadedNamespaces()} lists all loaded \emph{namesvpaces}, including those that aren't on the search path.
      \vskip1ex
      The function \texttt{search()} returns the current search path for \texttt{R} objects.
      \vskip1ex
      \texttt{search()} returns many package \emph{namesvpaces}, but not all the loaded \emph{namesvpaces}.
    \column{0.5\textwidth}
      \vspace{-1em}
        <<echo=TRUE,eval=FALSE>>=
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      \texttt{sessionInfo()} lists those packages as "loaded via a \emph{namesvpace} (and not attached)"
    \column{0.5\textwidth}
      \vspace{-1em}
        <<echo=TRUE,eval=FALSE>>=
# get session info,
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \begin{itemize}
        \item objects from \emph{not attached} \emph{namesvpaces},
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      Their search path starts in the package \emph{namesvpace}, then the global environment and then finally the regular search path.
      \vskip1ex
...
parSapply(cluster, c("VTI", "IEF", "DBC"),
          function(symbol)
            NROW(index(get(symbol, envir=rutils::etf_env))))
# zoo function referenced using "::" in child process:
parSapply(cluster, c("VTI", "IEF", "DBC"),
          function(symbol)
            NROW(zoo::index(get(symbol, envir=rutils::etf_env))))
...
summary(microbenchmark(
  ...
  quan_tile = quantile(datav, probs=confl),
...
library(rutils)
prices <- quantmod::Cl(rutils::etf_env$VTI)
startd <- as.numeric(prices[1, ])
returns <- rutils::diff_it(log(prices))
...
sum(datav < quan_tile)/nrows
# Generate importance sample
lambdav <- (-1.5)  # Tilt parameter
data_tilt <- datav + lambdav  # Tilt the random numbers
# Cumulative probability from importance sample
sum(data_tilt < quan_tile)/nrows
weights <- exp(-lambdav*data_tilt + lambdav^2/2)
sum((data_tilt < quan_tile)*weights)/nrows
# Bootstrap of standard errors of cumulative probability
nboot <- 1000
boot_data <- sapply(1:nboot, function(x) {
  datav <- rnorm(nrows)
  naivemc <- sum(datav < quan_tile)/nrows
  datav <- (datav + lambdav)
  weights <- exp(-lambdav*datav + lambdav^2/2)
  isample <- sum((datav < quan_tile)*weights)/nrows
...
va_r <- data_tilt[findInterval(confl, cumprob)]
sum((data_tilt < va_r)*data_tilt*weights)/sum((data_tilt < va_r)*weights)
...
boot_data <- sapply(1:nboot, function(x) {
  ...
  cumprob <- cumsum(weights)/nrows
  va_r <- data_tilt[findInterval(confl, cumprob)]
  isample <- sum((data_tilt < va_r)*data_tilt*weights)/sum((data_tilt < va_r)*weights)
  c(naivemc=naivemc, importmc=isample)
}) # end sapply
apply(boot_data, MARGIN=1,
  function(x) c(mean=mean(x), sd=sd(x)))
...
nrows <- 1000
pro_b <- 0.1
datav <- rbinom(n=nrows, size=1, pro_b)
head(datav, 33)
fre_q <- sum(datav)/nrows
# Tilted binomial sample
lambdav <- 5
p_tilted <- lambdav*pro_b/(1 + pro_b*(lambdav - 1))
weigh_t <- (1 + pro_b*(lambdav - 1))/lambdav
...
nboot <- 1000
boot_data <- sapply(1:nboot, function(x) {
  c(naivemc=sum(rbinom(n=nrows, size=1, pro_b))/nrows,
...
# Or
path_weights <- exp(-lambdav*colSums(data_tilt) + nrows*lambdav^2/2)
...
prices <- eapply(sp500_env, quantmod::Cl)
...
datav <- cbind(indeks[index(etf_env$VTI)], etf_env$VTI[, 4])
...
b_w <- 10*mad(rutils::diff_it(returns, lagg=10))
...
ma_d <- mad(returns)
# b_w <- mad(rutils::diff_it(returns))
...
chart.CumReturns(returns, lwd=2, ylab="",
  legend.loc="topleft", main="ETF Cumulative Returns")
    ...
    \column{0.5\textwidth}
    \vspace{-1em}
      \includegraphics[width=0.45\paperwidth]{figure/perf_analytics_cumrets.png}
  \end{columns}
\end{block}
...
dygraphs::dygraph(datav, main="VTI Drawdowns") %>%
  dyAxis("y", label=colnamev[1], independentTicks=TRUE) %>%
  dyAxis("y2", label=colnamev[2], valueRange=c(min(datav[, "Drawdowns"]), 5), independentTicks=TRUE) %>%
...
text(x=risk_ret$Skewness, y=risk_ret$Kurtosis,
      ...
      @
    \column{0.5\textwidth}
      \includegraphics[width=0.45\paperwidth]{figure/etf_skewsharp.png}
...
\begin{block}{Required}
  \begin{itemize}[]
    \item Study all the lecture slides in \texttt{FRE7241\_Lecture\1.pdf}, and run all the code in \texttt{FRE7241\_Lecture\1.R},
  \end{itemize}
\end{block}
