...
\begin{block}{Graduate Assistant}
  The graduate assistant (GA) will be Shu Ye \href{shu.ye@nyu.edu}{shu.ye@nyu.edu}\\
...
dygraph(x_ts, main="VTI and IEF prices") %>%
  ...
  dyRangeSelector()
    ...
    \column{0.5\textwidth}
      ...
      The \emph{dygraphs} package in \texttt{R} is an interface to the \emph{dygraphs} \texttt{JavaScript} charting library,
      \vskip1ex
      Interactive \emph{dygraphs} plots require running \texttt{JavaScript} code, which can be embedded in \texttt{HTML} documents, and displayed by web browsers,
      \vskip1ex
      But \emph{pdf} documents can't run \texttt{JavaScript} code, so they can't display interactive \emph{dygraphs} plots,
  \end{columns}
\end{block}
