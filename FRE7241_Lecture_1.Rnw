...
\begin{block}{Teaching Assistant}
  The teaching assistant (TA) will be announced later.\\
...
%%%%%%%%%%%%%%%
\subsection{Operations on \texttt{zoo} Time Series}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The method \texttt{diff.zoo()} returns the difference between a \texttt{zoo} time series and its proper lagged version from the past, given a positive \emph{"lag"} value,
      \vskip1ex
...
%%%%%%%%%%%%%%%
\subsection{Sequential Joining \texttt{zoo} Time Series}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
...
%%%%%%%%%%%%%%%
\subsection{Merging \texttt{zoo} Time Series}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
...
%%%%%%%%%%%%%%%
\subsection{Coercing \texttt{zoo} Time Series Into Class \texttt{ts}}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
...
%%%%%%%%%%%%%%%
\subsection{Coercing \texttt{zoo} Time Series Into Class \texttt{xts}}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
...
%%%%%%%%%%%%%%%
\subsection{Plotting Multiple \texttt{xts} Using Packages \texttt{xts} and \texttt{quantmod}}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The function \texttt{qplot()} is the simplest function in the \texttt{ggplot2} package, and allows creating line and bar plots,
      \vskip1ex
...
%%%%%%%%%%%%%%%
\subsection{Plotting Multiple \texttt{xts} Using Package \texttt{ggplot2}}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      Multiple \texttt{xts} time series can be plotted using the function \texttt{ggplot()} from package \texttt{ggplot2},
      \vskip1ex
      But \texttt{ggplot2} functions don't accept time series objects, so time series must be first formatted into \texttt{data frames},
      \vspace{-1em}
...
ggplot(data=data_frame,
  ...
  theme(  # add legend and title
    ...
    \column{0.5\textwidth}
      ...
      Time series with multiple columns must be reshaped into a single column, which can be performed using the function \texttt{melt()} from package \texttt{reshape2},
  \end{columns}
\end{block}
...
%%%%%%%%%%%%%%%
\subsection{Interactive Time Series Plots Using Package \texttt{dygraphs}}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The function \texttt{dygraph()} from package \texttt{dygraphs} creates interactive, zoomable plots from \texttt{xts} time series,
      \vskip1ex
      The function \texttt{dyOptions()} adds options (like colors, etc.) to a \texttt{dygraph} plot,
      \vskip1ex
      The function \texttt{dyRangeSelector()} adds a date range selector to the bottom of a \texttt{dygraphs} plot,
...
dygraph(x_ts, main="VTI and IEF prices") %>%
  ...
  dyRangeSelector()
    ...
    \column{0.5\textwidth}
      ...
      The \texttt{dygraphs} package in \texttt{R} is an interface to the \texttt{dygraphs} \texttt{JavaScript} charting library,
      \vskip1ex
      Interactive \texttt{dygraphs} plots require running \texttt{JavaScript} code, which can be embedded in \texttt{HTML} documents, and displayed by web browsers,
      \vskip1ex
      But \emph{pdf} documents can't run \texttt{JavaScript} code, so they can't display interactive \texttt{dygraphs} plots,
  \end{columns}
\end{block}
...
%%%%%%%%%%%%%%%
\subsection{Interactive Time Series Plots Using Package \texttt{plotly}}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The function \texttt{plot\_ly()} from package \texttt{plotly} creates interactive plots from data residing in \texttt{data frames},
      \vskip1ex
      The function \texttt{add\_trace()} adds elements to a \texttt{plotly} plot,
      \vskip1ex
      The function \texttt{layout()} modifies the layout of a \texttt{plotly} plot,
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.4\textwidth}
      ...
      \vskip1ex
      The function \texttt{.subset\_xts()} allows fast subsetting of \texttt{xts} time series, which is at least three times faster than the bracket \texttt{"[]"} notation,
    \column{0.6\textwidth}
      \vspace{-1em}
      <<echo=(-(1))>>=
library(xts)  # load package xts
