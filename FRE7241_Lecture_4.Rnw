...
\subtitle{Lecture\#4, Spring 2023}
...
confun <- function(actual, fcast, threshold) {
    conf <- table(actual, (fcast < threshold))
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      <<echo=TRUE,eval=FALSE>>=
# Simulate in-sample VTI strategy
posit <- rep(NA_integer_, NROW(retp))
posit[1] <- 0
posit[forecastops] <- (-1)
posit[forecastbot] <- 1
posit <- zoo::na.locf(posit)
...
%%%%%%%%%%%%%%%
\subsection{Forecasting Returns Using Logistic Regression}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
...
%%%%%%%%%%%%%%%
\subsection{Logistic Forecasting Returns Strategy In-sample}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
...
objfun <- function(weights, retp) {
  retp <- retp %*% weights
  -sum(retp^2) + 1e7*(1 - sum(weights^2))^2
}  # end objfun
...
retspca <- xts::xts(retp %*% pcad$rotation, order.by=dates)
round(cov(retspca), 3)
  ...
  \begin{columns}[T]
    ...
    \column{0.5\textwidth}
      ...
      The expected value of the \emph{RSS} is equal to the \emph{degrees of freedom} $(n-2)$ times the variance $\sigma^2_\varepsilon$ of the \emph{residuals} $\varepsilon_i$: $\mathbb{E}[RSS] = (n-2) \sigma^2_\varepsilon$.
        <<echo=TRUE,eval=TRUE>>=
# Calculate de-meaned explanatory (predv) and response vectors
predzm <- predictor - mean(predv)
respzm <- response - mean(respv)
...
points(x=predv, y=model$fitted.values, pch=16, col="blue")
      @
    \column{0.5\textwidth}
      \includegraphics[width=0.45\paperwidth]{figure/reg_scatter_plot.png}
        <<echo=TRUE,eval=FALSE>>=
# Plot response without noise
lines(x=predv, y=(respv-noise), col="red", lwd=3)
...
# Extract residuals
datav <- cbind(predv, model$residuals)
...
alphasd <- residsd*
  sqrt(1/nrows + mean(predv)^2/sum(predzm^2))
      @
  \end{columns}
\end{block}
...
respv <- (-3 + 2*predictor + rnorm(nrows, sd=8))
model <- lm(formulav)  # Perform regression
...
# Define explanatory (predv) and response variables
    ...
    respv <- (1 + 0.2*predictor +
      rnorm(NROW(predv), sd=stdev))
    reg_stats(data.frame(predv, respv))
    }))
# Plot in loop
par(mfrow=c(NCOL(statsmat), 1))
for (it in 1:NCOL(statsmat)) {
...
invpred <- MASS::ginv(predv)
# Calculate the influence matrix
influencem <- predv %*% invpred
# Plot the leverage vector
ordern <- order(predv[, 2])
plot(x=predv[ordern, 2], y=diag(influencem)[ordern],
...
fittedv <- drop(predv %*% betas)
residuals <- drop(respv - fittedv)
degf <- (NROW(predv) - NCOL(predv))
residsar <- sqrt(sum(residuals^2)/degf)
fitcovar <- residsar*influencem
...
fittedv <- lapply(1:50, function(it) {
  ...
  respv <- response + rnorm(nrows, sd=1.0)
...
predictor2 <- MASS::ginv(crossprod(predv))
...
predsd <- sqrt(predvn %*% predictor2 %*% t(predvn))
predv <- cbind(
  prediction=drop(predvn %*% betas),
  stddev=diag(residsar*predsd))
# Or: Perform loop over predictorn
predv <- apply(predvn, MARGIN=1, function(predv) {
  # Calculate predicted values
  prediction <- predv %*% betas
  # Calculate standard deviation
  predsd <- sqrt(t(predv) %*% predictor2 %*% predv)
  predictsd <- residsar*predsd
...
ncols <- 5
set.seed(1121)  # initialize random number generator
predv <- matrix(rnorm(nrows*ncols), ncol=ncols)
# Add column names
colnames(predv) <- paste0("col", 1:ncols)
...
# Calculate the regression alpha
alpha <- mean(respv) - sum(colSums(predv)*betas)/nrows
...
sapply(residuals %*% predv, all.equal, target=0)
# Residuals are orthogonal to the fitted values
all.equal(sum(residuals*fittedv), target=0)
# Sum of residuals is equal to zero
...
all.equal(fitted_zm,
  model$fitted.values - mean(respv),
  check.attributes=FALSE)
# Calculate the residuals
respzm <- response - mean(respv)
...
respv <- 0.2*predictor + omitv + 0.2*rnorm(30)
...
# Inverse of predictor matrix squared
predictor2 <- MASS::ginv(crossprod(predv))
# predictor2 <- t(predv) %*% predictor
# Variance of residuals
residsar <- sum(residuals^2)/degf
# Calculate covariance matrix of betas
beta_covar <- residsar*predictor2
# Round(beta_covar, 3)
...
all.equal(MASS::ginv(crossprod(predv)),
          invpred %*% t(invpred))
      @
  \end{columns}
\end{block}
...
model <- lm(formulav, data=data.frame(cbind(respv, predv)))
modelsum <- summary(model)
...
degf <- (nrows - ncols)
# Adjusted R-squared
rsquared_adj <- (1-sum(residuals^2)/degf/var(respv))
...
legend("topright", inset=0.05, title="degrees of freedom",
       labelv, cex=0.8, lwd=2, lty=1, col=colorv)
      @
  \end{columns}
\end{block}
...
predictor_trans <- predv %*% trans_mat
# Calculate the influence matrix
influence_trans <- predictor_trans %*% MASS::ginv(predv_trans)
