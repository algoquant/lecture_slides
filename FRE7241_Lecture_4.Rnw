...
pnls <- rutils::diff_it(closep)*position_s
...
\begin{block}{}
  \begin{columns}[T]
    ...
    \column{0.6\textwidth}
      \vspace{-1em}
      <<echo=TRUE,eval=FALSE>>=
simu_ewma <- function(ohlc, lambdav=0.01, wid_th=351, bid_offer=0.001, tre_nd=1) {
  nrows <- NROW(ohlc)
...
lambdavs <- seq(from=1e-5, to=0.05, by=0.01)
...
pnls <- lapply(lambdavs, function(lambdav) {
  ...
  startd + cumsum(simu_ewma(ohlc=ohlc,
    lambdav=lambdav, wid_th=wid_th)[, "pnls"])
})  # end lapply
...
returns <- mclapply(lambdavs, function(lambdav) {
  library(quantmod)
  # Simulate EWMA strategy and calculate returns
  startd + cumsum(simu_ewma(ohlc=ohlc,
    lambdav=lambdav, wid_th=wid_th)[, "pnls"])
...
pnls <- lapply(lambdavs, function(lambdav) {
  ...
  startd + cumsum(simu_ewma(
    ohlc=ohlc, lambdav=lambdav, wid_th=wid_th, tre_nd=(-1))[, "pnls"])
})  # end lapply
pnls <- do.call(cbind, pnls)
colnames(pnls) <- paste0("lambda=", lambdavs)
# Plot EWMA strategies with custom line colors
...
plot(x=lambdavs, y=sharper, t="l",
     ...
     as function of the decay parameter lambda")
revert_returns <- rutils::diff_it(pnls)
...
close_rets <- rutils::diff_it(closep)
