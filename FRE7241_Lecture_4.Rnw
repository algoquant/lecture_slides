...
%%%%%%%%%%%%%%%
\subsection{Defining Non-overlapping Time Intervals}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      Non-overlapping \emph{intervals} can be defined by a vector of \emph{endpoints} and a vector of \emph{startpoints}.
      \vskip1ex
      If the \emph{startpoints} are defined as the single-period lag of \emph{endpoints}, then the \emph{intervals} are \emph{contiguous} (each \emph{startpoint} is the \emph{endpoint} of the previous interval).
      \vskip1ex
      If the \emph{startpoints} are defined as the single-period lag of \emph{endpoints} plus \texttt{1}, then the \emph{intervals} are \emph{exclusive} (each \emph{startpoint} is the previous \emph{endpoint} plus \texttt{1}).
    \column{0.5\textwidth}
      \vspace{-1em}
      <<echo=TRUE,eval=FALSE>>=
...
tail(cbind(in_nov, residual_s))
    ...
    \column{0.5\textwidth}
      ...
      Accurate forecasting requires knowing the order of the \emph{AR(p)} process and its coefficients.
      <<echo=TRUE,eval=FALSE>>=
# Plot with legend
...
%%%%%%%%%%%%%%%
\subsection{Rolling Forecasting Using Autoregressive Models}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The coefficients of the \emph{AR(p)} process can be calculated over a rolling look-back interval, and then used for forecasting.
      <<echo=TRUE,eval=FALSE>>=
len_gth <- 1e3
# Simulate AR(3) time series using filter()
co_eff <- c(0.1, 0.39, 0.5)
...
ms_e <- mean(fore_casts-se_ries)^2
    ...
    \column{0.5\textwidth}
      ...
      The mean squared error (MSE) of a forecasting is the average of the squared differences between the forecasts minus the actual values.
      <<echo=TRUE,eval=FALSE>>=
# Plot with legend
plot.default(se_ries, 
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The mixture of two normal distributions produces a distribution with fat tails (lepto-kurtosis).
      \vskip1ex
