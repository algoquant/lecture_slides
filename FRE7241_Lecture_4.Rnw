...
\subtitle{Lecture\#4, Spring 2019}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      The accuracy of the \emph{AR(p)} forecasting model depends on the two \emph{meta-parameters}: \texttt{look\_back} (the length of look-back interval) and \texttt{p} (the order of the \emph{AR(p)} process).
      \vskip1ex
      The two \emph{meta-parameters} can be chosen by minimizing the \emph{MSE} of the model forecasts in a \emph{back-test} simulation.
      <<echo=TRUE,eval=FALSE>>=
# Mean squared error
