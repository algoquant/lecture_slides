...
regs <- HighFreq::roll_reg(respv=closep, predm=datev, lookb=lookb, controlv=controlv)
...
posv <- rutils::lagit(rutils::roll_sum(sign(retp), lookb=2))/2
pnls <- (-retp*posv)
      @
    \column{0.5\textwidth}
      \vspace{-1em}
...
sim_fcasts <- function(lookb=100, ordern=5, fixedlb=TRUE) {
  ...
  fcasts <- sapply((lookb+1):nrows, function(tday) {
    # Define rolling look-back range
    if (fixedlb)
      startp <- max(1, tday-lookb) # Fixed look-back
    else
...
lookbs <- seq(20, 600, 40)
fcasts <- parLapply(cluster, lookbs, sim_fcasts, ordern=6)
# Perform parallel bootstrap under Mac-OSX or Linux
fcasts <- mclapply(lookbs, sim_fcasts, ordern=6, mc.cores=ncores)
      @
    \column{0.5\textwidth}
      \vspace{-1em}
...
lookb <- lookbs[which.min(mse[, 1])]
# Plot forecasting MSE
plot(x=lookbs, y=mse[, 1],
