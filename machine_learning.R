...
solvecd <- function(respv, predm, maxit = 1000, tol = 1e-6) {
...
# # Calculate bias and variance for different lambda values
# lambdav <- c(0, 10, 100, 1000, 10000)
# coeffm <- sapply(lambdav, function(lambdaf) {
#   invmat = MASS::ginv(pred2 + lambdaf*unitmat)
#   bias = - lambdaf * invmat %*% betav
#   var = covm * invmat %*% pred2 %*% invmat
#   list(bias=bias, variance=diag(var))
# }) end sapply
# rownames(coeffm) <- colnames(predm)
# colnames(coeffm) <- paste("lambda", lambdav, sep="=")
# round(coeffm, 4)
...
# # Calculate LASSO regression coefficients
# 
# # Soft-thresholding function for LASSO
# # Applies the rule: sign(z) * max(|z| - lambda, 0)
...
# 
#   # Standardize predictors (mean=0, sd=1) and center y
#   X <- scale(X)
...
#     beta_old <- beta
# 
#     # Loop through each predictor (coordinate descent)
#     for (j in 1:p) {
#       # Compute partial residual: y minus all other predictors except j
#       r_j <- y - X %*% beta + X[, j] * beta[j]
# 
#       # Compute z_j = correlation between X_j and residual
#       z_j <- sum(X[, j] * r_j)
# 
#       # Update beta_j using soft-thresholding
#       beta[j] <- soft_threshold(z_j / n, lambda) / (sum(X[, j]^2) / n)
#     }
# 
#     # Check convergence: if change in coefficients is very small, stop
...
# }
# 
# # Example usage with Credit dataset
# library(ISLR)
# data(Credit)
# 
# # Select predictors and response
# X <- as.matrix(Credit[, c("Age", "Cards", "Education", "Income", "Limit", "Rating")])
# y <- Credit$Balance
# 
# # Set lambda (penalty term)
# lambda <- 0.1
# 
# # Compute LASSO coefficients
# lasso_coeffs <- lasso_cd(X, y, lambda)
# lasso_coeffs
# 
# 
# lambdaf <- 1000
# unitmat<- diag(ncol(predm))
# coeffl <- drop(MASS::ginv(t(predm) %*% predm + lambdaf*unitmat) %*% t(predm) %*% respv)
# names(coeffl) <- colnames(predm)
# # Calculate the RRS and the penalty term
# rrs <- sum((respv - predm %*% coeffl)^2)
# penalty <- lambdaf*sum(coeffl^2)
# c(RSS=rrs, Penalty=penalty, Objective=rrs + penalty)
