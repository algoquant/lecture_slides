...
\begin{block}{}
  \begin{columns}[T]
    \column{0.4\textwidth}
      ...
      \vskip1ex
      If the object doesn't contain a string, then \texttt{assign()} throws an error,
    \column{0.6\textwidth}
      \vspace{-1em}
      <<>>=
...
\begin{block}{}
  \begin{columns}[T]
    ...
    \column{0.5\textwidth}
      \vspace{-2em}
        <<>>=
vec_var1 <- rnorm(1000000)
vec_var2 <- rnorm(1000000)
big_vector <- numeric(1000000)
system.time(  # sum vectors using "for" loop
  for(i in 1:length(vec_var1)) {
    big_vector[i] <- vec_var1[i] + vec_var2[i]
  }  # end for
)  # end system.time
# sum vectors using vectorized "+"
system.time(big_vector <- vec_var1 + vec_var2)
