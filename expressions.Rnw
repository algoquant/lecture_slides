...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The function \texttt{quote()} accepts \texttt{symbols} or \texttt{expressions}, and returns them without evaluating them,
      \vskip1ex
      The function \texttt{substitute()} substitutes object names (that may be part of \texttt{expressions}), with their corresponding values, and returns an \texttt{expression},
      \vskip1ex
      \texttt{substitute()} looks up the object names in either named \texttt{lists} (symbol-value pairs) or \texttt{environments}, and evaluates the objects in them,
      \vskip1ex
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      \texttt{deparse()} converts \texttt{symbols} and \texttt{expressions} into character strings,
      \vskip1ex
      \texttt{quote()} accepts \texttt{symbols} and \texttt{expressions}, and returns them without evaluating them,
      \vskip1ex
      The function \texttt{substitute()} substitutes object names (that may be part of \texttt{expressions}), with their corresponding values, and returns an \texttt{expression},
    \column{0.5\textwidth}
      \vspace{-1em}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The square braces (brackets) \texttt{"[]"} operator is reserved for subsetting (dereferencing) the elements of vectors, matrices, and lists,
    \column{0.5\textwidth}
      \vspace{-1em}
        <<echo=(-(1:1))>>=
rm(list=ls())
...
%%%%%%%%%%%%%%%
\subsection{draft: The \protect\emph{split-apply-combine} Procedure ChickWeight}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
