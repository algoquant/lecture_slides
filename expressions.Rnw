...
\begin{block}{}
  \begin{columns}[T]
    \column{0.4\textwidth}
      ...
      \vskip1ex
      \texttt{assign()} looks for the variable name in the specified \texttt{environment}, and assigns a value to it,
      \vskip1ex
      If \texttt{assign()} can't find the variable name, then it creates it,
      \vskip1ex
      \texttt{assign()} expects a character string as its argument, 
      \vskip1ex
      If a variable name is passsed to \texttt{assign()}, then it evaluates that variable to get the string it contains,
      \vskip1ex
      If the variable doesn't contain a string, then \texttt{assign()} throws an error,
    \column{0.6\textwidth}
      \vspace{-1em}
      <<>>=
my_var <- 1  # create new variable
assign(x="my_var", value=2)  # assign value to existing variable
my_var
rm(my_var)  # remove my_var
assign(x="my_var", value=3)  # create new variable from name
my_var
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.4\textwidth}
      \texttt{assign()} allows creating new variables from lists or vectors of names (character strings), such as column names,
      \vskip1ex
    \column{0.6\textwidth}
      \vspace{-1em}
      <<>>=
