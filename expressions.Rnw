...
\begin{block}{}
  \begin{columns}[T]
    ...
    \column{0.5\textwidth}
      <<>>=
my_var1 <- 3  # "<-" and "=" are valid assignment operators
my_var1
my_var1 = 3
my_var1
...
rm(list=ls())
my_var1 <- -1
if (my_var1) {  # zero is FALSE, all other numbers are TRUE
  my_var2 <- 4
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      \texttt{R} has a function \texttt{ifelse()} for performing \texttt{if-else} statements on vectors,
      \vskip1ex
      \texttt{ifelse()} performs element-wise over the standard if-then-else construct is that
it is vectorized, thus potentially much faster.
      \texttt{R} has a function \texttt{ifelse()} for vectorized \texttt{if-else} statements,
      \vskip1ex
