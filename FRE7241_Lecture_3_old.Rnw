...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      \vspace{-2em}
      <<drawdown_plot,echo=(-1),eval=TRUE,fig.show='hide'>>=
library(quantmod)
library(PerformanceAnalytics)
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      \vspace{-1em}
      <<corr_plot,echo=(-1),eval=TRUE,fig.show='hide'>>=
library(quantmod)
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      \vspace{-1em}
      <<pca_plot,echo=(-(1:2)),eval=TRUE,fig.height=5,fig.show='hide'>>=
...
\begin{block}{Recommended}
  \begin{itemize}[]
    ...
    \item Read chapters 4, 5, 9-10: \citetitle{ruppertbook}
  \end{itemize}
\end{block}
