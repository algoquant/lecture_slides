...
datestart <- max(dates[(dates < date_min) & (draw_downs == 0)])
date_end <- min(dates[(dates > date_min) & (draw_downs == 0)])
...
dygraphs::dygraph(datav, main="VTI Drawdowns") %>%
  ...
  dyEvent(datestart, "start drawdown", col="blue") %>%
...
va_r <- quantile(returns, probs=confl)
# Or by sorting
...
# high_skew <- namesv[(nrows-51):nrows]
# Calculate returns and log prices
...
sapply(colnames(datav), function(name) {
  ...
  c_var <- mean(x[x < va_r])
  factorv <- 1
  if (name == colnames(datav)[2]) {factorv <- hold_p}
  sqrt(252/factorv)*mean(x)/c(Sharpe=stddev, Dowd=-va_r, DowdC=-c_var)
})  # end sapply
...
cum_rets <- (cum_rets -
  c(rep_len(0, look_back), cum_rets[1:(nrows-look_back)]))
cum_rets2 <- cumsum(returns^2)
cum_rets2 <- (cum_rets2 -
  c(rep_len(0, look_back), cum_rets2[1:(nrows-look_back)]))
variance2 <- (cum_rets2 - cum_rets^2/look_back)/(look_back-1)
all.equal(variance[-(1:look_back)], as.numeric(variance2)[-(1:look_back)])
# Same, using package rutils
cum_rets <- rutils::roll_sum(returns, look_back=look_back, min_obs=1)
cum_rets2 <- rutils::roll_sum(returns^2, look_back=look_back, min_obs=1)
variance2 <- (cum_rets2 - cum_rets^2/look_back)/(look_back-1)
...
variance <- roll::roll_var(returns,
  weights=rev(weightv), width=look_back)
colnames(variance) <- "VTI.variance"
class(variance)
head(variance)
sum(is.na(variance))
...
variance <- HighFreq::roll_var_ohlc(log(sp_y), method="yang_zhang",
  look_back=look_back)
# Plot range volatility
variance <- xts:::xts(sqrt(variance), order.by=index(sp_y))
...
vx_x <- na.omit(rutils::etfenv$prices$VXX)
dates <- zoo::index(vx_x)
look_back <- 41
vx_x <- log(vx_x)
# Calculate rolling VTI volatility
closep <- get("VTI", rutils::etfenv)[dates]
closep <- log(closep)
volat <- sqrt(HighFreq::roll_var_ohlc(ohlc=closep, look_back=look_back, scalit=FALSE))
volat[1:look_back] <- volat[look_back+1]
# Plot dygraph of VXX and VTI volatility
datav <- cbind(vx_x, volat)
...
betav <- drop(cov(vx_x, volat)/var(volat))
alpha <- drop(mean(vx_x) - betav*mean(volat))
# Calculate regression residuals
fit_ted <- (alpha + betav*volat)
residuals <- (vx_x - fit_ted)
...
likeli_hood <- function(om_ega, alpha, betav) {
  # Estimate the rolling variance from the returns
  variance <- numeric(nrows)
  variance[1] <- om_ega/(1-alpha-betav)
  for (i in 2:nrows) {
...
likeli_hood(om_ega, alpha, betav)
...
library(microbenchmark)
summary(microbenchmark(
  Rcode=likeli_hood(om_ega, alpha, betav),
...
likeli_hood <- function(alpha, betav) {
...
lik_mat <- sapply(alphas, function(alpha) sapply(betas,
  function(betav) likeli_hood(alpha, betav)))
...
likeli_hood <- function(x, returns) {
  alpha <- x[1]; betav <- x[2]; om_ega <- x[3]
  -HighFreq::lik_garch(omega=om_ega, alpha=alpha, beta=betav, returns=returns)
}  # end likeli_hood
# Initial parameters
par_init <- c(alpha=0.2, beta=0.4, omega=variance/0.2)
...
dygraphs::dygraph(xts::xts(sqrt(variance), index(returns)),
...
ohlc, method="yang_zhang", scalit=FALSE))
})  # end sapply
# Analyze bootstrapped variance
boot_data <- t(boot_data)
head(boot_data)
...
addEventLines(xts(rep("endpoint", NROW(endp)-1), index(closep)[endp]),
        col="red", lwd=2, pos=4)
# Or
plot_theme <- chart_theme()
...
agg_regate <- function(xtes) c(max=max(xtes), min=min(xtes))
# Perform aggregations over look_backs list
agg_s <- sapply(look_backs,
    function(look_back) agg_regate(closep[look_back])
)  # end sapply
...
# Define aggregation function
agg_regate <- function(xtes)
  c(max=max(xtes), min=min(xtes))
# Perform aggregations over rolling interval
agg_s <- roll_agg(closep, look_back=look_back, FUN=agg_regate)
class(agg_s)
dim(agg_s)
# library(rutils)  # Load package rutils
...
agg_regate <- function(xtes)  max(xtes)
...
agg_s <- apply.rolling(closep, width=look_back, FUN=agg_regate)
...
max_minr <- xts::xts(max_minr, index(closep))
...
max_minarma <- xts::xts(max_minr, index(closep))
max_min <- cbind(TTR::runMax(x=closep, n=look_back),
           TTR::runMin(x=closep, n=look_back))
