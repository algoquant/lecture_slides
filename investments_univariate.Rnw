...
vtis <- rutils::etf_env$returns$VTI
vtis <- na.omit(vtis)
...
costs <- bid_offer*drop(rutils::diff_it(mar_gin))/2
wealth_diff <- drop(rutils::diff_it(wealth))
...
sqrt(252)*sapply(rutils::diff_it(wealth), 
...
for (it in 2:NROW(rets_percent)) {
  ...
  if (sum(abs(wealth[it, ] - sum(wealth[it, ])*weights))/sum(wealth[it, ]) > thresh_old) {
    # cat("Rebalance at:", it, "\n")
...
returns <- na.omit(rutils::etf_env$returns$VTI["2008/2009"])
dates <- index(returns)
nrows <- NROW(returns)
...
thresh_old <- quantile(forecasts, 0.95)
# Calculate confusion matrix in-sample
confu_sion <- table(actual=!top_s, forecast=(forecasts < thresh_old))
confu_sion
# Calculate FALSE positive (type I error)
sum(!top_s & (forecasts > thresh_old))
...
con_fuse(!top_s, forecasts, threshold=thresh_old)
...
vtis <- rutils::diff_it(closep)  # VTI returns
pnls <- vtis*po_s
colnames(pnls) <- "EWMA"
wealth <- cbind(vtis, pnls)
...
pnls_lag <- vtis*po_s
# Determine trade dates right after EWMA has crossed prices
dates <- which(rutils::diff_it(po_s) != 0)
...
lambdavs <- seq(from=0.001, to=0.008, by=0.001)
# Perform lapply() loop over lambdavs
pnls <- lapply(lambdavs, function(lambdav) {
  # Simulate EWMA strategy and calculate returns
  sim_ewma(ohlc=ohlc, lambda=lambdav, look_back=look_back, bid_offer=0, lagg=2)[, "pnls"]
})  # end lapply
pnls <- do.call(cbind, pnls)
colnames(pnls) <- paste0("lambda=", lambdavs)
      @
    \column{0.5\textwidth}
      \vspace{-1em}
...
# Perform parallel loop over lambdavs under Windows
pnls <- parLapply(cluster, lambdavs, function(lambdav) {
  library(quantmod)
  # Simulate EWMA strategy and calculate returns
  sim_ewma(ohlc=ohlc, lambda=lambdav, look_back=look_back)[, "pnls"]
})  # end parLapply
stopCluster(cluster)  # Stop R processes over cluster under Windows
# Perform parallel loop over lambdavs under Mac-OSX or Linux
pnls <- mclapply(lambdavs, function(lambdav) {
  library(quantmod)
  # Simulate EWMA strategy and calculate returns
  sim_ewma(ohlc=ohlc, lambda=lambdav, look_back=look_back)[, "pnls"]
})  # end mclapply
...
plot(x=lambdavs, y=sharper, t="l",
     xlab="lambda", ylab="Sharpe",
     main="Performance of EWMA Trend Following Strategies
     as Function of the Decay Parameter Lambda")
# Find optimal lambda
lambdav <- lambdavs[which.max(sharper)]
      @
    \column{0.5\textwidth}
      \vspace{-1em}
...
ewma_trend <- sim_ewma(ohlc=ohlc, lambda=lambdav, look_back=look_back, bid_offer=0, lagg=2)
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      <<echo=TRUE,eval=FALSE>>=
source("/Users/jerzy/Develop/lecture_slides/scripts/ewma_model.R")
lambdavs <- seq(0.05, 1.0, 0.05)
# Perform lapply() loop over lambdavs
pnls <- lapply(lambdavs, function(lambdav) {
  # Simulate EWMA strategy and calculate returns
  sim_ewma(ohlc=ohlc, lambda=lambdav, look_back=look_back, trend=(-1))[, "pnls"]
})  # end lapply
pnls <- do.call(cbind, pnls)
colnames(pnls) <- paste0("lambda=", lambdavs)
...
ewma_revert <- sim_ewma(ohlc=ohlc, bid_offer=0.0,
  lambda=lambdav, look_back=look_back, trend=(-1))
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      <<echo=TRUE,eval=FALSE>>=
source("/Users/jerzy/Develop/lecture_slides/scripts/ewma_model.R")
lambdavs1 <- seq(from=0.05, to=0.15, by=0.01)
lambdavs2 <- seq(from=0.03, to=0.1, by=0.01)
# Perform sapply() loops over lambdas
sharper <- sapply(lambdavs1, function(lambda1) {
  sapply(lambdavs2, function(lambda2) {
    if (lambda1 > lambda2) {
...
colnames(sharper) <- lambdavs1
rownames(sharper) <- lambdavs2
# Calculate the PnLs for the optimal strategy
whichv <- which(sharper == max(sharper, na.rm=TRUE), arr.ind=TRUE)
lambda1 <- lambdavs1[whichv[2]]
lambda2 <- lambdavs2[whichv[1]]
pnls <- sim_ewma2(ohlc=ohlc, lambda1=lambda1, lambda2=lambda2, 
                    look_back=look_back, bid_offer=0.0, trend=1, lagg=2)[, "pnls"]
wealth <- cbind(vtis, pnls)
      @
...
library(rutils)
# Calculate a vector of daily VTI log returns
prices <- log(quantmod::Cl(rutils::etf_env$VTI))
vtis <- rutils::diff_it(prices)
vtis <- as.numeric(vtis)
nrows <- NROW(vtis)
# Define predictor matrix for forecasting
order_max <- 5
