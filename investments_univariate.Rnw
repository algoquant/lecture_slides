...
kelly_wealthv <- cumprod(1 + kelly_ratio*retsp)
...
# Calculate wealth paths
kelly_ratio <- drop(mean(retsp)/var(retsp))
wealthv <- cumprod(1 + kelly_ratio*retsp)
...
wealthv <- cbind(cumprod(1 + weightv$VTI*retsp$VTI),
                 cumprod(1 + retsp$VTI))
...
summary(model)
# Plot residual scatterplot
residuals <- (design$risk_parity - model$coeff["VTI"]*vti)
...
residuals <- (sell_inmay - model$coeff["VTI"]*retsp)
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The averaging of the signal improves strategy performance because of the bias-variance tradeoff.
      <<echo=TRUE,eval=FALSE>>=
# Simulate in-sample VTI strategy with averaging
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      <<echo=TRUE,eval=FALSE>>=
# Simulate in-sample VTI strategy
negav <- HighFreq::roll_sum(matrix(forecastneg), 5)/5
posav <- HighFreq::roll_sum(matrix(forecastpos), 5)/5
posit <- (negav - posav)
