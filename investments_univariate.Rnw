...
# Define predictor matrix for forecasting
order_max <- 5
predictor <- sapply(1:order_max, rutils::lagit, input=retvti)
predictor <- cbind(rep(1, nrows), predictor)
colnames(predictor) <- paste0("pred", 1:NCOL(predictor))
...
# Calculate mean squared errors
mse <- sapply(forecastv, function(x) {
  c(mse=mean((retvti[outsample] - x)^2), cor=cor(retvti[outsample], x))
})  # end sapply
mse <- t(mse)
rownames(mse) <- names(forecastv)
# Plot forecasting MSE
...
# Define predictor matrix for forecasting
order_max <- 5
design <- sapply(1:order_max, rutils::lagit, input=retvti)
