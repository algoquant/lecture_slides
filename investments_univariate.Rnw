...
wealth_fsa2 <- cumprod(1+retsp) %*% weights
...
wealth_fsa <- cumprod(1+retsp) %*% weights
# Wealth of proportional allocations (with rebalancing)
wealth_pda <- cumprod(1 + retsp %*% weights)
...
wealth_fda <- cumsum(retsp_std %*% weights)
# Plot log wealth
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The function \texttt{sim\_ewma2()} performs a simulation of the \emph{Dual EWMA} strategy, given an \emph{OHLC} time series of prices, and two decay parameters $\lambda_1$ and $\lambda2$.
      \vskip1ex
