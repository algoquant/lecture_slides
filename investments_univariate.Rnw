...
utilfun <- function(kellyfrac, returns) {
  sapply(kellyfrac, function(x) sum(log(1 + x*returns)))
}  # end utilfun
utilfun(1, vti)
utilfun(c(1, 4), vti)
# Plot the logarithmic utility
curve(expr=utilfun(x, returns=vti),
...
unlist(optimize(f=function(x) utilfun(x, w_b=0), interval=c(1, 4)))
# Kelly weight for bonds
unlist(optimize(f=function(x) utilfun(x, w_s=0), interval=c(1, 14)))
...
rets_weighted <- retsp %*% weightv
retsx <- lapply(retsp, function(x) (rets_weighted - x))
...
shadev <- posit[dates]
dates <- c(zoo::index(shadev), end(posit))
...
for (i in 1:NROW(shadev)) {
    dyplot <- dyplot %>% dyShading(from=dates[i], to=dates[i+1], color=shadev[i])
}  # end for
...
for (i in 1:NROW(shadev)) {
    dyplot <- dyplot %>% 
      dyShading(from=dates[i], to=dates[i+1], color=shadev[i])
}  # end for
# Plot the dygraph object
dyplot
      @
...
sim_ewma <- function(ohlc, lambda=0.01, look_back=333, bid_offer=0.001, 
  ...
  weightv <- weightv/sum(weightv)
  ewma <- HighFreq::roll_wsum(close, weights=weightv)
