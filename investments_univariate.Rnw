...
\begin{block}{}
  \begin{columns}[T]
    ...
    \column{0.5\textwidth}
      ...
      <<echo=TRUE,eval=FALSE>>=
# Wealth of fixed shares (without rebalancing)
wealth_fsa <- drop(apply(retp, 2, function(x) cumprod(1+x)) %*% weightv)-1
...
retp <- cbind(retvti, trendopt, revertopt, combstrat)
colnames(retp) <- c("VTI", "Trending", "Reverting", "EWMA combined")
sqrt(252)*sapply(retp, function(xtsv) mean(xtsv)/sd(xtsv))
      @
    \column{0.5\textwidth}
      \vspace{-1em}
...
dygraphs::dygraph(cumsum(retp)[endd], main="Performance of Combined EWMA Strategies") %>%
  dyOptions(colors=colorv, strokeWidth=2) %>%
  dyLegend(show="always", width=500)
# Standard plot of EWMA strategy wealth
plot_theme <- chart_theme()
...
weightv[weightv<0] <- 0
weightv <- weightv/sum(weightv)
retp <- cbind(pnltrend, pnlrevert)
retp <- retp %*% weightv
retp <- xts::xts(retp, order.by=zoo::index(retp))
retp <- cbind(retp, retp)
colnames(retp) <- c("VTI", "EWMA PnL")
# Plot dygraph of EWMA strategy wealth
colorv <- c("blue", "red")
dygraphs::dygraph(cumsum(retp)[endd], main="Performance of Ensemble of EWMA Strategies") %>%
  dyOptions(colors=colorv, strokeWidth=2) %>%
  dyLegend(show="always", width=500)
# Standard plot of EWMA strategy wealth
plot_theme <- chart_theme()
plot_theme$col$line.col <- colorv
quantmod::chart_Series(cumsum(retp), theme=plot_theme,
