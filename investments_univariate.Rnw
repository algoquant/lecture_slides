...
curve(expr=utilfun(x, retvti=retvti),
...
retsp <- cbind(retsp, weightv[1]*retsp$VTI + weightv[2]*retsp$IEF)
...
wealthv <- cbind(cumprod(1 + weightv$VTI*retsp$VTI), cumprod(1 + retsp$VTI))
...
wealthv <- cumprod(1 + weightv$VTI*retsp$VTI)
margin <- (retsp$VTI - 1)*wealthv + 1
...
retvti <- retsp$VTI
design <- cbind(retsp, 0.5*(retvti+abs(retvti)), retvti^2)
colnames(design)[3:4] <- c("merton", "treynor")
# Merton-Henriksson test
regmod <- lm(IEF ~ VTI + merton, data=design); summary(regmod)
      @
    \column{0.5\textwidth}
      \vspace{-1em}
      ...
      <<echo=TRUE,eval=FALSE>>=
# Treynor-Mazuy test
regmod <- lm(IEF ~ VTI + treynor, data=design); summary(regmod)
# Plot residual scatterplot
x11(width=6, height=5)
residv <- (design$IEF - regmod$coeff["VTI"]*retvti)
...
retvti <- rutils::diffit(pricev)/rutils::lagit(pricev, lagg=1, pad_zeros=FALSE)
      @
    \column{0.5\textwidth}
      \vspace{-1em}
...
wealthm <- mclapply(bootd, function(retsp) {
...
regmod <- lm(fixed ~ VTI + treynor, data=design)
summary(regmod)
# Plot fitted (predicted) response values
coefreg <- summary(regmod)$coeff
fittedv <- regmod$fitted.values - coefreg["VTI", "Estimate"]*retvti
points.default(x=retvti, y=fittedv, pch=16, col="blue")
text(x=0.05, y=0.6*max(residv), paste("Fixed Ratio t-value =", round(coefreg["treynor", "t value"], 2)))
      @
  \end{columns}
\end{block}
...
%%%%%%%%%%%%%%%
\subsection{draft: Risk Parity Strategy for Stocks}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The risk parity strategy contradicts the \emph{CAPM} model because it avoids risk, while the \emph{CAPM} model says that higher returns can only be achieved by taking more risk.
    \column{0.5\textwidth}
      \vspace{-1em}
      <<echo=TRUE,eval=FALSE>>=
# Calculate dollar and percentage returns for VTI and IEF.
...
design <- cbind(wealth$sell_in_may, 0.5*(retvti+abs(retvti)), retvti^2)
colnames(design) <- c("VTI", "merton", "treynor")
# Perform Merton-Henriksson test
regmod <- lm(sell_inmay ~ VTI + merton, data=design)
summary(regmod)
# Perform Treynor-Mazuy test
regmod <- lm(sell_inmay ~ VTI + treynor, data=design)
summary(regmod)
...
all.equal(pnls, pnls2)
    ...
    \column{0.5\textwidth}
      ...
      <<echo=TRUE,eval=FALSE>>=
# Combine data
wealthv <- xts::xts(cbind(retvti, pnls), dates)
colnamev <- c("VTI", "Strategy")
colnames(wealthv) <- colnamev
# Calculate Sharpe and Sortino ratios
sqrt(252)*sapply(wealthv, 
...
retvti <- rutils::diffit(closep)  # VTI returns
pnls <- retvti*posit
colnames(pnls) <- "EWMA"
wealthv <- cbind(retvti, pnls)
...
design <- cbind(pnls, retvti, retvti^2)
design <- na.omit(design)
colnames(design) <- c("EWMA", "VTI", "treynor")
regmod <- lm(EWMA ~ VTI + treynor, data=design)
summary(regmod)
# Plot residual scatterplot
residv <- (design$EWMA - regmod$coeff["VTI"]*retvti)
residv <- regmod$residuals
plot.default(x=retvti, y=residv, xlab="VTI", ylab="residuals")
...
wealthv <- cbind(retvti, revertopt)
colnames(wealthv) <- c("VTI", "EWMA PnL")
# Plot dygraph of EWMA strategy wealth
colorv <- c("blue", "red")
...
retsp <- cbind(retvti, trendopt, revertopt, combstrat)
...
calc_sharpe <- function(ohlc, lambda1, lambda2, look_back, bid_offer, trend, lagg) {
  if (lambda1 >= lambda2) return(NA)
  pnls <- sim_ewma2(ohlc=ohlc, lambda1=lambda1, lambda2=lambda2, look_back=look_back, 
...
sharpem <- outer(lambdav1, lambdav2, FUN=calc_sharpe, ohlc=ohlc, 
                 look_back=look_back, bid_offer=0.0, trend=1, lagg=2)
# Or perform two sapply() loops over lambda vectors
sharpem <- sapply(lambdav2, function(lambda2) {
...
colnames(sharpem) <- lambdav2
rownames(sharpem) <- lambdav1
      @
  \end{columns}
\end{block}
...
# Calculate matrix of PnLs
sharpem <- outer(lambdav1, lambdav2, 
...
# Dygraphs plot with custom line colors
colorv <- c("blue", "red")
dygraphs::dygraph(pricev["2008/2010"], main="VTI VWAP Prices") %>%
  dyOptions(colors=colorv, strokeWidth=2)
# Plot VWAP prices with custom line colors
x11(width=6, height=5)
plot_theme <- chart_theme()
plot_theme$col$line.col <- colorv
quantmod::chart_Series(pricev["2008/2010"], theme=plot_theme,
...
regmod <- lm(VWAP ~ VTI + treynor, data=design)
summary(regmod)
# Plot residual scatterplot
residv <- (design$VWAP - regmod$coeff["VTI"]*retvti)
residv <- regmod$residuals
...
%%%%%%%%%%%%%%%
\subsection{draft: Engle-Granger Two-step Procedure for Cointegration}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
...
residv <- xts::xts(residv, zoo::index(pricev))
dygraphs::dygraph(residv, main="XLB and XLE Cointegration Residuals")
      @
  \end{columns}
\end{block}
...
\begin{block}{}
  \begin{columns}[T]
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.45\paperwidth]{figure/leverage_func.png}
        <<echo=TRUE,eval=FALSE>>=
lambdav <- c(0.5, 1, 1.5)
...
orderp <- 5
predictor <- sapply(1:orderp, rutils::lagit, input=response)
...
# Define predictor matrix for forecasting
order_max <- 5
predictor <- sapply(1:order_max, rutils::lagit, input=response)
predictor <- cbind(rep(1, nrows), predictor)
colnames(predictor) <- paste0("pred", 1:NCOL(predictor))
...
mse <- sapply(forecastv, function(x) {
  c(mse=mean((response - x)^2), cor=cor(response, x))
})  # end sapply
mse <- t(mse)
...
# Calculate mean squared errors
mse <- sapply(forecastv, function(x) {
  c(mse=mean((response[outsample] - x)^2), cor=cor(response[outsample], x))
})  # end sapply
mse <- t(mse)
rownames(mse) <- names(forecastv)
# Plot forecasting MSE
...
nagg <- 5
predictor <- roll::roll_mean(retvti, width=nagg, min_obs=1)
response <- retvti
# Define predictor matrix for forecasting
...
# Define predictor matrix for forecasting
order_max <- 5
design <- sapply(1:order_max, rutils::lagit, input=response)
colnames(design) <- paste0("pred", 1:NCOL(design))
# Add response equal to VTI
design <- cbind(retvti, design)
...
nagg <- 5
predictor <- rutils::roll_sum(retvti, look_back=nagg)
# Define predictor matrix for forecasting
order_max <- 5
predictor <- sapply(1+nagg*(0:order_max), rutils::lagit,
                     input=predictor)
...
look_back <- 100
forecastv <- sapply((look_back+1):nrows, function(endd) {
  ...
  # Calculate fitted coefficients
  coeff <- drop(designinv %*% retvti[rangev])
  # Calculate forecast
  drop(predictor[endd, ] %*% coeff)
})  # end sapply
# Add warmup period
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      <<echo=TRUE,eval=FALSE>>=
# Mean squared error
mean((retvti - forecastv)^2)
# Correlation
cor(forecastv, retvti)
      @
    \column{0.5\textwidth}
      \vspace{-1em}
...
# Perform parallel bootstrap under Mac-OSX or Linux
forecastv <- mclapply(look_backs, sim_forecasts, response=retvti, 
...
# Perform parallel loop under Windows
orderv <- 2:6
forecastv <- parLapply(cluster, orderv, sim_forecasts, response=retvti, 
  nagg=5, look_back=look_back)
stopCluster(cluster)  # Stop R processes over cluster under Windows
# Perform parallel bootstrap under Mac-OSX or Linux
forecastv <- mclapply(orderv, sim_forecasts, response=retvti, 
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      <<echo=TRUE,eval=FALSE>>=
# Simulate the rolling autoregressive forecasts
forecastv <- sim_forecasts(retvti, ordern=ordern, look_back=look_back)
# Calculate strategy PnLs
pnls <- sign(forecastv)*retvti
...
forecastv <- sim_forecasts(retvti, ordern=ordern, look_back=look_back)
pnls5 <- cumsum(sign(forecastv)*retvti)
# Calculate PnLs for ordern=3
forecastv <- sim_forecasts(retvti, ordern=ordern, look_back=look_back)
pnls3 <- cumsum(sign(forecastv)*retvti)
      @
    \column{0.5\textwidth}
      \vspace{-1em}
...
forecastv <- sim_forecasts(retvti, ordern=ordern, look_back=look_back, 
                           rollp=TRUE)
pnls_roll <- cumsum(sign(forecastv)*retvti)
# Calculate PnLs for expanding look-back
forecastv <- sim_forecasts(retvti, ordern=ordern, look_back=look_back, 
                           rollp=FALSE)
pnls_expand <- cumsum(sign(forecastv)*retvti)
      @
    \column{0.5\textwidth}
      \vspace{-1em}
...
forecastv <- sim_forecasts(retvti, ordern=3, look_back=look_back, rollp=FALSE)
pnls <- cumsum(sign(forecastv)*retvti)
# Calculate PnLs for capped VTI returns
forecastv <- sim_forecasts(capped, ordern=3, look_back=look_back, rollp=FALSE)
pnls_capped <- cumsum(sign(forecastv)*retvti)
      @
    \column{0.5\textwidth}
      \vspace{-1em}
