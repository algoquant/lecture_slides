...
sum((datat < quantilev)*weightv)/nrows
...
bootd <- sapply(1:nboot, function(x) {
  ...
  isample <- sum((datav < quantilev)*weightv)/nrows
...
calc_quant <- function(lambda) {
  ...
  sapply(1:nboot, function(boo_t) {
    cumprob <- cumsum(weightv[, boo_t])/nrows
    datat[findInterval(confl, cumprob), boo_t]
  })  # end sapply
}  # end calc_quant
      @
    \column{0.5\textwidth}
