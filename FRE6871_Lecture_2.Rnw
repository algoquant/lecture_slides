...
\begin{block}{}
  \begin{columns}[T]
    \column{0.4\textwidth}
      Installing a package means downloading its executable files, and saving them to the local drive, so they can be called by the \texttt{R} system,
      \vskip1ex
      The function \texttt{install.packages()} installs packages from the \texttt{R} command line,
      \vskip1ex
      Most widely used packages (like \texttt{AER}) are available on \texttt{CRAN}:\\
      \url{http://cran.r-project.org/web/packages/AER/}
      \vskip1ex
      Packages that aren't available on \texttt{CRAN} may be available on \texttt{R-Forge} or \texttt{GitHub},\\
...
\begin{block}{Required}
  \begin{itemize}[]
    \item Using the function \texttt{which()} perform the following:
    \item Create a data frame that is a subset of \texttt{mtcars} and that contains only cars with 6 cylinders,
    \item Find the name of the car with the highest horsepower among 6 cylinder cars,
    \item Calculate the horsepower and the weight of that car,
    \item Create a \texttt{function} called "mult\_dots", which takes a '...' argument, and a single numeric argument called "fac\_tor", as follows: function (..., fac\_tor),
    \item The \texttt{function} "mult\_dots" should sum up the '...' argument, multiply the sum by "fac\_tor", and return the result,
    \item Apply the \texttt{function} "mult\_dots" so that it adds "1, 2, 3" and then multiplies the sum by "2",
  \end{itemize}
\end{block}
\pause
