...
# plot histogram
hist(dax_rets, breaks=30, main="", xlim=c(-0.04, 0.04), 
     ylim=c(0, 60), xlab="", ylab="", freq = FALSE)
lines(density(dax_rets),  # draw a line
      col='red', lwd=2)
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      Regression model mis-specification can lead to errors in estimation,
      \vskip1ex
      \emph{Omitted Variable Bias} occurs in a regression model that omits predictors affecting the response variable,
      \vskip1ex
      In case of \emph{OVB}, the parameter estimates are biased,
      \vskip1ex
      The \emph{t}-statistics, \emph{p}-values, and \emph{R}-squared all suggest a statistically significant regression,
      \vskip1ex
...
# regression mis-specified because depends only on one explanatory variable
reg_model <- lm(depend_var ~ indep_var, data=design_matrix)
...
reg_formula <- depend_var ~ indep_var
reg_model <- lm(reg_formula)  # perform regression
# summary suggests statistically significant regression
reg_model_sum <- summary(reg_model)
reg_model_sum$coefficients
reg_model_sum$r.squared
# Durbin-Watson test shows residuals are autocorrelated
