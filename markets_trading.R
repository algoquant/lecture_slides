...
# slopet <- function(tau, lambda) (1 - exp(-lambda * tau)) / (lambda * tau)
# curvet <- function(tau, lambda) (1 - exp(-lambda * tau)) / (lambda * tau) - exp(-lambda * tau)
# yns <- function(tau, beta0, beta1, beta2, lambda) {
#   return(beta0 + beta1 * slopet(tau, lambda) + beta2 * curvet(tau, lambda))
# }   end yns
# # Plot the Nelson-Siegel components
# tau <- seq(0.1, 30, by=0.2)
# lambda <- 0.2
# beta0 <- 1.0; beta1 <- -1.5; beta2 <- 3.0
# matplot(tau, cbind(beta0, beta1 * slopet(tau, lambda), beta2 * curvet(tau, lambda)),
#   type="l", lwd=3, lty=1,
...
#  lty=1, lwd=3, inset=0.05, cex=1.0)

# # Plot the Nelson-Siegel yield curve
# yieldv <- yns(tau, beta0, beta1, beta2, lambda)
...
#   yns(tau, beta0, beta1, beta2, lambda)
# })   end lapply
...
#   yieldf <- yns(tau, beta0, beta1, beta2, lambda)
#   return(sum((yieldv - yieldf)^2))
# }   end objfun
# # Run DEoptim
# set.seed(1234)
# library(DEoptim)
# optiml <- DEoptim(fn = objfun,
#   tau=tau, yieldv=yieldv,
#   lower = c(-20, -20, -20, -20),
#   upper = c(20, 20, 20, 20),
#   control = DEoptim.control(trace=FALSE, storepopfrom = 1, itermax=500))
# # The optimal parameters
# optimp <- optiml$optim$bestmem
# beta0 <- optimp[1]; beta1 <- optimp[2]; beta2 <- optimp[3]; lambda <- optimp[4]

# # Plot the Nelson-Siegel yield curve
# yieldv <- yns(tau, beta0, beta1, beta2, lambda)
# matplot(tau, cbind(yieldv, ycurves[, "2025"]),
#   type="l", lty=1, lwd=3, col=c("red", "blue"),
#   main="Nelson-Siegel and Actual Yield Curve",
#   xlab="Maturity (years)", ylab="Yield (%)")
...
# all.equal(sum(apply(retpca, 2, var)), sum(apply(retp, 2, var)))
...
# ratiop <- as.numeric(Cl(ESH5[indeks]))/as.numeric(Cl(ESZ4[indeks]))
...
# dygraphs::dygraph(quantmod::Cl(pricev["2024-09/"]),
#   main="Continuous E-mini Futures Prices") %>%
#   dyOptions(colors="blue", strokeWidth=2)

# # Download VIX index data from CBOE
# vixindex <- data.table::fread("https://cdn.cboe.com/api/global/us_indices/daily_prices/VIX_History.csv", skip=1)
# class(vixindex)
# dim(vixindex)
# tail(vixindex)
...
# process_ed <- lapply(pricesp500, formohlc, pricenv=sp500env)
...
# fundata <- getQuote(paste(symbolv, sep="", collapse=";"), src="yahoo", what=metricd)
# viewFinancials(fundata,  period="Q")
# 
...
# crspcomp <- read.csv(file="/Users/jerzy/Develop/lecture_slides/data/crspcomp_indices.csv")
# colnames(crspcomp) <- crspcomp[1, ]
# crspcomp <- crspcomp[-1, ]
# print(xtable(crspcomp), comment=FALSE, size="tiny", include.rownames=FALSE)
