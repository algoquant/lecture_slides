...
%%%%%%%%%%%%%%%
\subsection{Risk Parity Weights for Stocks}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \end{displaymath}
      The \emph{Kelly criterion} favors stocks with lower volatilities.  Given two stocks with the same \emph{Sharpe} ratios, the stock with the lower volatility will have a higher \emph{Kelly ratio}.
    \column{0.5\textwidth}
      \vspace{-1em}
      <<echo=TRUE,eval=FALSE>>=
  ...
  \begin{columns}[T]
    \column{0.5\textwidth}
     ...
     \column{0.5\textwidth}
      ...
      <<echo=TRUE,eval=FALSE>>=
# VTI returns
retm <- na.omit(rutils::etfenv$returns$VTI)
datev <- zoo::index(retm) ##  Dates vector
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The function \texttt{sim_momt()} simulates (backtests) a \emph{momentum strategy} which buys equal dollar amounts of the best performing stocks.
      \vskip1ex
      The function \texttt{sim_momt()} can be used to find the best choice of \emph{momentum strategy} parameters.
    \column{0.5\textwidth}
      \vspace{-1em}
      <<echo=TRUE,eval=FALSE>>=
sim_momt <- function(retp, objfun, lookb=12, rebalf="months", topstocks=10, volt=0.01, 
...
# pnll <- lapply(lookbv, sim_momt, retp=retp, endd=endd, objfun=objfun)
# Perform parallel loop under Mac-OSX or Linux
library(parallel)  ##  Load package parallel
ncores <- detectCores() - 1
pnll <- mclapply(lookbv, sim_momt, retp=retp, endd=endd, objfun=objfun, mc.cores=ncores)
sharpev <- sqrt(252)*sapply(pnll, function(pnl) mean(pnl)/sd(pnl))
      @
    \column{0.5\textwidth}
      \vspace{-1em}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      Performing a \emph{backtest} simulation allows finding the optimal \emph{momentum} (trading) strategy parameters, such as the \emph{look-back interval}.
      \vskip1ex
      The function \texttt{sim_momw()} simulates a \emph{momentum strategy} which buys dollar amounts proportional to the past performance of the stocks.
      \vskip1ex
      The function \texttt{sim_momw()} can be used to find the best choice of \emph{momentum strategy} parameters.
      \begin{itemize}
        \item Performs a parallel loop over the end points and calculate the momentum returns using the stock returns.
        \item At each end point, selects the in-sample stock returns over the look-back interval, and calculates the performance statistic for each stock.
        \item Sets the momentum weights equal to the performance statistic.
        \item Calculates the in-sample portfolio PnLs as the product of the weights times the in-sample stock returns.
        \item Scales the weights so the in-sample PnL volatility is the same as the target volatility.
        \item Calculates the out-of-sample portfolio PnLs as the product of the weights times the out-of-sample stock returns.
        \item Returns a time series of the momentum strategy returns.
      \end{itemize}
    \column{0.5\textwidth}
      \vspace{-1em}
      <<echo=TRUE,eval=FALSE>>=
...
# Perform backtests for vector of look-back intervals
lookbv <- seq(3, 15, by=1)
# pnll <- lapply(lookbv, sim_momw, retp=retp, endd=endd, objfun=objfun)
# Or perform parallel loop under Mac-OSX or Linux
library(parallel)  ##  Load package parallel
ncores <- detectCores() - 1
pnll <- mclapply(lookbv, sim_momw, retp=retp, endd=endd, objfun=objfun, mc.cores=ncores)
sharpev <- sqrt(252)*sapply(pnll, function(pnl) mean(pnl)/sd(pnl))
# Plot Sharpe ratios of momentum strategies
plot(x=lookbv, y=sharpev, t="b",
  main="Momentum Sharpe as Function of Look-back Interval",
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      <<echo=TRUE,eval=FALSE>>=
# Calculate best pnls of momentum strategy
sharpev <- sqrt(252)*sapply(pnls, function(pnl) mean(pnl)/sd(pnl))
whichmax <- which.max(sharpev)
lambdav[whichmax]
pnls <- pnls[, whichmax]
# Calculate the Sharpe and Sortino ratios
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      Performing \emph{PCA} on returns with \texttt{NA} values is more complicated, so they are removed from the data.
    \column{0.5\textwidth}
      \vspace{-1em}
      <<echo=TRUE,eval=FALSE>>=
...
volt <- 0.005 ##  Target volatility of momentum strategy
library(parallel)  ##  Load package parallel
ncores <- detectCores() - 1
pnll <- mclapply(lookbv, sim_momw, retp=retp, endd=endd, objfun=objfun, volt=volt, mc.cores=ncores)
sharpev <- sqrt(252)*sapply(pnll, function(pnl) mean(pnl)/sd(pnl))
# Plot Sharpe ratios of momentum strategies
plot(x=lookbv, y=sharpev, t="b",
  main="Momentum Sharpe as Function of Look-back Interval",
  xlab=paste0("look-back (", rebalf, ")"), ylab="Sharpe")
      @
  \end{columns}
\end{block}
...
weightv <- lapply(2:npts, function(tday) {
  ...
  ##  Scale weights so in-sample pnl volatility is same as target
  pnls <- retis %*% weightv
  return(weightv*volt/sd(pnls))
})  ##  end lapply
weightv <- rutils::do_call(rbind, weightv)
# Plot of momentum weights
retvti <- retp$VTI
retvtic <- cumsum(retvti)
datav <- cbind(retvtic[endd], weightv)
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      <<echo=TRUE,eval=FALSE>>=
# Calculate ETF betas
betetf <- sapply(retp, function(x) cov(retvti, x)/var(retvti))
# Momentum beta is equal weights times ETF betas
betam <- weightv %*% betetf
colnames(betam) <- "beta"
datav <- cbind(betam, retvtic[endd])
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The \emph{Treynor-Mazuy} test doesn't indicate any significant \emph{market timing} skill for the momentum strategy.
      \vskip1ex
      This is probably because the momentum strategy suffers momentum crashes, which are large draw-downs that occur after the market rallies from a sharp sell-off.  These momentum crashes are very difficult to forecast, so they reduce the apparent \emph{market timing} skill of the momentum strategy.
      <<echo=TRUE,eval=FALSE>>=
# Create the design matrix for the Treynor-Mazuy test
desm <- cbind(pnls, retvti)
# Aggregate the returns over the monthly end points
desm <- HighFreq::roll_sumep(desm, endd=(endd[-1]-1))
# Add the Treynor-Mazuy term, which is the square of the market returns
desm <- cbind(desm, desm[, 2]^2)
colnames(desm) <- c("pnls", "VTI", "Treynor")
# Perform the Treynor-Mazuy test
...
regmod <- lm(pnls ~ VTI + Treynor, data=desm); summary(regmod)
    ...
    \vspace{-1em}
      ...
      <<echo=TRUE,eval=FALSE>>=
# Plot the residual scatterplot
resids <- regmod$residuals
plot.default(x=desm[, "VTI"], y=resids, xlab="VTI", ylab="momentum")
title(main="Treynor-Mazuy Market Timing Test\n for Momentum vs VTI", line=0.5)
# Plot the fitted (predicted) response values
coefreg <- summary(regmod)$coeff
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      Most stocks with \emph{positive returns} also have \emph{negative skewness}.
      \vskip1ex
      The momentum strategy returns have less negative skewness compared to the negative skewness of \emph{VTI}.
      \vskip1ex
      The momentum strategy is a genuine \emph{market anomaly}, because it has both positive returns and less negative skewness.
      <<echo=TRUE,eval=FALSE>>=
# Standardize the returns
pnlsd <- (pnls-mean(pnls))/sd(pnls)
retvtisd <- (retvti-mean(retvti))/sd(retvti)
# Calculate skewness and kurtosis
apply(cbind(retvtisd, pnlsd), 2, function(x)
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The advantage of the momentum strategy is that it's negatively correlated to the \emph{All-Weather} returns, so when it's combined with it, it reduces the risk and increases the \emph{Sharpe ratio}.
      <<echo=TRUE,eval=FALSE>>=
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.6\textwidth}
      The \emph{momentum} strategy PnLs are calculated by multiplying the out-of-sample stock returns times the past portfolio weights.
      \vskip1ex
      The \emph{momentum} portfolio weights are calculated using the past stock returns, so they depend on the length of the \emph{look-back} interval.
      \vskip1ex
\fbox{\parbox{\columnwidth-1.5em}{
      The dependence on the length of the \emph{look-back interval} is an example of the \emph{bias-variance tradeoff}.
}}
      The longer the \emph{look-back interval} the less \emph{variance} but the more \emph{bias}, and vice versa.
      \vskip1ex
      ...
      \vskip1ex
      \emph{Backtesting} can be used to determine the optimal length of the \emph{look-back interval}.
      \vskip1ex
\fbox{\parbox{\columnwidth-1.5em}{
...
\fbox{\parbox{\columnwidth-1.5em}{
      ...
      \vskip1ex
      Momentum strategies don't account for correlations between stocks, so they can produce portfolios with high risk.
    \column{0.4\textwidth}
  \end{columns}
\end{block}
...
%%%%%%%%%%%%%%%
\section{Portfolio Momentum Strategies}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The \emph{out-of-sample} performance of the \emph{portfolio optimization} strategy can be improved by applying dimension reduction to the inverse of the covariance matrix.
      \vskip1ex
      For small portfolios, the \emph{in-sample} Sharpe ratio may also improve because dimension reduction reduces the portfolio volatility.
      <<echo=TRUE,eval=FALSE>>=
...
%%%%%%%%%%%%%%%
\subsection{Portfolio Momentum Strategy}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      In a \emph{portfolio momentum strategy}, the portfolio is optimized periodically and held out-of-sample.
      \setlength{\leftmargini}{1.0em}
      \begin{itemize}
        \item Calculate the \emph{end points} for portfolio rebalancing,
...
%%%%%%%%%%%%%%%
\subsection{Portfolio Momentum Strategy Performance}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
...
%%%%%%%%%%%%%%%
\subsection{Portfolio Momentum Strategy With Dimension Reduction}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
...
%%%%%%%%%%%%%%%
\subsection{Portfolio Momentum Strategy With Return Shrinkage}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
...
%%%%%%%%%%%%%%%
\subsection{draft: Weekly ETF Portfolio Momentum Strategy With Shrinkage}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
...
%%%%%%%%%%%%%%%
\subsection{Function for Portfolio Momentum Strategy}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
...
%%%%%%%%%%%%%%%
\subsection{Portfolio Momentum With Different Look-backs}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      Multiple \emph{portfolio momentum} strategies can be backtested by calling the function \texttt{roll\_portf()} in a loop over a vector of \emph{look-back} parameters.
      <<echo=TRUE,eval=FALSE>>=
...
%%%%%%%%%%%%%%%
\subsection{Portfolio Momentum With Dimension Reduction}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      Multiple \emph{portfolio momentum} strategies can be backtested by calling the function \texttt{roll\_portf()} in a loop over a vector of the dimension reduction parameter.
      <<echo=TRUE,eval=FALSE>>=
...
dimax <- dimv[which.max(profilev)]
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.45\paperwidth]{figure/portf_rolling_etf_monthly_multidim.png}
      <<echo=TRUE,eval=FALSE>>=
# Calculate the Sharpe and Sortino ratios
sqrt(252)*sapply(pnls, function(x) 
  c(Sharpe=mean(x)/sd(x), Sortino=mean(x)/sd(x[x<0])))
...
%%%%%%%%%%%%%%%
\subsection{draft: Portfolio Momentum With Return Shrinkage}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      Multiple \emph{portfolio momentum} strategies can be backtested by calling the function \texttt{roll\_portf()} in a loop over a vector of return shrinkage parameters.
      \vskip1ex
...
# Calculate the Maximum Sharpe weights the in-sample interval
covmat <- cov(retis, use="pairwise.complete.obs")
invreg <- MASS::ginv(covmat)
...
%%%%%%%%%%%%%%%
\subsection{Optimal Dimension Reduction Out-of-Sample}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The optimal out-of-sample dimension reduction parameter is \texttt{dimax=9}, which represents relatively weak dimension reduction.
      \vskip1ex
      The dependence of the out-of-sample strategy pnls on the \emph{dimax} parameter reflects the \emph{bias-variance tradeoff}.  
      \vskip1ex
...
dimax <- dimv[which.max(profilev)]
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.45\paperwidth]{figure/portf_maxsharpe_outsample_dimax.png}
      <<echo=TRUE,eval=FALSE>>=
# Plot of cumulative portfolio returns
colorv <- colorRampPalette(c("blue", "red"))(NCOL(pnls))
...
%%%%%%%%%%%%%%%
\subsection{Optimal Shrinkage Parameter Out-of-Sample}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The dependence of the out-of-sample strategy pnls on the $\alpha$ parameter reflects the \emph{bias-variance tradeoff}.  
      \vskip1ex
...
alphac <- alphav[which.max(profilev)]
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.45\paperwidth]{figure/portf_maxsharpe_outsample_alpha.png}
      <<echo=TRUE,eval=FALSE>>=
# Plot of cumulative portfolio returns
colorv <- colorRampPalette(c("blue", "red"))(NCOL(pnls))
endw <- rutils::calc_endpoints(pnls["2015/"], interval="weeks")
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      A \emph{portfolio momentum} strategy consists of rebalancing a portfolio over the end points:
      \setlength{\leftmargini}{1.0em}
      \begin{enumerate}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The performance of the \emph{portfolio momentum} strategy can be improved by applying dimension reduction and return shrinkage.
      <<echo=TRUE,eval=FALSE>>=
# Calculate the Sharpe and Sortino ratios
wealthv <- cbind(retew, pnls)
colnames(wealthv) <- c("EqualWeight", "MaxSharpe")
...
%%%%%%%%%%%%%%%
\subsection{Portfolio Momentum Strategy With Dimension Reduction and Shrinkage}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The optimal dimension reduction parameter for this portfolio of stocks is equal to \texttt{dimax=9}, which represents relatively weak dimension reduction.
      \vskip1ex
      The dependence of the out-of-sample strategy pnls on the \emph{dimax} parameter reflects the \emph{bias-variance tradeoff}.  
      \vskip1ex
      If the \emph{dimax} parameter is too small, it creates excessive bias, but if it's too large, it doesn't suppress the variance enough.
...
%%%%%%%%%%%%%%%
\subsection{Optimal Portfolio Optimization Strategy}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The optimal \emph{portfolio momentum} strategy, with dimension reduction and return shrinkage, has both a higher \emph{Sharpe} ratio and higher absolute returns than the equal weight portfolio.
      \vskip1ex
      Combining the optimal portfolio momentum strategy with the equal weight portfolio results in an even higher \emph{Sharpe} ratio.
    \column{0.5\textwidth}
      \vspace{-1em}
