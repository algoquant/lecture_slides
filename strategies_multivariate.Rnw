...
pnls <- btmomdaily(retp=retp, look_back=152, bidask=bidask)
# Perform sapply loop over look_backs
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The dimension reduction rolling weekly strategy performs better than the standard strategy because dimension reduction allows using shorter \texttt{look\_back} intervals since it suppresses the response noise.
      \vskip1ex
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The optimal value of the look-back interval for this portfolio of stocks is equal to \texttt{look\_back=10} months, which roughly agrees with the research literature on momentum strategies.
      \vskip1ex
      The dependence on the length of the \emph{look-back interval} is an example of the \emph{bias-variance tradeoff}.  
      \vskip1ex
      If the \emph{look-back interval} is too long, then the data has large \emph{bias} because the distant past may have little relevance to today.  
