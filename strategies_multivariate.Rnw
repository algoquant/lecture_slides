...
riskret <- mclapply(retp, function(retp) {
  ...
  meanvti <- mean(retvti)
  betac <- drop(cov(retp, retvti))/varvti
  resid <- retp - betac*retvti
  alphac <- mean(retp) - betac*meanvti
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The low volatility portfolio has a higher \emph{Sharpe ratio} and also higher absolute returns than the high volatility portfolio.
      <<echo=TRUE,eval=FALSE>>=
# Calculate the median volatilities
medianv <- matrixStats::rowMedians(volat)
# Calculate the wealth of low volatility stocks
weightv <- (volat <= medianv)
weightv <- rutils::lagit(weightv)
retlow <- rowMeans(weightv*retp, na.rm=TRUE)
# Calculate the wealth of high volatility stocks
weightv <- (volat > medianv)
weightv <- rutils::lagit(weightv)
rethigh <- rowMeans(weightv*retp, na.rm=TRUE)
      @
    \column{0.5\textwidth}
      \vspace{-1em}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The \emph{Long-Short Volatility} strategy has a higher \emph{Sharpe ratio} and also higher absolute returns than the proportional dollar strategy.
      <<echo=TRUE,eval=FALSE>>=
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      In the \emph{Risk Parity} strategy the dollar portfolio weights are rebalanced daily so that their dollar volatilities remain equal.
      \vskip1ex
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The risk parity strategy for stocks has a significantly higher \emph{Sharpe ratio} than the proportional dollar strategy because it's overweight low volatility stocks.  But it has lower absolute returns.
      \vskip1ex
...
wealthv <- xts(cbind(retew, pnls), index(retp))
...
pnll <- lapply(lookbs, btmomtop, retp=retp, endd=endd, objfun=objfun)
# Perform parallel loop under Mac-OSX or Linux
library(parallel)  # Load package parallel
ncores <- detectCores() - 1
pnll <- mclapply(lookbs, btmomtop, retp=retp, endd=endd, objfun=objfun, mc.cores=ncores)
...
# Perform backtests for vector of look-back intervals
lookbs <- seq(3, 15, by=1)
pnll <- lapply(lookbs, btmomweight, retp=retp, endd=endd, objfun=objfun)
# Or perform parallel loop under Mac-OSX or Linux
library(parallel)  # Load package parallel
ncores <- detectCores() - 1
pnll <- mclapply(lookbs, btmomweight, retp=retp, endd=endd, objfun=objfun, mc.cores=ncores)
sharper <- sqrt(252)*sapply(pnll, function(pnl) mean(pnl)/sd(pnl))
# Plot Sharpe ratios of momentum strategies
plot(x=lookbs, y=sharper, t="l",
  main="Momentum Sharpe as Function of Look-back Interval",
...
# Perform sapply loop over lookbs
lookbs <- seq(90, 190, by=10)
pnls <- sapply(lookbs, btmomdaily,
...
# Perform sapply loop over lookbs
lookbs <- seq(2, 15, by=1)
pnls <- lapply(lookbs, roll_portf,
  retp=retp, retx=retx, endd=endd, dimax=dimax)
pnls <- do.call(cbind, pnls)
colnames(pnls) <- paste0("lookb=", lookbs)
pnlsums <- sapply(pnls, sum)
lookb <- lookbs[which.max(pnlsums)]
      @
    \column{0.5\textwidth}
      \vspace{-1em}
...
pnls <- lapply(alphav, function(alphac) {
  controlv <- HighFreq::param_portf(method="maxsharpe", 
            dimax=dimax, alpha=alphac)
  HighFreq::back_test(retx=retx, retp=retp,
            startp=startp, endd=endd, controlv=controlv)
...
lookbs <- seq(from=5, to=16, by=1)
pnls <- lapply(lookbs, function(lookb) {
  startp <- c(rep_len(0, lookb), endd[1:(npts-lookb)])
...
profilev <- sapply(pnls, sum)
plot(x=lookbs, y=profilev, t="l", main="Strategy PnL as Function of Look-back Interval",
