...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The risk parity strategy for stocks does not perform much better than the price-weighted (fixed shares) strategy because their correlations are positive.
      \vskip1ex
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The low volatility portfolio has higher risk-adjusted returns than the high volatility portfolio.
      <<echo=TRUE,eval=FALSE>>=
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The \emph{Long-Short Volatility} strategy has higher risk-adjusted returns than the fixed share portfolio.
      <<echo=TRUE,eval=FALSE>>=
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      If the \emph{look-back interval} is too long, then the data has high \emph{bias} because the distant past may have little relevance to today.  
      \vskip1ex
...
%%%%%%%%%%%%%%%
\subsection{draft: Backtesting the Momentum Strategy}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
...
%%%%%%%%%%%%%%%
\subsection{Generalized Inverse of Singular Covariance Matrix}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
