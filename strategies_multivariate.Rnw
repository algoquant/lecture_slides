...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \end{displaymath}
      The \emph{Kelly criterion} favors stocks with low returns.  Given two stocks with the same \emph{Sharpe} ratios, the stock with the lower returns will have a higher \emph{Kelly ratio}.
    \column{0.5\textwidth}
      \vspace{-1em}
      <<echo=TRUE,eval=FALSE>>=
