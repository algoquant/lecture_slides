...
retmv <- (retsp %*% weightmv)
all.equal(retmv, (retsp %*% covinv %*% unitv)/c11)
...
sqrt(252)*sapply((retsp - riskf), function(x) mean(x)/sd(x))
      @
  \end{columns}
\end{block}
...
wealthv <- cbind(retsp %*% weightms, retsp %*% weightmv)
...
# Specify matrix of linear constraint coefficients
coeffm <- rbind(cbind(rbind(1, retsm),
