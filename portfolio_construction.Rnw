...
randportf <- sapply(1:nportf, function(it) {
  ...
  weightv <- weightv/sum(weightv)
  retsp <- etfenv$returns[, symbolv] %*% weightv
...
thresh <- qnorm(defprobs)
assets <- qnorm(unifm)
# Simulate defaults
defaultv <- colSums(assets < thresh)
mean(defaultv)
      @
    \column{0.5\textwidth}
    \vspace{-1em}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      To simplify, we can assume that the number of assets is very large and that they all have the same default probabilities $p_i=p$, so the default threshold is equal to $t=\Phi^{-1}(p)$.
      \vskip1ex
      Then the \emph{asset value} $a_i$ follows the \emph{Normal} distribution with mean equal to $\sqrt{\rho} s$ and standard deviation equal to $\sqrt{1-\rho}$
      \vskip1ex
      So the conditional default probability $p(s)$, given the systematic factor $s$, is equal to:
      \begin{displaymath}
        p(s) = \Phi(\frac{t - \sqrt{\rho} s}{\sqrt{1-\rho}})
      \end{displaymath}
      If the systematic factor $s$ is fixed, then the defaults are all independent, and the percentage of the portfolio that defaults $x$, is equal to the conditional default probability $x = p(s)$.
    \column{0.5\textwidth}
      We can invert the previous formula to obtain the systematic factor $s$:
      \begin{displaymath}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      <<echo=TRUE,eval=FALSE>>=
# Define cumulative default distribtution function
cumdefdistr <- function(x, thresh=(-2), rho=0.2)
  pnorm((sqrt(1-rho)*qnorm(x) - thresh)/sqrt(rho))
cumdefdistr(x=0.2, thresh=qnorm(defprob), rho=rho)
# Plot cumulative default distribtution function
defprob <- 0.4; thresh <- qnorm(defprob)
curve(expr=cumdefdistr(x, thresh=thresh, rho=0.05),
...
curve(expr=cumdefdistr(x, thresh=thresh, rho=0.2),
...
defdistr <- function(x, thresh=(-2), rho=0.2)
  sqrt((1-rho)/rho)*exp(-(sqrt(1-rho)*qnorm(x) -
  thresh)^2/(2*rho) + qnorm(x)^2/2)
# Define parameters
rho <- 0.2 ; rho_sqrt <- sqrt(rho) ; rho_sqrtm <- sqrt(1-rho)
defprob <- 0.3; thresh <- qnorm(defprob)
defdistr(0.03, thresh=thresh, rho=rho)
# Plot probability distribution of defaults
curve(expr=defdistr(x, thresh=thresh, rho=0.1),
      xlim=c(0, 1.0), lwd=3,
    ...
    \column{0.5\textwidth}
      ...
      <<echo=TRUE,eval=FALSE>>=
# Plot default distribution with higher correlation
curve(expr=defdistr(x, thresh=thresh, rho=0.3),
...
curve(expr=defdistr(x, thresh=thresh, rho=0.01),
      xlab="default percentage", ylab="", lwd=2,
      col="green", main="Distribution of Defaults")
# Plot default distribution with high correlation
curve(expr=defdistr(x, thresh=thresh, rho=0.99),
...
rho <- 0.1; lgd <- 0.4
defprob <- 0.05; thresh <- qnorm(defprob)
# Define Vasicek cumulative loss distribution
cumlossdistr <- function(x, thresh=(-2), rho=0.2, lgd=0.4)
  pnorm((sqrt(1-rho)*qnorm(x/lgd) - thresh)/sqrt(rho))
# Define Vasicek loss distribution function
lossdistr <- function(x, thresh=(-2), rho=0.2, lgd=0.4)
  sqrt((1-rho)/rho)*exp(-(sqrt(1-rho)*qnorm(x/lgd) - thresh)^2/(2*rho) + qnorm(x/lgd)^2/2)/lgd
integrate(lossdistr, low=0, up=lgd, thresh=(-2), rho=rho, lgd=lgd)
      @
    \column{0.5\textwidth}
      \vspace{-1em}
...
x11(width=6, height=5)
curve(expr=lossdistr(x, thresh=thresh, rho=rho),
...
lossdistr <- function(x, thresh=-2, rho=0.1, lgd=0.4) {
  qnormv <- ifelse(x/lgd < 0.999, qnorm(x/lgd), 3.1)
  sqrt((1-rho)/rho)*exp(-(sqrt(1-rho)*qnormv - thresh)^2/(2*rho) + qnormv^2/2)/lgd
}  # end lossdistr
      @
    \column{0.5\textwidth}
      \vspace{-1em}
      ...
      \vspace{-1em}
      <<echo=TRUE,eval=FALSE>>=
defprob <- 0.2; thresh <- qnorm(defprob)
rho <- 0.1; lgd <- 0.4
...
densv <- sapply(vars, lossdistr, thresh=thresh, rho=rho)
# Draw shaded polygon
...
# Calculate CVaR
cvar <- integrate(function(x) x*lossdistr(x, thresh=thresh, rho=rho, lgd=lgd),
  low=varisk, up=lgd)$value
cvar <- cvar/integrate(lossdistr, low=varisk, up=lgd, thresh=thresh, rho=rho, lgd=lgd)$value
# Plot probability distribution of losses
curve(expr=lossdistr(x, thresh=thresh, rho=rho),
      type="l", xlim=c(0, 0.06),
      xlab="loss percentage", ylab="density", lwd=3,
...
varfun <- function(x, thresh=qnorm(0.1), rho=0.1, lgd=0.4)
  lgd*pnorm((sqrt(rho)*qnorm(x) + thresh)/sqrt(1-rho))
varfun(x=0.99, thresh=thresh, rho=rho, lgd=lgd)
# Plot VaR
curve(expr=varfun(x, thresh=thresh, rho=rho, lgd=lgd),
...
integrate(function(x) x*lossdistr(x, thresh=thresh, rho=rho, lgd=lgd),
          low=0.0, up=lgd)
# Calculate confidence levels corresponding to VaR values
vars <- seq(0.07, 0.12, 0.001)
confls <- sapply(vars, function(varisk) {
  integrate(lossdistr, low=varisk, up=lgd, thresh=thresh, rho=rho, lgd=lgd)
})  # end sapply
...
losses <- lgd*colSums(assets < thresh)/nassets
      @
  \end{columns}
\end{block}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.7\textwidth}
      ...
      <<echo=TRUE,eval=FALSE>>=
calcvar <- function(thresh, # Default thresholds
...
stderr_var <- apply(boot_data[, 1:7], MARGIN=2,
    function(x) c(mean=mean(x), sd=sd(x)))
stderr_cvar <- apply(boot_data[, 8:14], MARGIN=2,
    function(x) c(mean=mean(x), sd=sd(x)))
...
calcvar <- function(defprobs, # Default probabilities
  ...
  thresh <- qnorm(runif(1, min=0.5, max=1.5)*defprobs)
  # Simulate losses under Vasicek model
...
stopCluster(cluster)  # Stop R processes over cluster
# Calculate vectors of standard errors of VaR and CVaR from boot_data data
stderr_var_param <- apply(boot_data[, 1:7], MARGIN=2,
    function(x) c(mean=mean(x), sd=sd(x)))
stderr_cvar_param <- apply(boot_data[, 8:14], MARGIN=2,
    function(x) c(mean=mean(x), sd=sd(x)))
      @
    \column{0.5\textwidth}
...
losses <-
  lgd*colSums(assets < thresh)/nassets
# Calculate VaRs
confls <- seq(0.93, 0.99, 0.01)
vars <- quantile(losses, probs=confls)
...
cond_thresh <- outer(rho_sqrtm*thresh, -rho_sqrt*sysv, FUN="+")
