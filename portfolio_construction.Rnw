...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The technique of \emph{regularization} is designed to reduce the overfitting of the parameters in a model.
      \vskip1ex
      \emph{Ridge regularization} adds a penalty term to the objective function, proportional to the sum of squares of the weights: $\mathbf{w}^T \mathbf{w} = \sum_{i=1}^n w^2_i$.
      \vskip1ex
      ...
      \end{displaymath}
      Where the parameter $\lambda$ determines the amount of regularization.
      \vskip1ex
...
%%%%%%%%%%%%%%%
\subsection{Portfolio Optimization With \protect\emph{Elastic Net Regularization}}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \end{displaymath}
      The $\lambda$ parameter determines the amount of regularization, and $\alpha$ controls the balance between the \emph{ridge} and \emph{LASSO} penalties.
    \column{0.5\textwidth}
      \vspace{-1em}
        <<echo=TRUE,eval=FALSE>>=
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      When default probabilities are random, the standard error of the \emph{VaR} decreases with higher confidence level, because the whole loss distribtution shifts, so it has a larger effect at lower confidence levels.
      <<echo=TRUE,eval=FALSE>>=
library(parallel)  ##  load package parallel
ncores <- detectCores() - 1  ##  number of cores
compclust <- makeCluster(ncores)  ##  Initialize compute cluster
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      When default probabilities are random, the \emph{scaled} (relative) standard errors of \emph{VaR} and \emph{CVaR} decrease with higher confidence level, because the whole loss distribtution shifts, so it has a larger effect at lower confidence levels.
      <<echo=TRUE,eval=FALSE>>=
      @
    \column{0.5\textwidth}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      It's not possible to apply importance sampling to the credit loss distribtution because the distribtution is not known.
      \vskip1ex
