...
uni_form <- matrix(runif(n_simu*n_assets), 
                   ncol=n_simu)
sum(uni_form < default_probs)/n_simu
      @
  \end{columns}
\end{block}
...
asset_values <- matrix(rnorm(n_simu*n_assets), ncol=n_simu)
asset_values <- t(rho_sqrt*system_atic + t(rho_sqrtm*asset_values))
loss_es <- 
  l_gd*colSums(asset_values < default_thresh)/n_assets
      @
    \column{0.5\textwidth}
...
# table_losses <- table(loss_es)/n_simu
...
calc_var <- function(default_thresh, 
                     ...
                     conf_levels=seq(0.93, 0.99, 0.01)) {
  # Define model parameters
  n_assets <- NROW(default_thresh)
  # Simulate losses under Vasicek model
  system_atic <- rnorm(n_simu)
  asset_values <- matrix(rnorm(n_simu*n_assets), ncol=n_simu)
  asset_values <- t(rho_sqrt*system_atic + t(rho_sqrtm*asset_values))
  loss_es <- l_gd*colSums(asset_values < default_thresh)/n_assets
