...
%%%%%%%%%%%%%%%
\subsection{The \protect\emph{Mean-Variance Efficient Portfolios}}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      A portfolio which has the smallest variance, given a target return, is a \emph{mean-variance efficient portfolio}.
      \vskip1ex
      ...
      \vskip1ex
      Where $\mathbf{\bar{r}}$ is the vector of the mean stock returns. 
      \vskip1ex
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The risk-free bond is a hypothetical bond that has a return equal to $r_f$ and zero price risk (standard deviation $\sigma=0$).
      \vskip1ex
      The \emph{tangent} line connects the risk-free point $(\sigma=0, r=r_f)$ with a single tangent point $(\sigma=\sigma, r=r_t)$ on the \emph{efficient frontier}. 
      \vskip1ex
      A \emph{tangent} line can be drawn at every point on the \protect\emph{efficient frontier}.
      \vskip1ex
      The slope $\beta$ of the \emph{tangent} line can be calculated by differentiating the portfolio variance $\sigma^2$ with respect to the target return $r_t$:
      \begin{align*}
      ...
      \end{align*}
      The \emph{tangent} line connects the \emph{tangent} point $(\sigma=\sigma, r=r_t)$ on the \protect\emph{efficient frontier} with a \emph{risk-free} point $(\sigma=0, r=r_f)$.
    \column{0.5\textwidth}
      The \emph{risk-free} rate $r_f$ can be calculated as the intercept of the tangent line with the y-axis:
      \begin{align*}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The \emph{Capital Market Line} (CML) is the tangent line connecting the risk-free point $(\sigma=0, r=r_f)$ with a single tangent point $(\sigma=\sigma, r=r_t)$ on the \emph{efficient frontier}. 
      \vskip1ex
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      We can calculate the \emph{maximum Sharpe} portfolio weights by setting the derivative of the \emph{Sharpe ratio} with respect to the weights $\mathbf{w}$ to zero:
      \begin{displaymath}
    ...
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The target return of the \emph{maximum Sharpe} portfolio is equal to:
      \begin{align*}
        ...
        \frac{\mathbf{\bar{r}}^T \mathbb{C}^{-1} \mathbbm{1} \, r_f - \mathbf{\bar{r}}^T \mathbb{C}^{-1} \, \mathbf{\bar{r}}}{\mathbbm{1}^T \mathbb{C}^{-1} \mathbbm{1} \, r_f - \mathbf{\bar{r}}^T \mathbb{C}^{-1} \mathbbm{1}} = 
...
%%%%%%%%%%%%%%%
\subsection{\protect\emph{Maximum Sharpe} Portfolio of Two Stocks}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \begin{displaymath}
        \mathbb{C}^{-1} = \frac{1}{(\sigma^2_1 \sigma^2_2 - \sigma^2_{12})}
        \begin{bmatrix}
          \sigma^2_2 & - \sigma_{12} \\
          -\sigma_{12} & \sigma^2_1
        \end{bmatrix}
      \end{displaymath}
      Where $\sigma^2_1$ and $\sigma^2_2$ are the stock variances and $\sigma_{12}$ is their covariance.
      \vskip1ex
      The \emph{maximum Sharpe} portfolio weights are equal to:
      \begin{displaymath}
        \mathbf{w} = \frac{\mathbb{C}^{-1} \mu}{\mathbbm{1}^T \mathbb{C}^{-1} \mu} = 
        \frac{1}{D} 
          \begin{bmatrix}
            \sigma^2_2 \mu_1 - \sigma_{12} \mu_2 \\
            - \sigma_{12} \mu_1 + \sigma^2_1 \mu_2
          \end{bmatrix}
      \end{displaymath}
      Where $\mu_1$ and $\mu_2$ are the mean excess returns of the two stocks, and
      $D = \sigma^2_2 \mu_1 + \sigma^2_1 \mu_2 - \sigma_{12} (\mu_1 + \mu_2)$.
    \column{0.5\textwidth}
      \vspace{-1em}
      <<echo=TRUE,eval=FALSE>>=
...
%%%%%%%%%%%%%%%
\subsection{draft: Recursive Matrix Inverse}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      Diverges for singular matrices.
      \vskip1ex
      The inverse of a square matrix $\mathbb{C}$ can be calculated approximately using the recursive \emph{Schulz formula}:
      \begin{displaymath}
        \mathbb{C}_{i+1}^{-1} = 2 \mathbb{C}_i^{-1} - \mathbb{C}_i^{-1} \mathbb{C} \mathbb{C}_i^{-1}
      \end{displaymath}
      The \emph{Schulz formula} requires a good initial value for the inverse matrix $\mathbb{C}_1^{-1}$ or else the recursion diverges.
      \vskip1ex
      If the initial inverse matrix $\mathbb{C}_1^{-1}$ is very close to the actual inverse $\mathbb{C}^{-1}$, then the \emph{Schulz formula} produces a very good approximation with just a few iterations.
      \vskip1ex
      The \emph{Schulz formula} is useful for updating the inverse when the matrix $\mathbb{C}$ changes only slightly.  For example, for updating the inverse of the covariance matrix as it changes slowly over time.
      <<echo=TRUE,eval=FALSE>>=
# Load stock returns
...
%%%%%%%%%%%%%%%
\subsection{draft: Efficient Frontier for Two-stock Portfolio}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The covariance matrix for two stocks is equal to:
      \begin{displaymath}
        ...
        \end{pmatrix}
      \end{displaymath}
      Where $\sigma_{11}$ is the variance, and $\sigma_{12}$ is the covariance of returns between the two stocks.
      \vskip1ex
      The excess returns of a two-stock portfolio are equal to:
      \vskip1ex
      Any linear combination of a portfolio of two stocks is a maximum Sharpe portfolio corresponding to some value of the risk-free rate.
      \vskip1ex
      \begin{displaymath}
        R = w \mu_1 + (1 - w) \mu_2
      \end{displaymath}
...
%%%%%%%%%%%%%%%
\subsection{draft: Efficient Frontier of Principal Components}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
...
%%%%%%%%%%%%%%%
\subsection{Portfolio Optimization Using \texttt{optim()}}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
...
%%%%%%%%%%%%%%%
\subsection{Optimization Algorithms Using \texttt{optim()}}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
...
\begin{table}[h!]
\centering
\caption{Comparison of optimization methods for portfolio optimization}
\tiny
\begin{tabular}{@{}p{1.5cm}p{1.5cm}p{2.5cm}p{2.5cm}p{2.5cm}@{}}
\toprule
\textbf{Method} & \textbf{Type} & \textbf{Strengths} & \textbf{Weaknesses} & \textbf{Suitability for Portfolio Optimization} \\
\midrule
...
\begin{block}{}
  \begin{columns}[T]
    ...
    \column{0.5\textwidth}
      ...
      \end{align*}
      If the variables are centered, then the OLS estimate for $\beta$ is given by equating the RSS derivative to zero:
      \begin{flalign*}
...
%%%%%%%%%%%%%%%
\subsection{\protect\emph{Efficient Portfolios} With \protect\emph{Ridge Regularization}}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      Portfolio optimization has a very large number of parameters, equal to the number of stocks in the portfolio.  This causes the \emph{overfitting} of the stock portfolio weights to the in-sample returns.
      \vskip1ex
      The technique of \emph{regularization} is designed to reduce the overfitting of the parameters in a model, by adding a penalty term to the objective function.
      \vskip1ex
      The \emph{Ridge regularization} penalty term is proportional to the sum of squares of the weights: $\mathbf{w}^T \mathbf{w} = \sum_{i=1}^n w^2_i$.
      \vskip1ex
      The Lagrangian for the efficient portfolio weights with \emph{ridge regularization} is:
      \begin{displaymath}
        \mathcal{L} = \mathbf{w}^T \mathbb{C} \, \mathbf{w} - \lambda_1 (\mathbf{w}^T \mathbbm{1} - 1) - \lambda_2 (\mathbf{w}^T \mathbf{r} - r_t) + \lambda \mathbf{w}^T \mathbf{w}
      \end{displaymath}
      Where the regularization intensity $\lambda$ determines the amount of regularization.
      \vskip1ex
      The Lagrangian can be rewritten using the ridge covariance matrix $\mathbb{C}_r = \mathbb{C} + \lambda \mathbbm{1}$:
      \begin{displaymath}
        \mathcal{L} = \mathbf{w}^T \mathbb{C}_r \, \mathbf{w} - \lambda_1 (\mathbf{w}^T \mathbbm{1} - 1) - \lambda_2 (\mathbf{w}^T \mathbf{r} - r_t)
      \end{displaymath}
      This is the efficient portfolio Lagrangian with the ridge covariance matrix $\mathbb{C}_r$.
    \column{0.5\textwidth}
      \emph{Ridge regularization} adds a constant to the variances of the returns and reduces their correlations.
      \vskip1ex
\fbox{\parbox{\columnwidth-1.5em}{
      \emph{Ridge regularization} shrinks the covariance matrix towards the unit matrix, and shrinks the portfolio weights towards equal weights.
}}
  \end{columns}
\end{block}
...
%%%%%%%%%%%%%%%
\subsection{Portfolio Optimization With \protect\emph{Ridge Regularization}}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      Maximizing the efficient portfolio Lagrangian is equivalent to maximizing the \emph{Sharpe ratio}:
      \begin{displaymath}
        SR = \frac{\mathbf{w}^T \mu}{\sigma}
      \end{displaymath}
      Where $\mu = \mathbf{r} - r_f$ is the vector of excess returns, and $\sigma = \sqrt{\mathbf{w}^T \mathbb{C}_r \, \mathbf{w}}$ is the standard deviation of portfolio with the ridge covariance matrix $\mathbb{C}_r = \mathbb{C} + \lambda \mathbbm{1}$.
      \vskip1ex
      The portfolio weights satisfying the linear constraint $\mathbf{w}^T \mathbbm{1} = 1$ are:
      \begin{displaymath}
        \mathbf{w} = \frac{\mathbb{C}_r^{-1} \mu}{\mathbbm{1}^T \mathbb{C}_r^{-1} \mu}
      \end{displaymath}
      These are the weights of the \emph{maximum Sharpe} portfolio, with the vector of mean excess returns equal to $\mu$, and the covariance matrix equal to $\mathbb{C}_r$.
    \column{0.5\textwidth}
      \vspace{-1em}
      <<echo=TRUE,eval=FALSE>>=
...
%%%%%%%%%%%%%%%
\subsection{draft: Portfolio Optimization With \protect\emph{Elastic Net Regularization}}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The \emph{regularization} techniques often add a penalty term to the objective function.
      \vskip1ex
      ...
      \vskip1ex
      The \emph{elastic net} regularization penalty is a linear combination of the \emph{Ridge} penalty and the \emph{LASSO} penalty.
      The \emph{elastic net} objective function $O(x)$ is: 
      \begin{displaymath}
        O(x) = \frac{\mathbf{w}^T \mu}{\sigma} - \lambda ( (1-\alpha) \sum_{i=1}^n w^2_i + \alpha \sum_{i=1}^n|w_i| )
      \end{displaymath}
      The $\lambda$ intensity parameter determines the amount of regularization, and $\alpha$ controls the balance between the \emph{Ridge} and \emph{LASSO} penalties.
    \column{0.5\textwidth}
      \vspace{-1em}
        <<echo=TRUE,eval=FALSE>>=
...
%%%%%%%%%%%%%%%
\subsection{draft: Portfolio Weights With Regularization}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
...
%%%%%%%%%%%%%%%
\subsection{Optimal Stock Portfolio Out-of-Sample}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The tangent point on the \emph{efficient frontier} is the \emph{tangency portfolio}.
...
%%%%%%%%%%%%%%%
\subsection{Vasicek Model With Random Default Probabilities}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.7\textwidth}
      ...
      \vskip1ex
      The below function \texttt{calc\_var()} can simulate the \emph{Vasicek} model with random default probabilities.
      <<echo=TRUE,eval=FALSE>>=
...
%%%%%%%%%%%%%%%
\subsection{Standard Errors Due to Random Default Probabilities}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The standard error of the \emph{CVaR} decreases with higher confidence level, because when default probabilities are random, the whole loss distribution shifts, so it has a larger effect at lower confidence levels.
      <<echo=TRUE,eval=FALSE>>=
library(parallel)  ##  load package parallel
ncores <- detectCores() - 1  ##  number of cores
compclust <- makeCluster(ncores)  ##  Initialize compute cluster
...
%%%%%%%%%%%%%%%
\subsection{Relative Errors Due to Random Default Probabilities}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      When default probabilities are random, the \emph{scaled} (relative) standard errors of \emph{VaR} and \emph{CVaR} decrease with higher confidence level, because the whole loss distribution shifts, so it has a larger effect at lower confidence levels.
      <<echo=TRUE,eval=FALSE>>=
      @
    \column{0.5\textwidth}
...
%%%%%%%%%%%%%%%
\subsection{depr: Simulating Credit Portfolios Using Importance Sampling}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      It's not possible to apply importance sampling to the credit loss distribution because the distribution is not known.
      \vskip1ex
      Importance sampling can be used to improve the accuracy of VaR and CVaR calculations for credit portfolios. 
    \column{0.5\textwidth}
      \vspace{-1em}
      <<echo=TRUE,eval=FALSE>>=
##########################################
