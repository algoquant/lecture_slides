...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \begin{align*}
        w_{min} = \operatorname*{arg\,max}_{w} [ \, w^T r - \\
...
# inefficient way to simulate using for() loop
...
mean(de_faults)
# efficient way to simulate using vectorized functions
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      Empirical studies have found that the correlation $\rho$ can vary between \texttt{5\%} to \texttt{20\%}, depending on the default risk, 
      \vskip1ex
      Obligors with higher default risk tend to also have higher correlation, since they are more  sensitive to the economic conditions,
    \column{0.5\textwidth}
      \vspace{-1em}
      <<echo=TRUE,eval=FALSE>>=
...
# efficient way to simulate asset values using vectorized functions
...
cor(asset_values)
# inefficient way to simulate asset values using for() loop
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      Default correlations are the correlations between the Boolean vectors of default events, and default correlation is much lower than the corresponding asset correlation,
    \column{0.5\textwidth}
      \vspace{-1em}
      <<echo=TRUE,eval=FALSE>>=
# calculate random default probabilities
...
# efficient way to calculate number of defaults using vectorized functions
# calculate vector of number of defaults
de_faults <- 
  colSums(t(t(asset_values) < default_thresh))
de_faults / num_simu
default_probs
# inefficient way to calculate number of defaults using for() loop
