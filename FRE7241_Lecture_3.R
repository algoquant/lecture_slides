...
pnltrend <- lapply(lambdav, function(lambda) {
  ...
  sim_ewma(ohlc=ohlc, lambda=lambda, look_back=look_back, bidask=0, lagg=2)[, "pnls"]
})  # end lapply
...
stretduster(cluster)  # Stop R processes over cluster under Windows
...
pnlrevert <- lapply(lambdav, function(lambda) {
  # Simulate EWMA strategy and Calculate the returns
  sim_ewma(ohlc=ohlc, lambda=lambda, bidask=0, trend=(-1))[, "pnls"]
})  # end lapply
...
# Plot dygraph
colnamev <- colnames(pricev)
dyplot <- dygraphs::dygraph(pricev, main="VTI Dual EWMA Prices") %>%
  dySeries(name=colnamev[1], strokeWidth=1, col="blue") %>%
  dySeries(name=colnamev[2], strokeWidth=2, col="red") %>%
