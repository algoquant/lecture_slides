...
# default values can be specified in the argument list
  my_var1 + 2*my_var2 + glob_var
# the function returns the last evaluated statement
}  # end MyFunc
MyFunc(3, 2)  # glob_var doesn't exist yet!
glob_var <- 10
...
\begin{block}{}
  \begin{columns}[T]
    ...
    \column{0.6\textwidth}
      \vspace{-1em}
        <<>>=
FunRobust <- function(in_var1=NULL, in_var2=NULL) {
# returns the sum of its two arguments
  if (is.na(in_var2)) {  # check if in_var2 was input
    warning("in_var2 wasn't input")
    in_var1  # in_var2 wasn't input
  } else if (is.na(in_var2)) {
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.4\textwidth}
      ...
      \vskip1ex
      The \texttt{S3} system is based on the \texttt{R} class system and on \emph{generic functions},
      \vskip1ex
      ...
      \vskip1ex
      \emph{Generic function} methods follow the naming convention \texttt{gmethod.class()},
      \vskip1ex
