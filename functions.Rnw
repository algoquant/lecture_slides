...
test_func("se")  # partial string matching
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.4\textwidth}
      The dots \texttt{"..."} argument binds with any number of additional arguments, that aren't already bound by name or position,
      \vskip1ex
      The dots \texttt{"..."} argument is used when the number of arguments isn't known in advance, and allows functions to accept an indefinite number of arguments,
      \vskip1ex
      Arguments that appear after the dots \texttt{"..."} argument must be matched by their full name, and can't be partially name matched,
      \vskip1ex
      The dots \texttt{"..."} argument is often passed on to another nested function,
    \column{0.6\textwidth}
      \vspace{-1em}
        <<echo=(-1)>>=
rm(list=ls())
match_dots <- function(in_put, ...) {
  paste0("in_put=", in_put, "  dots=", ...)
}  # end match_dots
match_dots(10, 2)  # dots matched
match_dots(2, in_put=10, 1)  # dots matched
match_dots(10, 1, 2, 3)  # dots matched
match_dots <- function(..., in_put) {
  paste0("in_put=", in_put, "  dots=", ...)
}  # end match_dots
match_dots(2, 10)  # dots not matched
# arguments after dots must be matched by full name
match_dots(in_put=10, 1, 2, 3)
match_dots <- function(arg1, arg2, ...) {
  arg1 + 2*arg2 + sum(...)
}  # end match_dots
match_dots(3, 2)  # match arguments by position
match_dots(3, 2, 5, 8)  # extra arguments matched to dots
      @
  \end{columns}
\end{block}
...
\begin{block}{}
  \begin{columns}[T]
    ...
    \column{0.7\textwidth}
      \vspace{-1em}
        <<>>=
sum_dots <- function(in_put, ...) {  # define recursive function
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.4\textwidth}
      ...
      \vskip1ex
      The super-assignment operator \texttt{"<<-"} allows operations on objects outside a function's \emph{evaluation} environment, which are called \emph{side effects}, 
      \vskip1ex
      The super-assignment operator \texttt{"<<-"} allows creating functions that produce \emph{side effects}, 
      \vskip1ex
      The function \texttt{load()} is an example of a function that produces \emph{side effects}, 
      \vskip1ex
...
rm(list=ls())  # remove all objects
glob_var <- 1  # define a global variable
side_effect <- function() {  # explore function scope
...
side_effect()
glob_var  # global variable was modified as side effect
rm(list=ls())  # remove all objects
ls()  # list objects
...
func_tional <- function(func_arg, in_put) {
# produce function name from argument
  func_arg <- match.fun(func_arg)
  func_arg(in_put)  # apply input function
}  # end func_tional

func_tional(sqrt, 4)

# first list element is function name
# remaining elements are arguments to function
func_tional <- function(list_arg) {
# produce function name from argument
  func_arg <- match.fun(list_arg[[1]])
# if arguments are in list, then need to use "do.call"
...
\begin{block}{}
  \begin{columns}[T]
    ...
    \column{0.6\textwidth}
      \vspace{-1em}
        <<>>=
# functional accepts function and '...' arguments
func_tional <- function(func_arg, ...) {
  func_arg <- match.fun(list_arg[[1]])
# apply input function to '...' arguments
  func_arg(...)
}  # end func_tional
