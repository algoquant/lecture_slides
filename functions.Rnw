...
test_func(3, 2, 1)  # too many arguments
      @
      All the actual arguments must be \emph{bound} to formal arguments, and if not then an error \texttt{"unused argument"} is produced,
      \vskip1ex
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.3\textwidth}
      ...
      \vskip1ex
      The function \texttt{vapply()} is similar to \texttt{sapply()}, but it always attempts to simplify its output to a matrix, and if it can't then it throws an error,
      \vskip1ex
...
vapply(2:4, function(num) c(el1=num, el2=2*num), 
       FUN.VALUE=c(0,0))
# vapply throws an error if it can't simplify
