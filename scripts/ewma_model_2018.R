...
simu_ewma <- function(oh_lc, lamb_da=0.01, wid_th=251, bid_offer=0.001, tre_nd=1) {
  ...
  in_dic <- tre_nd*xts::xts(sign(as.numeric(clos_e) - ew_ma), order.by=index(oh_lc))
  ...
  prices_lag <- rutils::lag_xts(clos_e)
  ...
  re_turns[trade_dates] <- position_lagged[trade_dates] * (op_en[trade_dates] - prices_lag[trade_dates]) + position_s[trade_dates] * (clos_e[trade_dates] - op_en[trade_dates]) - cost_s
