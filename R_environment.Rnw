...
\begin{block}{}
  \begin{columns}[T]
    \column{0.4\textwidth}
      ...
      \vskip1ex
      It lists packages that are loaded and attached to the search path,
      \vskip1ex
      It also lists packages that are loaded, but \emph{not} attached to the search path,
    \column{0.6\textwidth}
      \vspace{-1em}
      <<>>=
...
%%%%%%%%%%%%%%%
\subsection{File Listing and Handling Functions}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.4\textwidth}
      The function \texttt{list.files()} 
      is similar to \texttt{paste()}, but automatically uses the correct separator for the platform,
      \vskip1ex
...
list.dirs()
list.files(R.home())
list.dirs(R.home("etc"))
list.files(R.home("etc"))
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.4\textwidth}
      ...
      These functions are wrappers for \texttt{read.table()} and \texttt{write.table()},
      \vskip1ex
      Very often \texttt{.csv} files contain custom date-time formats, which need to be passed into  \texttt{read.zoo()} for proper formatting,
    \column{0.6\textwidth}
      \vspace{-1em}
      <<echo=4:30>>=
rm(list=ls())
