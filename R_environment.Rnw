...
\begin{block}{}
  \begin{columns}[T]
    \column{0.4\textwidth}
      \texttt{as.numeric()} convets bad data to \texttt{NA},
      \vskip1ex
...
mat_scrub <- as.numeric(as.matrix(mat_read))
mat_scrub  # vector with NAs
# coerce from vector back to matrix
mat_scrub <- matrix(mat_scrub, ncol=num_col, dimnames=dim_names)
mat_scrub
# perform sapply over columns and coerce to numeric vectors
mat_scrub <- sapply(mat_read, function(col_umn) {
  as.numeric(as.vector(col_umn))
}  # end anon function
)  # end sapply
dimnames(mat_scrub) <- dimnames(mat_read)
# overwrite NAs with zero
mat_scrub[is.na(mat_scrub)] <- 0
mat_scrub  # matrix without NAs
      @
  \end{columns}
\end{block}
