...
options("stringsAsFactors")  # show
options(stringsAsFactors=FALSE)  # set
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.4\textwidth}
      ...
      \begin{itemize}
        \item \texttt{Windows} user \texttt{personal} directory: \texttt{"\textasciitilde{}"} (\texttt{"\%USERPROFILE\%/Documents"}),
      ...
      \end{itemize}
      By default, the \texttt{R} user \texttt{HOME} directory is the \texttt{Windows} user \texttt{personal} directory,
      \vskip1ex
...
%%%%%%%%%%%%%%%
\subsection{Reading and Writing from \texttt{*.Rdata} Files}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.4\textwidth}
      The function \texttt{save()} writes selected objects to a binary file,
      \vskip1ex
      The object names can be passed to the "\texttt{...}" argument, in which case they are not evaluated,
      \vskip1ex
      The object names can also be passed to the "\texttt{list}" argument, in which case they are evaluated,
      \vskip1ex
      The function \texttt{load()} reads data from \texttt{*.RData} files, and \emph{invisibly} returns a vector of names of objects created in the workspace,
    \column{0.6\textwidth}
      \vspace{-1em}
      <<echo=TRUE>>=
rm(list=ls())  # remove all objects
# create a symbol name containing "my_var"
sym_bol <- "my_var"
# assign a value to the name "my_var"
assign(sym_bol, 100)
ls()  # list all objects
ls()[1]  # list first object
# save "my_var" to a binary file
save("my_var", file=paste0(sym_bol, ".RData"))
# save first list object "my_var" by passing it to the "..." argument
save(ls()[1], file=paste0(sym_bol, ".RData"))
# save first list object "my_var" by passing it to the "list" argument
save(list=ls()[1], file=paste0(sym_bol, ".RData"))
# load data from file
