...
print(xtable(gambl_e), comment=FALSE)
    ...
    \column{0.5\textwidth}
      ...
      \vskip1ex
      And the expected value of the wealth after the gamble is equal to: $p \, (1 + k_f a) + q \, (1 - k_f b) = 1 + k_f \, \mu$.
      \vskip1ex
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.9\textwidth}
      The terminal wealth after $n$ repeated gambles with $k$ wins is equal to: $(1 + k_f a)^k (1 - k_f b)^{n-k}$.
      \vskip1ex
      And the expected value of the wealth is equal to: 
      \begin{displaymath}
        w(k_f) = \sum_{k=0}^n {\binom{n}{k} p^k q^{n-k} {(1 + k_f a)^k (1 - k_f b)^{n-k}}}
      \end{displaymath}
      We can then find the fraction $k_f$ which maximizes the expected wealth $w(k_f)$:
      \begin{flalign*}
        \frac{\mathrm{d} w(k_f)}{\mathrm{d} k_f} = \sum_{k=0}^n \binom{n}{k} p^k q^{n-k} {(1 + k_f a)^k (1 - k_f b)^{n-k}} (\frac{a k}{1 + k_f a} - \frac{b (n-k)}{1 - k_f b}) = \\
        \frac{a}{1 + k_f a} \sum_{k=0}^n \binom{n}{k} p^k q^{n-k} {k} - \\
        \sum_{k=0}^n \binom{n}{k} p^k q^{n-k} {(1 + k_f a)^k (1 - k_f b)^{n-k}} (\frac{a k}{1 + k_f a} - \frac{b (n-k)}{1 - k_f b}) \\
      \end{flalign*}
      If the investor chooses to risk only a fraction $k_f$ of wealth, then the wealth after the gamble is either $1 + k_f a$ (with probability $p$), or $1 - k_f b$ (with probability $q = 1-p$).
      \vskip1ex
      ...
      \begin{flalign*}
        u(k_f) &= \sum_{k=0}^n \binom{n}{k} p^k q^{n-k} \log((1 + k_f a)^k (1 - k_f b)^{n-k}) &\\
        &= \log(1 + k_f a) \sum_{k=0}^n {\binom{n}{k} p^k q^{n-k} k} + &\\
        & \log(1 - k_f b) \sum_{k=0}^n {\binom{n}{k} p^k q^{n-k} (n-k)} &\\
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The terminal wealth after $n$ rounds with $k$ wins is equal to: $w(k_f) = (1 + k_f a)^k (1 - k_f b)^{n-k}$.
      \vskip1ex
      If the number of rounds $n$ is very large, then the number of wins is almost always equal to $k = n \, p$, and the terminal wealth is equal to: $w(k_f) = (1 + k_f a)^{np} (1 - k_f b)^{nq}$.
    \column{0.5\textwidth}
      Without loss of generality we can assume that $p = q = \frac{1}{2}$.
      \vskip1ex
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \begin{flalign*}
        & \frac{\mathrm{d} w(k_f)}{\mathrm{d} k_f} = n p a (1 + k_f a)^{np-1} (1 - k_f b)^{nq} & \\
        & - n q b (1 + k_f a)^{np} (1 - k_f b)^{nq-1} & \\
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \end{flalign*}
      Where $\varsigma = \frac{1}{\sigma^3} \int {r^3 \, p(r) \, \mathrm{d}r}$ is the \emph{skewness}, and $\kappa = \frac{1}{\sigma^4} \int {r^4 \, p(r) \, \mathrm{d}r}$ is the \emph{kurtosis}.
      \vskip1ex
...
%%%%%%%%%%%%%%%
\subsection{depr: Backtesting the \protect\emph{EWMA} \protect\emph{Momentum} Strategy}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
...
%%%%%%%%%%%%%%%
\subsection{draft: Rolling Portfolio Optimization Strategy}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      Repeat previous slide but regularize solve(), same as in HighFreq::back\_test()
      \vskip1ex
      Reproduce function HighFreq::calc\_weights() in R. 
      \vskip1ex
      A \emph{rolling portfolio optimization} strategy consists of rebalancing a portfolio over a vector of end points: 
      \setlength{\leftmargini}{1.0em}
      \begin{enumerate}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      Modify this code so it uses matrix of look-back intervals and look-forward intervals.
      \vskip1ex
