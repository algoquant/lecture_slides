...
sqrt(252)*sum(re_turns)/sd(re_turns)/NROW(pnl_s)
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.45\paperwidth]{figure/vwap_strat_open.png}
      <<echo=TRUE,eval=FALSE>>=
# Plot both strategies
...
text(x=0.05, y=0.15, paste("Treynor test t-value =", round(summary(mod_el)$coeff["treynor", "t value"], 2)))
      @
    \column{0.5\textwidth}
    \vspace{-1em}
