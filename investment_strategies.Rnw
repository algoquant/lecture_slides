...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      To limit the portfolio leverage, the weights can be scaled so that the sum of their absolute values (or their squares) is equal to \texttt{1}: $\sum_{i=1}^n {w_i^2} = 1$
      \vskip1ex
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      Multiple \emph{S\&P500} \emph{momentum} strategies can be simulated by calling the function \texttt{backtest\_rolling()} in a loop over a vector of \emph{width} parameters.
      \vskip1ex
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      Multiple \emph{S\&P500} \emph{mean-reverting} strategies can be simulated by calling the function \texttt{backtest\_rolling()} in a loop over a vector of \emph{width} parameters.
      \vskip1ex
