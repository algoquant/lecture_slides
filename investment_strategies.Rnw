...
%%%%%%%%%%%%%%%
\subsection{dev: Identifying Managers With Skill}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
...
re_turns <- 
  rutils::env_etf$re_turns[, c("IEF", "VTI")]
...
sqrt(260)*sapply(re_turns, function(x) mean(x)/sd(x))
      @
    \column{0.5\textwidth}
      \vspace{-1em}
...
dygraphs::dygraph(price_s, main="Stock and Bond Portfolio") %>% 
  dyOptions(colors=c("orange","blue","green"), strokeWidth=c(1,1,3)) %>%
...
reg_model <- lm(momentum_rets ~ vti_b)
summary(reg_model)
      @
    \column{0.5\textwidth}
    \vspace{-1em}
...
# rescale to VTI
in_dex <- as.numeric(first(env_etf$VTI)[, 4])*in_dex / 
  as.numeric((in_dex[start(env_etf$VTI)]))
da_ta <- cbind(in_dex[index(env_etf$VTI)], env_etf$VTI[, 4])
