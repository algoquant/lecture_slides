...
# perform aggregation
agg_regate(oh_lc, lamb_das, look_back)
      @
  \end{columns}
\end{block}
...
\begin{block}{}
  \begin{columns}[T]
    ...
    \column{0.6\textwidth}
      \vspace{-1em}
      <<echo=TRUE,eval=FALSE>>=
# adjust end_points so they are greater than EWMA look_back
end_points[(end_points > 0) & (end_points <= look_back)] <- 
  look_back + 1
# start_points are end_points lagged by look_back
len_gth <- NROW(end_points)
...
agg_s <- lapply(look_backs, 
  function(look_back, ...) {
    ...
    }, lamb_das=lamb_das, look_back=look_back)  # end lapply
...
re_turns <- parLapply(clus_ter, lamb_das, function(lamb_da) {
  ...
  simu_ewma(oh_lc=oh_lc, 
            lamb_da=lamb_da, look_back=look_back)[, "re_turns"]
})  # end parLapply
