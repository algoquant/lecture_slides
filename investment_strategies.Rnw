...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \end{displaymath}
      The \emph{leverage} $\kappa$ is equal to the \emph{margin debt} $m$ divided by the total wealth $w_t$: $\kappa = m / w_t$.
      \vskip1ex
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      In order to avoid ruin, the investor may choose to maintain a fixed \emph{leverage ratio} equal to $\kappa$, so that the amount invested in the \emph{risky asset} is proportional to the \emph{wealth}: $\kappa \, w_t$.
      \vskip1ex
      This requires buying the \emph{risky asset} when its price increases, and selling it when it drops.
      \vskip1ex
      The return on the \emph{risky asset} in a single period is equal to: $\kappa \, w_t \, r_t$, so the \emph{terminal wealth} at time $t$ is equal to the compounded returns: 
      \begin{displaymath}
...
\begin{block}{}
  \begin{columns}[T]
    ...
    \column{0.5\textwidth}
      ...
      \end{flalign*}
      Where $\mathbb{E}[\Delta w]$ is the expected change of wealth, $\sigma^2 = \int {{\Delta w}^2 \, p(w) \, \mathrm{d}w}$ is the \emph{variance} of The change in wealth, and $\mu_3 = \int {{\Delta w}^3 \, p(w) \, \mathrm{d}w} = \sigma^3 \hat{s}$ and $\mu_4 = \int {{\Delta w}^4 \, p(w) \, \mathrm{d}w} = \sigma^4 \hat{k}$ are the third and fourth moments, proportional to the \emph{skewness} $\hat{s}$ and the \emph{kurtosis} $\hat{k}$.
  \end{columns}
\end{block}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The dollar amount of the \emph{risky asset} traded is equal to the change in \emph{margin}, equal to: $\Delta m_t = \Delta [(\kappa_t - 1) w_t]$.
      \vskip1ex
    ...
    \column{0.5\textwidth}
      ...
      The wealth of the Kelly Strategy after accounting for the \emph{bid-offer spread} is then equal to:
      \begin{displaymath}
        w_t = \prod_{i=1}^t {(1 + \kappa \, r_i - \frac{\delta}{2} \frac{\left| \Delta m_i^{nc} \right|}{w_i^{nc}})}
      \end{displaymath}
      The effect of the \emph{bid-offer spread} is to reduce the effective asset returns by an amount proportional to the \emph{bid-offer spread}.
      <<echo=TRUE,eval=FALSE>>=
# bid_offer equal to 10 bps for liquid ETFs
