...
rets_percent <- rets_dollar/rutils::lag_it(price_s, lagg=1)
...
weight_s <- c(0.5, 0.5)
rets_dollar <- rets_dollar[, c("VTI", "IEF")]
re_turns <- rets_dollar %*% weight_s
weal_th <- cumsum(re_turns)
# Wealth for equal dollar amount of shares (with rebalancing)
rets_percent <- rets_percent[, c("VTI", "IEF")]
re_turns <- (rets_percent %*% weight_s)
...
ex_tra <- lapply(rets_percent, function(x)
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The value of the \emph{risky asset} $a_t$ is equal to the wealth $w_t$ times the \emph{leverage} $k_f$: $a_t = k_f w_t$.
      \vskip1ex
      So the \emph{margin debt} $m_t$ is proportional to the wealth $w_t$: $m_t = (k_f - 1) w_t + 1$.
      \vskip1ex
      The wealth changes from $w_{t-1}$ to: $w_t = w_{t-1} (1 + k_f \, r_t)$, while the value of the \emph{risky asset} changes from $a_{t-1} = k_f w_{t-1}$ to: $a_t = k_f w_{t-1} (1 + r_t)$, so that the leverage changes from $k_f$ to:
      \begin{displaymath}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \begin{displaymath}
        c_t = \frac{\delta}{2} k_f (k_f - 1) w_{t-1} \left| r_t \right|
      \end{displaymath}
    \column{0.5\textwidth}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      If the transaction costs are much less than the change in wealth $c_t \ll \left| \Delta w_t \right|$, then they can be calculated approximately as the absolute value of the change in \emph{margin} $m_t^{nc}$ for a wealth path with no transaction costs:
      \begin{displaymath}
        c_t = \frac{\delta}{2} \left| \Delta m_t^{nc} \right|
      \end{displaymath}
    \column{0.5\textwidth}
...
%%%%%%%%%%%%%%%
\subsection{draft: Backtesting Framework with Overlapping Aggregations}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
...
%%%%%%%%%%%%%%%
\subsection{draft: Performing Overlapping Aggregations}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
...
pnl_s <- star_t + cumsum(pnl_s)
    ...
    \column{0.5\textwidth}
      ...
      <<echo=TRUE,eval=FALSE>>=
# Plot the backtest
chart_Series(x=cl_ose[end_p[-n_rows]],
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      A \emph{portfolio optimization} strategy consists of:
      \setlength{\leftmargini}{1.0em}
      \begin{enumerate}
        \item Calculating the maximum Sharpe ratio portfolio weights in the in-sample interval,
        \item Applying the weights and calculating the portfolio returns in the out-of-sample interval.
      \end{enumerate}
      <<echo=TRUE,eval=FALSE>>=
...
in_verse <- MASS::ginv(cov(ex_cess["/2018"]))
weight_s <- in_verse %*% colMeans(ex_cess)
weight_s <- drop(weight_s/sqrt(sum(weight_s^2)))
      @
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.5\paperwidth]{figure/backtest_portfolio.png}
      <<echo=TRUE,eval=FALSE>>=
# Out-of-sample portfolio returns
rets_oos <- re_turns["2019/"]
rets_portf <- xts(rets_oos %*% weight_s, index(rets_oos))
in_dex <- xts(rowMeans(rets_oos), index(rets_oos))
# Plot cumulative portfolio returns
weal_th <- cumsum(cbind(rets_portf, in_dex))
...
%%%%%%%%%%%%%%%
\subsection{Regularized Inverse of \protect\emph{S\&P500} Covariance Matrix}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
...
\begin{block}{}
  \begin{columns}[T]
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      <<echo=TRUE,eval=FALSE>>=
# In-sample excess returns
risk_free <- 0.03/252
ex_cess <- (re_turns["/2018"] - risk_free)
ex_cess <- colMeans(ex_cess)
# Shrink the returns to their mean
al_pha <- 0.7
ex_cess <- (1 - al_pha)*ex_cess + al_pha*mean(ex_cess)
# In-sample maximum Sharpe weights
...
%%%%%%%%%%%%%%%
\subsection{Out-of-Sample \protect\emph{S\&P500} Portfolio Returns}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The out-of-sample portfolio returns are equal to the out-of-sample stock returns times the weights.
      \vskip1ex
      The weights can be scaled to match the volatility of the equally weighted portfolio.
      <<echo=TRUE,eval=FALSE>>=
# Out-of-sample portfolio returns
rets_oos <- re_turns["2019/"]
rets_portf <- xts(rets_oos %*% weight_s, index(rets_oos))
...
in_dex <- xts(rowMeans(rets_oos), index(rets_oos))
# Plot cumulative out-of-sample strategy returns
weal_th <- cbind(rets_portf, in_dex*sd(rets_portf)/sd(in_dex))
weal_th <- cumsum(na.omit(weal_th))
colnames(weal_th) <- c("Strategy", "Equal Weight Portfolio")
dygraphs::dygraph(weal_th, main="S&P500 Portfolio Optimization Strategy") %>%
  dyOptions(colors=c("red","blue"), strokeWidth=2) %>%
  dyLegend(show="always")
      @
    \column{0.5\textwidth}
      \includegraphics[width=0.5\paperwidth]{figure/backtest_sp500.png}
      \vspace{-1em}
      <<echo=TRUE,eval=FALSE>>=
      @
  \end{columns}
