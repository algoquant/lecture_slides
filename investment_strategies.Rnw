...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \begin{displaymath}
        P_i^{EWMA} = (1-\exp(-\lambda)) \sum_{j=0}^{\infty} \exp(-\lambda (i-j)) P_{i-j}
      \end{displaymath}
...
position_lagged <- rutils::lag_xts(po_sitions)
# calculate daily profits and losses
pn_l <- position_lagged*(cl_ose - prices_lag)
...
\begin{block}{}
  \begin{columns}[T]
    ...
    \column{0.6\textwidth}
      ...
      <<echo=(-(1:1)),eval=FALSE>>=
library(HighFreq)
# source model and define data and parameters
source("C:/Develop/R/scripts/ewma_model.R")
win_dow <- 51
lamb_das <- seq(0.001, 0.01, 0.001)
oh_lc <- rutils::env_etf$VTI["/2011"]
...
# create list of look-back intervals
look_backs <- lapply(seq_along(end_points)[-1], function(in_dex) start_points[in_dex]:end_points[in_dex])
# calculate aggregations over look-back intervals
sharpe_ratios <- roll_agg(x_ts=oh_lc, 
                          look_backs=look_backs, 
                          FUN=agg_regate, 
                          lamb_das=lamb_das,
                          win_dow=win_dow)
# calculate optimal EWMA parameters
lamb_das <- seq(0.001, 0.01, 0.001)
      @
  \end{columns}
\end{block}
...
%%%%%%%%%%%%%%%
\subsection{old: Look-back Intervals for Overlapping Aggregations}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
...
start_points <- 
  end_points[c(rep_len(1, look_back-1), 
               1:(len_gth-look_back+1))]
# index of look-back interval
in_dex <- start_points[20]:end_points[20]
# second warmup interval spans two months
warm_up <- oh_lc[start_points[3]:end_points[3]]
dim(warm_up)
head(warm_up)
tail(warm_up)
      @
...
agg_s <- lapply(2:len_gth, 
  function(in_dex, ...) {
    agg_regate(oh_lc[start_points[in_dex]:end_points[in_dex]], ...)
    }, lamb_das=lamb_das, win_dow=win_dow)  # end lapply
# rbind list into single xts or matrix
agg_s <- rutils::do_call_rbind(agg_s)
if (!is.xts(agg_s))
...
%%%%%%%%%%%%%%%
\subsection{Calculating Rolling Portfolio Weights}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      As an example, the portfolio weights can be calculated as proportional to the ratio of the trailing mean return divided by the \emph{MAD}, 
      \vskip1ex
      Constraints can also be applied to the portfolio weights, 
      \vskip1ex
      As an example, the portfolio weights can be de-meaned (so that their sum is equal to zero), to create a long-short portfolio with a small beta, 
      \vskip1ex
      Finally the portfolio weights can also be scaled (so that their absolute sum is equal to \texttt{1}), to maintain constant portfolio leverage, 
      <<echo=TRUE,eval=FALSE>>=
...
%%%%%%%%%%%%%%%
\subsection{old: Performing Backtesting}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
