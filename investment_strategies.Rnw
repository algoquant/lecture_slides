...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      To limit the portfolio leverage, the weights can be scaled so that the sum of their absolute values (or their squares) is equal to $1$: $\sum_{i=1}^n {w^2_i} = 1$
      \vskip1ex
...
%%%%%%%%%%%%%%%
\subsection{draft: Backtesting the EWMA Momentum Strategy}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
...
pnl_s <- star_t + cumsum(pnl_s)
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.45\paperwidth]{figure/backtest_ewma.png}
      <<echo=TRUE,eval=FALSE>>=
# Plot the backtest
...
%%%%%%%%%%%%%%%
\subsection{Backtest of the Momentum Strategy}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
...
\begin{block}{}
  \begin{columns}[T]
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.45\paperwidth]{figure/momentum_profile.png}
      <<echo=TRUE,eval=FALSE>>=
...
weal_th <- cbind(weal_th, wealth_aw)
colnames(weal_th) <- c("Momentum Strategy", "All_weather")
      @
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.45\paperwidth]{figure/momentum_etf_optim.png}
      <<echo=TRUE,eval=FALSE>>=
# Plot the Momentum strategy and benchmark
dygraphs::dygraph(weal_th, main="Momentum Strategy") %>%
...
quantmod::chart_Series(weal_th, theme=plot_theme, lwd=2,
             name="Momentum PnL")
legend("topleft", legend=colnames(weal_th),
  inset=0.1, bg="white", lty=1, lwd=6,
  col=plot_theme$col$line.col, bty="n")
...
%%%%%%%%%%%%%%%
\subsection{Momentum Strategy Market Betas}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
...
weal_th <- cbind(ret_mom, ret_aw, 0.5*(ret_mom + ret_aw))
...
weal_th <- apply(weal_th, MARGIN=2,
  function(x) {cumsum(x)}
)  # end apply
weal_th <- xts::xts(weal_th, date_s)
      @
...
pnl_s <- rowMeans(weight_s*re_turns)
    ...
    \column{0.5\textwidth}
    \vspace{-1em}
      \includegraphics[width=0.45\paperwidth]{figure/momentum_daily_etf.png}
      <<echo=TRUE,eval=FALSE>>=
# Calculate transaction costs
bid_offer <- 0.001
...
weal_th <- cbind(wealth_aw, weal_th)
colnames(weal_th) <- c("all_weather", "momentum")
col_names <- colnames(weal_th)
dygraphs::dygraph(weal_th, main="Momentum vs All-Weather") %>%
  dyAxis("y", label=col_names[1], independentTicks=TRUE) %>%
  dyAxis("y2", label=col_names[2], independentTicks=TRUE) %>%
  dySeries(name=col_names[1], axis="y", col="blue") %>%
...
momentum_daily <- function(re_turns, look_back=252, bid_offer=0.001, trend=1, ...) {
  ...
  weight_s <- rutils::lag_it(weight_s)
  # Calculate momentum profits and losses
  pnl_s <- trend*rowMeans(weight_s*re_turns)
  # Calculate transaction costs
  cost_s <- 0.5*bid_offer*rowSums(abs(rutils::diff_it(weight_s)))
  cumsum(pnl_s - cost_s)
}  # end momentum_daily
...
tail(weal_th)
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.45\paperwidth]{figure/momentum_daily_etf_mult.png}
      <<echo=TRUE,eval=FALSE>>=
# Plot EWMA strategies with custom line colors
plot_theme <- chart_theme()
...
weal_th <- xts::xts(weal_th, index(re_turns))
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.45\paperwidth]{figure/momentum_sp500_mult.png}
      <<echo=TRUE,eval=FALSE>>=
...
weal_th <- xts::xts(weal_th, index(price_s))
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.45\paperwidth]{figure/sp500_revert_mult.png}
      <<echo=TRUE,eval=FALSE>>=
# Plot EWMA strategies with custom line colors
plot_theme <- chart_theme()
plot_theme$col$line.col <-
