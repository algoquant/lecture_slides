...
# Calculate the EWMA of the squared residuals
resids <- (fcasts - respv)
...
resids <- (fcasts - resps)
# Calculate the variance of the residuals
vars <- sum(resids^2)/(NROW(preds)-NROW(coeff))
# Calculate the predictor matrix squared
...
fcasts <- xts::xts(fcasts, zoo::index(retp)[-(1:lookb)])
      @
  \end{columns}
\end{block}
...
%%%%%%%%%%%%%%%
\subsection{EWMA Mean-Reversion Strategy For Daytime Returns}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
...
pnls <- (-retd*posv)
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.45\paperwidth]{figure/strat_ar_ewma_daytime.png}
      <<echo=TRUE,eval=FALSE>>=
...
endd <- rutils::calc_endpoints(wealthv, interval="weeks")
dygraphs::dygraph(cumsum(wealthv)[endd],
  main="EWMA Strategy For Daytime VTI Returns") %>%
  dyOptions(colors=c("blue", "red"), strokeWidth=2) %>%
  dyLegend(show="always", width=300)
      @
  \end{columns}
