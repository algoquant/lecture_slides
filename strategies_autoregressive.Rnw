...
predm <- cbind(rep(1, nrows), predm)
colnames(predm) <- c("phi0", paste0("lag", 1:orderp))
# Add the volume predictor matrix
...
predm <- cbind(predm, predx)
# Add the SVXY predictor matrix
predx <- lapply(1:orderp, rutils::lagit, input=retp$SVXY)
predx <- rutils::do_call(cbind, predx)
colnames(predx) <- paste0("SVXY", 1:orderp)
predm <- cbind(predm, predx)
...
%%%%%%%%%%%%%%%
\subsection{draft: Kitchen Sink Autoregressive Strategy}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
...
fcasts <- fcasts/fcastv
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.45\paperwidth]{figure/ar_kitchen_sink.png}
      <<echo=TRUE,eval=FALSE>>=
# Calculate the autoregressive strategy PnLs
pnls <- respv*fcasts
...
colnames(wealthv) <- c("VTI", "Kitchen_sink")
sqrt(252)*sapply(wealthv[insample, ], function(x) 
  c(Sharpe=mean(x)/sd(x), Sortino=mean(x)/sd(x[x<0])))
sqrt(252)*sapply(wealthv[outsample, ], function(x) 
  c(Sharpe=mean(x)/sd(x), Sortino=mean(x)/sd(x[x<0])))
...
%%%%%%%%%%%%%%%
\subsection{draft: Kitchen Sink Strategy With Dimension Reduction}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
...
%%%%%%%%%%%%%%%
\subsection{draft: Shrinkage of the Kitchen Sink Coefficients}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
...
optiml <- suppressMessages(DEoptim::DEoptim(fn=objfun,
  respv=respv[insample, ],
  predm=predm[insample, ],
  lambdaf=2.0, 
  upper=rep(10000, ncoeff),
...
# Extract the AR coefficients
coeff <- optiml$optim$bestmem/sum(abs(optiml$optim$bestmem))
...
%%%%%%%%%%%%%%%
\subsection{draft: Kitchen Sink Strategy With Shrinkage}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
