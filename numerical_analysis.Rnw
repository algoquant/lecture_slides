...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      All the different \texttt{R} loops have similar speed, but \texttt{apply()}, \texttt{sapply()}, and \texttt{lapply()} are sometimes faster than \texttt{for()} loops.
      \vskip1ex
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The command \texttt{Rprof(filename)} turns on the profiling of \texttt{R} expressions, and saves the profiling data into the file \texttt{filename}.
      \vskip1ex
      If an \texttt{R} expression is executed after profiling is enabled, then its profiling data is written to the file \texttt{filename}.
      \vskip1ex
  ...
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \texttt{ifelse()} is much faster than performing an element-wise loop in \texttt{R}.
        <<func_ifelse,echo=TRUE,eval=FALSE,fig.show='hide'>>=
...
%%%%%%%%%%%%%%%
\subsection{Parallel Bootstrap of the \protect\emph{Median Absolute Deviation}}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      The \emph{bootstrapped} estimator values are then used to calculate the probability distribution of the estimator and its standard error.
      \vskip1ex
      Bootstrapping shows that for stock returns, the \emph{Median Absolute Deviation} (\emph{MAD}) has a smaller relative standard error than the standard deviation does.
      \vskip1ex
...
%%%%%%%%%%%%%%%
\subsection{Bootstrap of Time Series of Prices}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
...
%%%%%%%%%%%%%%%
\subsection{Block Bootstrap of Time Series of Prices}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
...
text(x=barl, y=0.7*max(densv$y), pos=2, "barrier")
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.45\paperwidth]{figure/simu_block_barrier.png}
      <<echo=TRUE,eval=FALSE>>=
...
bootd <- sapply(1:nboot, function(x) {
  ...
  impsample <- sum((datav < quantv)*weightv)/nsimu
  c(naivemc=naivemc, impsample=impsample)
}) ##  end sapply
...
bootd <- sapply(1:nboot, function(x) {
  ...
  weightv <- exp(-lambdaf*datat + lambdaf^2/2)
  cumprob <- cumsum(weightv)/nsimu
  impsample <- datat[findInterval(confl, cumprob)]
  c(naivemc=naivemc, impsample=impsample)
}) ##  end sapply
apply(bootd, MARGIN=1, function(x) c(mean=mean(x), sd=sd(x)))
      @
...
bootd <- sapply(1:nboot, function(x) {
  ...
  cumprob <- cumsum(weightv)/nsimu
  varisk <- datat[findInterval(confl, cumprob)]
  impsample <- sum((datat <= varisk)*datat*weightv)/sum((datat <= varisk)*weightv)
  c(naivemc=naivemc, impsample=impsample)
}) ##  end sapply
apply(bootd, MARGIN=1, function(x) c(mean=mean(x), sd=sd(x)))
      @
...
boot_var <- function(lambdaf, confl=0.05) {
  datat <- datav + lambdaf  ##  Tilt the random numbers
  weightv <- exp(-lambdaf*datat + lambdaf^2/2)
  ##  Calculate quantiles for columns
  bootd <- sapply(1:NCOL(datav), function(coln) {
    cumprob <- cumsum(weightv[, coln])/nsimu
    datat[findInterval(confl, cumprob), coln]
  })  ##  end sapply
  return(sd(bootd))
}  ##  end boot_var
...
# Calculate standard errors of VaR for vector of tilt parameters
stderr <- sapply(lambdav, boot_var, confl=0.02)
# Calculate the optimal tilt parameter
lambdav[which.min(stderr)]
# Plot the standard errors
plot(x=lambdav, y=stderr,
...
zoo::plot.zoo(patht[, sample(npaths, 20)], main="Paths of Brownian Motion", xlab="time steps", ylab=NA, plot.type="single")
# Calculate path weights
weightv <- exp(-lambdaf*datat + lambdaf^2/2)
weightv <- matrixStats::colProds(weightv)
# Or
...
rgl::rglwidget(elementId="plot3drgl", width=800, height=800)
      @
    \column{0.5\textwidth}
      \includegraphics[width=0.45\paperwidth]{figure/rgl_rastrigin.jpeg}
  \end{columns}
\end{block}
...
%%%%%%%%%%%%%%%
\subsection{Newton-Raphson Optimization}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The \emph{Newton-Raphson} method finds the minimum of the objective function by finding the root of its first derivative, using a recursive formula:
      \begin{displaymath}
...
%%%%%%%%%%%%%%%
\subsection{Newton-Raphson Multivariate Optimization}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The \emph{Newton-Raphson} iterative formula for the minimum of a multivariate function, states that the update of the solution is equal to the inverse Hessian matrix times the function gradient:
      \begin{displaymath}
        x_{n+1} = x_n - H_n^{-1} \nabla f_n
      \end{displaymath}
...
%%%%%%%%%%%%%%%
\subsection{Quasi-Newton BFGS Optimization}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The \emph{BFGS} method updates the solution of the function minimum $x_n$ by multiplying the inverse Hessian matrix $H^{-1}_n$ times the function gradient $\nabla f_n$:
      \begin{displaymath}
        x_{n+1} = x_n + \alpha \delta_n
      \end{displaymath}
      Where $\delta_n = - H^{-1}_n \nabla f_n$ is the increment of $x_n$, and $\alpha$ is a factor to satisfy the Armijo condition.
      \vskip1ex
      ...
      \vskip1ex
      The inverse of the Hessian matrix $H^{-1}_n$ is updated at each step of the iteration using the \emph{BFGS} formula:
      \begin{displaymath}
        H^{-1}_{n+1} = (I - \rho s y^T) H^{-1}_n (I - \rho y s^T) + \rho s s^T
      \end{displaymath}
      ...
      \vskip1ex
      The updated inverse Hessian satisfies the secant condition: $H^{-1}_{n+1} (\nabla f_{n+1} - \nabla f_n) = x_{n+1} - x_n$.
    \column{0.5\textwidth}
      \vspace{-1em}
        <<echo=TRUE,eval=FALSE>>=
...
%%%%%%%%%%%%%%%
\subsection{BFGS Optimization Path}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The \emph{BFGS} method is a popular quasi-Newton method, but it requires a large amount of memory for large multivariate optimization problems.
      \vskip1ex
      The \emph{L-BFGS-B} method is a more efficient version of the \emph{BFGS} method, which uses less memory.
      \vskip1ex
      The \emph{Rosenbrock} function is used to test optimization algorithms.  It has a global minimum at the point (1, 1) where the function value is 0.
        <<echo=TRUE,eval=FALSE>>=
...
optiml <- optim(c(-1.2, 1), rosefun, method = "L-BFGS-B")
optiml$par      # Minimum coordinates
...
zg <- outer(xg, yg, rosefun2)
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.45\paperwidth]{figure/optim_rosenbrock.jpeg}
        <<echo=TRUE,eval=FALSE>>=
# Draw 3d surface plot of function
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The inverse Hessian $H^{-1}_{j, n}$ for coordinate $j$ at step $n+1$ is updated using the one-dimensional \emph{BFGS} step:
      \begin{displaymath}
        H^{-1}_{j, n+1} = H^{-1}_{j, n} - \frac{\delta_x^2}{\delta_g}
      \end{displaymath}
      ...
      \vskip1ex
      Coordinate descent doesn't need to multiply and store large matrices in memory like the \emph{BFGS} method does.
    \column{0.5\textwidth}
      \vspace{-1em}
        <<echo=TRUE,eval=FALSE>>=
...
%%%%%%%%%%%%%%%
\subsection{Coordinate Descent Optimization Performance}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      Coordinate descent performs well for high-dimensional problems like portfolio optimization.
      \vskip1ex
      But in many cases it's not as fast as the \emph{BFGS} method.
      \vskip1ex
      Coordinate descent can fail to find the minimum of some objective functions, like the \emph{Rosenbrock} function.
    \column{0.5\textwidth}
      \vspace{-1em}
      <<echo=TRUE,eval=FALSE>>=
...
optiml <- rutils::optim_coordescent(rosefun, x0 = c(-1.2, 1))
