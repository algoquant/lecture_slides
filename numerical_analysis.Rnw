...
lambdas <- seq(-3.0, -1.2, by=0.2)
# Calculate vector of quantiles for tilt parameters
quantiles <- sapply(lambdas, calc_quant)
...
lambdas[which.min(stdevs)]
# Plot the standard deviations
x11(width=6, height=5)
plot(x=lambdas, y=stdevs, 
...
datat <- datav + lambda  # Tilt the random numbers
paths_tilt <- matrixStats::colCumsums(datat)
# Calculate path weights
weights <- exp(-lambda*datat + lambda^2/2)
path_weights <- matrixStats::colProds(weights)
# Or
path_weights <- exp(-lambda*colSums(datat) + nrows*lambda^2/2)
...
summary(microbenchmark(
  ...
  cppcode=inner_mult(1:10000, 1:10000),
  cppcode_sugar=inner_lagmugar(1:10000, 1:10000),
  times=10))[, c(1, 4, 5)]
      @
  \end{columns}
\end{block}
...
boot_data <- function(datav, nboot=nboot) {
  ...
  retsp <- numeric(nrows)
  pricets <- numeric(nrows)
  pricets[1] <- eq_price
  for (i in 2:nrows) {
    retsp[i] <- thetav*(eq_price - pricets[i-1]) + volat*rnorm(1)
    pricets[i] <- pricets[i-1] + retsp[i]
  }  # end for
  pricets
}  # end boot_data
...
sim_ou <- function(nrows=1000, eq_price=5.0,
  ...
  pricets[1] <- eq_price
  for (i in 2:nrows) {
    retsp[i] <- theta*(eq_price - pricets[i-1]) + volat*rnorm(1)
    pricets[i] <- pricets[i-1] + retsp[i]
  }  # end for
  pricets
}  # end sim_ou
...
NumericVector sim_ou_rcpp(double eq_price,
  ...
  NumericVector pricets*nrows);
  NumericVector retsp*nrows);
  pricets[0] = eq_price;
  for (int it = 1; it < nrows; it++) {
    retsp[it] = theta*(eq_price - pricets[it-1]) + volat*innov[it-1];
    pricets[it] = pricets[it-1] + retsp[it];
  }  // end for
  return pricets;
}")  # end cppFunction
...
library(microbenchmark)
summary(microbenchmark(
  rcode=sim_ou(nrows*nrows, eq_price=eq_price, volat=sigmav, theta=thetav),
  cppcode=sim_ou_rcpp(eq_price=eq_price, volat=sigmav, theta=thetav, innov=rnorm(nrows)),
  times=10))[, c(1, 4, 5)]
      @
  \end{columns}
\end{block}
...
NumericVector sim_ou_rcpp(double eq_price,
  ...
  pricets[0] = eq_price;
  for (int it = 1; it < nrows; it++) {
    retsp[it] = thetav*(eq_price - pricets[it-1]) + volat*innov[it-1];
    pricets[it] = pricets[it-1] + retsp[it];
  }  // end for
  return pricets;
}  // end sim_ou_rcpp
...
arma::vec calc_weights(const arma::mat& returns, // Portfolio returns
  ...
  case method::max_sharpe: {
    ...
    // weights = calc_inv(cov(returns), dimax)*meancols;
    weights = calc_inv(cov(returns), eigen_thresh, dimax)*meancols;
    break;
  }  // end max_sharpe
  ...
  case method::min_var: {
    ...
    weights = calc_inv(cov(returns), eigen_thresh, dimax)*arma::ones(returns[ncols);
    break;
  }  // end min_var
...
arma::mat back_test(const arma::mat& excess, // Portfolio excess returns
  ...
  for (arma::uword it = 1; it < endp.size(); it++) {
    ...
    weights = coeff*calc_weights(excess.rows(startp(it-1), endp(it-1)), method, eigen_thresh, dimax, confi, alpha, scale, vol_target);
