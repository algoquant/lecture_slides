...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The function \texttt{fitdistr()} from package \texttt{MASS} fits a univariate distribution to a sample of data, by performing \emph{maximum likelihood} optimization, 
\vspace{-2em}
      <<echo=TRUE,eval=FALSE>>=
# initial parameters
