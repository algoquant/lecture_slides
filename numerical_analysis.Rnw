...
profvis::profvis({
  ...
  reg_model <- lm(price ~ carat, data=ggplot2::diamonds)
...
de_sign <- data.frame(res_ponse, predic_tor)
...
plot(density(boot_strap["predic_tor", ]),
     lwd=2, xlab="regression slopes",
     main="Bootstrapped regression slopes")
abline(v=mean(boot_strap["predic_tor", ]),
       lwd=2, col="red")
      @
    \column{0.5\textwidth}
      \hspace*{-1em}
...
price_s <- sim_ou(len_gth=len_gth, eq_price=eq_price, vol_at=vol_at, the_ta=the_ta)
      @
    \column{0.5\textwidth}
      \vspace{-1em}
      <<echo=TRUE,eval=FALSE>>=
...
price_s <- sim_ou_rcpp(eq_price=eq_price, 
  vol_at=vol_at, 
  the_ta=the_ta, 
