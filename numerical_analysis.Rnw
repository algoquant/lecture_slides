...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The command \texttt{Rprof(filen)} turns on the profiling of \texttt{R} expressions, and saves the profiling data into the file \texttt{filen}.
      \vskip1ex
      If an \texttt{R} expression is executed after profiling is enabled, then its profiling data is written to the file \texttt{filen}.
      \vskip1ex
