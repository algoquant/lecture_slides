...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The \emph{p}-value for DAX returns is extremely small, and we conclude that the \emph{null hypothesis} is FALSE, and the DAX returns are not from a normally distributed population,
    \column{0.5\textwidth}
      \vspace{-1em}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      \vspace{-2.5em}
      <<dax_filter,echo=2:30,tidy=FALSE,fig.show='hide'>>=
...
par(mfrow=c(2,1))  # set plot panels
# autocorrelation from "stats"
Acf(dax.diff[, 2], lag=10, xlab=NA, ylab=NA)
# autocorrelation from "forecast"
Pacf(dax.diff[, 2], lag=10, xlab=NA, ylab=NA)
      @
...
head(acf.dax$acf)
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      A function that returns \texttt{invisible} doesn't automatically return a value,
      <<echo=1:30,tidy=FALSE>>=
my.acf <- function (ts.data, xlab=xlab, 
                    ylab=ylab, main=main, ...)
...
\begin{block}{}
  \begin{columns}[T]
      ...
      The function \texttt{arima.sim()} simulates an ARIMA process,
      \vspace{-1em}
      <<ar_param,echo=2:30,fig.height=7,tidy=FALSE,fig.show='hide'>>=
# ARIMA processes
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.3\textwidth}
      ...
      \vskip1ex
      The function \texttt{lm()} from the base package \texttt{stats} fits a linear model into the data,
    \column{0.7\textwidth}
      \vspace{-1em}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The \emph{Durbin-Watson} is designed to test the \emph{null hypothesis} that the autocorrelations of regression residuals are equal to zero,
      \vskip1ex
      The test statistic is:
      \begin{displaymath}
        DW = \frac {\sum_{i=2}^{n} (e_{i} - e_{i-1})^2} {\sum_{i=1}^{n} e_{i}^2}
      \end{displaymath}
      Where $e_{i}$ are the regression residuals,
      \vskip1ex
      The value of the \emph{Durbin-Watson} statistic \emph{DW} is between 0 and 4,
      \vskip1ex
      \emph{DW}=2 indicates zero autocorrelations,
      \vskip1ex
...
\begin{block}{Required}
  \begin{itemize}[]
    \item create a zoo object helper function
    \item input is zoo object 
  \end{itemize}
\end{block}
\pause
