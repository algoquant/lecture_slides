...
returns <- rutils::etf_env$returns$VTI
...
bwidth <- 10*mad(rutils::diff_it(returns, lagg=10))
# Calculate the kernel density
densityv <- sapply(1:nrows, function(i_d) {
  sum(dnorm(returns-returns[i_d], sd=bwidth))
})  # end sapply
ma_d <- mad(returns)
plot(returns, densityv, xlim=c(-5*ma_d, 5*ma_d),
     t="l", col="blue", lwd=3,
...
plot(densityv, xlim=c(-5*ma_d, 5*ma_d),
     xlab="returns", ylab="density",
...
ma_d <- mad(returns)
# bwidth <- mad(rutils::diff_it(returns))
plot(density(returns, bw=ma_d/10), xlim=c(-5*ma_d, 5*ma_d),
...
%%%%%%%%%%%%%%%
\subsection{draft: The \protect\emph{Kolmogorov-Smirnov} Test for Probability Distributions}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      Needs more work: KS test for VTI returns vs t-distribution
      \vskip1ex
      The \emph{Kolmogorov-Smirnov} test is designed to test the \emph{null hypothesis} that two samples: $\{x1, \ldots , x_n\}$ and $\{y1, \ldots , y_n\}$ were obtained from the same probability distribution.
      \vskip1ex
...
returns <- as.numeric(na.omit(rutils::etf_env$returns$VTI))
returns <- (returns - mean(returns))/sd(returns)
...
returns <- rutils::etf_env$returns[, c("VTI", "DBC", "IEF")]
returns <- na.omit(returns)
...
risk_ret <-
  PerformanceAnalytics::table.Stats(rutils::etf_env$returns)
class(risk_ret)
...
prices <- na.omit(rutils::etf_env$prices[, c("VXX", "SVXY")])
prices <- prices["2017/"]
colnamev <- c("VXX", "SVXY")
...
closep <- log(na.omit(rutils::etf_env$prices$VTI))
...
filter_ed <- filter(closep, filter=weights,
...
weights <- weights/sum(weights)
weights_rev <- rev(weights)
filter_ed <- roll::roll_sum(closep, width=NROW(weights), weights=weights_rev)
# Copy warmup period
filter_ed[1:look_back] <- closep[1:look_back]
# Combine prices with smoothed prices
prices <- cbind(closep, filter_ed)
colnames(prices)[2] <- "VTI Smooth"
# Calculate standard deviations of returns
sapply(rutils::diff_it(prices), sd)
# Plot dygraph
...
weights <- weights/sum(weights)
filter_ed <- roll::roll_sum(closep, width=NROW(weights), weights=weights, online=FALSE)
# Copy warmup period
filter_ed[1:(2*look_back)] <- closep[1:(2*look_back)]
# Center the data
filter_ed <- rutils::lagit(filter_ed, -(look_back-1), pad_zeros=FALSE)
# Combine prices with smoothed prices
prices <- cbind(closep, filter_ed)
colnames(prices)[2] <- "VTI Smooth"
# Calculate standard deviations of returns
...
filter_ed <- cbind(closep, filter_ed)
...
ewma_rfilter <- .Call(stats:::C_rfilter, closep, lambdav, c(as.numeric(closep[1])/(1-lambdav), double(NROW(closep))))[-1]
# Or R code
# ewma_rfilter <- filter(closep, filter=lambdav, init=as.numeric(closep[1, 1])/(1-lambdav), method="recursive")
ewma_rfilter <- (1-lambdav)*ewma_rfilter
# Calculate EWMA prices recursively using RcppArmadillo
ew_ma <- HighFreq::run_mean(closep, lambda=lambdav)
all.equal(drop(ew_ma), ewma_rfilter)
# Compare the speed of C++ code with RcppArmadillo
library(microbenchmark)
summary(microbenchmark(
  run_mean=HighFreq::run_mean(closep, lambda=lambdav),
  rfilter=.Call(stats:::C_rfilter, closep, lambdav, c(as.numeric(closep[1])/(1-lambdav), double(NROW(closep)))),
  times=10))[, c(1, 4, 5)]
      @
    \column{0.5\textwidth}
      \vspace{-1em}
...
volume_rec <- .Call(stats:::C_rfilter, volumes, lambdav, c(as.numeric(volumes[1])/(1-lambdav), double(NROW(volumes))))[-1]
price_rec <- .Call(stats:::C_rfilter, volumes*closep, lambdav, c(as.numeric(volumes[1]*closep[1])/(1-lambdav), double(NROW(closep))))[-1]
vwap_rec <- price_rec/volume_rec
# Calculate VWAP prices recursively using RcppArmadillo
vwap_arma <- HighFreq::run_mean(closep, lambda=lambdav, weights=volumes)
...
adf_stats <- sapply(returns, function(x)
  suppressWarnings(tseries::adf.test(x)$statistic)
)  # end sapply
names(adf_stats) <- colnames(returns)
      @
    \column{0.5\textwidth}
      \vspace{-1em}
...
color_s <- colorRampPalette(c("blue", "red"))(NCOL(returns))
colnamev <- colnames(returns)
dyplot <- dygraphs::dygraph(returns["2019"], main="Fractional Returns") %>%
  dyAxis("y", label=colnamev[1], independentTicks=TRUE) %>%
  dySeries(name=colnamev[1], axis="y", label=colnamev[1], strokeWidth=2, col=color_s[1])
for (i in 2:NROW(colnamev))
  dyplot <- dyplot %>%
  dyAxis("y2", label=colnamev[i], independentTicks=TRUE) %>%
  dySeries(name=colnamev[i], axis="y2", label=colnamev[i], strokeWidth=2, col=color_s[i])
...
volumes <- quantmod::Vo(rutils::etf_env$VTI)
look_back <- 21
volume_mean <- roll::roll_mean(volumes, width=look_back, min_obs=1)
volume_sd <- roll::roll_sd(rutils::diff_it(volumes), width=look_back, min_obs=1)
volume_sd[1] <- 0
...
volat_sd <- roll::roll_sd(rutils::diff_it(volat), width=look_back, min_obs=1)
volat_sd[1] <- 0
...
ma_d <- HighFreq::roll_var(closep, look_back=look_back, method="nonparametric")
# ma_d <- TTR::runMAD(closep, n=look_back)
# Calculate time series of z-scores
z_scores <- (closep - medi_an)/ma_d
...
ma_d <- rutils::lagit(ma_d, lagg=-half_back)
z_scores <- (closep - medi_an)/ma_d
z_scores[1:look_back, ] <- 0
tail(z_scores, look_back)
range(z_scores)
...
returns <- rutils::diff_it(log(SPY["2012-02-13", 4]))
sd(returns)
# SPY returns multiple days (includes overnight jumps)
returns <- rutils::diff_it(log(SPY[, 4]))
sd(returns)
# Table of time intervals - 60 second is most frequent
indeks <- rutils::diff_it(.index(SPY))
table(indeks)
...
vx_x <- na.omit(rutils::etf_env$prices$VXX)
dates <- zoo::index(vx_x)
look_back <- 41
vx_x <- log(vx_x)
# Calculate rolling VTI volatility
closep <- get("VTI", rutils::etf_env)[dates]
closep <- log(closep)
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      Student's \emph{t-distribution} with two degrees of freedom is often used to represent asset returns.
      \vskip1ex
...
returns <- returns / rutils::diff_it(.index(SPY["2012-02-13"]))
returns[1] <- 0
# Minutely SPY volatility scaled to unit per minute
60*sd(returns)
...
returns <- log(rutils::etf_env$VTI[,2] /
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      <<echo=TRUE,eval=FALSE>>=
# Number of data points
nrows <- NROW(rutils::etf_env$VTI["2019/"])
# Number of npoints that fit over nrows
npoints <- 21
nagg <- nrows %/% npoints
# Stub interval at beginning
...
weights <- exp(-0.1*1:look_back)
weights <- weights/sum(weights)
filter_ed <- stats::filter(closep, filter=weights,
                         method="convolution", sides=1)
...
quan_tiles <- runquantile(x=closep, k=look_back,
...
closep <- quantmod::Cl(rutils::etf_env$VTI["2019"])
