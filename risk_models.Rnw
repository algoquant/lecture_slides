...
all.equal(garch_data, cbind(re_turns, vari_ance), 
  check.attributes=FALSE)
      @
      The \emph{GARCH} process must be simulated using explicit loops, either in \texttt{R} or in \texttt{C++}.
  \end{columns}
\end{block}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The \emph{GARCH(1,1)} volatility model produces volatility clustering - periods of high volatility followed by a quick decay.
      \vskip1ex
...
%%%%%%%%%%%%%%%
\subsection{draft: \protect\emph{GARCH} Returns Kurtosis}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The expected value of the variance $\sigma^2$ of \emph{GARCH(1,1)} returns is proportional to the parameter $\omega$:
      \begin{displaymath}
        \sigma^2 = \frac{\omega}{1 - \alpha - \beta}
      \end{displaymath}
      Under the \emph{GARCH(1,1)} volatility model, the returns follow the process: $r_i = \mu + \sigma_i \xi_i$.
      \vskip1ex
      So the \emph{conditional} distribution of returns is normal with standard deviation equal to $\sigma_i$:
      \begin{displaymath}
        \phi(x, \mu, \sigma_i) = \frac{e^{-(x-\mu)^2/2\sigma^2_i}}{\sigma_i\sqrt{2 \pi}}
      \end{displaymath}
      The \emph{GARCH} returns
      is a volatility model defined by two coupled equations:
      \begin{flalign*}
        r_i &= \mu + \sigma_{i-1} \xi_i \\
        \sigma^2_i &= \omega + \alpha (r_i - \mu)^2 + \beta \sigma^2_{i-1}
      \end{flalign*}
      The expected value of the kurtosis of returns $\kappa$ is equal to:
      \begin{flalign*}
        \kappa = 3 + \frac{6 \alpha^2}{1 - \alpha^2 - (\alpha + \beta)^2}
      \end{flalign*}
...
om_ega^2*(1+al_pha+be_ta)/(1-al_pha-be_ta)/(1-2*al_pha^2-(al_pha+be_ta)^2)/(om_ega/(1-al_pha-be_ta))^2

3*(1-(al_pha+be_ta)^2)/(1-2*al_pha^2-(al_pha+be_ta)^2)
...
foo <- sapply(1:1e2, function(x) {
  garch_data <- HighFreq::sim_garch(omega=om_ega, alpha=al_pha, beta=be_ta, innov=matrix(rnorm(n_rows)))
...
histo_gram <- hist(foo[, 2], col="lightgrey",
  xlab="returns", breaks=200, xlim=c(0, 10),
  ylab="frequency", freq=FALSE,
  main="GARCH Returns Histogram")
lines(density(foo[, 2], adjust=1.5), lwd=3, col="blue")
...
%%%%%%%%%%%%%%%
\subsection{\protect\emph{GARCH} Volatility Estimation}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The \emph{GARCH} model can be used to estimate the rolling volatility of empirical (historical) returns.
      \vskip1ex
      ...
      \begin{flalign*}
        \sigma^2_i &= \omega + \alpha (r_i - \mu)^2 + \beta \sigma^2_{i-1}
      \end{flalign*}
      If the returns from the \emph{GARCH(1,1)} simulation are used in the above formula, then it produces the simulated \emph{GARCH(1,1)} volatility.
      \vskip1ex
      But to estimate the rolling volatility of historical returns, the parameters $\omega$, $\alpha$, and $\beta$ must be estimated through model calibration.
    \column{0.5\textwidth}
      \vspace{-1em}
      <<echo=TRUE,eval=FALSE>>=
# Simulate the GARCH process using Rcpp
...
%%%%%%%%%%%%%%%
\subsection{draft: \protect\emph{GARCH} Likelihood Function}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \begin{displaymath}
        \phi(x, \mu, \sigma_i) = \frac{e^{-(x-\mu)^2/2\sigma^2_i}}{\sigma_i\sqrt{2 \pi}}
      \end{displaymath}
      Note that the \emph{unconditional} distribution of returns is \emph{not} normal, since their standard deviation is time-dependent, so they are \emph{leptokurtic} (fat tailed).
      \vskip1ex
      The \emph{log-likelihood} function $\mathcal{L}(\theta|\bar{x})$ is a function of the parameters of a statistical model $\theta$, given a sample of observed values $\bar{x}$, taken under the model's probability distribution $p(x|\theta)$:
      \begin{displaymath}
        \mathcal{L}(\omega, \alpha, \beta | r_i) = - \sum_{i=1}^n (\frac{r^2_i}{\sigma^2_i} + \log(\sigma^2_i))
      \end{displaymath}
...
HighFreq::lik_garch(omega=om_ega, alpha=al_pha,  beta=be_ta, returns=matrix(re_turns))
...
# Create matrix of likelihood values
alphas <- seq(from=0.1, to=0.3, len=100)
betas <- seq(from=0.7, to=0.9, len=100)
sums <- seq(from=0.981, to=0.999, len=50)
diffs <- seq(from=0.8, to=0.99, len=50)
lik_mat <- sapply(sums, function(y) sapply(diffs,
  function(x) HighFreq::lik_garch(omega=om_ega, alpha=(y-x)/2,  beta=(y+x)/2, returns=matrix(re_turns))))

sum(!is.finite(lik_mat))
lik_mat[!is.finite(lik_mat)] <- NA
lik_mat <- apply(lik_mat, 2, na.locf, na.rm=FALSE)
lik_mat <- apply(lik_mat, 2, na.locf, fromLast=TRUE, na.rm=FALSE)
class(lik_mat)
dim(lik_mat)
...
library(rgl)
# Draw 3d surface plot of likelihood function
rgl::persp3d(sums, diffs, lik_mat, col="green",
        xlab="sum", ylab="diff", zlab="likelihood")
# Render the surface plot
rgl::rglwidget(elementId="plot3drgl")


rgl::persp3d(x=function(al_pha, be_ta) {
  HighFreq::lik_garch(omega=om_ega, alpha=al_pha,  beta=be_ta, returns=matrix(re_turns))}, 
  xlim=c(0.1, 0.3), ylim=c(0.7, 0.9),
  col="green", axes=FALSE, omega=om_ega, returns=matrix(re_turns))
...
%%%%%%%%%%%%%%%
\subsection{\protect\emph{GARCH} Volatility Forecasts}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The one-step-ahead forecast for $(r_i - \mu)^2 = (\sigma_{i-1} \xi_i)^2$ is equal to $\sigma^2_{i-1}$, so the forecast for the variance $\sigma^2_i$ is equal to: $\omega + (\alpha + \beta) \sigma^2_{i-1}$
      \vskip1ex
      So if no new shocks arrive, then the variance gradually settles to the equilibrium value $\sigma^2$, such that its forecast is equal to itself: $\sigma^2 = \omega + (\alpha + \beta) \sigma^2$.
      \vskip1ex
      This gives: $\sigma^2 = \frac{\omega}{1 - \alpha - \beta}$. \\
      Which is the long-term expected value of the variance.
      \vskip1ex
      So if no new shocks arrive, then the variance decays exponentially to its equilibrium value: $\sigma^2_i = \sigma^2 + (\alpha + \beta) (\sigma^2_{i-1} - \sigma^2) = \sigma^2 + (\alpha + \beta)^i (\sigma^2_1 - \sigma^2)$
    \column{0.5\textwidth}
      \vspace{-1em}
