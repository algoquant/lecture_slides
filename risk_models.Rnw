...
volum <- coredata(quantmod::Vo(ohlc))
...
volumroll <- HighFreq::roll_vec(volum, look_back=look_back)/look_back
...
retsc <- ifelse(volum > 0, sqrt(volumroll)*retvti/sqrt(volum), 0)
retsc <- sd(retvti)*retsc/sd(retsc)
# retsc <- ifelse(volum > 1e4, retvti/volum, 0)
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \end{displaymath}
      Where the decay parameter $\lambda$ determines the rate of decay of the \emph{RSI} weightv, with larger values of $\lambda$ producing faster decay, giving more weight to recent prices, and vice versa,
      \vspace{-1em}
      <<echo=TRUE,eval=FALSE>>=
...
retvti <- lapply(deltav, function(deltav) {
  weightv <- (deltav - 0:(look_back-2)) / 1:(look_back-1)
...
retvti <- do.call(cbind, retvti)
retvti <- cbind(closep, retvti)
colnames(retvti) <- c("VTI", paste0("frac_", deltav))
# Calculate ADF test statistics
adfstats <- sapply(retvti, function(x)
  suppressWarnings(tseries::adf.test(x)$statistic)
)  # end sapply
names(adfstats) <- colnames(retvti)
      @
    \column{0.5\textwidth}
      \vspace{-1em}
...
colorv <- colorRampPalette(c("blue", "red"))(NCOL(retvti))
colnamev <- colnames(retvti)
dyplot <- dygraphs::dygraph(retvti["2019"], main="Fractional Returns") %>%
  dyAxis("y", label=colnamev[1], independentTicks=TRUE) %>%
...
volum <- quantmod::Vo(rutils::etfenv$VTI)
look_back <- 21
volumean <- roll::roll_mean(volum, width=look_back, min_obs=1)
volumsd <- roll::roll_sd(rutils::diffit(volum), width=look_back, min_obs=1)
volumsd[1] <- 0
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \begin{displaymath}
        z_i = \frac{2 p_t - p_{t-k} - p_{t+k}}{\sigma_t}
      \end{displaymath}
...
retvti <- rutils::diffit(closep)
volat <- roll::roll_sd(retvti, width=look_back, min_obs=1)
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The trailing \emph{z-score} $z_i$ of a price $p_t$ can be defined as the \emph{standardized residual} of the linear regression with respect to time $t_i$ or some other variable:
      \begin{displaymath}
        z_i = \frac{p_t - (\alpha + \beta t_i)}{\sigma_t}
      \end{displaymath}
      Where $\alpha$ and $\beta$ are the \emph{regression coefficients}, and $\sigma_t$ is the standard deviation of the residuals.
      \vskip1ex
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The parameter $\lambda$ determines the rate of decay of the \emph{EWMA} weightv, with smaller values of $\lambda$ producing faster decay, giving more weight to recent realized variance, and vice versa.
      \vskip1ex
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The function \texttt{HighFreq::run\_var()} calculates the trailing variance of a \emph{time series} of returns, by recursively weighing the past variance estimates $\sigma^2_{t-1}$, with the squared differences of the returns minus the trailing means $(r_t - \mu_t)^2$, using the decay factor $\lambda$:
      \begin{flalign*}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \end{flalign*}
      Where $\sigma^2_t$ is the time-dependent variance, equal to the weighted average of the point \emph{realized} variance $(r_t - \mu)^2$ and the past variance $\sigma^2_{t-1}$, and $\xi_t$ are standard normal \emph{innovations}.
      \vskip1ex
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \begin{flalign*}
        r_t &= \mu + \sigma_{t-1} \xi_t \\
        \sigma^2_t &= \omega + \alpha (r_t - \mu)^2 + \beta \sigma^2_{t-1}
      \end{flalign*}
      Where $\sigma^2_t$ is the time-dependent variance, equal to the weighted average of the point \emph{realized} variance $(r_t - \mu)^2$ and the past variance $\sigma^2_{t-1}$, and $\xi_t$ are standard normal \emph{innovations}.
      \vskip1ex
      The parameter $\alpha$ is the weight associated with recent realized variance updates, and $\beta$ is the weight associated with the past variance.
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      Under the \emph{GARCH(1,1)} volatility model, the returns follow the process: $r_t = \mu + \sigma_{t-1} \xi_t$.  (We can assume that the returns have been de-meaned, so that $\mu = 0$.)
      \vskip1ex
...
likmat <- sapply(alphas, function(alpha) sapply(betas,
...
rgl::persp3d(alphas, betas, likmat, col=color[zcols],
