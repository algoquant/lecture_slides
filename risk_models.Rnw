...
volatf <- sqrt(HighFreq::run_var(retp, lambda=lambdafa))[, 2]
volats <- sqrt(HighFreq::run_var(retp, lambda=lambdasl))[, 2]
# Calculate the recursive trailing z-scores of VTI volatility
volatd <- volatf - volats
volatsd <- sqrt(HighFreq::run_var(rutils::diffit(volatd), lambda=lambdafa)[, 2])
volatsd[1] <- 0
