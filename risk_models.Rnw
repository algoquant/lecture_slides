...
knitr::kable(risk_ret[c("VXX", "SVXY"), c("Sharpe", "Skewness", "Kurtosis")])
      @
      \includegraphics[width=0.5\paperwidth]{figure/vix_vxx_svxy.png}
      <<echo=TRUE,eval=FALSE>>=
# dygraph plot of VTI drawdowns
...
text(x=risk_ret$Skewness, y=risk_ret$Kurtosis,
      ...
      @
    \column{0.5\textwidth}
      \includegraphics[width=0.5\paperwidth]{figure/etf_skew_sharp.png}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      Feature engineering means deriving predictive data elements (features) from a large input data set.
      \vskip1ex
      ...
      \vskip1ex
      Feature engineering can use both \emph{domain knowledge} and \emph{}
...
%%%%%%%%%%%%%%%
\subsection{depr: Filtering Time Series Using Function \texttt{filter()}}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
...
filter_ed <- filter(clos_e, filter=co_eff,
...
all.equal(filter_ed, filter_fast)
...
filter_fast <- .Call(stats:::C_cfilter, clos_e, filter=co_eff,
...
library(microbenchmark)
summary(microbenchmark(
  fast_filter=.Call(stats:::C_cfilter, clos_e, filter=co_eff, sides=1, circular=FALSE),
  R_filter=filter(clos_e, filter=co_eff, method="convolution", sides=1)
  ), times=10)[, c(1, 4, 5)]
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      Smoothing reduces the \emph{variance} in the data but it increases its \emph{bias}, because it introduces a time lag.
      \vskip1ex
      ...
      <<echo=TRUE,eval=FALSE>>=
# Copy over NA values
filter_ed <- zoo::na.locf(filter_ed, fromLast=TRUE)
...
sapply(re_turns, sd)
# Plot dygraph
dygraphs::dygraph(price_s["2009"], main="VTI Prices and Smoothed Prices") %>%
  dyOptions(colors=c("black", "red"), strokeWidth=2)
      @
    \column{0.5\textwidth}
      \hspace*{-1em}
      \includegraphics[width=0.45\paperwidth]{figure/filter_vti.png}
  \end{columns}
\end{block}
...
title(main="ACF of Smoothed VTI Returns", line=-1)
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.45\paperwidth]{figure/filter_acf.png}
  \end{columns}
\end{block}
...
colnames(price_s) <- c("VTI", "VTI EWMA")
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.45\paperwidth]{figure/filter_ewma.png}
      <<echo=TRUE,eval=FALSE>>=
...
colnames(price_s) <- c(sym_bol, paste(sym_bol, "VWAP"))
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.45\paperwidth]{figure/filter_vwap.png}
      <<echo=TRUE,eval=FALSE>>=
# Dygraphs plot with custom line colors
col_ors <- c("blue", "red")
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      $r_i = p^{ewma\_fast}_i - p^{ewma\_slow}_i$
      \vskip1ex
...
ewma_slow <- roll::roll_sum(clos_e, width=look_back, weights=weight_s, min_obs=1)
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.45\paperwidth]{figure/filter_returns.png}
      <<echo=TRUE,eval=FALSE>>=
# Calculate VTI returns
...
\begin{block}{}
  \begin{columns}[T]
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.45\paperwidth]{figure/filter_fracret.png}
      <<echo=TRUE,eval=FALSE>>=
...
names(adf_stats) <- colnames(re_turns)
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.45\paperwidth]{figure/filter_fracrets.png}
      <<echo=TRUE,eval=FALSE>>=
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The \emph{volume z-score} is equal to the volume $v_i$ minus the trailing average volumes $\bar{v_i}$ divided by the volatility of the volumes $\sigma_i$:
      {\scriptsize
      \begin{displaymath}
...
hist(volume_scores, breaks=1e2)
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.45\paperwidth]{figure/filter_volume_zscores.png}
      <<echo=TRUE,eval=FALSE>>=
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      Extreme prices can be identified using the \emph{price z-score} equal to the price difference with neighboring prices divided by the volatility of returns $\sigma_i$:
      {\scriptsize
      \begin{displaymath}
        z_i = \frac{2 p_i - p_{i-l} - p_{i+l}}{\sigma_i}
      \end{displaymath}
      }
      Where $p_{i-l}$ and $p_{i+l}$ are the lagged and advanced prices.
      \vskip1ex
      The lag parameter $l$ determines the scale of the extreme local prices, with smaller $l$ producing larger z-scores for more local price extremes.
      <<echo=TRUE,eval=FALSE>>=
...
price_scores <- ifelse(vol_at > 0, price_scores/vol_at, 0)
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.45\paperwidth]{figure/filter_price_zscores.png}
      <<echo=TRUE,eval=FALSE>>=
...
%%%%%%%%%%%%%%%
\subsection{Labelling the Tops and Bottoms of Prices}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
...
pnl_s <- cumsum(re_turns*position_s)
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.45\paperwidth]{figure/filter_labels.png}
      <<echo=TRUE,eval=FALSE>>=
...
%%%%%%%%%%%%%%%
\subsection{draft: \protect\emph{Hampel Filter} for Outlier Detection}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      need new HighFreq function see code comment
      \vskip1ex
      ...
      {\scriptsize
      \begin{displaymath}
        z_i = \frac{x_i - \operatorname{median}(\mathbf{x})}{\operatorname{MAD}}
      \end{displaymath}
      }
...
medi_an <- roll::roll_median(clos_e, width=win_dow)
# Calculate time series of z-scores
ma_d <- TTR::runMAD(clos_e, n=win_dow)

# This can't work - need new HighFreq function
foo <- roll::roll_median(abs(clos_e-medi_an), width=win_dow)
all.equal(unname(coredata(ma_d[-(1:(2*win_dow+1))])), unname(coredata(foo[-(1:(2*win_dow+1))])))
foo <- cbind(ma_d, foo)
tail(foo, 22)

hampel_scores <- (clos_e - medi_an)/ma_d
hampel_scores[1:win_dow, ] <- 0
tail(hampel_scores, win_dow)
range(hampel_scores)
      @
    \column{0.5\textwidth}
      \vspace{-1em}
...
histo_gram <- hist(hampel_scores, col="lightgrey",
  xlab="z-scores", breaks=50, xlim=c(-4, 4),
  ylab="frequency", freq=FALSE, main="Hampel Z-scores histogram")
lines(density(hampel_scores, adjust=1.5), lwd=3, col="blue")
      @
  \end{columns}
\end{block}
...
medi_an <- TTR::runMedian(clos_e, n=win_dow)
ma_d <- TTR::runMAD(clos_e, n=win_dow)
hampel_scores <- (clos_e - medi_an)/ma_d
hampel_scores[1:win_dow, ] <- 0
tail(hampel_scores, win_dow)
...
is_bad <- (abs(hampel_scores) > thresh_old)
sum(is_bad)
...
con_fuse(is_jump, hampel_scores, thresh_old=thresh_old)
# Define vector of discrimination thresholds
threshold_s <- seq(from=0.2, to=5.0, by=0.2)
# Calculate error rates
error_rates <- sapply(threshold_s, con_fuse,
  actu_al=is_jump, z_scores=hampel_scores)  # end sapply
