...
%%%%%%%%%%%%%%%
\subsection{Fat Tails of Asset Returns}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      So large values are produced only when all the contributing binomial variables are of the same sign, which is very improbable.
      \vskip1ex
      This produces extremely low tail probabilities (thin tails), but the probabilities of large negative asset returns decreases much slower, close to that of the negative power of the returns (fat tails).
      \vskip1ex
...
%%%%%%%%%%%%%%%
\subsection{Package \protect\emph{PerformanceAnalytics} for Risk and Return Analysis}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
...
library(PerformanceAnalytics)  # Load package "PerformanceAnalytics"
chart.CumReturns(
  rutils::etf_env$re_turns[, c("XLF", "DBC", "IEF")], lwd=2,
...
n_cores <- detectCores() - 1  # Number of cores
clus_ter <- makeCluster(n_cores)  # Initialize compute cluster
clusterExport(clus_ter, c("n_rows", "da_ta"))
boot_data <- parLapply(clus_ter, 1:10000,
  function(x) {
    sampl_e <- da_ta[sample.int(n_rows, replace=TRUE)]
    c(sd=sd(sampl_e), mad=mad(sampl_e))
...
%%%%%%%%%%%%%%%
\subsection{draft: The Downside Deviation of Asset Returns}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      Empirical asset returns have negative skewness, so their downside risk is often greater than their upside risk (at least for short horizons).
      \vskip1ex
      In addition, investors are mostly concerned with downside risk, not upside risk (unless they are shorting the asset). 
      \vskip1ex
      For normally distributed data the \emph{MAD} has a larger standard error than the standard deviation.
      \vskip1ex
      But for distributions with fat tails (like asset returns), the standard deviation has a larger standard error than the \emph{MAD}.
...
%%%%%%%%%%%%%%%
\subsection{draft: Drawdown Risk}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      Add drawdown code from homework
      \vskip1ex
      <<drawdown_plot,eval=FALSE,echo=(-(1:1)),fig.width=7,fig.height=6,fig.show="hide">>=
options(width=200)
library(PerformanceAnalytics)
chart.Drawdown(rutils::etf_env$re_turns[, "VTI"], ylab="",
               main="VTI drawdowns")
      @
      \vskip27ex
      <<echo=(-(1:2)),eval=FALSE>>=
options(width=200)
library(PerformanceAnalytics)
table.Drawdowns(rutils::etf_env$re_turns[, "VTI"])
      @
    \column{0.5\textwidth}
    \vspace{-1em}
...
%%%%%%%%%%%%%%%
\subsection{draft: Value at Risk (\protect\emph{VaR})}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The \emph{\emph{Value at Risk}} (\emph{VaR}) is equal to the quantile of the return distribution at a given confidence level $\alpha$:
      \begin{displaymath}
        \mathrm{CVaR} = \frac{1}{\alpha} \int_{0}^\alpha \mathrm{VaR}(p) \, \mathrm{d}p
      \end{displaymath}
      The \emph{\emph{Conditional Value at Risk}} is also called the \emph{Expected Shortfall} (\emph{ES}), or the Expected Tail Loss (\emph{ETL}).
      \vspace{-1em}
      <<echo=(-(1:2)),eval=FALSE>>=
...
%%%%%%%%%%%%%%%
\subsection{draft: Conditional Value at Risk (\protect\emph{CVaR})}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
...
library(PerformanceAnalytics)
chart.Histogram(rutils::etf_env$re_turns[, 1], main="",
  xlim=c(-0.06, 0.06),
  methods = c("add.density", "add.normal"))
# Add title
title(main=paste(colnames(rutils::etf_env$re_turns[, 1]),
                 "density"), line=-1)
      @
    \column{0.5\textwidth}
    \vspace{-2em}
...
library(PerformanceAnalytics)
chart.Boxplot(rutils::etf_env$re_turns[,
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      <<echo=(-1),eval=FALSE>>=
library(PerformanceAnalytics)
table.Stats(etf_env$re_turns[, c("VTI", "IEF", "DBC", "VXX")])
risk_return <- table.Stats(etf_env$re_turns)
# Transpose the data frame
class(risk_return)
risk_return <- as.data.frame(t(risk_return))
# Copy risk_return into etf_env and save to .RData file
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \begin{displaymath}
        S_{r}=\frac{E[R-R_t]}{\sqrt{\sum_{i=1}^{n} ([R_i-R_t]_{-})^2}}
      \end{displaymath}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \begin{displaymath}
        \sigma \propto {\Delta t} ^ H
      \end{displaymath}
      Where \texttt{H} is the \emph{Hurst} exponent.
      \vskip1ex
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The \emph{Hurst exponent} $H$ can be calculated by regressing the volatility against the aggregation intervals.
        <<echo=TRUE,eval=FALSE>>=
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The \emph{Rescaled Range} $RS_{\Delta t}$ is proportional to the length of the aggregation interval $\Delta t$ raised to the power of the \emph{Hurst exponent} $H$:
      \begin{displaymath}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      During the May 6, 2010 \emph{flash crash}, \emph{range} volatility spiked more than the \emph{Close-to-Close} volatility.
      <<echo=TRUE,eval=FALSE>>=
library(HighFreq)  # Load HighFreq
