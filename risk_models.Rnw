...
# Calculate skew and kurtosis of VTI returns
calcmom(retvti, moment=3)
calcmom(retvti, moment=4)
      @
  \end{columns}
\end{block}
...
ks.test(sample(retvti, replace=TRUE), ptdistr, dfree=3, locv=locv, scalev=scalev)
      @
  \end{columns}
\end{block}
...
# Plot the Normal probability distribution
curve(expr=dnorm(x, mean=mean(retvti), sd=sd(retvti)), lwd=3, col="green", add=TRUE, log="y")
# Add legend
...
madv <- mad(retvti)
# bwidth <- mad(rutils::diffit(retvti))
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      Investors typically prefer larger \emph{odd moments} of the return distribution (mean, skewness), and smaller \emph{even moments} (varv, kurtosis).
      \vskip1ex
...
sum(is.na(retsp))
# retsp <- retsp["2000/"]
skews <- sapply(retsp, calcmom, moment=3)
# skews <- sapply(retsp, calcmom, moment=4)
# skews <- sapply(retsp, sd, na.rm=TRUE)
skews <- sort(skews)
namesv <- names(skews)
nrows <- NROW(namesv)
# Select high skew and low skew ETFs
...
# Calculate returns and log prices
low_skew <- rowMeans(retsp[, low_skew], na.rm=TRUE)
low_skew[1] <- 0
high_skew <- rowMeans(retsp[, high_skew], na.rm=TRUE)
high_skew[1] <- 0
wealthv <- cbind(high_skew, low_skew)
wealthv <- xts::xts(wealthv, zoo::index(retsp))
wealthv <- cumsum(wealthv)
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The function \texttt{roll::roll\_sum()} calculates the \emph{weighted} trailing sum (convolution) even faster than \texttt{stats:::C\_cfilter()}.
    \column{0.5\textwidth}
      \vspace{-1em}
      <<echo=TRUE,eval=FALSE>>=
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \end{displaymath}
      The decay parameter $\lambda$ determines the rate of decay of the \emph{RSI} weights, with larger values of $\lambda$ producing faster decay, giving more weight to recent pricev, and vice versa,
      \vspace{-1em}
      <<echo=TRUE,eval=FALSE>>=
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \end{displaymath}
      The decay parameter $\lambda$ determines the rate of decay of the \emph{EWMA} weights, with smaller values of $\lambda$ producing faster decay, giving more weight to recent pricev, and vice versa.
      \vskip1ex
...
ewmacpp <- HighFreq::run_mean(closep, lambda=lambda, weightv=0)
all.equal(drop(ewmacpp), ewmar)
# Compare the speed of C++ code with RcppArmadillo
library(microbenchmark)
summary(microbenchmark(
  filtercpp=HighFreq::run_mean(closep, lambda=lambda, weightv=0),
...
look_back <- 21
vwap <- roll::roll_sum(closep*volumv, width=look_back, min_obs=1)
volumroll <- roll::roll_sum(volumv, width=look_back, min_obs=1)
vwap <- vwap/volumroll
colnames(vwap) <- "VWAP"
...
weightv <- weightv/sum(weightv)
ewmaf <- roll::roll_sum(closep, width=look_back, weights=weightv, min_obs=1)
lambda <- 0.05
weightv <- exp(lambda*1:look_back)
weightv <- weightv/sum(weightv)
ewmas <- roll::roll_sum(closep, width=look_back, weights=weightv, min_obs=1)
      @
    \column{0.5\textwidth}
      \vspace{-1em}
...
retvti <- roll::roll_sum(closep, width=look_back, weights=weightv, min_obs=1, online=FALSE)
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The \emph{true range} is the difference between high and low prices is a proxy for the spot volatility in a bar of data.
      \vskip1ex
...
%%%%%%%%%%%%%%%
\subsection{Recursive Volatility Z-Scores}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
...
colnames(pricev) <- c("VTI", "Z-Scores")
colnamev <- colnames(pricev)
dygraphs::dygraph(pricev["2008/2009"], main="VTI Recursive Volatility Z-Scores") %>%
  dyAxis("y", label=colnamev[1], independentTicks=TRUE) %>%
  dyAxis("y2", label=colnamev[2], independentTicks=TRUE) %>%
  dySeries(name=colnamev[1], axis="y", label=colnamev[1], strokeWidth=2, col="blue") %>%
  dySeries(name=colnamev[2], axis="y2", label=colnamev[2], strokeWidth=2, col="red") %>%
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The trailing price z-score is equal to the difference between the current price $p_t$ minus the trailing average price $\bar{p}_{t-k}$, divided by the volatility of returns $\sigma_t$:
      \begin{displaymath}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The recursive trailing price z-score is equal to the difference between the current price $p_t$ minus the trailing average price $\bar{r}$, divided by the volatility of returns $\sigma_t$:
      \begin{flalign*}
        & \bar{r}_t = \lambda \bar{r}_{t-1} + (1-\lambda) r_t \\
        & \sigma^2_t = \lambda \sigma^2_{t-1} + (1-\lambda) (r_t - \bar{r}_t)^2 \\
...
pricer <- (closep - HighFreq::run_mean(closep, lambda=lambda, weightv=0))
pricer <- ifelse(volat > 0, pricer/volat, 0)
# Plot dygraph of the trailing z-scores of VTI prices
pricev <- xts::xts(cbind(pricez, pricer), datev)
colnames(pricev) <- c("Z-Scores", "Recursive")
colnamev <- colnames(pricev)
dygraphs::dygraph(pricev["2008/2009"], main="VTI Recursive Trailing Price Z-Scores") %>%
  dyOptions(colors=c("blue", "red"), strokeWidth=2) %>% 
  dyLegend(show="always", width=500)
      @
  \end{columns}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The regression z-scores represent the second derivative (curvature) of the stock pricev, since the price level and slope are subtracted.
      \vskip1ex
      The regression z-scores can be used as a rich or cheap indicator, either relative to past pricev, or relative to prices in a stock pair.
      \vskip1ex
      ...
      \vskip1ex
      The function \texttt{HighFreq::roll\_reg()} calculates rolling regressions and their residuals.
    \column{0.5\textwidth}
      \vspace{-1em}
...
dygraphs::dygraph(datav["2008/2009"], main="VTI Regression Z-Scores") %>%
  dyAxis("y", label=colnamev[1], independentTicks=TRUE) %>%
  dyAxis("y2", label=colnamev[2], independentTicks=TRUE) %>%
  dySeries(name=colnamev[1], axis="y", label=colnamev[1], strokeWidth=2, col="blue") %>%
  dySeries(name=colnamev[2], axis="y2", label=colnamev[2], strokeWidth=2, col="red") %>%
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The trailing regressions of the stock price $p_t$ with respect to the predictor (explanatory) variables $\varkappa_t$ can be calculated using the following recursive formulas:
      \begin{flalign*}
        & \bar{p}_t = \lambda \bar{p}_{t-1} + (1-\lambda) p_t \\
        & \bar{\varkappa}_t = \lambda \bar{\varkappa}_{t-1} + (1-\lambda) \varkappa_t \\
        & \sigma^{cov}_t = \lambda \sigma^{cov}_{t-1} + (1-\lambda) (p_t - \bar{p}_t)^T (\varkappa_t - \bar{\varkappa}_t) \\
        & \sigma^2_t = \lambda \sigma^2_{t-1} + (1-\lambda) (\varkappa_t - \bar{\varkappa}_t)^T (\varkappa_t - \bar{\varkappa}_t) \\
        & \beta_t = \lambda \beta_{t-1} + (1-\lambda) \sigma^{-2}_t \sigma^{cov}_t \\
        & \alpha_t = \bar{p}_t - \beta_t \bar{\varkappa}_t
      \end{flalign*}
      Where $\alpha_t$ and $\beta_t$ are the \emph{regression coefficients}, 
      $\sigma^{cov}_t$ is the covariance matrix between the response and the predictor data, 
      and $\sigma^2_t$ is the covariance matrix of the predictors ($\sigma^{-2}_t$ is the inverse of the covariance).
      \vskip1ex
...
datav <- cbind(closep, regz[, NCOL(regz)])
colnames(datav) <- c("VTI", "Betas")
colnamev <- colnames(datav)
dygraphs::dygraph(datav["2008/2009"], main="VTI Recursive Regression Betas") %>%
  dyAxis("y", label=colnamev[1], independentTicks=TRUE) %>%
  dyAxis("y2", label=colnamev[2], independentTicks=TRUE) %>%
  dySeries(name=colnamev[1], axis="y", label=colnamev[1], strokeWidth=2, col="blue") %>%
  dySeries(name=colnamev[2], axis="y2", label=colnamev[2], strokeWidth=2, col="red") %>%
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The trailing \emph{z-score} $z_t$ of the stock price $p_t$ is equal to the standardized residual $\epsilon_t$:
      \begin{flalign*}
    ...
    \column{0.5\textwidth}
      ...
      <<echo=TRUE,eval=FALSE>>=
# Plot dygraph of z-scores of VTI prices
datav <- cbind(closep, regz[, 1])
colnames(datav) <- c("VTI", "Z-Scores")
colnamev <- colnames(datav)
dygraphs::dygraph(datav["2008/2009"], main="VTI Recursive Regression Z-Scores") %>%
  dyAxis("y", label=colnamev[1], independentTicks=TRUE) %>%
  dyAxis("y2", label=colnamev[2], independentTicks=TRUE) %>%
  dySeries(name=colnamev[1], axis="y", label=colnamev[1], strokeWidth=2, col="blue") %>%
  dySeries(name=colnamev[2], axis="y2", label=colnamev[2], strokeWidth=2, col="red") %>%
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The function \texttt{stats:::C\_cfilter()} calculates the convolution of a vector or a time series with a filter of coefficients (weightv).
      \vskip1ex
...
weightv <- exp(-0.1*1:look_back)
weightv <- weightv/sum(weightv)
varv <- .Call(stats:::C_cfilter, retvti^2, filter=weightv, sides=1, circular=FALSE)
...
%%%%%%%%%%%%%%%
\subsection{Recursive Realized Volatility Estimator}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \begin{flalign*}
        r_t &= \sigma_{t-1} \xi_t \\
        \sigma^2_t &= \omega + \alpha r_t^2 + \beta \sigma^2_{t-1}
      \end{flalign*}
...
lines(density(retsp, adjust=1.5), lwd=3, col="blue")
    ...
    \column{0.5\textwidth}
      ...
      <<echo=TRUE,eval=FALSE>>=
# Fit t-distribution into GARCH returns
fitobj <- MASS::fitdistr(retsp, densfun="t", df=2, lower=c(-1, 1e-7))
locv <- fitobj$estimate[1]
scalev <- fitobj$estimate[2]
curve(expr=dt((x-locv)/scalev, df=2)/scalev,
...
c(mu=mean(retsp), omega=omega,alpha=alpha, beta=betav)
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      Under the \emph{GARCH(1,1)} volatility model, the returns follow the process: $r_t = \sigma_{t-1} \xi_t$.  (We can assume that the returns have been de-meaned.)
      \vskip1ex
      ...
      \begin{displaymath}
        \sigma^2_t = \omega + \alpha r^2_t + \beta \sigma^2_{t-1}
      \end{displaymath}
      The \emph{GARCH} process must be simulated using an explicit loop, so it's better to perform it in \texttt{C++} instead of \texttt{R}.
    \column{0.5\textwidth}
      \vspace{-1em}
...
summary(microbenchmark(
  ...
  Rcpp=HighFreq::lik_garch(omega=omega, alpha=alpha, beta=betav, returns=matrix(retsp))
  ), times=10)[, c(1, 4, 5)]
      @
  \end{columns}
\end{block}
...
likefun <- function(alpha, betav) {
  ...
  -HighFreq::lik_garch(omega=omega, alpha=alpha, beta=betav, returns=retsp)
}  # end likefun
...
garch_data <- HighFreq::sim_garch(omega=omega, alpha=alpha, 
  beta=betav, innov=retvti, is_random=FALSE)
all.equal(garch_data[, 2], variance, check.attributes=FALSE)
# Plot dygraph of the estimated GARCH volatility
dygraphs::dygraph(xts::xts(sqrt(varv), zoo::index(retvti)), 
...
shapiro.test(coredata(retsp))
tseries::jarque.bera.test(retsp)
# Fit VTI returns using MASS::fitdistr()
fitobj <- MASS::fitdistr(retsp, densfun="t", df=2)
fitobj$estimate; fitobj$sd
# Calculate moments of standardized returns
sapply(3:4, moments::moment, x=(retsp - mean(retsp))/sd(retsp))
      @
    \column{0.5\textwidth}
      \vspace{-1em}
...
\begin{block}{}
  \begin{columns}[T]
    ...
    \column{0.5\textwidth}
      A \emph{rolling aggregation} is performed over a vector of \emph{end points} in time.
      \vskip1ex
...
getAnywhere(C_cfilter)
str(stats:::C_cfilter)
filterfast <- .Call(stats:::C_cfilter, closep, 
