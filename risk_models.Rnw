...
retsd <- sd(retp)
qqnorm(retp/retsd, main="VTI Q-Q Plot",
       ylim=c(-1000*retsd, 1000*retsd), 
       xlab="Normal Quantiles", ylab="VTI Quantiles (sd)")
# Fit a line to the normal quantiles
qqline(retp/retsd, col="red", lwd=2)
# Perform Shapiro-Wilk test of normality
shapiro.test(as.numeric(retp[1:499]))
      @
    \column{0.5\textwidth}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      Some investors argue that positive returns don't represent risk, only those returns that are less than the target rate of return $r_t$.
      \vskip1ex
      The downside deviation (\emph{DD}, or semi-deviation) $\sigma_{d}$ is equal to the standard deviation of the returns minus the target rate of return $r_t$.
      \vskip1ex
      There are two methods of calculating the \emph{DD}.
      \vskip1ex
      The first method is to sum over the full time series, while setting the positive values to zero:
      \begin{displaymath}
        \sigma^2_{d} = \frac{1}{n} \sum_{i=1}^{n} \min(r_i-r_t, 0)^2
      \end{displaymath}
      But this can underestimate the volatility when the excess returns are positive, because it ignores the positive values.
      \vskip1ex
      The second method is to sum only over the negative values:
      \begin{displaymath}
        \sigma^2_{d} = \frac{1}{k} \sum_{(r_i - r_t) < 0}^{n} (r_i - r_t)^2
      \end{displaymath}
      But this can overestimate the volatility of streaming (live) returns, when the excess returns are positive, because it ignores the positive values.
    \column{0.5\textwidth}
      \vspace{-1em}
      <<echo=TRUE,eval=FALSE>>=
library(PerformanceAnalytics)
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
\fbox{\parbox{\columnwidth-1.5em}{
      The distribution of stock returns has a long left tail of negative returns representing the risk of loss.
}}
      \vskip1ex
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The skewness of monthly returns is larger than for daily returns, but their kurtosis and tail risks are lower.
      <<echo=TRUE,eval=FALSE>>=
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \end{displaymath}
      Where $r_t$ is the filtered output vector, $\varphi_i$ are the filter coefficients, and $\xi_t$ are standard normal innovations.
      \vskip1ex
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The \emph{Exponential Moving Average Price} (\emph{EMA}) is defined as the weighted average of prices over a trailing interval:
      \begin{displaymath}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The Exponential Moving Average \emph{EMA} prices can be calculated recursively as follows:
      \begin{displaymath}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      Time-varying volatility can be more accurately estimated using an \emph{Exponential Moving Average} (\emph{EMA}) variance estimator.
      \vskip1ex
...
%%%%%%%%%%%%%%%
\subsection{Estimating Trailing Variance Using Package \protect\emph{roll}}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
...
%%%%%%%%%%%%%%%
\subsection{Exponential Moving Average \protect\emph{EMA} Volatility Estimator}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The exponential moving average (\emph{EMA}) variance of the returns $r_t$ can be calculated by recursively weighting the past variance estimates $\sigma^2_{t-1}$, with the squared differences of the returns minus their trailing means $(r_t - \bar{r}_t)^2$:
      \begin{flalign*}
        & \bar{r}_t = \lambda \bar{r}_{t-1} + (1 - \lambda) r_t \\
        & \sigma^2_t = \lambda^2 \sigma^2_{t-1} + (1 - \lambda^2) (r_t - \bar{r}_t)^2
      \end{flalign*}
      Where $\bar{r}_t$ and $\sigma^2_t$ are the \emph{EMA} mean and variance at time $t$, and $\lambda$ is the decay factor.
      \vskip1ex
      The decay factor $\lambda$ determines how quickly the mean and variance estimates are updated, with smaller values of $\lambda$ producing faster updating, giving more weight to recent prices, and vice versa.
      \vskip1ex
      The function \texttt{HighFreq::run\_var()} calculates the \emph{EMA} mean and variance of the returns $r_t$.
    \column{0.5\textwidth}
      \vspace{-1em}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \end{flalign*}
      Where $\sigma^2_t$ is the time-dependent variance, equal to the weighted average of the point \emph{realized} variance $(r_t - \bar{r}_t)^2$ and the past variance $\sigma^2_{t-1}$, and $\xi_t$ are standard normal innovations.
      \vskip1ex
...
garchsim <- HighFreq::sim_garch(omega=omega, alphac=alphac,
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The source of uncertainty are the returns $r_t$, which are proportional to the standard normal innovations $\xi_t$.
      \vskip1ex
      The parameter $\alpha$ is the weight associated with recent realized variance updates, and $\beta$ is the weight associated with the past variance.
      \vskip1ex
      The long-term equilibrium value of the variance is proportional to the parameter $\omega$:
      \begin{displaymath}
        \sigma^2_{eq} = \frac{\omega}{1 - \alpha - \beta}
      \end{displaymath}
      So the sum of $\alpha$ plus $\beta$ should be less than $1$, otherwise the volatility is explosive.
      \vskip1ex
\fbox{\parbox{\columnwidth-1.5em}{
      The difference between the \emph{GARCH(1,1)} estimator and the \emph{EMA} estimator is that the \emph{GARCH(1,1)} decays to a non-zero equilibrium value $\sigma^2_{eq}$, while the \emph{EMA} estimator decays to zero.
}}
    \column{0.5\textwidth}
      \vspace{-1em}
      <<echo=TRUE,eval=FALSE>>=
...
all.equal(garchsim, cbind(retp, varv), check.attributes=FALSE)
      @
      The \emph{GARCH} process is path dependent, so it must be simulated using an explicit loop, and it's better to perform it in \texttt{C++} instead of \texttt{R}.
      \vskip1ex
      NYU Professor 
      \href{https://www.stern.nyu.edu/faculty/bio/robert-engle}{Robert Engle}
      was awarded the Nobel Prize in Economics for developing the \emph{GARCH} volatility model.
  \end{columns}
\end{block}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      <<echo=TRUE,eval=FALSE>>=
# Specify GARCH model
garchspec <- fGarch::garchSpec(model=list(ar=c(0, 0), omega=omega,
  alpha=alphac, beta=betac))
# Simulate GARCH model
garchsim <- fGarch::garchSim(spec=garchspec, n=nrows)
retp <- as.numeric(garchsim)
# Calculate kurtosis of GARCH returns
...
kurt <- sapply(1:1e4, function(x) {
  garchsim <- HighFreq::sim_garch(omega=omega, alphac=alphac,
    betac=betac, innov=matrix(rnorm(nrows)))
  retp <- garchsim[, 1]
...
HighFreq::lik_garch(omega=omega, alphac=alphac,
  betac=betac, returns=matrix(retp))
# Benchmark speed of likelihood calculations
library(microbenchmark)
summary(microbenchmark(
  Rcode=likefun(omega, alphac, betac),
  Rcpp=HighFreq::lik_garch(omega=omega, alphac=alphac, betac=betac, returns=matrix(retp))
  ), times=10)[, c(1, 4, 5)]
      @
  \end{columns}
\end{block}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \begin{displaymath}
        \sigma^2_{eq} = \frac{\omega}{1 - \alpha - \beta} = \frac{1}{n-1} \sum_{t=1}^n (r_t-\bar{r})^2
      \end{displaymath}
...
likefun <- function(alphac, betac) {
  ...
  -HighFreq::lik_garch(omega=omega, alphac=alphac, betac=betac, returns=retp)
}  ##  end likefun
...
cbind(actual=c(alphac=alphac, betac=betac, omega=omega),
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The flat shape of the \emph{GARCH} likelihood function makes it difficult for \emph{gradient} optimization methods to find the best parameters.
      \vskip1ex
...
likefun <- function(param, retp) {
  alphac <- param[1]; betac <- param[2]; omega <- param[3]
  -HighFreq::lik_garch(omega=omega, alphac=alphac, betac=betac, returns=retp)
}  ##  end likefun
# Initial parameters
initp <- c(alphac=0.2, beta=0.4, omega=varv/0.2)
# Find max likelihood parameters using quasi-Newton method
fitobj <- optim(par=initp,
  fn=likefun, ##  Log-likelihood function
  method="L-BFGS-B", ##  Quasi-Newton method
  retp=retp,
  upper=c(0.4, 0.55, 2*varv), ##  Upper constraint
  ...
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \begin{displaymath}
        \sigma^2_t = \omega + \beta \sigma^2_{t-1} + \alpha r_t^2
      \end{displaymath}
      The \emph{GARCH} estimator of the trailing variance is a generalization of the exponential moving average (\emph{EMA}) variance estimator:
      \begin{displaymath}
        \sigma^2_t = \lambda \sigma^2_{t-1} + (1 - \lambda) r^2_t
      \end{displaymath}
      The main difference is that the \emph{GARCH} model has a non-zero equilibrium value of the variance $\sigma^2_{eq}$, while the \emph{EMA} estimator decays to zero.
      <<eval=FALSE,echo=TRUE>>=
# Calculate VTI returns
retp <- na.omit(rutils::etfenv$returns$VTI)
# Find max likelihood parameters using DEoptim
...
garchsim <- HighFreq::sim_garch(omega=omega, alphac=alphac,
  betac=betac, innov=retp, is_random=FALSE)
all.equal(garchsim[, 2], varv, check.attributes=FALSE)
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The variance forecasts gradually decay to the equilibrium value $\sigma^2_{eq}$, such that the forecast is equal to itself: $\sigma^2_{eq} = \omega + (\alpha + \beta) \sigma^2_{eq}$.
      \vskip1ex
      This gives: $\sigma^2_{eq} = \frac{\omega}{1 - \alpha - \beta}$, which is the long-term expected value of the variance.
      \vskip1ex
      So the variance forecasts decay exponentially to their equilibrium value $\sigma^2$ at the decay rate equal to $(\alpha + \beta)$:
      \begin{displaymath}
        \sigma^2_{t+1} - \sigma^2_{eq} = (\alpha + \beta) (\sigma^2_t - \sigma^2_{eq})
      \end{displaymath}
      \vspace{-1em}
      <<echo=TRUE,eval=FALSE>>=
# Simulate GARCH model
...
print(xtable(gamblev), comment=FALSE)
    ...
    \column{0.5\textwidth}
      If the investor chooses to risk only a fraction $k$ of wealth, then the return on the gamble is either $k a$ (with probability $p$), or $- k b$ (with probability $q = 1-p$).
      \vskip1ex
      The fraction $k$ can be greater than $1$ (leveraged investing), or it can be negative (shorting).
      \vskip1ex
      And the expected return on the gamble is equal to: $p \, k a - q \, k b = k \, \mu$.
      \vskip1ex
      If an investor makes decisions exclusively based on the expected return $\mu$, then they would either invest all their wealth ($k = 1$) on the gamble if $\mu > 0$, or choose not to invest at all ($k = 0$) if $\mu < 0$.
      \vskip1ex
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      An investor with \emph{logarithmic utility} invests only a fraction $k$ of their wealth in a gamble, depending on the risk-return of the gamble.
      \vskip1ex
      If the initial wealth is equal to $1$, then the expected value of \emph{logarithmic utility} for the binary gamble is equal to: $u(k) = p \, \log(1 + k a) + q \, \log(1 - k b)$.
      \vskip1ex
...
log_util <- function(frac, p=0.3, a=20, b=1) {
  p*log(1+frac*a) + (1-p)*log(1-frac*b)
}  ##  end log_util
# Plot utility
curve(expr=log_util, xlim=c(0, 1),
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \begin{displaymath}
        \frac{\mathrm{d} u(k_r)}{\mathrm{d} k_r} = \frac{p \, a}{1 + k_r a} - \frac{q \, b}{1 - k_r b} = 0
      \end{displaymath}
      \begin{displaymath}
        k_r = \frac{p}{b}-\frac{q}{a} = \frac{p \, a - q \, b}{b \, a} = \frac{\mu}{b \, a}
      \end{displaymath}
      The optimal betting fraction $k_r$ is called the \emph{Kelly ratio}, and it's proportional to the expected return (\emph{edge}) on the gamble $\mu$, and inversely proportional to the product of the loss amount $b$ and the win amount $a$.
      \vskip1ex
      The \emph{Kelly ratio} can be greater than $1$ (leveraged investing), or it can be negative (shorting).
      \vskip1ex
      The \emph{Kelly ratio} is zero if the ratio of the probabilities of losing and winning is equal to the betting odds: $\frac{q}{p} = \frac{a}{b}$. 
      \vskip1ex
      As the betting odds $\frac{a}{b}$ increase the \emph{Kelly ratio} approaches its maximum value of $\frac{p}{b}$.
    \column{0.5\textwidth}
      \vspace{-1em}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The \emph{Kelly criterion} states that investors who want to maximize their logarithmic utility should bet a fraction of their capital equal to the \emph{Kelly ratio}.
      \vskip1ex
      ...
      \vskip1ex
      For example, if the betting odds are very large $a \to \infty$, then the \emph{Kelly ratio}: $k_r = \frac{p}{b}$.
    \column{0.5\textwidth}
    \vspace{-1em}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The expected payout $\mu = p \, a - q \, b$ for most lottery tickets is negative.
      \vskip1ex
      Therefore the \emph{Kelly ratio} $k_r = \frac{\mu}{b \, a}$ for most lottery tickets is also negative, so investors shouldn't be expected to buy these lottery tickets.
      \vskip1ex
      But in reality many people do buy lottery tickets with negative expected payouts, which means that either their utility functions are not logarithmic, or that they're not assigning probabilities to the outcomes in a rational way.
      \vskip1ex
\fbox{\parbox{\columnwidth-1.5em}{
      The demand for lottery tickets can be explained by assuming a high demand for positive \emph{skewness}, which is higher than the demand for a positive payout.
}}
      \vskip1ex
    ...
    \column{0.5\textwidth}
      The \emph{Prospect Theory} of behavioral economics (developed by Kahneman and Tversky), argues that people overestimate small probabilities, so they assign a much higher probability to the chance of winning the lottery than the actual probability, making the expected payout of the gamble appear positive.
      \vskip1ex
      ...
      \vskip1ex
      The variance is then equal to: $\sigma^2 = p \, q \, (a + 1)^2 = a$.
      \vskip1ex
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The wealth of a Kelly Strategy with a fixed leverage ratio $k$ is equal to:
      \begin{displaymath}
        w_n = \prod_{t=1}^n {(1 + k \, r_t)}
      \end{displaymath}
      The \emph{Kelly ratio} $k_r$ provides the optimal leverage to maximize the utility of wealth, by balancing the benefit of leveraging higher positive returns, with the risk of ruin due to excessive leverage.
      \vskip1ex
...
wealthp <- cbind(kelly_wealth, wealthyper, wealthsub)
...
quantmod::chart_Series(wealthp, theme=themev,
             name="Wealth Paths")
legend("topleft", legend=colnames(wealthp),
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      In multiperiod betting the investor participates in multiple rounds of sucessive gambles, and in each round they risk a fixed fraction $k$ of their current outstanding wealth.
      \vskip1ex
      Let $r_i$ be the random return on the gamble in period $i$, and let $w_i = (1 + k r_i)$ be the random wealth increment.
      \vskip1ex
      In the binary gamble the return $r_i$ is equal to either $a$ (with probability $p$), or $-b$ (with probability $q = 1-p$).
      \vskip1ex
      Then the terminal wealth after $n$ rounds of betting is equal to the compounded wealth increments: $w_n = \prod_{i=1}^n w_i = \prod_{i=1}^n (1 + k r_i)$.
      \vskip1ex
      And the cumulative utility is equal to the sum of the individual utilities:
      \begin{displaymath}
        u_n = \log(w_n) = \sum_{i=1}^n \log(w_i) = \sum_{i=1}^n \log(1 + k r_i) = \sum_{i=1}^n u_i
      \end{displaymath}
      The individual utilities are all maximized by the same fraction $k = k_r$ equal to the \emph{Kelly ratio}, so the \emph{Kelly ratio} for multiperiod betting is the same as for single period betting:
      \begin{displaymath}
        k = \frac{p}{b}-\frac{q}{a} = k_r
      \end{displaymath}
    \column{0.5\textwidth}
  \end{columns}
\end{block}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      In multiperiod betting the investor participates in $n$ rounds of sucessive gambles, and in each round they risk a fixed fraction $k$ of their current outstanding wealth.
      \vskip1ex
      In each round the wealth is multiplied by either $(1 + k a)$ (win) or $(1 - k b)$ (loss), so that the current outstanding wealth changes over time.
      \vskip1ex
      The terminal wealth after $n$ rounds with $m$ wins is equal to: $w_n = (1 + k a)^m (1 - k b)^{n-m}$.
      \vskip1ex
      If the number of rounds $n$ is very large, then the number of wins is almost always equal to $m = n \, p$, and the terminal wealth is equal to: $w_n = (1 + k a)^{np} (1 - k b)^{nq}$.
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.45\paperwidth]{figure/kelly_multi.png}
      <<echo=TRUE,eval=FALSE>>=
# Wealth of multiperiod binary betting
wealth_fun <- function(f, a, b, n, p) {
  m <- n*p
  (1+f*a)^m * (1-f*b)^(n-m)
}  ##  end wealth
curve(expr=wealth_fun(x, a=0.8, b=0.1, n=1e3, p=0.15), xlim=c(0, 1),
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.9\textwidth}
      The betting fraction $k$ that maximizes the terminal wealth is found by setting the derivative of $w_n$ to zero:
      \begin{flalign*}
        & \frac{\mathrm{d} w_n}{\mathrm{d} k} = n p a (1 + k a)^{np-1} (1 - k b)^{nq} - n q b (1 + k a)^{np} (1 - k b)^{nq-1} & \\
        & = (\frac{n p a}{1 + k a} - \frac{n q b}{1 - k b}) (1 + k a)^{np} (1 - k b)^{nq} = 0 &
      \end{flalign*}
      We can then solve for the optimal betting fraction $k$:
      \begin{flalign*}
        \frac{p a}{1 + k a} - \frac{q b}{1 - k b} = 0 \\
        p a (1 - k b) - q b (1 + k a) = 0 \\
        p a - q b - k a b = 0 \\
        k = \frac{p a - q b}{a b} = \frac{p}{b} - \frac{q}{a} = k_r
      \end{flalign*}
      The above is just the \emph{Kelly ratio} $k_r$ that maximizes the utility.
      \vskip1ex
      So the \emph{Kelly ratio} $k_r$ that maximizes the utility also maximizes the terminal wealth.
    \column{0.1\textwidth}
  \end{columns}
\end{block}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.9\textwidth}
      The terminal wealth after $n$ repeated gambles with $m$ wins is equal to: $(1 + k_r a)^m (1 - k_r b)^{n-m}$.
      \vskip1ex
      And the expected value of the wealth is equal to:
      \begin{displaymath}
        w(k_r) = \sum_{m=0}^n {\binom{n}{m} p^m q^{n-m} {(1 + k_r a)^m (1 - k_r b)^{n-m}}}
      \end{displaymath}
      We can then find the fraction $k_r$ which maximizes the expected wealth $w(k_r)$:
      \begin{flalign*}
        \frac{\mathrm{d} w(k_r)}{\mathrm{d} k_r} = \sum_{m=0}^n \binom{n}{m} p^m q^{n-m} {(1 + k_r a)^m (1 - k_r b)^{n-m}} (\frac{a m}{1 + k_r a} - \frac{b (n-m)}{1 - k_r b}) = \\
        \frac{a}{1 + k_r a} \sum_{m=0}^n \binom{n}{m} p^m q^{n-m} {m} - \\
        \sum_{m=0}^n \binom{n}{m} p^m q^{n-m} {(1 + k_r a)^m (1 - k_r b)^{n-m}} (\frac{a m}{1 + k_r a} - \frac{b (n-m)}{1 - k_r b}) \\
      \end{flalign*}
      If the investor chooses to risk only a fraction $k_r$ of wealth, then the wealth after the gamble is either $1 + k_r a$ (with probability $p$), or $1 - k_r b$ (with probability $q = 1-p$).
      \vskip1ex
      (with probability $p$), or $1 - b$ (with probability $q = 1-p$).
      initial wealth is equal to $1$, and the
      The \emph{Kelly ratio} for multiperiod betting can be found by maximizing the expected \emph{utility} of the final wealth distribution:
      \begin{flalign*}
        u(k_r) &= \sum_{m=0}^n \binom{n}{m} p^m q^{n-m} \log((1 + k_r a)^m (1 - k_r b)^{n-m}) &\\
        &= \log(1 + k_r a) \sum_{m=0}^n {\binom{n}{m} p^m q^{n-m} m} + &\\
        & \log(1 - k_r b) \sum_{m=0}^n {\binom{n}{m} p^m q^{n-m} (n-m)} &\\
        &= n \, p \, \log(1 + k_r a) + n \, q \, \log(1 - k_r b)
      \end{flalign*}
      The above is just the single period \emph{utility} multiplied by the number of rounds of betting $n$.
      \vskip1ex
      The \emph{Kelly ratio} $k_r$ for multiperiod betting is the same as for single period betting:
      \begin{displaymath}
        k_r = \frac{p}{b}-\frac{q}{a}
      \end{displaymath}
    \column{0.1\textwidth}
  \end{columns}
\end{block}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      Let $r_t$ be the percentage stock returns, so that the stock price $p_n$ at time $T$ is given by:
      \begin{displaymath}
        p_T = p_0 \prod_{t=1}^T {(1 + r_t)}
      \end{displaymath}
      The initial investor wealth at time $t=0$ is equal to $1$ dollar, and they also borrow on margin $m$ dollars to invest in the stock.
      \vskip1ex
      The investor's \emph{wealth} at time $t$ is equal to (the margin borrowing rate is assumed to be zero):
      \begin{displaymath}
        w_T = 1 + m \, \frac{p_T - p_0}{p_0}
      \end{displaymath}
      The \emph{leverage} $k$ is equal to the \emph{margin debt} $m$ divided by the total wealth $w_T$: $k = m / w_T$.
      \vskip1ex
      If the stock price drops then the \emph{leverage} increases, because the \emph{margin debt} is fixed while the wealth drops.
      \vskip1ex
...
set.seed(1121, "Mersenne-Twister", sample.kind="Rejection")
pricep <- sapply(1:3, function(x) cumprod(1 + rnorm(1e3, sd=0.01)))
colorv <- c("blue", "red", "green")
matplot(pricep, type="l", lwd=2, main="Simulated Stock Prices",
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      In order to avoid ruin, the investor may choose to maintain a fixed \emph{leverage ratio} equal to $k$, so that the amount invested in the stock is proportional to the \emph{wealth}: $k \, w_t$.
      \vskip1ex
      This requires buying the stock when its price increases, and selling it when it drops.
      \vskip1ex
      The return on the stock investment in a single period is equal to: $k \, w_t \, r_t$, so the \emph{terminal wealth} at time $T$ is equal to the compounded returns:
      \begin{displaymath}
        w_T = (1 + k \, r_1) \ldots (1 + k \, r_T) = \prod_{t=1}^T {(1 + k \, r_t)}
      \end{displaymath}
      The utility of the \emph{terminal wealth} is equal to the sum of the utilities of single periods:
      \begin{flalign*}
        & \mathbbm{E}[\log{w_T}] = \mathbbm{E}[\log((1 + k \, r_1) \ldots (1 + k \, r_T))] &\\
        & = \sum_{t=1}^T {\mathbbm{E}[\log{(1 + k \, r_t)}]} = T \, \mathbbm{E}[\log{(1 + k \, r)}]
      \end{flalign*}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \begin{displaymath}
        u_t = \mathbbm{E}[\log{w_t}] = t \, \mathbbm{E}[\log{(1 + k \, r)}] = t \, u_r
      \end{displaymath}
      The utility of the asset returns $u_r$ is equal to:
      \begin{displaymath}
        u_r = \mathbbm{E}[\log{(1 + k \, r)}] = \int {\log(1 + k \, r) \, p(r) \, \mathrm{d}r}
      \end{displaymath}
      The leverage $k$ is limited so that $(1 + k \, r) > 0$ for all return values $r$.
      \vskip1ex
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      If the returns (gamble payouts) are small $r \ll 1$, then the \emph{logarithmic utility} $u_r$ can be expanded in the moments of the return distribution:
      \begin{flalign*}
        & u_r = \mathbbm{E}[\log{(1 + k \, r)}] = \int {\log(1 + k \, r) \, p(r) \, \mathrm{d}r} & \\
        & = \int {(k \, r - \frac{(k \, r)^2}{2} + \frac{(k \, r)^3}{3} - \frac{(k \, r)^4}{4}) \, p(r) \, \mathrm{d}r} & \\
        & = k \bar{r} - \frac{k^2 \sigma^2}{2} + \frac{k^3 \sigma^3 \varsigma}{3} - \frac{k^4 \sigma^4 \kappa}{4}
      \end{flalign*}
      ...
      \begin{displaymath}
        \frac{\mathrm{d} u_r}{\mathrm{d}k} = \bar{r} - k_r \sigma^2 + k_r^2 \sigma^3 \varsigma - k_r^3 \sigma^4 \kappa = 0
      \end{displaymath}
      % wippp
...
%%%%%%%%%%%%%%%
\subsection{Wealth Paths for Different Leverage Ratios}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      A \emph{Kelly Strategy} is multiperiod betting with a fixed leverage ratio $k$.
      The terminal wealth of a \emph{Kelly Strategy} is equal to:
      \begin{displaymath}
        w_T = \prod_{t=1}^T {(1 + k \, r_t)}
      \end{displaymath}
      The \emph{Kelly ratio} $k$ provides the optimal leverage to maximize the utility of wealth, by balancing the benefit of leveraging higher positive returns, with the risk of ruin due to excessive leverage.
      \vskip1ex
      If the mean asset returns are positive, then a higher leverage ratio provides higher returns.
      \vskip1ex
      But if the leverage is too high, then the losses in periods with negative returns wipe out most of the wealth, so then it's slow to recover.
...
kellyr <- drop(mean(retp)/var(retp))
wealthk <- cumprod(1 + kellyr*retp)
wealthyper <- cumprod(1 + 1.5*kellyr*retp)
wealthsub <- cumprod(1 + 0.5*kellyr*retp)
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The \emph{margin debt} $m_t$ is equal to the dollar amount borrowed to purchase the stock investment.
      \vskip1ex
      The wealth $w_t$ at time $t$ is equal to the initial wealth $w_0 = 1$ plus the dollar amount of the stock investment $a_t$, minus the \emph{margin debt} $m_t$: $w_t = 1 + a_t - m_t$.
      \vskip1ex
      The dollar amount of the stock investment $a_t$ is equal to the wealth $w_t$ times the \emph{leverage} $k$: $a_t = k w_t$.
      \vskip1ex
      So the \emph{margin debt} $m_t$ is proportional to the wealth $w_t$: $m_t = (k - 1) w_t + 1$.
      \vskip1ex
      The wealth changes from $w_{t-1}$ to: $w_t = w_{t-1} (1 + k \, r_t)$, while the dollar amount of the stock investment changes from $a_{t-1} = k w_{t-1}$ to: $a_t = k w_{t-1} (1 + r_t)$, so that the leverage changes from $k$ to:
      \begin{displaymath}
        \frac{k w_{t-1} (1 + r_t)}{w_{t-1} (1 + k \, r_t)} = \frac{k (1 + r_t)}{1 + k \, r_t}
      \end{displaymath}
    \column{0.5\textwidth}
      In order to maintain a fixed \emph{leverage ratio} equal to $k$, the investor must actively trade the stock and the \emph{margin debt} $m_t$ changes over time.
      \vskip1ex
      The change in margin in a single time period is equal to:
      \begin{displaymath}
        \Delta m_t = (k - 1) \Delta w_t = k (k - 1) w_{t-1} r_t
      \end{displaymath}
      The dollar amount of the stock traded is equal to the change in \emph{margin}.
      \vskip1ex
      Therefore the investor must borrow on margin and buy the stock when its price increases and sell it when it drops.
  \end{columns}
\end{block}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The \emph{transaction costs} $c^r$ due to the \emph{bid-ask spread} are equal to half the \emph{bid-ask spread} $\delta$ times the absolute value of the traded dollar amount of the stock:
      \begin{displaymath}
      ...
      \begin{displaymath}
        c^r = \frac{\delta}{2} k (k - 1) w_{t-1} \left| r_t \right|
      \end{displaymath}
    \column{0.5\textwidth}
      The wealth of the Kelly Strategy after accounting for the \emph{bid-ask spread} is then equal to:
      \begin{displaymath}
        w_T = \prod_{t=1}^T {(1 + k \, r_t - \frac{\delta}{2} k (k - 1) \left| r_t \right|)}
      \end{displaymath}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      In reality investors don't know the probability of winning or the odds of the gamble, so they can't accurately Calculate the optimal \emph{Kelly ratio}.
      \vskip1ex
      The \emph{Kelly ratio}: $k_r = \frac{\bar{r}}{\sigma^2}$ is especially sensitive to the uncertainty of the expected returns $\bar{r}$.
      \vskip1ex
      If the expected returns are over-estimated, then it can produce an inflated value of the \emph{Kelly ratio}, leading to ruin.
      \vskip1ex
      ...
      \vskip1ex
      So in practice many investors apply only half the theoretical \emph{Kelly ratio} (the Half-Kelly), to reduce the risk of ruin.
    \column{0.5\textwidth}
      Perform bootstrap simulation to obtain the standard error of the \emph{Kelly ratio}.
      <<echo=TRUE,eval=FALSE>>=
# Plot several Kelly curves
curve(expr=kelly_ratio(x, b=1), xlim=c(0, 5),
      ylim=c(-1, 1.5), xlab="win amount",
      ylab="Kelly ratio", main="", lwd=2)
abline(h=0.5, lwd=2, col="red")
text(x=1.5, y=0.5, pos=3, cex=0.8, labels="b=1.0; max fraction=0.5")
curve(expr=kelly_ratio(x, b=0.5), add=TRUE, main="", lwd=2)
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \begin{displaymath}
        u = \sum_{t=1}^T {\log(1 + stocku \, r^s_i + bondu \, r^b_i)}
      \end{displaymath}
...
log_util <- function(stocku, bondu) {
...
unlist(optimize(f=function(x) log_util(x, bondu=0), interval=c(1, 4)))
# Kelly weight for bonds
unlist(optimize(f=function(x) log_util(x, stocku=0), interval=c(1, 14)))
# Vectorized utility of stock and bond portfolio
utilityv <- function(weightv) {
  log_util(weightv[1], weightv[2])
}  ##  end utilityv
# Optimize with respect to vector argument
optiml <- optim(fn=utilityv, par=c(3, 10),
                method="L-BFGS-B",
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The \emph{margin debt} $m_t$ is proportional to the wealth $w_t$: $m_t = (k_r - 1) w_t + 1$.
      \vskip1ex
      The dollar amount of the stock traded is equal to the change in \emph{margin}, equal to: $\Delta m_t = \Delta [(k_r - 1) w_t]$.
      \vskip1ex
      If the transaction costs are large, then they will reduce the wealth and reduce the dollar amount of the stock held by the investor.
      \vskip1ex
    ...
    \column{0.5\textwidth}
      ...
      The wealth of the Kelly Strategy after accounting for the \emph{bid-ask spread} is then equal to:
      \begin{displaymath}
        w_T = \prod_{t=1}^T {(1 + k \, r_t - \frac{\delta}{2} \frac{\left| \Delta m_i^{nc} \right|}{w_t^{nc}})}
      \end{displaymath}
      The effect of the \emph{bid-ask spread} is to reduce the effective asset returns by an amount proportional to the \emph{bid-ask spread}.
      <<echo=TRUE,eval=FALSE>>=
# bidask equal to 1 bp for liquid ETFs
...
costv <- bidask*drop(rutils::diffit(marginv))/2
wealthd <- drop(rutils::diffit(wealthv))
costr <- ifelse(wealthd>0, costv/wealthd, 0)
range(costr)
hist(costr, breaks=10000, xlim=c(-0.02, 0.02))
# Scale and lag the transaction costs
costv <- rutils::lagit(abs(costv)/wealthv)
# ReCalculate the compounded Kelly wealth
wealthc <- cumprod(1 + retp$VTI*retp$VTI - costv)
# Plot compounded wealth
wealthv <- cbind(wealthv, wealthc)
colnames(wealthv) <- c("Kelly", "Including bid-ask")
dygraphs::dygraph(wealthv, main="Kelly Strategy With Transaction Costs") %>%
