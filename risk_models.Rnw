...
look_backs <- lapply(2:npts, function(indeks) {
...
# Define starting points as lag of endp
  startp <- c(rep_len(0, look_back), endp[1:(npts- look_back)])
# Perform aggregations over look_backs list
  aggs <- lapply(2:npts, function(indeks)
    FUN(xtsv[startp[indeks]:endp[indeks]], ...)
  )  # end lapply
# rbind list into single xts or matrix
  aggs <- rutils::do_call(rbind, aggs)
# Coerce aggs into xts series
...
startp <- ifelse(startp < 0, 0, startp)
# Perform lapply() loop over look_backs list
aggs <- lapply(2:NROW(endp), function(indeks) {
      xtsv <- closep[startp[indeks]:endp[indeks]]
      c(max=max(xtsv), min=min(xtsv))
})  # end lapply
# rbind list into single xts or matrix
aggs <- rutils::do_call(rbind, aggs)
