...
roll_ed <- roll::roll_sum(closep, width=look_back, weights=weightrev, min_obs=1)
...
eq_up <- exp(cumsum(sig_up*retsp))
eq_dn <- exp(cumsum(-1*sig_dn*retsp))
eq_all <- exp(cumsum(sig_nals*retsp))
      @
    \column{0.5\textwidth}
      \vspace{-1em}
...
plot.zoo(cbind(eq_all, eq_up, eq_dn)[endp], lwd=c(2, 2, 2),
  ...
  main=paste("RSI(2) strategy for", colnames(closep), "from",
             format(start(retsp), "%B %Y"), "to",
             format(end(retsp), "%B %Y")))
@
  \end{columns}
\end{block}
...
retsp <- lapply(delta_s, function(deltav) {
  weightv <- (deltav - 0:(look_back-2)) / 1:(look_back-1)
  weightv <- c(1, (-1)^(1:(look_back-1))*cumprod(weightv))
  weightv <- rev(weights - mean(weightv))
...
colnames(retsp) <- c("VTI", paste0("frac_", delta_s))
...
variance <- sapply(1:nrows, function(it) {
  retsp <- retsp[startp[it]:endp[it]]
  sum((retsp - mean(retsp))^2)
}) / (look_back)  # end sapply
# Use only vectorized functions
retscum <- cumsum(retsp)
retscum <- (retscum -
  c(rep_len(0, look_back), retscum[1:(nrows-look_back)]))
retscum2 <- cumsum(retsp^2)
retscum2 <- (retscum2 -
  c(rep_len(0, look_back), retscum2[1:(nrows-look_back)]))
variance2 <- (retscum2 - retscum^2/look_back)/(look_back)
all.equal(variance[-(1:look_back)], as.numeric(variance2)[-(1:look_back)])
# Same, using package rutils
retscum <- rutils::roll_sum(retsp, look_back=look_back, min_obs=1)
retscum2 <- rutils::roll_sum(retsp^2, look_back=look_back, min_obs=1)
variance2 <- (retscum2 - retscum^2/look_back)/(look_back)
...
summary(microbenchmark(
  sapply=sapply(1:nrows, function(it) {
    var(retsp[startp[it]:endp[it]])
  }),
  roll=roll::roll_var(retsp, width=look_back),
  times=10))[, c(1, 4, 5)]
      @
  \end{columns}
\end{block}
...
# Calculate rolling VTI variance using package roll
library(roll)  # Load roll
variance <- roll::roll_var(retsp, weights=rev(weightv), width=look_back)
colnames(variance) <- "VTI.variance"
class(variance)
head(variance)
sum(is.na(variance))
...
likefun <- function(omega, alpha, betav) {
  ...
  variance[1] <- omega/(1-alpha-betav)
  for (i in 2:nrows) {
    variance[i] <- omega + alpha*retsp[i]^2 + betav*variance[i-1]
  }  # end for
...
(last(volog) - log(sd(retsp)))/last(agglog)
...
aggint <- 500
nrows <- NROW(closep); naggs <- nrows %/% aggint
endp <- c(0, nrows - naggs*aggint + (0:naggs)*aggint)
# Or
# endp <- rutils::calc_endpoints(closep, interval=aggint)
...
startp <- c(rep_len(0, look_back), endp[1:(NROW(endp)-look_back)])
head(startp, 33)
      @
  \end{columns}
    \vspace{-1em}
...
aggs <- xts(aggs, order.by=zoo::index(closep[endp]))
      @
  \end{columns}
\end{block}
...
sum_roll <- roll::roll_sum(retsp, width=look_back, min_obs=1)
# Calculate rolling sum using rutils
sum_rutils <- rutils::roll_sum(retsp, look_back=look_back)
...
summary(microbenchmark(
  ...
  roll=roll::roll_sum(retsp, width=look_back),
  RcppRoll=RcppRoll::roll_sum(retsp, n=look_back),
  rutils=rutils::roll_sum(retsp, look_back=look_back),
  times=10))[, c(1, 4, 5)]
      @
  \end{columns}
\end{block}
...
# Calculate rolling sum using RcppRoll
sum_roll <- RcppRoll::roll_sum(retsp, align="right", n=look_back)
# Calculate rolling sum using rutils
sum_rutils <- rutils::roll_sum(retsp, look_back=look_back)
...
startp <- c(0, endp[1:(NROW(endp)-1)])
# Define list of look-back intervals for aggregations over past
look_backs <- lapply(2:NROW(endp), function(it) {
    startp[it]:endp[it]
})  # end lapply
look_backs[[1]]
look_backs[[2]]
...
roll_agg <- function(xtsv, endp, FUN, ...) {
  nrows <- NROW(endp)
...
# rbind list into single xts or matrix
  ...
  if (!is.xts(aggs))
    ...
    xts(aggs, order.by=zoo::index(xtsv[endp]))
  aggs
}  # end roll_agg
# Apply sum() over endp
aggs <- roll_agg(closep, endp=endp, FUN=sum)
aggs <- period.apply(closep, INDEX=endp, FUN=sum)
# Benchmark the speed of aggregation functions
summary(microbenchmark(
  roll_agg=roll_agg(closep, endp=endp, FUN=sum),
  period_apply=period.apply(closep, INDEX=endp, FUN=sum),
  times=10))[, c(1, 4, 5)]
aggs <- period.sum(closep, INDEX=endp)
head(aggs)
      @
  \end{columns}
\end{block}
...
startp <- ifelse(startp < 0, 0, startp)
# Perform lapply() loop over look_backs list
aggs <- lapply(2:NROW(endp), function(it) {
      xtsv <- closep[startp[it]:endp[it]]
      c(max=max(xtsv), min=min(xtsv))
})  # end lapply
# rbind list into single xts or matrix
aggs <- rutils::do_call(rbind, aggs)
