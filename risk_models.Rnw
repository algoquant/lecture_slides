...
# if n_rows==look_back*n_agg then whole number 
# of look_backs fit over price_s
end_points <- (1:n_agg)*look_back
# if (n_rows > look_back*n_agg) 
# then stub interval at beginning
end_points <- 
  n_rows-look_back*n_agg + (0:n_agg)*look_back
# stub interval at end
end_points <- c((1:n_agg)*look_back, n_rows)
...
end_points <- # define end_points with beginning stub
  n_rows-look_back*n_agg + (0:n_agg)*look_back
n_rows <- NROW(end_points)
n_points <- 4  # number of end points in look-back interval
# start_points are multi-period lag of end_points
start_points <- c(rep_len(1, n_points-1), 
  end_points[1:(n_rows-n_points+1)])
# define list of look-back intervals for aggregations over past
look_backs <- lapply(seq_along(end_points), 
  function(in_dex) {
    start_points[in_dex]:end_points[in_dex]
...
%%%%%%%%%%%%%%%
\subsection{draft: Alternative \protect\emph{Range} Volatility Estimators}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      <<echo=TRUE,eval=FALSE>>=
re_turns <- 
  ifelse(etf_env$VTI[, 2] > etf_env$VTI[, 3], 
  log((etf_env$VTI[, 2] - etf_env$VTI[, 3]) / 
    (etf_env$VTI[, 2] + etf_env$VTI[, 3])), 0)
