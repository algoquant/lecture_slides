...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The perspective plot shows that the \emph{log-likelihood} is much more sensitive to $\beta$ than to $\alpha$. 
      \vskip1ex
...
re_turns <- matrix(re_turns)
# Calculate likelihood as function of rotated parameters
...
%%%%%%%%%%%%%%%
\subsection{draft: \protect\emph{GARCH} Likelihood Function Optimization}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      The more parameters the harder it is to find their optimal values using optimization.
      \vskip1ex
      We can simplify the task of finding the parameter values that maximize the \emph{log-likelihood}, by assuming that the expected value of the variance of \emph{GARCH} returns is equal to the realized (sample) variance:
      \begin{displaymath}
        \sigma^2 = \frac{\omega}{1 - \alpha - \beta} = \frac{1}{n-1} \sum_{i=1}^n (r_i-\bar{r})^2
      \end{displaymath}
      This way the \emph{log-likelihood} becomes a function of only two parameters, say $\alpha$ and $\beta$. 
      \vskip1ex
      It's convenient to express the \emph{log-likelihood} terms of the rotated parameters $x1 = \alpha + \beta$ and $x2 = \alpha - \beta$. 
      <<eval=FALSE,echo=TRUE>>=
...
# Find max likeli_hood parameters
optim_fit <- optim(par=par_init,
  fn=likeli_hood, # Log-likelihood function
  method="L-BFGS-B", # quasi-Newton method
  re_turns=re_turns, 
...
# Fix om_ega
likeli_hood <- function(x) {
  al_pha <- x[1]
  be_ta <- x[2]
  al_pha <- (al_pha + be_ta)/2
  be_ta <- (al_pha - be_ta)/2
  om_ega <- vari_ance*(1 - al_pha - be_ta)
  -HighFreq::lik_garch(omega=om_ega, alpha=al_pha, beta=be_ta, returns=re_turns)
}  # end likeli_hood
# Initial parameters
par_init <- c(al_pha=0.7, be_ta=-0.2)
# Find max likeli_hood parameters
optim_fit <- optim(par=par_init,
  fn=likeli_hood, # Log-likelihood function
  method="L-BFGS-B", # quasi-Newton method
  upper=c(0.9, 0.0), # Upper constraint
  lower=c(0.1, -0.3)) # Lower constraint
# Extract optimal parameters
al_pha <- optim_fit$par["al_pha"]
be_ta <- optim_fit$par["be_ta"]
# Optimal and actual parameters
options(scipen=10)  # Use fixed not scientific notation
cbind(actual=c(alpha=al_pha, beta=be_ta, omega=om_ega),
      optimal=c((al_pha + be_ta)/2, (al_pha - be_ta)/2, omega=om_ega))
...
%%%%%%%%%%%%%%%
\subsection{draft: \protect\emph{GARCH} Model Fitting to Stock Returns}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
