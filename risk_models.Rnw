...
re_turns <- rutils::diff_xts(log(quantmod::Cl(rutils::etf_env$VTI)))
...
be_ta <- 0.2 ; n_row <- 1000
re_turns <- numeric(n_row)
vari_ance <- numeric(n_row)
...
date_s <- seq.Date(from=Sys.Date()-n_row+1, 
  to=Sys.Date(), length.out=n_row)
...
# simulate GARCH model
garch_sim <- 
  fGarch::garchSim(spec=garch_spec, n=n_row)
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \end{displaymath}
      Where $\epsilon$ is a random process following the standard normal distribution \emph{N(0, 1)}. 
      \vskip1ex
...
in_dex <- index(rutils::etf_env$VTI)
n_row <- NROW(in_dex)
half_length <- n_row %/% 2
