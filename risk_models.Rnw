...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      A look-back \emph{interval} is a time \emph{interval} for performing aggregations over the past, starting from a \emph{startpoint} and ending at an \emph{endpoint}, 
      \vskip1ex
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.4\textwidth}
      ...
      \vskip1ex
      But it's often necessary to perform aggregations over \emph{overlapping} intervals, defined by a vector of \emph{endpoints} and a look-back \emph{interval}, 
      \vskip1ex
      The \emph{startpoints} are defined as the \emph{endpoints} lagged by the interval width (number of periods in the look-back \emph{interval}), 
      \vskip1ex
      Each point in time has an associated look-back \emph{interval}, which starts at a certain number of periods in the past (\emph{start\_point}) and ends at that point (\emph{end\_point}), 
      \vskip1ex
    ...
    \column{0.6\textwidth}
      ...
      <<echo=(-(1:1)),eval=FALSE>>=
library(HighFreq)  # load package HighFreq
look_back <- 4  # number of end points per look-back interval
end_points <- # define end_points with beginning stub
  c(0, n_row-look_back*num_agg+look_back*(0:num_agg))
len_gth <- NROW(end_points)
# start_points are multi-period lag of end_points
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      It's not possible to vectorize the calculation of rolling realized variance in \texttt{R}, so it must be calculated using an \texttt{apply()} loop,
      \vskip1ex
      The package \emph{roll} contains functions for calculating \emph{weighted} rolling aggregations over \texttt{vectors} and \texttt{time series} objects: 
      \begin{itemize}
      ...
      \end{itemize}
      The \emph{roll} functions are about \texttt{1,000} times faster than \texttt{apply()} loops! because they perform calculations in parallel in compiled \texttt{C++} code,
      \vskip1ex
...
start_points <-  end_points[
  c(rep_len(1, look_back-1), 1:(len_gth-look_back+1))]
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      <<echo=TRUE,eval=FALSE>>=
# calculate realized VTI variance
...
%%%%%%%%%%%%%%%
\subsection{draft: \protect\emph{EWMA} Realized Variance Estimator}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      Time-varying volatility can be more accurately estimated using an \emph{Exponentially Weighted Moving Average} (\emph{EWMA}) \emph{realized} variance estimator, 
      \vskip1ex
      If the returns have zero \emph{expected} mean, then the \emph{EWMA} \emph{realized} variance estimator can be written approxiamtely as:
      \begin{displaymath}
      ...
      \vskip1ex
      Heteroskedasticity refers to statistical distributions whose parameters vary with time,
      \vskip1ex
      The \emph{RcppRoll} functions accept \emph{xts} objects, but they return matrices, not \emph{xts} objects, 
      \vskip1ex
      The rolling \emph{RcppRoll} functions are much faster than performing \texttt{apply()} loops, because they are \emph{compiled} functions (compiled from \texttt{C++} code),
...
var_running <- rutils::diff_xts(log(rutils::env_etf$VTI[, 4]))^2
# calculate average realized VTI variance using rutils
look_back <- 31
var_avg <- rutils::roll_sum(var_running, 
                  win_dow=look_back)/look_back
# calculate EWMA VTI variance using filter()
weight_s <- exp(0.1*1:look_back)
weight_s <- weight_s/sum(weight_s)
var_ewma <- filter(var_running, filter=weight_s, sides=1)
var_ewma[1:(look_back-1)] <- var_ewma[look_back]
# plot EWMA prices
# set plot_theme plot line colors
# plot_theme <- chart_theme()
# plot_theme$col$line.col <- c("orange", "blue")
# var_ewma <- xts(cbind(cl_ose, var_ewma), order.by=index(cl_ose))
# colnames(var_ewma) <- c("VTI", "VTI EWMA")
# chart_Series(var_ewma, theme=plot_theme, name="EWMA prices")

library(RcppRoll)  # load RcppRoll
var_ewma <- RcppRoll::roll_var(var_running, 
    align="right", n=look_back, weights=weight_s)
var_ewma <- xts(var_ewma, 
    order.by=index(env_etf$VTI[-(1:(look_back-1)), ]))
