...
legend("topleft", legend=colnames(rates),
       col=colorv, lty=1, lwd=6, inset=0.05, cex=1.0)
      @
  \end{columns}
\end{block}
...
# Add legend
legend("topleft", legend=colnames(rates),
       col=colorv, lty=1, lwd=4, inset=0.05, cex=0.8)
      @
    \column{0.5\textwidth}
    \vspace{-1em}
...
objfun <- function(weights, retp) {
  retp <- retp %*% weights
  -1e7*var(retp) + 1e7*(1 - sum(weights*weights))^2
}  # end objfun
...
objfun <- function(weights, retp) {
  retp <- retp %*% weights
  -1e7*var(retp) + 1e7*(1 - sum(weights^2))^2 +
