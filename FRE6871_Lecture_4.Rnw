  ...
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \begin{itemize}
        ...
        \item test \emph{statistic} (from the data sample),
        \item \emph{p}-value (conditional probability of observing the test statistic value, assuming the \emph{null hypothesis} is \texttt{TRUE}),
        \item \emph{significance level} $\alpha$, to determine whether to accept or reject the \emph{null hypothesis},
      \end{itemize}
      If the \emph{p}-value is less than the \emph{significance level} $\alpha$, then the \emph{null hypothesis} is rejected, otherwise it's accepted as \texttt{TRUE},
      \vskip1ex
...
head(test_frame[!test_frame$result, ])
      ...
      \vskip1ex
      It's possible for the probability of a hypothesis to be high, but when testing it, to obtain a very small \emph{p}-value, purely by chance,
  \end{columns}
\end{block}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \end{align*}
      $\mathbbm{1}$ is the unit vector, and $\mathbbm{1}^T x = x^T \mathbbm{1} = \sum_{i=1}^n {x_i}$
    \column{0.5\textwidth}
      ...
      \begin{align*}
        \alpha = \bar{z}^T \mathbbm{1} - \beta \bar{x}^T \mathbbm{1}
      \end{align*}
      The solution for $\beta$ is:
      \begin{flalign*}
        & (z - (\bar{z}^T \mathbbm{1} - \beta \bar{x}^T \mathbbm{1}) \mathbbm{1} - \beta x)^T x = 0\\
        & (z^T x - (\bar{z}^T \mathbbm{1} - \beta \bar{x}^T \mathbbm{1}) \mathbbm{1}^T x - \beta x^T x) = 0\\
        & (z^T x - (\bar{z}^T \mathbbm{1}) (x^T \mathbbm{1}) + \beta (x^T \mathbbm{1})^2 / n - \beta x^T x) = 0\\
        & \beta = \frac {z^T x - (x^T \mathbbm{1}) (z^T \mathbbm{1}) / n } {x^T x - (\mathbbm{1}^T x)^2 / n}
      \end{flalign*}
...
%%%%%%%%%%%%%%%
\subsection{Solving \protect\emph{Linear Regression} Using Linear Algebra}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
...
%%%%%%%%%%%%%%%
\subsection{Residuals of a \protect\emph{Linear Regression}}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      A Linear Regression model with \texttt{k} explanatory variables $\{x_j\}$, is defined by the formula:
      \begin{displaymath}
      ...
      \end{displaymath}
      The response variable $z$ and the \texttt{k} explanatory variables $\{x_j\}$ each contain \texttt{n} observations, and the form the columns of matrix $\mathbb{X}$,
      \vskip1ex
    ...
    \column{0.5\textwidth}
      ...
      \begin{align*}
        \alpha = z^T \mathbbm{1} - \beta \mathbb{X}^T \mathbbm{1}
      \end{align*}
      If the variables are de-meaned, then $\alpha = 0$, and the \emph{OLS} estimate for $\beta$ is given by equating the \emph{RSS} derivative to zero:
      \begin{flalign*}
        & RSS_\beta = - 2 (z - \beta \mathbb{X})^T \mathbb{X} = 0\\
        & \mathbb{X}^T z - \beta \mathbb{X}^T \mathbb{X} = 0\\
        & \beta = (\mathbb{X}^T \mathbb{X})^{-1} \mathbb{X}^T z
      \end{flalign*}
      The matrix $\mathbb{C} = \mathbb{X}^T \mathbb{X} / (k-1)$ is the covariance matrix of the matrix $\mathbb{X}$, and it's invertible only if the columns of $\mathbb{X}$ are linearly independent,
      \vskip1ex
