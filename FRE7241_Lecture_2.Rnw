...
nrows/((nrows-1)*(nrows-2))*
  sum(((retp - retsm)/sdrets)^3)
# Kurtosis of VTI returns
nrows*(nrows+1)/((nrows-1)^3)*
  sum(((retp - retsm)/sdrets)^4)
...
plot(density(retp, adjust=3), xlab="VTI Returns", ylab="Density",
...
rets_scaled <- ifelse(volumes > 0, 
  sqrt(volume_roll)*retp/sqrt(volumes), 0)
rets_scaled <- sd(retp)*rets_scaled/sd(rets_scaled)
# rets_scaled <- ifelse(volumes > 1e4, retp/volumes, 0)
# Calculate moments of scaled returns
nrows <- NROW(retp)
sapply(list(retp=retp, rets_scaled=rets_scaled),
...
indeks <- round(confl*NROW(retp))
varisk <- sortv[indeks]
# PerformanceAnalytics VaR
PerformanceAnalytics::VaR(retp,
...
wealthm <- sapply(bootd, function(retp) {
...
residuals <- (sell_inmay - model$coeff[2]*retp)
...
all.equal(pnls, pnls2)
    ...
    \column{0.5\textwidth}
      ...
      <<echo=TRUE,eval=FALSE>>=
# Combine data
wealthv <- xts::xts(cbind(retp, pnls), dates)
colnamev <- c("VTI", "Strategy")
colnames(wealthv) <- colnamev
# Calculate Sharpe and Sortino ratios
sqrt(252)*sapply(wealthv, 
