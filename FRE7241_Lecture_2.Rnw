...
\subtitle{Lecture\#2, Spring 2025}
...
colv <- c("Margin", "Cumulative Transaction Costs")
colnames(datav) <- colv
...
wealthv <- xts::xts(wealthv, datev)
colv <- c("Wealth", "Cumulative Transaction Costs")
...
wealthfs <- 1 + cumsum(retsc %*% weightv)
...
# Calculate the Sharpe and Sortino ratios
wealthv <- xts::xts(cbind(wealthfs, wealthev), zoo::index(pricev))
...
dygraphs::dygraph(priceb["2008-09/2009-09"], 
  ...
  dySeries(name=colv[1], strokeWidth=2, col="blue") %>%
  dySeries(name=colv[2], strokeWidth=2, strokePattern="dashed", col="red") %>%
...
\begin{block}{Required}
  \begin{itemize}[]
    ...
    \texttt{Clarke Risk Parity Minimum Variance Portfolios.pdf} and
