...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      If \texttt{x} follows the \emph{Normal} distribution $N(x, \mu, \sigma)$, then the exponential of \texttt{x}: $y = e^x$ follows the \emph{Log-normal} distribution:
      \begin{displaymath}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The function \texttt{arima()} from the base package \emph{stats} calibrates (fits) an \emph{ARIMA} model to a univariate time series.
      \vskip1ex
