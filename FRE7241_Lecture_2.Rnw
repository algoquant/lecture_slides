...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The proper benchmark for a stock picker is the median of random portfolios, not the stock index, which is the mean.
      <<echo=TRUE,eval=FALSE>>=
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The strategy out-of-sample performance is only slightly better than the index.
      <<echo=TRUE,eval=FALSE>>=
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \begin{itemize}
        \item Specify a portfolio of stocks and their time series of returns,
