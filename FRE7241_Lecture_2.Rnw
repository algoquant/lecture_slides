...
\subtitle{Lecture\#2, Spring 2023}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      Multiplying the weights times the dollar returns is equivalent to buying a \emph{number of shares} proportional to the weights (aka \emph{Target Share Allocation} or TSA).
      \vskip1ex
      Multiplying the weights times the percentage returns is equivalent to investing in \emph{dollar amounts of stock} proportional to the weights (aka \emph{Target Dollar Allocation} or TDA).
      \vskip1ex
      ...
      \vskip1ex
      The portfolio with target dollar allocations has a slightly higher Sharpe ratio than the portfolio with a fixed number of shares.
      <<echo=TRUE,eval=FALSE>>=
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      Maintaining a \emph{target dollar allocation} of stock requires periodic \emph{rebalancing}, selling shares when their price goes up, and vice versa.
      \vskip1ex
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      In the \emph{fixed share strategy} (\emph{TSA}), the number of shares is fixed, with their initial dollar value equal to the portfolio weights.
      \vskip1ex
      ...
      \vskip1ex
      If the threshold level is very large then the \emph{PTA} strategy does not rebalance and it's the same as the \emph{TSA}.
    \column{0.5\textwidth}
      \vspace{-1em}
      <<echo=TRUE,eval=FALSE>>=
# Wealth of fixed shares (without rebalancing)
