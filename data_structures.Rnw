...
\begin{block}{}
  \begin{columns}[T]
    \column{0.3\textwidth}
      The \texttt{mtcars} data frame is included in the \texttt{MASS} package, and contains design and performance data for 93 automobiles,
      \vskip1ex
    \column{0.7\textwidth}
      \vspace{-1em}
        <<Cars93_hist>>=
...
# plot histogram of Highway MPG
hist(Cars93$MPG.highway, col="lightblue1", main="Distance per Gallon 1993", xlab="Highway MPG", breaks="FD")
      @
      \vspace{-2em}
      \hspace*{1em}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.4\textwidth}
      ...
      \vskip1ex
      The attributes of an object is a named \texttt{list} of \texttt{tag=value} pairs,
      \vskip1ex
      ...
      \vskip1ex
      The function \texttt{structure()} adds attributes to an object (specified as \texttt{tag=value} pairs), and returns it,
    \column{0.6\textwidth}
      \vspace{-1em}
      <<>>=
