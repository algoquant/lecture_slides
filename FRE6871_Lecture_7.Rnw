...
dframe <- data.frame(dates=zoo::index(prices),
    zoo::coredata(prices))
...
p_lot <- plot_ly(data=dframe, x=~dates, y=~VTI, type="scatter", mode="lines", name="VTI")
...
dframe <- read.table("mydata.txt", header=TRUE)
...
dframe <- as.data.frame(zoo_series)
dframe <- cbind(format(dates, "%m-%d-%Y %H:%M:%S"),
                    dframe)
head(dframe, 3)
# Write zoo series to csv file using write.table
write.table(dframe, file="zoo_series.csv",
