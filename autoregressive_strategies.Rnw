...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The \emph{in-sample} forecasts of the \emph{AR(n)} autoregressive model are calculated by multiplying the predictor matrix by the fitted AR coefficients:
      \begin{displaymath}
        f_t = \varphi_0 + \varphi_1 r_{t-1} + \varphi_2 r_{t-2} + \ldots + \varphi_n r_{t-n}
      \end{displaymath}
      The variance of the autoregressive coefficients $\varphi$ is given by:
      \begin{displaymath}
        \sigma^2_\varphi = \sigma^2_\varepsilon (\mathbb{P}^T \mathbb{P})^{-1}
      \end{displaymath}
      Where $\sigma^2_\varepsilon$ is the variance of the residuals.
      <<echo=TRUE,eval=FALSE>>=
...
%%%%%%%%%%%%%%%
\subsection{Performance of High and Low Volatility Autoregressive Coefficients}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
...
dygraphs::dygraph(datav[endd], main="Volatility of high Residuals") %>%
  ...
  dySeries(name="VTI", axis="y", label="VTI", strokeWidth=2, col="blue") %>%
  dySeries(name="residuals", axis="y2", label="residuals", strokeWidth=2, col="red")
...
predinv <- MASS::ginv(desv[rangev, -1])
# Calculate the fitted AR coefficients
coeff <- predinv %*% desv[rangev, 1]
