...
\begin{block}{}
  \begin{columns}[T]
    ...
    \column{0.6\textwidth}
      \vspace{-1em}
      <<eval=FALSE>>=
library()  # get all packages installed on your system
# get documentation for package "Ecdat"
packageDescription("Ecdat")  # get short description
help(package="Ecdat")  # get help
data(package="Ecdat")  # show all datasets
library(Ecdat)  # load package Ecdat
ls("package:Ecdat")  # list all objects in package
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.4\textwidth}
      ...
      \vskip1ex
      A package must be loaded for its \texttt{namespace} to be attached to the search path,
      \vskip1ex
      But packages may be loaded without their \texttt{namespace} being attached to the search path,
      \vskip1ex
      The function \texttt{loadedNamespaces()} lists all loaded \texttt{namespaces}, including those that are not on the search path,
      \vskip1ex
      ...
      \vskip1ex
      Packages that depend on other packages load them but they don't attach their \texttt{namespaces} to the search path,
      \vskip1ex
      loaded \texttt{namespaces} aren't always attached to the search path
      \vskip1ex
      they are reported as "loaded via a \texttt{namespace} (and not attached)"
