...
residuals <- drop(arimav - drop(predm %*% coeff))
residsar <- sum(residuals^2)/(nrows-NROW(coeff))
covar <- residsar*MASS::ginv(crossprod(predm))
coeffsd <- sqrt(diag(covar))
coefftv <- drop(coeff)/coeffsd
...
# Calculate t-values of AR(5) coefficients
residuals <- drop(retp - drop(predm %*% coeff))
residsar <- sum(residuals^2)/(nrows-NROW(coeff))
covar <- residsar*MASS::ginv(crossprod(predm))
coeffsd <- sqrt(diag(covar))
coefftv <- drop(coeff)/coeffsd
...
filterfast <- c(0, drop(predm %*% coeff))
...
predm <- predict(arima_fit, n.ahead=1)
# Or directly call predict.Arima()
# predm <- predict.Arima(arima_fit, n.ahead=1)
...
nagg <- 5
predm <- rutils::roll_sum(retp, look_back=nagg)
# Define predictor matrix for forecasting
ordmax <- 5
predm <- sapply(1+nagg*(0:ordmax), rutils::lagit,
                     input=predm)
...
sim_forecasts <- function(respv, nagg=5, ordern=5, 
  ...
  fcast <- sapply((look_back+1):nrows, function(endd) {
    ...
    rangev <- startp:(endd-1)
    # Invert the predictor matrix
    designinv <- MASS::ginv(predm[rangev, ])
    # Calculate fitted coefficients
...
# Define predictor matrix for forecasting
orderp <- 5
predm <- sapply(1:orderp, rutils::lagit, input=retp)
predm <- cbind(rep(1, nrows), predm)
colnames(predm) <- paste0("pred", 1:NCOL(predm))
predinv <- MASS::ginv(predm)
coeff <- drop(predinv %*% retp)
...
# Define predictor matrix for forecasting
ordmax <- 5
predm <- sapply(1:ordmax, rutils::lagit, input=retp)
predm <- cbind(rep(1, nrows), predm)
colnames(predm) <- paste0("pred", 1:NCOL(predm))
# Calculate forecasts as function of the AR order
fcast <- lapply(2:NCOL(predm), function(ordern) {
  # Calculate fitted coefficients
...
names(fcast) <- paste0("n=", 2:NCOL(predm))
      @
    \column{0.5\textwidth}
      \vspace{-1em}
...
nagg <- 5
predm <- roll::roll_mean(retp, width=nagg, min_obs=1)
respv <- retp
# Define predictor matrix for forecasting
predm <- sapply(1+nagg*(0:ordmax), rutils::lagit,
                     input=predm)
