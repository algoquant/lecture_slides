...
dates <- (rutils::diff_it(position_s) != 0)
...
trade_dates <- (rutils::diff_it(indic) != 0)
...
pnls <- lapply(lambdavs, function(lambdav) {
  ...
  simu_ewma(ohlc=ohlc, lambda=lambdav, look_back=look_back, bid_offer=0, lagg=2)[, "pnls"]
})  # end lapply
...
pnls <- parLapply(cluster, lambdavs, function(lambdav) {
  library(quantmod)
  # Simulate EWMA strategy and calculate returns
  simu_ewma(ohlc=ohlc, lambda=lambdav, look_back=look_back)[, "pnls"]
})  # end parLapply
...
ewma_trend <- simu_ewma(ohlc=ohlc, lambda=lambdav, look_back=look_back, bid_offer=0, lagg=2)
...
pnls <- lapply(lambdavs, function(lambdav) {
  # Simulate EWMA strategy and calculate returns
  simu_ewma(ohlc=ohlc, lambda=lambdav, look_back=look_back, trend=(-1))[, "pnls"]
})  # end lapply
pnls <- do.call(cbind, pnls)
colnames(pnls) <- paste0("lambda=", lambdavs)
...
for (indeks in 1:NROW(sigmavs)) {
  curve(expr=dlnorm(x, sdlog=sigmavs[indeks]),
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.45\paperwidth]{figure/lognormv_dist.png}
      \vspace{-1em}
      <<echo=TRUE,eval=FALSE>>=
...
sqrt(nrows)*sigmav
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.45\paperwidth]{figure/lognormv_skew.png}
      \vspace{-1em}
      <<echo=TRUE,eval=FALSE>>=
...
\begin{block}{}
  \begin{columns}[T]
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.45\paperwidth]{figure/lognormv_prob.png}
      \vspace{-1em}
      <<echo=TRUE,eval=FALSE>>=
...
per_centage <- rowSums((prices < indeks) & validp) / nums
...
\begin{block}{Required}
  Study all the lecture slides in \texttt{FRE7241\_Lecture\5.pdf}, and run all the code in \texttt{FRE7241\_Lecture\5.R}
\end{block}
