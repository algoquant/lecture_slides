...
dygraphs::dygraph(cumsum(wealthv), main="Yield Curve Strategy In-sample") %>%
  dyAxis("y", label=colnamev[1], independentTicks=TRUE) %>%
  dyAxis("y2", label=colnamev[2], independentTicks=TRUE) %>%
...
colnames(wealthv) <- c("VTI", "Strategy")
colnamev <- colnames(wealthv)
dygraphs::dygraph(cumsum(wealthv), main="Yield Curve Strategy Out-of-Sample") %>%
  dyAxis("y", label=colnamev[1], independentTicks=TRUE) %>%
  dyAxis("y2", label=colnamev[2], independentTicks=TRUE) %>%
  dySeries(name=colnamev[1], axis="y", col="blue", strokeWidth=2) %>%
  dySeries(name=colnamev[2], axis="y2", col="red", strokeWidth=2) %>%
...
colnames(wealthv) <- c("VTI", "Strategy")
colnamev <- colnames(wealthv)
dygraphs::dygraph(cumsum(wealthv), main="Rolling Yearly Yield Curve Strategy") %>%
  dyAxis("y", label=colnamev[1], independentTicks=TRUE) %>%
  dyAxis("y2", label=colnamev[2], independentTicks=TRUE) %>%
  dySeries(name=colnamev[1], axis="y", col="blue", strokeWidth=2) %>%
  dySeries(name=colnamev[2], axis="y2", col="red", strokeWidth=2) %>%
...
colnames(wealthv) <- c("VTI", "Strategy")
colnamev <- colnames(wealthv)
dygraphs::dygraph(cumsum(wealthv), main="Rolling Monthly Yield Curve Strategy") %>%
  dyAxis("y", label=colnamev[1], independentTicks=TRUE) %>%
  dyAxis("y2", label=colnamev[2], independentTicks=TRUE) %>%
  dySeries(name=colnamev[1], axis="y", col="blue", strokeWidth=2) %>%
  dySeries(name=colnamev[2], axis="y2", col="red", strokeWidth=2) %>%
...
colnames(wealthv) <- c("VTI", "Strategy")
colnamev <- colnames(wealthv)
dygraphs::dygraph(cumsum(wealthv), main="Rolling Weekly Yield Curve Strategy") %>%
  dyAxis("y", label=colnamev[1], independentTicks=TRUE) %>%
  dyAxis("y2", label=colnamev[2], independentTicks=TRUE) %>%
  dySeries(name=colnamev[1], axis="y", col="blue", strokeWidth=2) %>%
  dySeries(name=colnamev[2], axis="y2", col="red", strokeWidth=2) %>%
...
colnames(wealthv) <- c("VTI", "Strategy")
colnamev <- colnames(wealthv)
dygraphs::dygraph(cumsum(wealthv), main="Rolling Monthly Shrinkage YC Strategy") %>%
  dyAxis("y", label=colnamev[1], independentTicks=TRUE) %>%
  dyAxis("y2", label=colnamev[2], independentTicks=TRUE) %>%
  dySeries(name=colnamev[1], axis="y", col="blue", strokeWidth=2) %>%
  dySeries(name=colnamev[2], axis="y2", col="red", strokeWidth=2) %>%
...
colnames(wealthv) <- c("VTI", "Strategy")
colnamev <- colnames(wealthv)
dygraphs::dygraph(cumsum(wealthv), main="Rolling Weekly Shrinkage YC Strategy") %>%
  dyAxis("y", label=colnamev[1], independentTicks=TRUE) %>%
  dyAxis("y2", label=colnamev[2], independentTicks=TRUE) %>%
  dySeries(name=colnamev[1], axis="y", col="blue", strokeWidth=2) %>%
  dySeries(name=colnamev[2], axis="y2", col="red", strokeWidth=2) %>%
...
colnames(wealthv) <- c("VTI", "Strategy")
colnamev <- colnames(wealthv)
dygraphs::dygraph(cumsum(wealthv), main="Aggregated YC Strategy In-sample") %>%
  dyAxis("y", label=colnamev[1], independentTicks=TRUE) %>%
  dyAxis("y2", label=colnamev[2], independentTicks=TRUE) %>%
  dySeries(name=colnamev[1], axis="y", col="blue", strokeWidth=2) %>%
  dySeries(name=colnamev[2], axis="y2", col="red", strokeWidth=2) %>%
...
colnames(wealthv) <- c("VTI", "Strategy")
colnamev <- colnames(wealthv)
dygraphs::dygraph(cumsum(wealthv), main="Aggregated YC Strategy Out-of-Sample") %>%
  dyAxis("y", label=colnamev[1], independentTicks=TRUE) %>%
  dyAxis("y2", label=colnamev[2], independentTicks=TRUE) %>%
  dySeries(name=colnamev[1], axis="y", col="blue", strokeWidth=2) %>%
  dySeries(name=colnamev[2], axis="y2", col="red", strokeWidth=2) %>%
...
colors <- c("blue", "red")
dygraphs::dygraph(wealthv, main="Stock Index and Random Portfolio") %>%
...
wealthv <- xts::xts(rowMeans(wealthv), dates)
wealthv <- cbind(indeks, wealthv)
colnames(wealthv)[2] <- "Portfolio"
# Calculate the out-of-sample Sharpe and Sortino ratios
sqrt(252)*sapply(rutils::diffit(wealthv[(cutoff+1):nrows, ]), 
  function(x) c(Sharpe=mean(x)/sd(x), Sortino=mean(x)/sd(x[x<0])))
# Plot out-of-sample stock portfolio returns
dygraphs::dygraph(log(wealthv), main="Out-of-sample Log Prices of Stock Portfolio") %>%
...
colors <- c("blue", "red")
dygraphs::dygraph(log(wealthv[endp]), main="Log Stock Index and Momentum Strategy") %>%
  dyOptions(colors=colors, strokeWidth=2) %>%
  dyLegend(show="always", width=500)
      @
  \end{columns}
...
# Plot dygraph of stock index and momentum strategy
colors <- c("blue", "red")
dygraphs::dygraph(log(wealthv)[endp], main="Momentum Strategy and All-weather") %>%
  dyOptions(colors=colors, strokeWidth=2) %>%
  dyLegend(show="always", width=500)
      @
  \end{columns}
...
wealthv[, 1] <- wealthv[, 1]/as.numeric(wealthv[1, 1])
wealthv[, 2] <- wealthv[, 2]/as.numeric(wealthv[1, 2])
...
dygraphs::dygraph(wealthv[endp], main="VTI vs MTUM ETF") %>%
  dyOptions(colors=c("blue", "red"), strokeWidth=2) %>%
  dyLegend(width=500)
      @
    \column{0.5\textwidth}
...
vti <- retsp$VTI
predictor <- cbind(VTI=vti, 0.5*(vti+abs(vti)), vti^2)
colnames(predictor)[2:3] <- c("merton", "treynor")
model <- lm(pnls ~ VTI + merton, data=predictor); summary(model)
# Treynor-Mazuy test
model <- lm(pnls ~ VTI + treynor, data=predictor); summary(model)
...
dygraphs::dygraph(log(wealthv[endp]), main="ETF Momentum Strategy Combined with All-Weather") %>%
...
wealthv <- xts::xts(cbind(all_weather, pnls), order.by=dates)
...
dygraphs::dygraph(log(cumprod(1+wealthv))[endp], main="Daily Momentum Strategy vs All-Weather") %>%
  dyOptions(colors=c("blue", "red"), strokeWidth=2) %>%
  dyLegend(show="always", width=500)
      @
  \end{columns}
...
cor(wealthv)
# Plot dygraph of the momentum strategy returns
dygraphs::dygraph(cumsum(wealthv), main="Monthly Momentum Strategy vs All-Weather") %>%
  dyOptions(colors=c("blue", "red"), strokeWidth=2) %>%
  dyLegend(show="always", width=500)
      @
  \end{columns}
...
colnamev <- colnames(wealthv)
dygraphs::dygraph(cumsum(wealthv), main="Weekly ETF Rolling Portfolio Strategy With Shrinkage") %>%
  dyAxis("y", label=colnamev[1], independentTicks=TRUE) %>%
  dyAxis("y2", label=colnamev[2], independentTicks=TRUE) %>%
  dySeries(name=colnamev[1], axis="y", col="blue", strokeWidth=2) %>%
  dySeries(name=colnamev[2], axis="y2", col="red", strokeWidth=2) %>%
...
arma::mat back_test(const arma::mat& retsx, // Portfolio excess returns
  ...
  for (arma::uword it = 1; it < endp.size(); it++) {
    // cout << "it: " << it << endl;
    // Calculate portfolio weights
    weightv = coeff*calc_weights(retsx.rows(startp(it-1), endp(it-1)), method, eigen_thresh, dimax, confi, alpha, scale, vol_target);
...
pnls <- HighFreq::back_test(excess=retsx, returns=retsp,
  startp=startp, endp=endp, alpha=alpha, dimax=dimax, method="max_sharpe")
pnls <- pnls*sd(indeks)/sd(pnls)
      @
    \column{0.5\textwidth}
      \vspace{-1em}
