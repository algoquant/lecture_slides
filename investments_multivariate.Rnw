...
predv <- rutils::lagit(retsr)
regmod <- lm(retsp ~ predv)
summary(regmod)
# Regress VTI returns before and after 2012
summary(lm(retsp["/2012"] ~ predv["/2012"]))
summary(lm(retsp["2012/"] ~ predv["2012/"]))
      @
  \end{columns}
\end{block}
...
ordmax <- 5
predv <- sapply(1:ordmax, rutils::lagit, input=as.numeric(retsp))
colnames(predv) <- paste0("retslag", 1:NCOL(predv))
predv <- cbind(predv, rutils::lagit(retsr))
predv <- cbind(rep(1, NROW(predv)), predv)
colnames(predv)[1] <- "intercept"
respv <- retsp
      @
...
tvalues <- sapply(naggs, function(nagg) {
  predv <- roll::roll_mean(retsr, width=nagg, min_obs=1)
  predv <- cbind(rep(1, NROW(predv)), predv)
...
riskret <- sapply(retsp, function(retsp) {
  retsp <- na.omit(retsp)
  std <- sd(retsp)
  retvti <- retvti[zoo::index(retsp)]
  varvti <- drop(var(retvti))
  meanvti <- mean(retvti)
  betav <- drop(cov(retsp, retvti))/varvti
  resid <- retsp - betav*retvti
  alphav <- mean(retsp) - betav*meanvti
...
riskretis <- sapply(retsp[insample], function(retsp) {
  combv <- na.omit(cbind(retsp, retvti))
  if (NROW(combv) > 0) {
    retsp <- na.omit(retsp)
    std <- sd(retsp)
    retvti <- retvti[zoo::index(retsp)]
...
wealthrp <- exp(cumsum(rowSums(alloc*retsp, na.rm=TRUE)))
# Combined wealth
...
objfun <- function(retsp) {
  varv <- var(retsp)
  if (varv > 0) mean(retsp)/varv else 0
}  # end objfun
...
objfun <- function(retsp) mean(retsp)/max(sd(retsp), 1e-4)
# Objective function equal to Kelly ratio
objfun <- function(retsp) {
  varv <- var(retsp)
  if (varv > 0) mean(retsp)/varv else 0
...
\begin{block}{}
  \begin{columns}[T]
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      <<echo=TRUE,eval=FALSE>>=
btmomtop <- function(retsp, objfun, look_back=12, rebalf="months", nstocks=10, 
  bid_offer=0.0, endd=rutils::calc_endpoints(retsp, interval=rebalf), ...) {
...
endd <- rutils::calc_endpoints(retsp, interval="months")
pnll <- lapply(look_backs, btmomtop, retsp=retsp, endd=endd, objfun=objfun)
# Perform parallel loop under Mac-OSX or Linux
library(parallel)  # Load package parallel
ncores <- detectCores() - 1
pnll <- mclapply(look_backs, btmomtop, retsp=retsp, endd=endd, objfun=objfun, mc.cores=ncores)
...
\begin{block}{}
  \begin{columns}[T]
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      <<echo=TRUE,eval=FALSE>>=
btmomweight <- function(retsp, objfun, look_back=12, rebalf="months", 
  bid_offer=0.0, endd=rutils::calc_endpoints(retsp, interval=rebalf), ...) {
  # Perform loop over end points
  npts <- NROW(endd)
  pnls <- lapply(2:(npts-1), function(ep) {
    ...
    weightv <- weightv*sd(rowMeans(retsis))/sd(retportf)
    # Calculate the momentum portfolio returns
    retsp[(endd[ep]+1):endd[ep+1], ] %*% weightv
  })  # end lapply
...
# Perform backtests for vector of look-back intervals
look_backs <- seq(3, 15, by=1)
pnll <- lapply(look_backs, btmomweight, retsp=retsp, endd=endd, objfun=objfun)
# Or perform parallel loop under Mac-OSX or Linux
library(parallel)  # Load package parallel
ncores <- detectCores() - 1
pnll <- mclapply(look_backs, btmomweight, retsp=retsp, endd=endd, objfun=objfun, mc.cores=ncores)
sharper <- sqrt(252)*sapply(pnll, function(pnl) mean(pnl)/sd(pnl))
# Plot Sharpe ratios of momentum strategies
plot(x=look_backs, y=sharper, t="l",
  main="Momentum Sharpe as Function of Look-back Interval",
...
look_back <- 152
varm <- roll::roll_var(retsp, width=look_back, min_obs=1)
head(varm)
varm[1, ] <- retsp[1, ]^2
...
btmomdaily <- function(retsp, look_back=252, trend=1, bid_offer=0.0, ...) {
...
# Calculate the trailing Kelly ratio
  ...
  weightv <- rutils::lagit(weightv)
  # Calculate the momentum profits and losses
  pnls <- trend*rowSums(weightv*retsp)
  # Calculate the transaction costs
  costs <- 0.5*bid_offer*rowSums(abs(rutils::diffit(weightv)))
  (pnls - costs)
}  # end btmomdaily
...
pnls <- sapply(look_backs, btmomdaily, retsp=retsp)
# Scale the momentum volatility to the equal weight index
...
# Define backtest functional for daily momentum strategy
# If trend=(-1) then it backtests a mean reverting strategy
btmomdailyhold <- function(retsp, look_back=252, holdp=5, trend=1, bid_offer=0.0, ...) {
  stopifnot("package:quantmod" %in% search() || require("quantmod", quietly=TRUE))
  # Calculate the trailing variance
  varm <- roll::roll_var(retsp, width=look_back, min_obs=1)
  varm[1, ] <- retsp[1, ]^2
...
pnls <- sapply(holdpv, btmomdailyhold, look_back=180, retsp=retsp)
# Scale the momentum volatility to the equal weight index
pnls <- apply(pnls, MARGIN=2, function(pnl) indeksd*pnl/sd(pnl))
...
pnls <- sapply(look_backs, btmomdaily, retsp=retsp, trend=(-1))
# Scale the momentum volatility to the equal weight index
pnls <- apply(pnls, MARGIN=2, function(pnl) indeksd*pnl/sd(pnl))
colnames(pnls) <- paste0("look_back=", look_backs)
pnls <- xts::xts(pnls, datev)
...
# Define backtest functional for daily momentum strategy
# If trend=(-1) then it backtests a mean reverting strategy
btmomrun <- function(retsp, lambda=0.9, trend=1, holdp=5, bid_offer=0.0, ...) {
  stopifnot("package:quantmod" %in% search() || require("quantmod", quietly=TRUE))
  # Calculate the trailing variance
...
pnll <- mclapply(look_backs, btmomdaily, retsp=retspca[, 1:dimax], 
   mc.cores=ncores)
...
plot(x=look_backs, y=sharper, t="l",
  ...
  xlab="look-back (months)", ylab="Sharpe")
    ...
    \column{0.5\textwidth}
      ...
      <<echo=TRUE,eval=FALSE>>=
# Calculate a vector of monthly end points
endd <- rutils::calc_endpoints(retspca, interval="months")
...
pnls <- sapply(look_backs, btmomdaily, trend=(-1),
  retsp=retspca[, (dimax+1):NCOL(retspca)])
colnames(pnls) <- paste0("look_back=", look_backs)
# Scale the momentum volatility to the equal weight index
pnls <- apply(pnls, MARGIN=2, function(pnl) indeksd*pnl/sd(pnl))
pnls <- xts::xts(pnls, datev)
...
pnll <- mclapply(look_backs, btmomdaily, 
   retsp=retspca[, 1:dimax], mc.cores=ncores)
pnll <- lapply(pnll, function(pnl) indeksd*pnl/sd(pnl))
pnls <- do.call(cbind, pnll)
colnames(pnls) <- paste0("look_back=", look_backs)
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      <<echo=TRUE,eval=FALSE>>=
# Calculate ETF betas
betasetf <- sapply(retsp, function(x)
...
pnls <- btmomdaily(retsp=retsp, look_back=152, bid_offer=bid_offer)
# Perform sapply loop over look_backs
look_backs <- seq(90, 190, by=10)
...
# Define backtest functional for daily momentum strategy
# If trend=(-1) then it backtests a mean reverting strategy
btmomdaily <- function(retsp, look_back=252, holdp=5, trend=1, bid_offer=0.0, ...) {
  stopifnot("package:quantmod" %in% search() || require("quantmod", quietly=TRUE))
  
  posit <- matrixStats::rowRanks(retsp)
  posit <- (posit - rowMeans(posit))
  posit <- HighFreq::lagit(posit, lagg=1)
  trend*rowMeans(posit*retsp)
...
pnls <- btmomdaily(retsp=retsp, trend=(-1))
pnls <- xts::xts(pnls, datev)
colnames(pnls) <- "PnL"
...
# Perform sapply loop over holding periods
holdpv <- seq(2, 11, by=2)
pnls <- sapply(holdpv, btmomdaily, look_back=120, retsp=retsp)
colnames(pnls) <- paste0("holding=", holdpv)
pnls <- xts::xts(pnls, datev)
      @
    \column{0.5\textwidth}
...
btmomweight <- function(retsp,
                      ...
                      look_back=12, rebalf="months", bid_offer=0.0,
                      endd=rutils::calc_endpoints(retsp, interval=rebalf)[-1],
                      with_weights=FALSE, ...) {
...
pnls <- lapply(alphav, roll_portf, excess=retsx, 
  returns=retsp, endd=endd, look_back=look_back, dimax=dimax)
pnls <- do.call(cbind, pnls)
...
arma::mat back_test(const arma::mat& retsx, // Portfolio excess returns
  ...
  for (arma::uword it = 1; it < endd.size(); it++) {
    // cout << "it: " << it << endl;
    // Calculate portfolio weights
    weightv = coeff*calc_weights(retsx.rows(startp(it-1), endd(it-1)), method, eigen_thresh, dimax, confi, alpha, scale, vol_target);
