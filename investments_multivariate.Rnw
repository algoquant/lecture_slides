...
\begin{block}{}
  \begin{columns}[T]
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      <<echo=TRUE,eval=FALSE>>=
btmomtop <- function(retp, objfun, look_back=12, rebalf="months", nstocks=10, 
...
pnls <- roll_portf(excess=retsx, returns=retp, endd=endd, 
  look_back=look_back, dimax=dimax)
# Perform sapply loop over look_backs
look_backs <- seq(2, 15, by=1)
pnls <- lapply(look_backs, roll_portf,
  returns=retp, excess=retsx, endd=endd, dimax=dimax)
pnls <- do.call(cbind, pnls)
colnames(pnls) <- paste0("look_back=", look_backs)
...
arma::mat back_test(const arma::mat& retsx, // Portfolio excess returns
  ...
  for (arma::uword it = 1; it < endd.size(); it++) {
    // cout << "it: " << it << endl;
    // Calculate portfolio weights
    weightv = coeff*calc_weights(retsx.rows(startp(it-1), endd(it-1)), method, dimax, eigen_thresh, confi, alpha, scale, vol_target);
...
pnls <- HighFreq::back_test(excess=retsx, returns=retp,
...
pnls <- lapply(alphav, function(alpha) {
  HighFreq::back_test(excess=retsx, returns=retp,
  startp=startp, endd=endd, controlv=controlv)
})  # end lapply
