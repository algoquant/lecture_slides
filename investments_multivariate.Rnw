...
roll_portf <- function(excess, # Excess returns
                       ...
                       dimax=3, # Eigen shrinkage intensity
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The optimal values of the eigen shrinkage parameter \texttt{dimax} and the return shrinkage intensity parameter $\alpha$ can be determined using \emph{backtesting}.
      \vskip1ex
      The best eigen shrinkage parameter for this portfolio of stocks is equal to \texttt{dimax=33}, which means relatively weak eigen shrinkage.
      \vskip1ex
