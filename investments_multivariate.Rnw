...
objfun <- function(returns) prod(1+returns)
pnlsl <- lapply(look_backs, backtestmomb, returns=retsp, endp=endp, objfun=objfun)
profilev <- sapply(pnlsl, function(pnl) sum(pnl)/sd(pnl))
look_backs[which.max(profilev)]
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The function \texttt{momentum\_daily()} simulates a momentum strategy with \emph{daily rebalancing}.
      \vskip1ex
      A momentum strategy with \emph{daily rebalancing} requires more computations so compiled \texttt{C++} functions are preferred to \texttt{apply()} loops.
      \vskip1ex
      The package \emph{roll} contains extremely fast functions for calculating rolling aggregations using compiled \texttt{C++} code.
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The function \texttt{momentum\_daily\_hold()} simulates a momentum strategy with \emph{daily rebalancing} with a holding period.
    \column{0.5\textwidth}
      \vspace{-1em}
      <<echo=TRUE,eval=FALSE>>=
# Define backtest functional for daily momentum strategy
# If trend=(-1) then it backtests a mean reverting strategy
momentum_daily_hold <- function(retsp, look_back=252, holdperiod=5, bid_offer=0.001, trend=1, ...) {
  stopifnot("package:quantmod" %in% search() || require("quantmod", quietly=TRUE))
  # Calculate rolling variance
  variance <- roll::roll_var(retsp, width=look_back, min_obs=1)
  variance[1, ] <- 1
  ...
  # Calculate momentum profits and losses
  pnls <- trend*rowMeans(weightv*retsp)
  # Calculate transaction costs
  costs <- 0.5*bid_offer*rowSums(abs(rutils::diffit(weightv)))
  (pnls - costs)
}  # end momentum_daily_hold
@
  \end{columns}
\end{block}
...
pnls <- sapply(holdperiods, momentum_daily_hold, look_back=120,
                  retsp=retsp, bid_offer=bid_offer)
...
retsp <- zoo::na.locf(retsp, na.rm=FALSE)
ncols <- NCOL(retsp)
retsp <- retsp[, !(retsp[ncols %/% 10, ] == 0)]


pnls <- momentum_daily(retsp=retsp, trend=(-1))
pnls <- xts::xts(pnls, zoo::index(retsp))
colnames(pnls) <- "PnL"
...
# Perform sapply loop over holding periods
holdperiods <- seq(2, 11, by=2)
pnls <- sapply(holdperiods, momentum_daily, look_back=120, retsp=retsp)
colnames(pnls) <- paste0("holding=", holdperiods)
pnls <- xts::xts(pnls, zoo::index(retsp))
      @
    \column{0.5\textwidth}
...
retsx <- (retsp - riskf)
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.4\paperwidth]{figure/portf_etf_weights_insample.png}
      <<echo=TRUE,eval=FALSE>>=
...
portfis <- portfis*sd(indeks)/sd(portfis)
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.4\paperwidth]{figure/portf_etf_insample.png}
      <<echo=TRUE,eval=FALSE>>=
...
sapply(pnls, function(x) mean(x)/sd(x))
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.4\paperwidth]{figure/portf_etf_outsample.png}
      <<echo=TRUE,eval=FALSE>>=
# Plot cumulative portfolio returns
...
portfos <- xts::xts(retsos %*% weightv, zoo::index(retsos))
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.4\paperwidth]{figure/portf_etfs_outsample.png}
      <<echo=TRUE,eval=FALSE>>=
# Plot cumulative portfolio returns
pnls <- rbind(portfis, portfos)
indeks <- xts::xts(rowSums(retsp)/sqrt(nassets), dates)
...
portfos <- xts::xts(retsos %*% weightv, zoo::index(retsos))
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.4\paperwidth]{figure/portf_etfs_outsample_shrink.png}
      <<echo=TRUE,eval=FALSE>>=
# Plot cumulative portfolio returns
pnls <- rbind(portfis, portfos)
pnls <- pnls*sd(indeks)/sd(pnls)
...
retsxis <- (1 - alpha)*retsx["/2014"] + alpha*retsxm
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.4\paperwidth]{figure/portf_etfs_outsample_rets_shrink.png}
      <<echo=TRUE,eval=FALSE>>=
# Calculate portfolio weights
...
# Define backtest functional for daily momentum strategy
# If trend=(-1) then it backtests a mean reverting strategy
roll_portf <- function(retsp, look_back=252, eigen_max=3, holdperiod=5, bid_offer=0.0, trend=1, ...) {
  ...
  pnls <- lapply((look_back+1):(NROW(weeks)-1), function(i) {
    ...
    # invmat <- HighFreq::calc_inv(cov(retsis), eigen_max=eigen_max)
    # weightv <- invmat %*% colMeans(retsis - riskf)
    weightv <- HighFreq::calc_weights(retsis, method="max_sharpe", eigen_max=eigen_max, scale=FALSE)
    weightv <- drop(weightv/sqrt(sum(weightv^2)))
    # Calculate portfolio pnls out-of-sample
    xts::xts(retsos %*% weightv, zoo::index(retsos))
  })  # end lapply
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      <<echo=TRUE,eval=FALSE>>=
# Simulate a daily ETF momentum strategy
pnls <- roll_portf(retsp=retsp, look_back=41, eigen_max=eigen_max)
# Perform sapply loop over look_backs
look_backs <- seq(5, 50, by=2)
pnls <- lapply(look_backs, roll_portf,
  retsp=retsp, eigen_max=eigen_max)
pnls <- do.call(cbind, pnls)
colnames(pnls) <- paste0("look_back=", look_backs)
tail(pnls)
...
indeks <- xts::xts(rowSums(returns)/sqrt(nassets), dates)
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.4\paperwidth]{figure/portf_optim_outsample.png}
      <<echo=TRUE,eval=FALSE>>=
# Plot cumulative portfolio returns
pnls <- rbind(portfis, portfos)
pnls <- pnls*sd(indeks)/sd(pnls)
...
indeks <- xts::xts(rowSums(returns)/sqrt(nassets), dates)
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.4\paperwidth]{figure/portf_optim_outsample_shrink.png}
      <<echo=TRUE,eval=FALSE>>=
# Plot cumulative portfolio returns
pnls <- rbind(portfis, portfos)
pnls <- pnls*sd(indeks)/sd(pnls)
...
retsxis <- (1 - alpha)*retsx["/2010"] + alpha*retsxm
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.4\paperwidth]{figure/portf_optim_outsample_rets_shrink.png}
      <<echo=TRUE,eval=FALSE>>=
# Calculate portfolio weights
weightv <- invmat %*% colMeans(retsxis)
weightv <- drop(weightv/sqrt(sum(weightv^2)))
...
pnls <- lapply(2:nrows, function(i) {
    ...
    retsx <- retsx[startp[i-1]:endp[i-1], ]
    invmat <- MASS::ginv(cov(retsx))
    # Calculate the maximum Sharpe ratio portfolio weights
    weightv <- invmat %*% colMeans(retsx)
    weightv <- drop(weightv/sqrt(sum(weightv^2)))
    # Calculate the out-of-sample portfolio returns
...
pnls <- HighFreq::back_test(retsx=retsx, returns=returns100,
...
pnls <- lapply(alpha_s, function(alpha) {
  HighFreq::back_test(retsx=retsx, returns=returns100,
  startp=startp, endp=endp, alpha=alpha, eigen_max=eigen_max, method="max_sharpe")
})  # end lapply
