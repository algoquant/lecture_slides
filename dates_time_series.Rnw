...
\begin{block}{}
  \begin{columns}[T]
    \column{0.3\textwidth}
      \texttt{date-time} objects require a time zone to be uniquely specified,
      \vskip1ex
...
%%%%%%%%%%%%%%%
\subsection{Performance Measurement Using \texttt{PerformanceAnalytics}}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-2em}
\begin{block}{}
  \begin{columns}[T]
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      \vspace{-2em}
      <<drawdown_plot,echo=2:30,fig.show='hide'>>=
library(quantmod)
library(PerformanceAnalytics)
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      \vspace{-1em}
      <<corr_plot,echo=2:30,fig.show='hide'>>=
library(quantmod)
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      <<cluster_plot,echo=1:30,fig.width=6,fig.height=6,fig.show='hide'>>=
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      \vspace{-1em}
      <<pca_plot,echo=3:30,fig.height=5,fig.show='hide'>>=
