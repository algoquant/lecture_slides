...
ts_new <- window(ts_var, start=1992, end=1997)
      @
...
# plot histogram
hist(ts_rets, breaks=30, main="", xlim=c(-0.04, 0.04), 
     ylim=c(0, 60), xlab="", ylab="", freq = FALSE)
lines(density(ts_rets),  # draw a line
      col='red', lwd=2)
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      \vspace{-2em}
      <<tseries_intro,echo=7:30,eval=FALSE,fig.show='hide'>>=
set.seed(1121)  # initialize the random number generator
par(oma=c(1, 1, 1, 1))  # set outer margins
par(mgp=c(2, 0.5, 0))  # set axis title and labels
par(mar=c(5, 1, 1, 1))  # set plot margins
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      \vspace{-2em}
      <<zoo_plot_2y,echo=6:30,eval=FALSE,fig.show='hide'>>=
set.seed(1121)  # initialize the random number generator
par(oma=c(1, 1, 1, 1))  # set outer margins
par(mgp=c(2, 0.5, 0))  # set axis title and labels
par(mar=c(5, 1, 1, 1))  # set plot margins
