...
pricets <- rutils::etfenv$prices[, 1]
head(pricets)
sum(is.na(pricets))
library(quantmod)
pricezoo <- zoo::na.locf(pricets, na.rm=FALSE, fromLast=TRUE)
pricexts <- xts:::na.locf.xts(pricets, fromLast=TRUE)
all.equal(pricezoo, pricexts, check.attributes=FALSE)
library(microbenchmark)
summary(microbenchmark(
  zoo=zoo::na.locf(pricets, fromLast=TRUE),
  xts=xts:::na.locf.xts(pricets, fromLast=TRUE),
...
for (colnum in 2:NCOL(xtsv))
  lines(xtsv[, colnum], col=colors[colnum])
# Plot using quantmod
library(quantmod)
...
plotobj <- qplot(x=zoo::index(pricets),
                y=as.numeric(pricets),
                geom="line",
                main=names(pricets)) +
  xlab("") + ylab("") +
...
dframe <- data.frame(dates=zoo::index(pricets),
    zoo::coredata(pricets))
...
dygraph(pricets, main="VTI and IEF prices") %>%
...
pricesub <- pricets["2014-10-15/2015-01-10", 1:4]
first(pricesub)
last(pricesub)
# Subset Nov 2014 using a date string
pricesub <- pricets["2014-11", 1:4]
first(pricesub)
last(pricesub)
...
all.equal(pricets["2014-11", ], pricets["2014-11"])
# .subset_xts() is faster than the bracket []
library(microbenchmark)
summary(microbenchmark(
  bracket=pricets[10:20, ],
  subset=xts::.subset_xts(pricets, 10:20),
  times=10))[, c(1, 4, 5)]
      @
  \end{columns}
\end{block}
...
pricets <- rutils::etfenv$prices
all.equal(pricets[zoo::index(pricets) >= datev],
          pricets[paste0(datev, "/")])
# Boolean subsetting is slower because coercing string into date
library(microbenchmark)
summary(microbenchmark(
  boolean=(pricets[zoo::index(pricets) >= datev]),
  date=(pricets[paste0(datev, "/")]),
  times=10))[, c(1, 4, 5)]  # end microbenchmark summary
...
\begin{block}{}
  \begin{columns}[T]
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      <<echo=TRUE,eval=FALSE>>=
pricets <- HighFreq::SPY["2012-04"]
# Subset recurring time interval using "T notation",
pricets <- pricets["T10:30:00/T15:00:00"]
first(pricets["2012-04-16"])  # First element of day
last(pricets["2012-04-16"])  # Last element of day
...
pricets <- rutils::etfenv$prices[, c("VTI", "IEF")]
pricets <- na.omit(pricets)
str(pricets)  # Display structure of xts
# Subsetting zoo to single column drops dim attribute
pricezoo <- as.zoo(pricets)
dim(pricezoo)
dim(pricezoo[, 1])
# zoo with single column are vectors not matrices
c(is.matrix(pricezoo), is.matrix(pricezoo[, 1]))
# xts always have a dim attribute
rbind(base=dim(pricets), subs=dim(pricets[, 1]))
c(is.matrix(pricets), is.matrix(pricets[, 1]))
      @
  \end{columns}
\end{block}
...
rbind(base=dim(pricets), lag=dim(lag(pricets)))
...
endp <- xts::endpoints(pricets, on="hours")
head(endp)
...
pricesm <- to.period(x=pricets, period="months", name="MSFT")
...
pricesy <- to.period(x=pricesm, period="years", name="MSFT")
