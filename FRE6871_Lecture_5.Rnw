...
returns <- rutils::etf_env$returns[, c("XLF", "XLE")]
...
deg_free <- (NROW(risk_ret -.n_cols)
# Adjusted R-squared
r_squared_adj <- (1-sum(residuals^2)/deg_free/var(response))
...
1-pf(q=f_stat, df1.n_cols-1, df2:nrows.n_cols)
      @
  \end{columns}
\end{block}
...
returns <- na.omit(rutils::etf_env$returns)
...
# Plot three curves in loop
for (indeks in 1:3) {
  curve(expr=plogis(x, scale=lambdav[indeks]),
...
table(actual=!de_fault, forecast=(forecasts < thresh_old))
...
sum(!test_data$de_fault & (forecasts > thresh_old))
...
con_fuse <- function(actu_al, forecasts, thresh_old) {
    confu_sion <- table(actu_al, (forecasts < thresh_old))
    confu_sion <- confu_sion / rowSums(confu_sion)
    c(typeI=confu_sion[2, 1], typeII=confu_sion[1, 2])
  }  # end con_fuse
con_fuse(!test_data$de_fault, forecasts, thresh_old=thresh_old)
