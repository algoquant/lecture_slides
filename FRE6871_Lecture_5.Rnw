...
predm <- matrix(rnorm(nrows*ncols), ncol=ncols)
# Add column names
colnames(predm) <- paste0("col", 1:ncols)
...
alpha <- mean(respv) - sum(colSums(predm)*betas)/nrows
...
predictor2 <- MASS::ginv(crossprod(predm))
# predictor2 <- t(predm) %*% predictor
# Variance of residuals
residsar <- sum(residuals^2)/degf
...
all.equal(MASS::ginv(crossprod(predm)),
          invpred %*% t(invpred))
      @
  \end{columns}
\end{block}
...
fitsd <- cbind(fitted=fitv, stddev=fitsd)
...
cor_fitted <- drop(cor(respv, fitv))
...
predictor=cbind(intercept=rep(1, NROW(respv)), predm)
