...
se_ries <- rutils::etf_env$prices[, 1]
head(se_ries, 3)
sum(is.na(se_ries))
...
dim(matrixv)
do_call_rbind <- function(lists) {
  while (NROW(lists) > 1) {
# Index of odd list elements
    odd_index <- seq(from=1, to=NROW(lists), by=2)
# Bind odd and even elements, and divide lists by half
    lists <- lapply(odd_index, function(indeks) {
      if (indeks==NROW(lists)) return(listv[[indeks]])
      rbind(listv[[indeks]], listv[[indeks+1]])
    })  # end lapply
  }  # end while
# lists has only one element - return it
  listv[[1]]
}  # end do_call_rbind
