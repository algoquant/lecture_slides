...
\begin{block}{}
  \begin{columns}[T]
    ...
    \column{0.5\textwidth}
      ...
      \begin{flalign*}
        & (z - (z^T \mathbbm{1} - \beta x^T \mathbbm{1}) \mathbbm{1} - \beta x)^T x = 0\\
        & (z^T x - (z^T \mathbbm{1} - \beta x^T \mathbbm{1}) \mathbbm{1}^T x - \beta x^T x) = 0\\
        & (z^T x - (z^T \mathbbm{1}) (x^T \mathbbm{1}) + \beta (x^T \mathbbm{1})^2 - \beta x^T x) = 0\\
        & \beta = \frac {z^T x - (x^T \mathbbm{1}) (z^T \mathbbm{1}) } {x^T x - (\mathbbm{1}^T x)^2}
      \end{flalign*}
...
%%%%%%%%%%%%%%%
\subsection{draft: Solving \protect\emph{Linear Regression} Using Linear Algebra}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
...
%%%%%%%%%%%%%%%
\subsection{draft: Solving Multiple \protect\emph{Linear Regression} Using \texttt{solve()}}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    ...
    \column{0.5\textwidth}
      ...
      \begin{align*}
        \alpha = z^T \mathbbm{1} - \beta x^T \mathbbm{1}
      \end{align*}
      If the variables are de-meaned, then the \emph{OLS} estimate for $\beta$ is given by equating the \emph{RSS} derivative to zero:
      \begin{flalign*}
        & RSS_\beta = - 2 (z - \beta x)^T x = 0\\
        & x^T z - \beta x^T x = 0\\
        & \beta = (x^T x)^{-1} x^T z
