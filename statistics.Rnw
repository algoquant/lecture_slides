...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The formula "\texttt{z} \textasciitilde{} \texttt{x}" means the response variable \texttt{z} is explained by the \emph{predictor} \texttt{x} (also called the \emph{explanatory variable} or \emph{independent variable}),
      \vskip1ex
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      A Simple Linear Regression is a linear model between a response variable \texttt{z} and a single \emph{predictor} \texttt{x}, defined by the formula:
      \begin{displaymath}
      ...
      \vskip1ex
      The data consists of \texttt{n} pairs of observations $(x_i, y_i)$ of the response and \emph{predictor}, with the index \texttt{i} ranging from \texttt{1} to \texttt{n},
    \column{0.5\textwidth}
      \vspace{-1em}
...
# define predictor
len_gth <- 100
predic_tor <- rnorm(len_gth, mean=2)
noise <- rnorm(len_gth)
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \end{flalign*}
      $\beta$ is proportional to the correlation coefficient $\rho_{xy}$ between the response and \emph{predictor},
    \column{0.5\textwidth}
      \vspace{-1em}
        <<echo=TRUE,eval=TRUE>>=
# calculate de-meaned explanatory and response vectors
explain_zm <- predic_tor - mean(predic_tor)
...
al_pha <- mean(res_ponse) - be_ta*mean(predic_tor)
      @
      If the response and \emph{predictor} have zero mean, then $\alpha=0$ and $\beta=\frac {y^T x} {x^T x}$,
      \vskip1ex
      The residuals $\varepsilon = y - \alpha \mathbbm{1} - \beta x$ have zero mean: $RSS_\alpha = -2 \varepsilon^T \mathbbm{1} = 0$,
      \vskip1ex
      The residuals $\varepsilon$ are orthogonal to the \emph{predictor} \texttt{x}: $RSS_\beta = -2 \varepsilon^T x = 0$,
  \end{columns}
\end{block}
...
reg_formula <- res_ponse ~ predic_tor
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The fitted (predicted) values are the values of the response variable obtained from applying the regression model to the \emph{predictors},
        <<echo=TRUE,eval=FALSE>>=
...
points(x=predic_tor, y=reg_model$fitted.values,
       pch=16, col="blue")
      @
    \column{0.5\textwidth}
      \includegraphics[width=0.5\paperwidth]{figure/reg_scatter_plot.png}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \end{displaymath}
      The \emph{residuals} are the error terms associated with a particular realization of the response and \emph{predictors},
      \vskip1ex
      The fitted (predicted) values are the values of the response variable obtained from applying the regression model to the \emph{predictors},
        <<echo=TRUE,eval=FALSE>>=
# sum of residuals = 0 
...
par(mar=c(5, 5, 1, 1), oma=c(0, 0, 0, 0))
# extract residuals
resi_duals <- cbind(predic_tor, reg_model$residuals)
colnames(resi_duals) <- c("predictor", "residuals")
# plot residuals
plot(resi_duals)
...
reg_model_sum$
  coefficients["predic_tor", "Std. Error"]
sd(reg_model_sum$residuals)/sd(predic_tor)/
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      If the relationship between the response and \emph{predictors} is weak compared to the error terms (noise), then the regression will have low statistical significance,
      \vskip1ex
    \column{0.5\textwidth}
      \vspace{-1em}
        <<echo=(-(1:1)),eval=TRUE>>=
...
res_ponse <- 3 + predic_tor + rnorm(30, sd=8)
reg_model <- lm(reg_formula)  # perform regression
...
# create explanatory and response variables
  ...
  res_ponse <- 3 + 0.2*predic_tor +
    rnorm(NROW(predic_tor), sd=std_dev)
# specify regression formula
  reg_formula <- res_ponse ~ predic_tor
...
# create explanatory and response variables
    ...
    res_ponse <- 3 + 0.2*predic_tor +
      rnorm(NROW(predic_tor), sd=std_dev)
    reg_stats(data.frame(predic_tor, res_ponse))
    }))
# plot in loop
par(mfrow=c(NCOL(mat_stats), 1))
for (in_dex in 1:NCOL(mat_stats)) {
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      But the failure of the Durbin-Watson test doesn't reject the existence of a linear relationship between the response and \emph{predictors}, it just puts it in doubt,
      \vskip1ex
      Links:
...
predic_tor <- cumsum(rnorm(100))  # unit root time series
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      A multivariate \emph{linear regression} model with \texttt{k} \emph{predictors} $\{x_j\}$, is defined by the formula:
      \begin{displaymath}
      ...
      \vskip1ex
      The response variable $y$ and the \texttt{k} \emph{predictors} $\{x_j\}$ each contain \texttt{n} observations, and they form the columns of matrix $\mathbb{X}$,
      \vskip1ex
      The response variable $y$ is a vector of length \texttt{n}, and the \emph{predictor} $\mathbb{X}$ is a $(n,k)$-dimensional matrix,
    \column{0.5\textwidth}
      \vspace{-1em}
        <<echo=(-(1:1)),eval=TRUE>>=
set.seed(1121)  # initialize random number generator
...
len_gth <- 100
n_var <- 5
predic_tor <- matrix(rnorm(n_var*len_gth), nc=n_var)
noise <- rnorm(len_gth, sd=1.0)
# response equals linear form plus error terms
weight_s <- rnorm(n_var)
res_ponse <- -3 + predic_tor %*% weight_s + noise
      @
      \vspace{-1em}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The residuals have zero mean: $\varepsilon^T \mathbbm{1} = 0$, and they are orthogonal to the \emph{predictors}: $\varepsilon^T \mathbb{X} = 0$, and variance equal to: $\mathbbm{E}[\varepsilon \varepsilon^T] = {\sigma_\varepsilon}^2 \mathbbm{1}$
    \column{0.5\textwidth}
      \vspace{-1em}
        <<echo=TRUE,eval=TRUE>>=
# calculate de-meaned explanatory matrix
explain_zm <- t(t(predic_tor) - colMeans(predic_tor))
...
al_pha <- mean(res_ponse) - 
  sum(colSums(predic_tor)*drop(beta_s))/len_gth
...
fit_ted <- drop(al_pha + predic_tor %*% beta_s)
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      Let $y_i$ be a vector of \texttt{n} observations (\emph{responses}), and $x_{ij}$ be a $(n,k)$-dimensional matrix of \emph{predictors}, 
      \vskip1ex
      Let $\hat{y}_i$ be a vector of \emph{fitted values} of the model, using the matrix of \emph{predictors} $x_{ij}$, 
      \vskip1ex
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The residuals $\varepsilon = y - \hat{y}$ are orthogonal to the \emph{predictors}: $\varepsilon^T \mathbb{X} = 0$, and they have zero mean: $\varepsilon^T \mathbbm{1} = 0$, 
    \column{0.5\textwidth}
      \vspace{-1em}
        <<echo=TRUE,eval=TRUE>>=
...
# solve for regression betas
beta_s <- MASS::ginv(predic_tor) %*% res_ponse
all.equal(drop(beta_s), coef(reg_model), check.attributes=FALSE)
# calculate fitted values
fit_ted <- drop(predic_tor %*% beta_s)
all.equal(fit_ted, reg_model$fitted.values, check.attributes=FALSE)
# calculate residuals
resid_uals <- drop(res_ponse - fit_ted)
all.equal(resid_uals, reg_model$residuals, check.attributes=FALSE)
  ...
  \begin{columns}[T]
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
        <<echo=TRUE,eval=TRUE>>=
# degrees of freedom of residuals
deg_free <- len_gth-NCOL(predic_tor)
# variance of residuals
resid_var <- sum(resid_uals^2)/deg_free
# explanatory matrix squared
explain_squared <- crossprod(predic_tor)
# explain_squared <- t(predic_tor) %*% predic_tor
...
de_sign <- data.frame(  # design matrix
  predic_tor=1:30, omit_var=sin(0.2*1:30))
# response depends on both predictors
res_ponse <- with(de_sign,
          0.2*predic_tor + omit_var + 0.2*rnorm(30))
...
predic_tor <- seq(from=0.1, to=3.0, by=0.1)  # predictor
res_ponse <- 3 + 2*predic_tor + rnorm(30)
reg_formula <- res_ponse ~ predic_tor
reg_model <- lm(reg_formula)  # perform regression
new_data <- data.frame(predic_tor=0.1*31:40)
...
with(predict_lm, {
  ...
  lines(x=new_data$predic_tor, y=lwr, lwd=2, col="red")
  lines(x=new_data$predic_tor, y=upr, lwd=2, col="red")
})  # end with
      @
    \column{0.5\textwidth}
