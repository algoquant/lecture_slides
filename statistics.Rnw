...
%%%%%%%%%%%%%%%
\subsection{Simple Regression Example 2}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
...
indep_var <- 0.1*1:30
# create dependent variable plus noise
depend_var <- 3 + 2*indep_var + rnorm(30)
# perform regression
simple_reg <- lm(depend_var ~ indep_var)
summary(simple_reg)  # regression summary
      @
  \end{columns}
\end{block}
...
%%%%%%%%%%%%%%%
\subsection{Plotting a Regression 2}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
...
par(oma=c(1, 1, 1, 1), mgp=c(2, 0.5, 0), mar=c(5, 1, 1, 1), cex.lab=0.8, cex.axis=0.8, cex.main=0.8, cex.sub=0.5)
# plot(my_reg)  # plot diagnostic tests
plot(my_form)  # plot regression scatterplot
abline(my_reg, col="red")  # add reg line
title(main="Simple Regression", line=-1)
