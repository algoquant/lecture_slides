...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The \emph{null hypothesis} is that $\gamma=0$,
      \vskip1ex
      sample: $\{x_{1},\ldots ,x_{n}\}$ is from a normally distributed population,
      \vskip1ex
      \texttt{adf.test()} \emph{Augmented Dickey-Fuller} test for existence of unit roots,
      \vskip1ex
      The test statistic is:
      \begin{displaymath}
        JB= \frac{n}{6} (\hat{s}^2 + \frac{1}{4} (\hat{k} - 3)^2)
      ...
      If ${\varphi}_{1}=0$ (no autocorrelation of returns) then asset prices follow a \emph{Wiener} process (random walk),
      \vskip1ex
      An \emph{autoregressive} of order \emph{n} (\emph{AR(n)}) is defined as:
    \column{0.5\textwidth}
      \vspace{-1em}
      \hspace*{-1em}
      \includegraphics[width=0.5\paperwidth,valign=t]{figure/stat_unit_root-1}
