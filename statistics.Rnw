...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The data consists of $n$ pairs of observations $(x_i, z_i)$ of the response and explanatory variables, with the index $i$ ranging from $1$ to $n$,
    \column{0.5\textwidth}
      \vspace{-1em}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      A multivariate \emph{linear regression} model with $k$ explanatory variables $\{x_j\}$, is defined by the formula:
      \begin{displaymath}
      ...
      \vskip1ex
      $\varepsilon_i$ are the residuals, assumed to be normally distributed, independent, and stationary, with $\varepsilon$ a vector of length $k$,
      \vskip1ex
      The response variable $z$ and the $k$ explanatory variables $\{x_j\}$ each contain $n$ observations, and they form the columns of matrix $\mathbb{X}$,
      \vskip1ex
      The response variable $z$ is a vector of length $n$, and the explanatory variable $\mathbb{X}$ is a $(n,k)$-dimensional matrix,
    \column{0.5\textwidth}
      \vspace{-1em}
        <<echo=(-(1:1)),eval=TRUE>>=
set.seed(1121)  # initialize random number generator
