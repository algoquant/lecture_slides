...
% Title page setup
\title[Quantitive Finance]{Quantitive Finance}
...
etf_perf_stats <- etf_perf_stats[
  order(etf_perf_stats[, "Annualized Alpha"], 
        decreasing=TRUE), ]
save(file="C:/Develop/data/etf_analysis.RData")
      @
    \column{0.45\textwidth}
      \vspace{-1em}
      <<>>=
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      \vspace{-1em}
      <<corr_plot,echo=(-1),eval=FALSE,fig.show='hide'>>=
library(quantmod)
...
%%%%%%%%%%%%%%%
\subsection{Investment Strategy Backtesting Framework}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
...
\vspace{-1em}
\begin{block}{}
      <<back_weights,eval=FALSE,echo=TRUE,fig.width=15,fig.height=7,fig.show='hide'>>=
