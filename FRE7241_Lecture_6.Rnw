...
reg_stats <- HighFreq::roll_reg(respv=retsp$XLP, retsp=retsp$VTI, endd=(endd-1), startp=(startp-1))
betas <- reg_stats$VTI
all.equal(betas, betar)
# Compare the speed of RcppArmadillo with R code
library(microbenchmark)
summary(microbenchmark(
  Rcpp=HighFreq::roll_reg(respv=retsp$XLP, retsp=retsp$VTI, endd=(endd-1), startp=(startp-1)),
