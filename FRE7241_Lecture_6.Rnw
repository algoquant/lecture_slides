...
pnls <- lapply(2:(npts-1), function(ep) {
  ...
  # Calculate the momentum portfolio returns
  retsportf <- retsp[(endd[ep]+1):endd[ep+1], ] %*% weightv
  rowMeans(retsportf)
})  # end lapply
...
endd <- xts::endpoints(retsp, on="months")[-1]
...
betar <- sapply(1:NROW(endd), FUN=function(ep) {
    datav <- retsp[startp[ep]:endd[ep], ]
    # coef(lm(formulav, data=datav))[2]
    drop(cov(datav$XLP, datav$VTI)/var(datav$VTI))
  })  # end sapply
# Calculate rolling betas using RcppArmadillo
reg_stats <- HighFreq::roll_reg(response=retsp$XLP, retsp=retsp$VTI, endd=(endd-1), startp=(startp-1))
betas <- reg_stats$VTI
all.equal(betas, betar)
# Compare the speed of RcppArmadillo with R code
library(microbenchmark)
summary(microbenchmark(
  Rcpp=HighFreq::roll_reg(response=retsp$XLP, retsp=retsp$VTI, endd=(endd-1), startp=(startp-1)),
