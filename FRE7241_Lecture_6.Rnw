...
dygraphs::dygraph(wealth, main="In-sample Optimal Portfolio Returns") %>%
  ...
  dyLegend(width=500)
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.45\paperwidth]{figure/portf_etf_weights_insample.png}
      \includegraphics[width=0.45\paperwidth]{figure/portf_etf_insample.png}
  \end{columns}
\end{block}
...
indeks <- xts::xts(rowSums(returns)/sqrt(nassets), zoo::index(returns))
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.45\paperwidth]{figure/portf_stocks_outsample_reg.png}
      <<echo=TRUE,eval=FALSE>>=
# Plot cumulative portfolio returns
wealth <- rbind(portf_is, portf_os)
wealth <- wealth*sd(indeks)/sd(wealth)
...
rets_mean <- (1 - alpha)*rets_mean + alpha*mean(rets_mean)
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.45\paperwidth]{figure/portf_stocks_outsample_reg_shink.png}
      <<echo=TRUE,eval=FALSE>>=
# Calculate portfolio weights
...
arma::mat calc_inv(const arma::mat& matrixv, const arma::uword& max_eigen) {
  ...
  eigenvec = eigenvec.cols(eigenvec.n_cols-max_eigen, eigenvec.n_cols-1);
  eigenval = 1/eigenval.subvec(eigenval.n_elem-max_eigen, eigenval.n_elem-1);
...
arma::mat back_test(const arma::mat& excess, // Portfolio excess returns
  ...
  for (arma::uword it=1; it < endp.size(); it++) {
    // cout << "it: " << it << endl;
    // Calculate portfolio weights
    weights = coeff*calc_weights(excess.rows(startp(it-1), endp(it-1)), typev, max_eigen, probv, alpha, scalev);
