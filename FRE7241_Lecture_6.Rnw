...
%%%%%%%%%%%%%%%
\subsection{Forecasting Returns Using Autoregressive Models}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The model fitting procedure depends on two unknown \emph{meta-parameters}: the order of the \emph{AR(p)} model (\texttt{p = or\_der}) and the length of the look-back interval (\texttt{look\_back}).
    \column{0.5\textwidth}
    \vspace{-1em}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The \emph{backtesting} procedure allows determining the optimal \emph{meta-parameters} of the model fitting procedure: the order of the \emph{AR(p)} model (\texttt{p = or\_der}) and the length of look-back interval (\texttt{look\_back}).
    \column{0.5\textwidth}
    \vspace{-1em}
      <<echo=TRUE,eval=FALSE>>=
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The \emph{meta-parameters} of the \emph{backtesting} function are the order $p$ of the \emph{AR(p)} model and the length of look-back interval (\texttt{look\_back}).
    \column{0.5\textwidth}
    \vspace{-1em}
      <<echo=TRUE,eval=FALSE>>=
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The autoregressive strategy is dominated by a few isolated points with very large returns, without showing profits for the remaining points.
      <<echo=TRUE,eval=FALSE>>=
look_back <- 300
# Perform rolling forecasting
forecast_s <- sapply(look_back:n_rows, function(now) {
...
forecast_s <- c(numeric(look_back-1), forecast_s)
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.48\paperwidth]{figure/strat_ar_vti_perf.png}
      <<echo=TRUE,eval=FALSE>>=
...
capp_ed <- ifelse(foo < (-cut_off), -cut_off, capp_ed)
# Define predictor matrix for forecasting
de_sign <- sapply(1:or_der, rutils::lag_it, in_put=re_turns)
de_sign <- cbind(rep(1, n_rows), de_sign)
colnames(de_sign) <- paste0("pred_", 1:NCOL(de_sign))
...
colnames(de_sign)[1] <- "response"
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.48\paperwidth]{figure/strat_ar_vti_perf_plus.png}
      <<echo=TRUE,eval=FALSE>>=
# Perform rolling forecasting
look_back <- 600
forecast_s <- sapply(look_back:n_rows, function(now) {
