...
vti <- retp$VTI
desv <- cbind(retp, 0.5*(vti+abs(vti)), vti^2)
...
model <- lm(IEF ~ VTI + merton, data=desv); summary(model)
# Treynor-Mazuy test
model <- lm(IEF ~ VTI + treynor, data=desv); summary(model)
# Plot residual scatterplot
x11(width=6, height=5)
residuals <- (desv$IEF - model$coeff["VTI"]*vti)
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \end{displaymath}
      Where the decay parameter $\lambda$ determines the rate of decay of the \emph{EWMA} weightv, with smaller values of $\lambda$ producing faster decay, giving more weight to recent pricev, and vice versa.
      \vskip1ex
...
dygraphs::dygraph(pricev["2009"], main="VTI EWMA Prices") %>%
...
colnames(pricev) <- c("VTI", "VTI EWMA")
colnamev <- colnames(pricev)
dygraphs::dygraph(pricev["2009"], main="Recursive VTI EWMA Prices") %>%
  dySeries(name=colnamev[1], label=colnamev[1], strokeWidth=1, col="blue") %>%
  dySeries(name=colnamev[2], label=colnamev[2], strokeWidth=2, col="red") %>%
  dyLegend(show="always", width=500)
# Standard plot of  EWMA prices with custom line colors
...
colnamev <- colnames(pricev)
dygraphs::dygraph(pricev["2009"], main=paste(symbol, "EWMA Returns")) %>%
...
retp <- roll::roll_sum(closep, width=look_back, weights=weightv, min_obs=1, online=FALSE)
pricev <- cbind(closep, retp)
symbol <- "VTI"
colnames(pricev) <- c(symbol, paste(symbol, "Returns"))
# Plot dygraph of VTI Returns
colnamev <- colnames(pricev)
dygraphs::dygraph(pricev["2009"], main=paste(symbol, "Fractional Returns")) %>%
  dyAxis("y", label=colnamev[1], independentTicks=TRUE) %>%
  dyAxis("y2", label=colnamev[2], independentTicks=TRUE) %>%
  dySeries(name=colnamev[1], axis="y", label=colnamev[1], strokeWidth=2, col="blue") %>%
  dySeries(name=colnamev[2], axis="y2", label=colnamev[2], strokeWidth=2, col="red")
...
retp <- lapply(deltav, function(deltav) {
  weightv <- (deltav - 0:(look_back-2)) / 1:(look_back-1)
...
retp <- cbind(closep, retp)
colnames(retp) <- c("VTI", paste0("frac_", deltav))
# Calculate ADF test statistics
adfstats <- sapply(retp, function(x)
  suppressWarnings(tseries::adf.test(x)$statistic)
)  # end sapply
names(adfstats) <- colnames(retp)
      @
    \column{0.5\textwidth}
      \vspace{-1em}
...
colorv <- colorRampPalette(c("blue", "red"))(NCOL(retp))
colnamev <- colnames(retp)
dyplot <- dygraphs::dygraph(retp["2019"], main="Fractional Returns") %>%
  dyAxis("y", label=colnamev[1], independentTicks=TRUE) %>%
...
pricev <- cbind(closep, volumez)
colnames(pricev) <- c("VTI", "Z-scores")
colnamev <- colnames(pricev)
dygraphs::dygraph(pricev["2009"], main="VTI Volume Z-Scores") %>%
  dyAxis("y", label=colnamev[1], independentTicks=TRUE) %>%
  dyAxis("y2", label=colnamev[2], independentTicks=TRUE) %>%
  dySeries(name=colnamev[1], axis="y", label=colnamev[1], strokeWidth=2, col="blue") %>%
  dySeries(name=colnamev[2], axis="y2", label=colnamev[2], strokeWidth=2, col="red")
...
colnames(pricev) <- c("VTI", "Z-scores")
colnamev <- colnames(pricev)
dygraphs::dygraph(pricev["2009"], main="VTI Volatility Z-Scores") %>%
  dyAxis("y", label=colnamev[1], independentTicks=TRUE) %>%
  dyAxis("y2", label=colnamev[2], independentTicks=TRUE) %>%
  dySeries(name=colnamev[1], axis="y", label=colnamev[1], strokeWidth=2, col="blue") %>%
  dySeries(name=colnamev[2], axis="y2", label=colnamev[2], strokeWidth=2, col="red")
...
retp <- rutils::diffit(closep)
volat <- roll::roll_sd(retp, width=look_back, min_obs=1)
...
look_back <- 21
pricez <- drop(HighFreq::roll_zscores(respv=closep, design=dates, look_back=look_back))
...
model <- lm(EWMA ~ VTI + treynor, data=desv)
summary(model)
# Plot residual scatterplot
residuals <- (desv$EWMA - model$coeff["VTI"]*retp)
residuals <- model$residuals
x11(width=6, height=6)
plot.default(x=retp, y=residuals, xlab="VTI", ylab="residuals")
title(main="Treynor-Mazuy Market Timing Test\n for EWMA Crossover vs VTI", line=0.5)
# Plot fitted (predicted) response values
fittedv <- (model$coeff["(Intercept)"] + 
              model$coeff["treynor"]*retp^2)
...
retp <- cbind(retp, trendopt, revertopt, combstrat)
...
# Perform sapply() loops over lambdav
sharpem <- sapply(lambdav1, function(lambda1) {
  sapply(lambdav2, function(lambda2) {
    if (lambda2 > lambda1) {
...
colnames(sharpem) <- lambdav1
rownames(sharpem) <- lambdav2
# Calculate the PnLs for the optimal strategy
whichv <- which(sharpem == max(sharpem, na.rm=TRUE), arr.ind=TRUE)
lambda1 <- lambdav1[whichv[2]]
lambda2 <- lambdav2[whichv[1]]
pnls <- sim_ewma2(ohlc=ohlc, lambda1=lambda1, lambda2=lambda2, 
                    look_back=look_back, bid_offer=0.0, trend=1, lagg=2)[, "pnls"]
wealthv <- cbind(retp, pnls)
...
model <- lm(VWAP ~ VTI + treynor, data=desv)
summary(model)
# Plot residual scatterplot
residuals <- (desv$VWAP - model$coeff["VTI"]*retp)
residuals <- model$residuals
# x11(width=6, height=6)
plot.default(x=retp, y=residuals, xlab="VTI", ylab="residuals")
title(main="Treynor-Mazuy Market Timing Test\n for VWAP Crossover vs VTI", line=0.5)
# Plot fitted (predicted) response values
fittedv <- (model$coeff["(Intercept)"] + model$coeff["treynor"]*retp^2)
points.default(x=retp, y=fittedv, pch=16, col="red")
