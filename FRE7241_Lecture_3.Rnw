...
ew_ma <- roll::roll_sum(closep, width=look_back, weights=weights, min_obs=1)
# Copy over NA values
...
ma_d <- HighFreq::roll_var(closep, look_back=look_back, method="quantile")
...
ma_d <- HighFreq::roll_var(matrix(taq$price), look_back=look_back, method="quantile")
# ma_d <- TTR::runMAD(taq$price, n=look_back)
ma_d <- rutils::lagit(ma_d, lagg=-half_back, pad_zeros=FALSE)
# Calculate Z-scores
...
ma_d <- HighFreq::roll_var(matrix(taq$price), look_back=look_back, method="quantile")
ma_d <- rutils::lagit(ma_d, lagg=-1, pad_zeros=FALSE)
# Calculate Z-scores
z_scores <- ifelse(ma_d > 0, (taq$price - medi_an)/ma_d, 0)
range(z_scores); mad(z_scores)
