...
\begin{block}{}
  \begin{columns}[T]
    ...
    \column{0.5\textwidth}
      ...
      <<echo=TRUE,eval=FALSE>>=
# bidask equal to 10 bps for liquid ETFs
bidask <- 0.001
...
calc_sharpe <- function(ohlc, lambdaf, lambdas, look_back, bidask, trend, lagg) {
  if (lambdaf >= lambdas) return(NA)
...
sharpem <- outer(lambdafv, lambdasv, FUN=calc_sharpe, ohlc=ohlc, 
                 look_back=look_back, bidask=0.0, trend=1, lagg=2)
...
pnls <- sim_ewma2(ohlc=ohlc, lambdaf=lambdaf, lambdas=lambdas, 
                  look_back=look_back, bidask=0.0, trend=1, lagg=2)[, "pnls"]
wealthv <- cbind(retp, pnls)
colnames(wealthv)[2] <- "EWMA"
