...
vti <- retsp$VTI
desv <- cbind(retsp, 0.5*(vti+abs(vti)), vti^2)
...
desv <- cbind(pnls, retsp, retsp^2)
desv <- na.omit(desv)
colnames(desv) <- c("EWMA", "VTI", "treynor")
model <- lm(EWMA ~ VTI + treynor, data=desv)
summary(model)
# Plot residual scatterplot
residuals <- (desv$EWMA - model$coeff["VTI"]*retsp)
...
summary(model)
# Plot residual scatterplot
residuals <- (desv$VWAP - model$coeff["VTI"]*retsp)
residuals <- model$residuals
# x11(width=6, height=6)
plot.default(x=retsp, y=residuals, xlab="VTI", ylab="residuals")
