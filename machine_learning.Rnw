...
%%%%%%%%%%%%%%%
\subsection{draft: Fisher's \protect\emph{F-statistic} for Linear Regression}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \begin{displaymath}
        (y - y_{fit})^T (y_{fit} - \bar{y}) = 0
      \end{displaymath}
      Therefore the \emph{Residual Sum of Squares} $RSS = \varepsilon^T \varepsilon$ and the \emph{Explained Sum of Squares} $ESS = (y_{fit} - \bar{y})^T (y_{fit} - \bar{y})$ follow independent \emph{chi-squared} distributions with $(n - k)$ and $(k-1)$ degrees of freedom.
      \vskip1ex
      Then the \emph{F}-statistic, equal to the ratio of the \emph{ESS} divided by \emph{RSS}: 
      \begin{displaymath}
        F = \frac{ESS/(k-1)}{RSS/(n-k)}
      \end{displaymath}
      Follows the \emph{F-distribution} with $(n - k)$ and $(k-1)$ degrees of freedom (assuming that the \emph{residuals} are normally distributed).
      \vskip1ex
      The \emph{F-test} is used to test the \emph{null hypotheses} that an \emph{unrestricted} model with more parameters does not explain the variance of the \emph{response} better than a \emph{restricted} model with fewer parameters.
      \vskip1ex
      Let the \emph{unrestricted} model have $p_2$ parameters and $df_2 = n - p_2$ degrees of freedom, and the \emph{restricted} model have $p_1$ parameters and $df_1 = n - p_1$ degrees of freedom, with $p_1 < p_2$.
      \vskip1ex
...
f_stat <- e_ss*deg_free/r_ss/(n_cols-1)
...
%%%%%%%%%%%%%%%
\subsection{draft: Fisher's \protect\emph{F-statistic} of Model Significance}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
...
%%%%%%%%%%%%%%%
\subsection{draft: Fisher's \protect\emph{F-test} of Model Significance}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      Let $y^{fit}_i$ be a vector of \emph{fitted values} of the model, using the \emph{design matrix} $x_{ij}$.
      \vskip1ex
      Let the \emph{unrestricted} model have $p_2$ parameters, so its \emph{fitted values} have $df_2 = n - p_2$ degrees of freedom, and the \emph{restricted} model have $p_1$ parameters, so its \emph{fitted values} have $df_1 = n - p_1$ degrees of freedom, with $p_1 < p_2$.
      \vskip1ex
      Then the \emph{F}-statistic, defined as the ratio: 
      \begin{displaymath}
        F = \frac{(RSS_1 - RSS_2)/(df_1 - df_2)}{RSS_2/df_2}
