...
objfun <- function(weightv, retp) {
  retp <- retp %*% weightv
  -sum(retp^2) + 1e7*(1 - sum(weightv^2))^2
}  # end objfun
...
retspca <- xts::xts(retp %*% pcad$rotation, order.by=datev)
round(cov(retspca), 3)
...
stdevs <- sapply(retp, var, na.rm=TRUE)
# Calculate the returns of low and high volatility stocks
nstocks <- NCOL(retp)
medianv <- median(stdevs)
retlow <- retp[, names(stdevs[stdevs<=medianv])]
rethigh <- retp[, names(stdevs[stdevs>medianv])]
...
datav <- xts(cbind(volaapl, correlv), zoo::index(retp))
