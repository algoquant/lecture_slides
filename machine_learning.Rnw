...
barplot(weightv6, names.arg=names(weights2), xlab="", ylab="",
...
%%%%%%%%%%%%%%%
\subsection{Stock Correlations in Intervals of High and Low Volatility}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The trailing covariance can be updated using these recursive formulas with the weight decay factor $\lambda$:
      \begin{flalign*}
...
densityv <- density(predm[response])
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The update equation can also be written as the \emph{Exponentially Weighted Moving Average} (\emph{EWMA}) of the observed values $o_t$:
      \begin{displaymath}
        \bar{p}_t = (1 - k) \bar{p}_{t-1} + k o_t = k \sum_{j=0}^n (1 - k)^j o_{t-j}
      \end{displaymath}
      The Kalman filtered prices are equal to the \emph{EWMA} of the observed values $o_t$ even for more complex processes than Brownian motion, like \emph{ARIMA} processes.
      \vskip1ex
      The function \texttt{HighFreq::run\_mean()} calculates the \emph{EWMA} using the \texttt{C++} \emph{Armadillo} numerical library.
      \vskip1ex
      \begin{lstlisting}[language=R,basicstyle=\tiny\ttfamily\bfseries,backgroundcolor=\color{anti_flashwhite},showstringspaces=FALSE]
