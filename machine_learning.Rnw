...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      In \emph{Principal Component Regression} (\emph{PCR}), the \emph{design matrix} $\mathbb{X}$ is mulltiplied by a \emph{PCA rotation matrix} $\mathbb{W}$:
      \begin{displaymath}
        \mathbb{X}_{pca} = \mathbb{X} \mathbb{W}
      \end{displaymath}
      The matrix $\mathbb{W}$ is chosen so that the columns (principal components) of the \emph{PCA design matrix} $\mathbb{X}_{pca}$ form an orthogonal set of predictors.
      \vskip1ex
...
\begin{block}{}
  \begin{columns}[T]
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.4\paperwidth]{figure/logistic_func.png}
        <<echo=TRUE,eval=FALSE>>=
lambdas <- c(0.5, 1, 1.5)
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The \texttt{stu\_dent} column alone can be used to calculate the probability of default using single-factor \emph{logistic} regression.
      \vskip1ex
      ...
      \vskip1ex
      The reason that students are more likely to default is because they have higher credit balances than non-students - which is what the single-factor regression is showing.
      \vskip1ex
      But students are less likely to default than non-students that have the same credit balance - which is what the multifactor model is showing.
      \vskip1ex
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The \texttt{stu\_dent} column can be used to calculate the probability of default using \emph{logistic} regression.
      \vskip1ex
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The \emph{null hypothesis} is that \texttt{de\_fault = FALSE}.
      \vskip1ex
