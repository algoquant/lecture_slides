...
\subtitle{FRE6871 \& FRE7241, Fall 2025}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      Multivariate regression can also be solved recursively using \emph{coordinate descent}:
      \begin{flalign*}
      ...
      \vskip1ex
      Then the updated \emph{coefficient} $\beta_j$ is equal to the projection of the \emph{partial residual} $r_j$ onto the $j$-th \emph{predictor} vector $x_j$ divided by the sum of squared \emph{predictor} vector $x_j$.
      \vskip1ex
      The calculation is repeated  until the \emph{regression coefficients} $\beta$ converge.
        <<echo=TRUE,eval=TRUE>>=
      @
    \column{0.5\textwidth}
      \vspace{-1em}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \begin{displaymath}
        O(\beta) = (y - \mathbb{X} \boldsymbol\beta)^T (y - \mathbb{X} \boldsymbol\beta) + \lambda \sum_{i=1}^n \beta^2_i
      \end{displaymath}
      Where $\lambda$ is the regularization intensity parameter.
      \vskip1ex
      The \emph{ridge regression} coefficients can be calculated using matrix inversion as follows:
      \begin{displaymath}
        \beta_{ridge} = (\mathbb{X}^T \mathbb{X} + \lambda \mathbbm{1})^{-1} \mathbb{X}^T y = \mathbb{X}_{\lambda}^{-2} \mathbb{X}^T y
      \end{displaymath}
