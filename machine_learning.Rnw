...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \end{flalign*}
      The regression model with a centered (de-meaned) \emph{predictor matrix} produces the same \emph{fitted values} (only shifted by their mean) and \emph{residuals} as the original regression model, so it's equivalent to it.
      \vskip1ex
      But the centered regression model has a different \emph{influence matrix}, which maps the centered \emph{response vector} $\hat{y}$ into the centered \emph{fitted values} $\hat{y}_{fit}$.
    \column{0.5\textwidth}
      \vspace{-1em}
        <<echo=TRUE,eval=TRUE>>=
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      A large value of the \emph{F-statistic} $F$ indicates that the variance of $ESS$ is significantly greater than that of $RSS$, and that the regression is statistically significant.
    \column{0.5\textwidth}
      \vspace{-1em}
      <<echo=TRUE,eval=TRUE>>=
# F-statistic from lm()
...
%%%%%%%%%%%%%%%
\subsection{Microstructure Noise And Trading Volumes in High Frequency Data}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
...
%%%%%%%%%%%%%%%
\subsection{Filtering Microstructure Noise From High Frequency Data}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      Microstructure noise in high frequency data can be identified using a \emph{Hampel filter}.
      \vskip1ex
...
medianv <- HighFreq::roll_mean(pricev, look_back=look_back, method="nonparametric")
...
# Calculate the z-scores
medianv <- HighFreq::roll_mean(priceb, look_back=look_back, method="nonparametric")
...
medianv[1:look_back, ] <- priceb[1:look_back, ]
medianv <- rutils::lagit(medianv, lagg=(-half_back), pad_zeros=FALSE)
madv <- HighFreq::roll_var(priceb, look_back=look_back, method="nonparametric")
madv <- rutils::lagit(madv, lagg=(-half_back), pad_zeros=FALSE)
zscores <- ifelse(madv > 0, (priceb - medianv)/madv, 0)
...
errorr <- sapply(threshv, confun,
  actualv=ispike, zscores=zscores)  # end sapply
errorr <- t(errorr)
rownames(errorr) <- threshv
errorr <- rbind(c(1, 0), errorr)
errorr <- rbind(errorr, c(0, 1))
...
\begin{block}{}
  \begin{columns}[T]
    ...
    \column{0.5\textwidth}
      ...
      <<echo=TRUE,eval=FALSE>>=
# Calculate the EWMA VTI variance using compiled C++ function
look_back <- 51
...
pricewma[1] <- prico[1]
# Calculate the EWMA prices in R
for (it in 2:nrows) {
  pricewma[it] <- (1-kgain)*pricewma[it-1] + kgain*prico[it]
}  # end for
all.equal(pricewma, priceb)
# Calculate the EWMA prices using RcppArmadillo C++
