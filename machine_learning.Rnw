...
\subtitle{FRE6871 \& FRE7241, Spring 2026}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      Most covariance matrices are \emph{positive semi-definite} and all \emph{positive semi-definite} matrices are the covariance matrix of some multivariate distribution.
      \vskip1ex
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
\fbox{\parbox{\columnwidth-1.5em}{
      The \emph{Singular Value Decomposition} (\emph{SVD}) is a generalization for rectangular matrices, of the \emph{eigen decomposition} of square matrices.
}}
      \vskip1ex
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \end{displaymath}
      In the case of square positive definite matrices, the \emph{pseudo-inverse} matrix simplifies to the inverse: $(\mathbb{A}^T \mathbb{A})^{-1} \mathbb{A}^T = \mathbb{A}^{-1} (\mathbb{A}^T)^{-1} \mathbb{A}^T = \mathbb{A}^{-1}$
      \vskip1ex
      If the number of rows of $\mathbb{A}$ is greater than the number of columns, then the generalized inverse is only a left inverse $\mathbb{A}^{-1} \mathbb{A} = \mathbbm{1}$, and if the number of rows is less than the number of columns, then it's only a right inverse $\mathbb{A} \mathbb{A}^{-1} = \mathbbm{1}$.
      \vskip1ex
...
nrows <- 6 ; ncols <- 4 ; dimv <- min(nrows, ncols)
matv <- matrix(rnorm(nrows*ncols), nc=ncols)
# Calculate the generalized inverse of matv
invmat <- MASS::ginv(matv)
all.equal(matv %*% invmat, diag(dimv))
all.equal(invmat %*% matv, diag(dimv))
all.equal(matv, matv %*% invmat %*% matv)
# Random rectangular matrix: nrows < ncols
nrows <- 4 ; ncols <- 6 ; dimv <- min(nrows, ncols)
matv <- matrix(rnorm(nrows*ncols), nc=ncols)
# Calculate the generalized inverse of matv
invmat <- MASS::ginv(matv)
all.equal(matv %*% invmat, diag(dimv))
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
\fbox{\parbox{\columnwidth-1.5em}{
      The Cholesky matrix $\mathbb{L}$ can be considered the square root of the matrix $\mathbb{A}$.
}}
      \vskip1ex
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The \emph{regularized} inverse $\mathbb{C}_n^{-1}$ is calculated by removing the higher order eigenvalues that are very small, and keeping only the first $n$ \emph{eigenvalues}:
      \begin{displaymath}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
\fbox{\parbox{\columnwidth-1.5em}{
      This is an example of the \emph{bias-variance tradeoff} that is a central concept in machine learning.
}}
      \vskip1ex
...
%%%%%%%%%%%%%%%
\subsection{draft: Principal Components of Two Stocks}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
...
lmtest::dwtest(regmod)
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      % \includegraphics[width=0.45\paperwidth]{figure/pca_stocks.png}
      <<echo=TRUE,eval=FALSE>>=
...
%%%%%%%%%%%%%%%
\subsection{Conservation of Variance in PCA}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The matrix of \emph{principal component} returns $\mathbf{pc}$ is equal to the matrix $\mathbf{w}$ of the PCA weights (loadings) times the matrix of the return vectors $\mathbf{r}$:
      \begin{displaymath}
        \mathbf{pc} = \mathbf{w} \, \mathbf{r}
      \end{displaymath}
      Where the PCA weights $\mathbf{w}^T \mathbf{w} = \mathbbm{1}$ are orthonormal.
      \vskip1ex
      So the sum of the PCA variances is equal to:
      \begin{displaymath}
        \mathbf{pc}^T \mathbf{pc} = (\mathbf{w} \, \mathbf{r})^T \mathbf{w} \, \mathbf{r} = 
        \mathbf{r}^T \, (\mathbf{w}^T \mathbf{w}) \, \mathbf{r} = \mathbf{r}^T \mathbf{r}
      \end{displaymath}
      So the total variance is conserved in PCA: the sum of the PCA variances is equal to the sum of the stock variances.
      <<echo=TRUE,eval=FALSE>>=
      @
    \column{0.5\textwidth}
      \vspace{-1em}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
\fbox{\parbox{\columnwidth-1.5em}{
      \emph{Principal Component Analysis} (\emph{PCA}) can be applied to perform \emph{dimension reduction}, to explain the returns of a large number of correlated time series as linear combinations of a smaller number of principal component time series.
}}
      \vskip1ex
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
\fbox{\parbox{\columnwidth-1.5em}{
      The original time series can be calculated approximately from just the first few \emph{principal components}, showing how \emph{PCA} can be applied to perform \emph{dimension reduction}.
}}
      \vskip1ex
      The function \texttt{solve()} solves systems of linear equations, and also inverts square matrices.
      <<echo=TRUE,eval=FALSE>>=
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      Estimating the correlations of stock returns is complicated when their date ranges don't overlap in time.  
      Stocks may trade over different date ranges because of IPOs and corporate events (takeovers, mergers).
      \vskip1ex
      The function \texttt{cor()} calculates the correlation matrix of time series.  
      But it returns \texttt{NA} if the date ranges don't overlap in time.
      The argument \texttt{use="pairwise.complete.obs"} removes \texttt{NA} values from pairs of stock returns.
      \vskip1ex
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The \emph{residuals} $\varepsilon_i$ can be considered the source of error and uncertainty in the \emph{response vector} $y$: $y_i = \alpha + \beta x_i + \varepsilon_i$.
      \vskip1ex
...
plot(regmod, which=2, ask=FALSE)  ##  Plot just Q-Q
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \hspace*{-1em}
      \includegraphics[width=0.45\paperwidth]{figure/autocorr_reg-1}
  \end{columns}
\end{block}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \end{displaymath}
      The matrix $\mathbb{X}^{-1} = (\mathbb{X}^T \mathbb{X})^{-1} \mathbb{X}^T$ is the generalized inverse of the \emph{predictor matrix} $\mathbb{X}$.
      \vskip1ex
      The generalized inverse $\mathbb{X}^{-1}$ satisfies the equation:
      \begin{displaymath}
        \mathbb{X} \mathbb{X}^{-1} \mathbb{X} = \mathbbm{X}
      \end{displaymath}
      Which is a generalization of the inverse property: $\mathbb{X}^{-1} \mathbb{X} = \mathbbm{1}$.
    \column{0.5\textwidth}
      \vspace{-1em}
        <<echo=TRUE,eval=TRUE>>=
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \end{displaymath}
      Where $\mathbb{H} = \mathbb{X} (\mathbb{X}^T \mathbb{X})^{-1} \mathbb{X}^T$ is the \emph{influence matrix}.
      \vskip1ex
\fbox{\parbox{\columnwidth-1.5em}{
...
%%%%%%%%%%%%%%%
\subsection{Solution of Regression Using Coordinate Descent}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      Multivariate regression can also be solved recursively using the \emph{coordinate descent} algorithm, which optimizes the regression coefficients one at a time using formulas similar to the \emph{Newton-Raphson} algorithm.
      \vskip1ex
      The \emph{partial residual} $r_j$ is first calculated without the effect of the $j$-th \emph{predictor}:
      \begin{flalign*}
        & r_j = y - \sum_{i \ne j} {\beta_i x_i} = y - \mathbb{X}_{-j} \beta_{-j} \\
        & \beta_j = \frac{x_j r_j}{x_j^T x_j}
      \end{flalign*}
      Then the coefficient $\beta_j$ is updated, to be equal to the product of the \emph{partial residual} $r_j$ times the $j$-th \emph{predictor} vector $x_j$ divided by its sum of squares.
      \vskip1ex
      The \emph{coordinate descent} algorithm performs a \emph{Newton-Raphson} type of optimization of each coordinate independently, one at a time, until the regression coefficients converge.
      \vskip1ex
      The \emph{coordinate descent} algorithm can be faster than the \emph{OLS} solution when the number of predictors is large, because it avoids calculating the inverse of large predictor matrices.
      \vskip1ex
      It can also be faster when the predictor matrix is sparse, because it only uses the non-zero elements of the predictor matrix.
        <<echo=TRUE,eval=TRUE>>=
      @
    \column{0.5\textwidth}
      \vspace{-1em}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      Bootstrapping the regression of stock returns shows that the actual standard errors can be much larger as the theoretical standard errors reported by the function \texttt{lm()}.
      \vskip1ex
      This is because the function \texttt{lm()} assumes that the data is normally distributed, while in reality stock returns have very large skewness and kurtosis.
      \vskip1ex
      The \emph{null hypothesis} of the \emph{Jarque-Bera} test is that the data is normally distributed.
      \vskip1ex
      If the \emph{p}-value is very small, then the \emph{null hypothesis} can be assumed to be \texttt{FALSE}, and the data is not normally distributed.
    \column{0.5\textwidth}
      \vspace{-1em}
      <<echo=TRUE,eval=FALSE>>=
...
tseries::jarque.bera.test(retp$XLF)
sapply(retp, tseries::jarque.bera.test)
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
\fbox{\parbox{\columnwidth-1.5em}{
      The \emph{R-squared} measures how well the fitted values fit the response values, but it doesn't measure the statistical significance of the regression.  
}}
      \vskip1ex
      A regression may have a large \emph{R-squared}, but the coefficient \emph{p}-values may also be large, when the statistical significance of the regression is low.
    \column{0.5\textwidth}
      \vspace{-1em}
        <<echo=TRUE,eval=TRUE>>=
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      Fisher's \emph{F-distribution} is the distribution of the ratio of two independent sample variances, i.e. two independent \emph{Chi-squared} random variables divided by their respective degrees of freedom.
      \vskip1ex
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
\fbox{\parbox{\columnwidth-1.5em}{
      The \emph{null hypothesis} of the \emph{F-test} test is that the \emph{F-statistic} is not significantly greater than $1$ (the variance $\sigma^2_x$ is not significantly greater than $\sigma^2_y$).
}}
      \vskip1ex
      A large value of the \emph{F-statistic} indicates that the variances are unlikely to be equal.
      \vskip1ex
      The function \texttt{pf(q)} returns the cumulative probability of the \emph{F-distribution}, i.e. the cumulative probability that the \emph{F-statistic} is less than the quantile $q$.
      \vskip1ex
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
\fbox{\parbox{\columnwidth-1.5em}{
      The \emph{F}-statistic of linear regression is equal to the ratio of the explained variance divided by the residual variance:
}}
      \begin{displaymath}
        F = \frac{ESS/(k-1)}{RSS/(n-k)}
      \end{displaymath}
...
1-pf(q=fstat, df1=(ncols-1), df2=(nrows-ncols))
      @
      \fbox{\parbox{\columnwidth-1.5em}{
      The \emph{null hypothesis} of the \emph{F-test} is that the regression is not statistically significant - that the \emph{F-statistic} is not significantly greater than $1$ - that the variance of $ESS$ is not significantly greater than that of $RSS$.
      }}
      \vskip1ex
      A large value of the \emph{F-statistic} indicates that the $ESS$ is significantly greater than the $RSS$, and that the regression is able to explain the \emph{response} data well.
      \vskip1ex
      A regression model that better explains the \emph{response} data will have a larger \emph{ESS} and a smaller \emph{RSS}, so the \emph{F-statistic} will be significantly greater than $1$.
  \end{columns}
\end{block}
...
%%%%%%%%%%%%%%%
\subsection{Linear Transformation of the Predictor Matrix}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      This is equivalent to multiplying the \emph{predictor matrix} $\mathbb{X}$ by a transformation matrix $\mathbb{W}$:
      \begin{displaymath}
        \mathbb{X}_{trans} = \mathbb{X} \, \mathbb{W}
      \end{displaymath}
      The transformed \emph{predictor matrix} $\mathbb{X}_{trans}$ produces the same \emph{influence matrix} $\mathbb{H}$ as the original \emph{predictor matrix} $\mathbb{X}$:
      \begin{flalign*}
      ...
      \end{flalign*}
      Since the \emph{influence matrix} $\mathbb{H}$ is the same, the transformed regression model produces the same fitted values and \emph{residuals} as the original regression model, so it's equivalent to it.
      \vskip1ex
    \column{0.5\textwidth}
      \vspace{-1em}
        <<echo=TRUE,eval=TRUE>>=
# Define transformation matrix
matv <- matrix(rnorm(ncols^2, min=(-1), max=1), ncol=ncols)
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      In \emph{Principal Component Regression} (\emph{PCR}), the predictor matrix $\mathbb{X}$ is multiplied by the \emph{PCA rotation matrix} $\mathbb{W}$:
      \begin{displaymath}
        \mathbb{X}_{pca} = \mathbb{X} \mathbb{W}
      \end{displaymath}
      So that the principal component vectors form the columns of the new predictor matrix.
      \vskip1ex
      Since the new \emph{PCR} predictors $x^{pca}_i$ are orthogonal, the regression coefficients are simply: 
      \begin{displaymath}
        \beta_i = \frac{x^{pca}_i \cdot y}{\sigma^2_i}
      \end{displaymath}
      Where $x^{pca}_i \cdot y$ are the inner products of the \emph{PCR} predictors $x^{pca}_i$ times the \emph{response vector} $y$, and $\sigma^2_i = x^{pca}_i \cdot x^{pca}_i$ are the inner products (sum of squares) of the predictors $x^{pca}_i$.
    \column{0.5\textwidth}
      \vspace{-1em}
        <<echo=TRUE,eval=FALSE>>=
# Perform PCA of the predictors
...
%%%%%%%%%%%%%%%
\subsection{Dimension Reduction Using Principal Component Regression}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      If the predictor columns are \emph{collinear} then some of the \emph{PCR} predictor squares are zero $\sigma^2_i = 0$, and the associated regression coefficients are infinite (indeterminate) and should be discarded.
      \vskip1ex
      The regression can also become \emph{singular} if the number of rows of the predictor is too small, or is even less than the number of its columns.
      \vskip1ex
      The regression can be \emph{regularized} by removing the infinite or very large \emph{PCR} regression coefficients, and transforming the coefficients back to the original predictor coordinates.
      \vskip1ex
      This is called \emph{dimension reduction} - excluding the principal components with very small eigenvalues.
      \vskip1ex
      \emph{Dimension reduction} can also be applied to reduce model overfitting by reducing the number of effective predictors.
    \column{0.5\textwidth}
      \vspace{-1em}
        <<echo=TRUE,eval=FALSE>>=
# Create almost collinear predictors
...
%%%%%%%%%%%%%%%
\subsection{Ridge Regularized Regression}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The objective function of \emph{ridge regression} is equal to the \emph{Residual Sum of Squares} (\emph{RSS}) plus a penalty term proportional to the sum of squares of the coefficients:
      \begin{displaymath}
        \operatorname{O}(\beta) = (y - \mathbb{X} \boldsymbol\beta)^T (y - \mathbb{X} \boldsymbol\beta) + \lambda \boldsymbol\beta^T \boldsymbol\beta
      \end{displaymath}
      Where $\lambda$ is the regularization intensity parameter.
      \vskip1ex
      The \emph{ridge regression} coefficients $\boldsymbol\beta$ are calculated by minimizing the objective function $\operatorname{O}(\beta)$, by setting its derivative equal to zero:
      \begin{displaymath}
        \frac{d \operatorname{O}(\beta)}{d \boldsymbol\beta} = -2 \mathbb{X}^T (y - \mathbb{X} \boldsymbol\beta) + 2 \lambda \boldsymbol\beta = 0
      \end{displaymath}
      The coefficients $\boldsymbol\beta$ are then calculated by performing matrix inversion:
      \begin{displaymath}
        \boldsymbol\beta = (\mathbb{X}^T \mathbb{X} + \lambda \mathbbm{1})^{-1} \mathbb{X}^T y = \mathbb{X}_{\lambda}^{-2} \mathbb{X}^T y
      \end{displaymath}
      With $\mathbb{X}_{\lambda}^{-2} = (\mathbb{X}^T \mathbb{X} + \lambda \mathbbm{1})^{-1}$.
    \column{0.5\textwidth}
      \vspace{-1em}
        <<echo=TRUE,eval=FALSE>>=
...
pred2 <- (t(predm) %*% predm + lambdaf*unitmat)
betar <- drop(MASS::ginv(pred2) %*% t(predm) %*% respv)
names(betar) <- colnames(predm)
# Calculate the RSS and the penalty terms
rss <- sum((respv - predm %*% betar)^2)
penalty <- lambdaf*sum(betar^2)
c(RSS=rss, Penalty=penalty, Objective=rss + penalty)
# Calculate the ridge objective function:
ridgeobj <- function(respv, predm, betav, lambdaf) {
  rss <- sum((respv - predm %*% betav)^2)
  penalty <- lambdaf*sum(betav^2)
  residv <- respv - predm %*% betav
  return(rss + penalty)
} # end ridgeobj
# Calculate coefficients by minimizing the ridge objective
...
%%%%%%%%%%%%%%%
\subsection{Ridge Regression Coefficients}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      \emph{Ridge regression} shrinks the less significant coefficients towards zero, but it does not set any of them exactly to zero.
      \vskip1ex
      As the \emph{regularization} intensity parameter $\lambda$ becomes very large, the \emph{ridge} coefficients tend to zero:
      \begin{displaymath}
        \lambda \to \infty \quad \Rightarrow \quad
        \beta_{ridge} \to \frac{1}{\lambda} \mathbb{X}^T y
      \end{displaymath}
      When $\lambda$ is very large, all the \emph{ridge} coefficients tend to zero.
      \vskip1ex
      When $\lambda$ is zero, the \emph{ridge} coefficients are equal to the standard regression coefficients.
        <<echo=TRUE,eval=FALSE>>=
...
legend("topright", legend=rownames(coeffm), lwd=3,
  pch=19, lty=1, bty="n", col=colorv, cex=0.9)
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.45\paperwidth]{figure/reg_ridge.png}
  \end{columns}
\end{block}
...
%%%%%%%%%%%%%%%
\subsection{draft: Bias and Variance of Ridge Regression}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The bias of the \emph{ridge regression} coefficients is equal to the difference between their expected values minus the standard regression coefficients $\beta$:
      \begin{flalign*}
        & {bias}_{ridge} = \mathbbm{E}[\beta_{ridge}] - \beta = 
        \mathbbm{E}[(\mathbb{X}^T \mathbb{X} + \lambda \mathbbm{1})^{-1} \mathbb{X}^T y] - \beta = \\
        & ((\mathbb{X}^T \mathbb{X} + \lambda \mathbbm{1})^{-1} \mathbb{X}^T \mathbb{X} - \mathbbm{1}) \beta = 
        - \lambda (\mathbb{X}^T \mathbb{X} + \lambda \mathbbm{1})^{-1} \beta
      \end{flalign*}
      Since $\mathbbm{E}[y] = \mathbb{X} \beta$.
      \vskip1ex
      The variance of the \emph{ridge regression} coefficients is equal to:
      \begin{displaymath}
        \sigma^2_{ridge} = \sigma^2 \mathbb{X}_{\lambda}^{-2} \mathbb{X}^T \mathbb{X} \mathbb{X}_{\lambda}^{-2}
      \end{displaymath}
      Where $\sigma^2$ is the variance of the \emph{residuals} and
      $\mathbb{X}_{\lambda}^{-2} = (\mathbb{X}^T \mathbb{X} + \lambda \mathbbm{1})^{-1}$.
      \vskip1ex
      \emph{Ridge regression} shrinks the less significant coefficients towards zero, but it does not set any of them exactly to zero.
      \vskip1ex
      As the \emph{regularization} intensity parameter $\lambda$ becomes very large, the \emph{ridge} coefficients tend to zero:
      \begin{displaymath}
        ...
        \beta_{ridge} \to \frac{1}{\lambda} \mathbb{X}^T y
      \end{displaymath}
      Which is very biased, but has low variance.
      \vskip1ex
      As $\lambda$ is becomes large, the solution becomes more biased, but the variance of the estimates decreases.
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.45\paperwidth]{figure/reg_ridge.png}
        <<echo=TRUE,eval=FALSE>>=
...
% This is a copy from the previous section "Multivariate Regression"
%%%%%%%%%%%%%%%
\subsection{Solution of Regression Using Coordinate Descent}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
...
%%%%%%%%%%%%%%%
\subsection{LASSO Regularized Regression}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The objective function of \emph{LASSO regression} is equal to the \emph{RSS} plus a penalty term proportional to the sum of the absolute values of the coefficients:
      \begin{displaymath}
        O(\beta) = (y - \mathbb{X} \beta)^T (y - \mathbb{X} \beta) + \lambda \sum_{i=1}^n |\beta_i|
      \end{displaymath}
      Where $\lambda$ is the regularization intensity parameter.
      \vskip1ex
      The \emph{LASSO regression} coefficients can be calculated using \emph{coordinate descent} with soft-thresholding:
      \begin{flalign*}
        & r_j = y - \sum_{i \ne j} {\beta_i x_i} = y - \mathbb{X}_{-j} \beta_{-j} \\
        & c_j = x_j r_j \\
        & c_j = sign(c_j) \, max(|c_j| - \lambda, 0) \\
        & \beta_j = \frac{c_j}{x_j^T x_j}
      \end{flalign*}
      The \emph{partial residual} $r_j$ is calculated without the effect of the $j$-th \emph{predictor}.
      \vskip1ex
      Soft-thresholding is applied to the projection $c_j$ of the \emph{partial residual} $r_j$ onto the $j$-th \emph{predictor} vector $x_j$.
      \vskip1ex
      The \emph{coefficient} $\beta_j$ is equal to the projection $c_j$ divided by the sum of squared \emph{predictor} vector $x_j$.
      \vskip1ex
      The recursion is repeated  until the \emph{regression coefficients} $\beta$ converge.
    \column{0.5\textwidth}
      \vspace{-1em}
        <<echo=TRUE,eval=FALSE>>=
...
all.equal(betav, drop(betac), check.attributes=FALSE)
      @
      The soft-thresholding shrinks the coefficients to zero if they are less than the regularization parameter $\lambda$.
  \end{columns}
\end{block}
...
%%%%%%%%%%%%%%%
\subsection{draft: LASSO Regularized Regression Example}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      Where $\lambda$ is the regularization intensity parameter.
      \vskip1ex
      The \emph{LASSO regression} coefficients can be calculated using matrix inversion as follows:
      \begin{displaymath}
        \beta_{ridge} = (\mathbb{X}^T \mathbb{X} + \lambda \mathbbm{1})^{-1} \mathbb{X}^T y
      \end{displaymath}
      \emph{Ridge regression} shrinks the less significant regression coefficients towards zero, but it does not set any of them exactly to zero.
        <<echo=TRUE,eval=FALSE>>=
...
# Calculate the RSS and the penalty term
rss <- sum((respv - predm %*% coeffl)^2)
...
%%%%%%%%%%%%%%%
\subsection{Modeling Credit Defaults Using Student Status}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The \texttt{student} column alone can be used to calculate the probability of default using a single-factor \emph{logistic} regression.
      \vskip1ex
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The \texttt{student} column is a confounding variable because it's correlated with the \texttt{balance} column.
      \vskip1ex
...
%%%%%%%%%%%%%%%
\subsection{The Confusion Matrix of the Classification Model}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The \emph{ROC curve} is the plot of the \emph{true positive} rate, as a function of the \emph{false positive} rate, and illustrates the performance of the binary classifier.
      \vskip1ex
      The area under the \emph{ROC curve} (AUC) is a measure of the performance of the binary classification model.
      <<echo=TRUE,eval=FALSE>>=
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      Consider a price process $p_t$ which follows a \emph{Brownian Motion}:
      \begin{displaymath}
        p_t = p_{t-1} + \eta_t
      \end{displaymath}
      This equation is called the \emph{transition} (state) equation, and it describes the time evolution of the unobservable state variable $p_t$.
      \vskip1ex
      We can only observe (measure) the value of $p_t$ approximately due to the noise $\epsilon_t$:
      \begin{displaymath}
        o_t = p_t + \epsilon_t
      \end{displaymath}
      This equation is called the \emph{measurement} equation, and it describes the observed measurement variable $o_t$.
      \vskip1ex
      The innovations $\epsilon_t$ and $\eta_t$ follow \emph{Normal} distributions with standard deviations equal to $\sigma_{\epsilon}$ and $\sigma_{\eta}$
      \vskip1ex
      The above two equations define a very simple \emph{state space} model called a \emph{level model}.
      \vskip1ex
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.45\paperwidth]{figure/kalman_statespace.png}
      <<echo=TRUE,eval=FALSE>>=
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      Let $\hat{p}_t$ be the estimate of the price, let $\bar{p}_t$ be the expected value of the estimate, and let $\sigma^2_t$ be its variance.
      \vskip1ex
      The expected value $\hat{p}_t$ is the best estimate of the price $\bar{p}_t$.
      \vskip1ex
      Let $o_t$ be the observed price today.
      \vskip1ex
      The probability distribution of the price estimate $\hat{p}_t$, conditional on $\hat{p}_{t-1}$ and on $o_t$, is the product of two \emph{Standard} distributions centered on $\bar{p}_{t-1}$ and $o_t$:
      \begin{align*}
        & \phi(\hat{p}_t | \bar{p}_{t-1}, o_t) \propto \\
        & \exp(\frac{-(\hat{p}_t - o_t)^2}{2 \sigma^2_{\epsilon}}) \exp(\frac{-(\hat{p}_t - \bar{p}_{t-1})^2}{2 (\sigma^2_{t-1} + \sigma^2_{\eta})}) \propto \\
        & \exp(\frac{-(\hat{p}_t - \bar{p}_t)^2}{2 \sigma^2_t})
      \end{align*}
      We can interpret this expression as a \emph{likelihood function} for the expected value $\bar{p}_t$ of the estimate $\hat{p}_t$.
    \column{0.5\textwidth}
      The expected value $\bar{p}_t$ of the estimate $\hat{p}_t$ is given by:
      \begin{align*}
        & \bar{p}_t = \frac{\bar{p}_{t-1} \sigma^2_{\epsilon} + o_t (\sigma^2_{t-1} + \sigma^2_{\eta})}{\sigma^2_{\eta} + \sigma^2_{\epsilon} + \sigma^2_{t-1}} = \\
        & \bar{p}_{t-1} + k_{t-1} (o_t - \bar{p}_{t-1})
      \end{align*}
      Where $\sigma^2_t$ is the updated variance of $\hat{p}_t$ given by:
      \begin{displaymath}
        \sigma^2_t = \frac{\sigma^2_{\epsilon} (\sigma^2_{t-1} + \sigma^2_{\eta})}{\sigma^2_{t-1} + \sigma^2_{\eta} + \sigma^2_{\epsilon}} = (1 - k_{t-1}) (\sigma^2_{t-1} + \sigma^2_{\eta})
      \end{displaymath}
      The ratio: 
      \begin{displaymath}
        k_t = \frac{\sigma^2_t + \sigma^2_{\eta}}{\sigma^2_t + \sigma^2_{\eta} + \sigma^2_{\epsilon}}
      \end{displaymath}
      Is called the \emph{Kalman gain}.
      \vskip1ex
      The above equations can be combined to form the \emph{update equations}:
      \begin{align*}
        & \bar{p}_t = \bar{p}_{t-1} + k_{t-1} (o_t - \bar{p}_{t-1}) \\
        & \sigma^2_t = (1 - k_{t-1}) (\sigma^2_{t-1} + \sigma^2_{\eta}) \\
        & k_t = \frac{\sigma^2_t + \sigma^2_{\eta}}{\sigma^2_t + \sigma^2_{\eta} + \sigma^2_{\epsilon}}
      \end{align*}
      The update equations allow to \emph{recursively update} the expected value $\bar{p}_t$ and the variance $\sigma^2_t$ based on the past value $\bar{p}_{t-1}$ and the observed price $o_t$.
  \end{columns}
\end{block}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      In the long run, the Kalman filter settles into a steady state, with the estimate variance and the Kalman gain becoming constant: $\sigma^2_t = \sigma^2$ and $k_t = k$.
      \vskip1ex
      Then the update equations simplify to:
      \begin{align*}
        & \sigma^2 = (1 - k) (\sigma^2 + \sigma^2_{\eta}) \\
        & k = \frac{\sigma^2 + \sigma^2_{\eta}}{\sigma^2 + \sigma^2_{\eta} + \sigma^2_{\epsilon}}
      \end{align*}
      Which can be transformed to:
      \begin{align*}
        & k = \frac{\sigma^2_{\eta}}{\sigma^2 + \sigma^2_{\eta}} \\
        & \sigma^2 (\sigma^2 + \sigma^2_{\eta}) = \sigma^2_{\epsilon} \sigma^2_{\eta}
      \end{align*}
      With the solutions:
      \begin{align*}
        & \sigma^2 = -\frac{\sigma^2_{\eta}}{2} + \sqrt{\frac{\sigma^4_{\eta}}{4} + \sigma^2_{\epsilon} \sigma^2_{\eta}} \\
        & k = \frac{\sigma^2_{\eta}}{\sigma^2 + \sigma^2_{\eta}}
      \end{align*}
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.45\paperwidth]{figure/kalman_steady.png}
      \vspace{-1em}
      <<echo=TRUE,eval=FALSE>>=
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \begin{align*}
        & \sigma^2 = \sigma^2_{\eta} (-\frac{1}{2} + \sqrt{\frac{1}{4} + \frac{\sigma^2_{\epsilon}}{\sigma^2_{\eta}}}) \\
        & k = \frac{1}{\frac{1}{2} + \sqrt{\frac{1}{4} + \frac{\sigma^2_{\epsilon}}{\sigma^2_{\eta}}}}
      \end{align*}
      For very small observation volatility $\sigma_{\epsilon}$, the Kalman gain is close to \texttt{1}, and the best estimate of the price today $\bar{p}_t$ is the observed value today $o_t$.
      \vskip1ex
      While for very large observation volatility $\sigma_{\epsilon}$, the Kalman gain is close to \texttt{0}, and the best estimate of the price today $\bar{p}_t$ is the estimate from yesterday $\bar{p}_{t-1}$.
      \begin{displaymath}
        \bar{p}_t = \bar{p}_{t-1} + k (o_t - \bar{p}_{t-1})
      \end{displaymath}
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.45\paperwidth]{figure/kalman_volratio.png}
      <<echo=TRUE,eval=FALSE>>=
...
%%%%%%%%%%%%%%%
\subsection{Solution of the Kalman Filter}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
...
for (it in 2:nrows) {
  priceb[it] <- priceb[it-1] + kgain*(prico[it] - priceb[it-1])
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.45\paperwidth]{figure/kalman_solution.png}
      <<echo=TRUE,eval=FALSE>>=
...
%%%%%%%%%%%%%%%
\subsection{The Kalman Filter and Exponential Smoothing}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The update equation can also be written as the \emph{Exponential Moving Average} (\emph{EMA}) of the observed values $o_t$:
      \begin{displaymath}
        \bar{p}_t = (1 - k) \bar{p}_{t-1} + k o_t = k \sum_{j=0}^n (1 - k)^j o_{t-j}
      \end{displaymath}
      The Kalman filtered prices are equal to the \emph{EMA} of the observed values $o_t$ even for more complex processes than Brownian motion, like \emph{ARIMA} processes.
      \vskip1ex
      The function \texttt{HighFreq::run\_mean()} calculates the \emph{EMA} using the \texttt{C++} \emph{Armadillo} numerical library.
      \vskip1ex
      \begin{lstlisting}[language=R,basicstyle=\tiny\ttfamily\bfseries,backgroundcolor=\color{anti_flashwhite},showstringspaces=FALSE]
arma::mat run_mean(const arma::mat& tseries, double lambdaf) {
  ...
  for (arma::uword it = 1; it < nrows; it++) {
    ...
    meanm.row(it) = lambdaf*meanm.row(it-1) + 
...
%%%%%%%%%%%%%%%
\section{The Nearest Neighbor Algorithm}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      function \texttt{predict()} is a \emph{generic function} for forecasting based on a given model.
      \vskip1ex
      ...
      \vskip1ex
      The \emph{null hypothesis} is that \texttt{default = FALSE}.
      \vskip1ex
...
%%%%%%%%%%%%%%%
\subsection{draft: Machine Learning Is Not Artificial Intelligence}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    ...
    \column{0.4\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.3\paperwidth]{figure/MLisnotAI.jpeg}
  \end{columns}
\end{block}
...
\begin{block}{}
  \begin{columns}[T]
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.45\paperwidth]{figure/data_science.png}
  \end{columns}
\end{block}
...
%%%%%%%%%%%%%%%
\subsection{draft: Machine Learning}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
...
What is Machine Learning?
Machine Learning (ML) studies statistical models which can identify patterns in the data and make forecasts.
ML is closely related to statistics, but with an emphasis on forecasting.
ML models are divided into supervised learning or unsupervised learning.
Supervised learning models require a training set to calibrate the model parameters.
Examples of supervised learning models are linear regression, decision trees, support vector machines (SVM), and neural networks.
Unsupervised learning models don't require a training set.
Examples of unsupervised learning models are clustering models, like principal component analysis (PCA) and k-nearest neighbors (KNN).
ML models are also divided into classification and regression models.
An example of a regression model is linear regression.  An example of a classification model is logistic regression.
ML uses several techniques to calibrate models and improve forecasting.
First, ML uses cross-validation (backtesting) to determine the optimal model meta-parameters.
Second, ML uses estimator shrinkage to achieve a better tradeoff between their bias and variance.
