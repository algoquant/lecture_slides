...
bar <- covmat(retsp, anglev=anglev)
tan(anglev)
...
pricev <- rutils::etfenv$pricev[, symbolv]
...
summary(microbenchmark(
  ...
  times=10))[, c(1, 4, 5)]
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.45\paperwidth]{figure/pca_etf_load1.png}
      <<echo=TRUE,eval=FALSE>>=
...
optiml <- DEoptim::DEoptim(fn=objfun,
  ...
  retsp=retsp, control=list(parVar="weights1",
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.45\paperwidth]{figure/pca_etf_load2.png}
      <<echo=TRUE,eval=FALSE>>=
# PC2 weights
...
\begin{block}{}
  \begin{columns}[T]
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.45\paperwidth]{figure/pca_etf_scree.png} \\
...
retspca <- xts::xts(retsp %*% pcad$rotation, order.by=datev)
round(cov(retspca), 3)
...
max(eigenval)/min(eigenval)
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.45\paperwidth]{figure/pca_cond_num.png}
      <<echo=TRUE,eval=FALSE>>=
...
nrows <- 100
set.seed(1121)  # Initialize random number generator
retsp <- matrix(rnorm(nstocks*nrows), nc=nstocks)
...
condvec <- sapply(obsvec, function(nobs) {
  ...
  max(eigenval)/min(eigenval)
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.45\paperwidth]{figure/pca_cond_num2.png}
      <<echo=TRUE,eval=FALSE>>=
# Plot the condition numbers
...
cor(retsp$DAL, retsp$FOXA, use="pairwise.complete.obs")
cor(na.omit(retsp[, c("DAL", "FOXA")]))[2]
cormat <- cor(retsp, use="pairwise.complete.obs")
sum(is.na(cormat))
...
which(cumsum(eigenval)/sum(eigenval) > 0.8)[1]
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.45\paperwidth]{figure/pca_stock_scree.png}
      <<echo=TRUE,eval=FALSE>>=
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The returns of low volatility stocks have a higher correlation than those of high volatility stocks.
      \vskip1ex
      Low and high volatility stocks have different principal components.
      \vskip1ex
      The returns of low volatility stocks have a higher correlation so they can be explained by a smaller number of principal components, compared to high volatility stocks.
      \vskip1ex
      The correlation matrix of high volatility stocks may have a larger condition number than low volatility stocks.
    \column{0.5\textwidth}
      \vspace{-1em}
      <<echo=TRUE,eval=FALSE>>=
# Calculate the stock volatilities
stdevs <- sapply(retsp, var, na.rm=TRUE)
# Calculate the returns of low and high volatility stocks
nstocks <- NCOL(retsp)
medianv <- median(stdevs)
retlow <- retsp[, names(stdevs[stdevs<=medianv])]
rethigh <- retsp[, names(stdevs[stdevs>medianv])]
...
%%%%%%%%%%%%%%%
\subsection{Correlations of Stock Returns in Intervals of High and Low Volatility}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
...
colnames(retvti) <- "VTI"
retsp <- returns[zoo::index(retvti)]
...
stdcor <- sapply(2:npts, function(endp) {
  # cat("endp = ", endp, "\n")
  retsp <- retsp[endd[endp-1]:endd[endp]]
  cormat <- cor(retsp, use="pairwise.complete.obs")
  cormat[is.na(cormat)] <- 0
...
%%%%%%%%%%%%%%%
\subsection{Principal Component Analysis in Intervals of High and Low Volatility}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The trailing correlations of stock returns are typically higher with higher volatilities.
      \vskip1ex
      The function \texttt{HighFreq::run\_covar()} calculates the trailing covariance of two \emph{time series} using the decay factor $\lambda$:
      \begin{flalign*}
        & \bar{r}^1_t = \lambda \bar{r}^1_{t-1} + (1-\lambda) r^1_t \\
        & \bar{r}^2_t = \lambda \bar{r}^2_{t-1} + (1-\lambda) r^2_t \\
        & \operatorname{cov}_t = \lambda \operatorname{cov}_{t-1} + (1-\lambda) (r^1_t - \bar{r}^1_t) (r^2_t - \bar{r}^2_t)
      \end{flalign*}
      <<echo=TRUE,eval=FALSE>>=
# Calculate AAPL and XLK returns
retsp <- na.omit(cbind(returns$AAPL, rutils::etfenv$returns$XLK))
# Calculate the trailing correlations
lambda <- 0.99
covarv <- HighFreq::run_covar(retsp, lambda)
correlv <- covarv[, 1, drop=FALSE]/sqrt(covarv[, 2]*covarv[, 3])
volaapl <- sqrt(covarv[, 2])
# Scatterplot of trailing stock volatilities and correlations
endd <- rutils::calc_endpoints(retsp, interval="months")
plot(x=volaapl[endd], y=correlv[endd, ], 
 xlab="volatility", ylab="correlation",
 main="Trailing Stock Volatilities and Correlations")
# Plot trailing stock volatilities and correlations
datav <- xts(cbind(volaapl, correlv), zoo::index(retsp))
colnames(datav) <- c("volatility", "correlation")
colnamev <- colnames(datav)
dygraphs::dygraph(datav[endd], 
  main="AAPL Trailing Stock Volatility and Correlation") %>%
  dyAxis("y", label=colnamev[1], independentTicks=TRUE) %>%
  dyAxis("y2", label=colnamev[2], independentTicks=TRUE) %>%
  dySeries(name=colnamev[1], axis="y", label=colnamev[1], strokeWidth=2, col="blue") %>%
  dySeries(name=colnamev[2], axis="y2", label=colnamev[2], strokeWidth=2, col="red") %>%
...
datav <- cbind(cumsum(retsp$XLK), correlv)
...
%%%%%%%%%%%%%%%
\subsection{Portfolio Correlations}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The average correlations of a stock portfolio are typically higher with higher volatilities.
      <<echo=TRUE,eval=FALSE>>=
...
lambda <- 0.9
correlv <- sapply(retsp, function(retsp) {
  covarv <- HighFreq::run_covar(cbind(retvti, retsp), lambda)
  ...
  \begin{columns}[T]
    ...
    \column{0.5\textwidth}
      ...
      \includegraphics[width=0.45\paperwidth]{figure/reg_scatter_plot.png}
        <<echo=TRUE,eval=TRUE>>=
# Define explanatory (predv) variable
nrows <- 100
set.seed(1121)  # Initialize random number generator
predv <- runif(nrows)
noisev <- rnorm(nrows)
# Response equals linear form plus random noise
respv <- (-3 + 2*predv + noisev)
      @
...
predzm <- predv - mean(predv)
respzm <- respv - mean(respv)
# Calculate the regression beta
betav <- cov(predv, respv)/var(predv)
# Calculate the regression alpha
alpha <- mean(respv) - betav*mean(predv)
      @
  \end{columns}
\end{block}
...
points(x=predv, y=regmod$fitted.values, pch=16, col="blue")
      @
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.45\paperwidth]{figure/reg_scatter_plot.png}
...
lines(x=predv, y=(respv-noisev), col="red", lwd=3)
...
# Extract residuals
datav <- cbind(predv, regmod$residuals)
...
residsd <- sqrt(sum(residv^2)/degf)
# Standard error of beta
betasd <- residsd/sqrt(sum(predzm^2))
# Standard error of alpha
alphasd <- residsd*sqrt(1/nrows + mean(predv)^2/sum(predzm^2))
      @
  \end{columns}
\end{block}
...
respv <- (-3 + 2*predv + rnorm(nrows, sd=8))
regmod <- lm(formulav)  # Perform regression
...
# Define explanatory (predv) and response variables
  predv <- rnorm(100, mean=2)
  respv <- (1 + 0.2*predv + 
  rnorm(NROW(predv), sd=stdev))
# Specify regression formula
  formulav <- respv ~ predv
...
# Define explanatory (predv) and response variables
    ...
    respv <- (1 + 0.2*predv +
      rnorm(NROW(predv), sd=stdev))
    regstats(data.frame(predv, respv))
    }))
# Plot in loop
par(mfrow=c(NCOL(statsmat), 1))
for (it in 1:NCOL(statsmat)) {
...
predv <- cumsum(rnorm(100))  # Unit root time series
...
infmat <- predv %*% predinv
# Plot the leverage vector
ordern <- order(predv[, 2])
plot(x=predv[ordern, 2], y=diag(infmat)[ordern],
...
betav <- predinv %*% respv
fittedv <- drop(predv %*% betav)
residv <- drop(respv - fittedv)
degf <- (NROW(predv) - NCOL(predv))
residsd <- sqrt(sum(residv^2)/degf)
fitcovar <- residsd*infmat
fitsd <- sqrt(diag(fitcovar))
# Plot the standard deviations
fitsd <- cbind(fitted=fittedv, stdev=fitsd)
...
fittedv <- lapply(1:50, function(it) {
  ...
  respv <- respv + rnorm(nrows, sd=1.0)
...
predv2 <- MASS::ginv(crossprod(predv))
# Define new predictors
newdata <- (max(predv[, 2]) + 10*(1:5)/nrows)
# Calculate the predicted values and standard errors
predv <- cbind(rep(1, NROW(newdata)), newdata)
predsd <- sqrt(predv %*% predv2 %*% t(predv))
predv <- cbind(
  prediction=drop(predv %*% betav),
  stdev=diag(residsd*predsd))
# Or: Perform loop over predv
predv <- apply(predv, MARGIN=1, function(predv) {
  # Calculate predicted values
  prediction <- predv %*% betav
  # Calculate standard deviation
  predsd <- sqrt(t(predv) %*% predv2 %*% predv)
  predictsd <- residsd*predsd
  c(prediction=prediction, stdev=predictsd)
})  # end sapply
predv <- t(predv)
      @
  \end{columns}
  ...
  \begin{columns}[T]
    \column{0.5\textwidth}
      The variables $\sigma^2_\varepsilon$ and $\sigma^2_y$ follow the \emph{chi-squared} distribution with $d_{free} = (n-k-1)$ degrees of freedom, so the \emph{predicted value} $y_{pred}$ follows the \emph{t-distribution}.
        <<echo=TRUE,eval=FALSE>>=
# Prepare plot data
xdata <- c(predv[, 2], newdata)
xlim <- range(xdata)
ydata <- c(fittedv, predv[, 1])
# Calculate t-quantile
tquant <- qt(pnorm(2), df=degf)
predictlow <- predv[, 1]-tquant*predv[, 2]
predicthigh <- predv[, 1]+tquant*predv[, 2]
ylim <- range(c(respv, ydata, predictlow, predicthigh))
...
points(x=predv[, 2], y=respv, col="blue")
points(x=newdata, y=predv[, 1], pch=16, col="blue")
...
with(predictlm, {
  points(x=newdata$predv, y=fit, pch=16, col="blue")
  lines(x=newdata$predv, y=lwr, lwd=3, col="green")
  lines(x=newdata$predv, y=upr, lwd=3, col="red")
})  # end with
legend(x="topleft", # Add legend
       legend=c("predictions", "+2SD", "-2SD"),
       title=NULL, inset=0.05, cex=0.8, lwd=6, y.intersp=0.5,
...
ncols <- 5
set.seed(1121)  # initialize random number generator
predv <- matrix(runif(nrows*ncols), ncol=ncols)
# Add column names
colnames(predv) <- paste0("pred", 1:ncols)
...
predzm <- t(t(predv) - colMeans(predv))
# predv <- apply(predv, 2, function(x) (x-mean(x)))
respzm <- respv - mean(respv)
# Calculate the regression coefficients
betav <- drop(MASS::ginv(predzm) %*% respzm)
# Calculate the regression alpha
alpha <- mean(respv) - sum(colSums(predv)*betav)/nrows
...
residv <- drop(respv - fittedv)
all.equal(residv, regmod$residuals, check.attributes=FALSE)
# Residuals are orthogonal to predictor columns (predvs)
sapply(residv %*% predv, all.equal, target=0)
# Residuals are orthogonal to the fitted values
all.equal(sum(residv*fittedv), target=0)
# Sum of residuals is equal to zero
...
# Calculate the residuals
respzm <- respv - mean(respv)
residv <- drop(respzm - fitted_zm)
all.equal(residv, regmod$residuals,
          check.attributes=FALSE)
# Calculate the influence matrix
...
pcad <- prcomp(predv, center=FALSE, scale=FALSE)
# Calculate the PCA predictors
predpca <- predv %*% pcad$rotation
...
respv <- 0.2*predv + omitv + 0.2*rnorm(30)
# Mis-specified regression only one predictor
model_ovb <- lm(respv ~ predv)
regsum <- summary(regmod_ovb)
regsum$coeff
regsum$r.squared
# Durbin-Watson test shows residuals are autocorrelated
...
# Inverse of predictor matrix squared
predv2 <- MASS::ginv(crossprod(predv))
# predv2 <- t(predv) %*% predv
# Variance of residuals
residsd <- sum(residv^2)/degf
# Calculate covariance matrix of betas
betacovar <- residsd*predv2
# Round(betacovar, 3)
...
all.equal(MASS::ginv(crossprod(predv)), predinv %*% t(predinv))
      @
  \end{columns}
\end{block}
...
colnamev <- colnames(predv)
colnames(newdata) <- colnamev
newdatav <- as.matrix(newdata)
prediction <- drop(newdatav %*% betav)
predsd <- drop(sqrt(newdatav %*% betacovar %*% t(newdatav)))
      @
  \end{columns}
\end{block}
...
formulav <- paste0("respv ~ ",
  paste(colnames(predv), collapse=" + "), " - 1")
# Specify multivariate regression using formula
regmod <- lm(formulav, data=data.frame(cbind(respv, predv)))
regsum <- summary(regmod)
...
cor_fitted <- drop(cor(respv, fittedv))
...
infmat <- predv %*% MASS::ginv(predv)
infpca <- predpca %*% MASS::ginv(predpca)
all.equal(infmat, infpca)
# Calculate the regression coefficients
coeffv <- drop(MASS::ginv(predv) %*% respv)
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      This is called \emph{dimension reduction} - excluding the principal components with very small squares.
      \vskip1ex
...
legend("topleft", title="Scale parameters",
       paste("lambda", lambdav, sep="="), y.intersp=0.5,
...
predv <- c(sample1, sample2)
respv <- c(logical(100), !logical(100))
# Perform logit regression
logmod <- glm(respv ~ predv, family=binomial(logit))
class(logmod)
summary(logmod)
      @
    \column{0.5\textwidth}
...
ordern <- order(predv)
plot(x=predv[ordern], y=logmod$fitted.values[ordern],
     main="Category Densities and Logistic Function",
     type="l", lwd=4, col="orange", xlab="predictor", ylab="density")
densityv <- density(predv[response])
...
predv=cbind(intercept=rep(1, NROW(respv)), predv)
# Likelihood function of the logistic model
likefun <- function(coeff, respv, predv) {
  probs <- plogis(drop(predv %*% coeff))
  -sum(respv*log(probs) + (1-respv)*log((1-probs)))
}  # end likefun
# Run likelihood function
coeff <- c(1, 1)
likefun(coeff, respv, predv)
      @
  \end{columns}
\end{block}
...
%%%%%%%%%%%%%%%
\subsection{draft: Modeling Credit Defaults Using Student Status}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The \texttt{student} column can be used to calculate the probability of default using \emph{logistic} regression.
      \vskip1ex
...
table(actual=!default, forecast=(fcast < threshold))
...
table(actual=!testset$default, forecast=(fcast < threshold))
      @
  \end{columns}
\end{block}
...
confmat <- table(actual=!testset$default, 
      forecast=(fcast < threshold))
confmat
# Calculate FALSE positive (type I error)
sum(!testset$default & (fcast > threshold))
...
confun <- function(actualv, fcast, threshold) {
    confmat <- table(actualv, (fcast < threshold))
    confmat <- confmat / rowSums(confmat)
    c(typeI=confmat[2, 1], typeII=confmat[1, 2])
  }  # end confun
confun(!testset$default, fcast, threshold=threshold)
...
%%%%%%%%%%%%%%%
\subsection{draft: Receiver Operating Characteristic (ROC) Curve}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      a binary.
      \emph{area under the ROC curve} (AUC) is a measure of 
      a binary.
      The optimal \emph{threshold value} can be determined using \emph{cross-validation}. 
      \vskip1ex
      The \emph{ROC curve} is the plot of the \emph{true positive} rate, as a function of the \emph{false positive} rate, and illustrates the performance of a binary classifier.
      \vskip1ex
      The area under the \emph{ROC curve} (AUC) measures the classification ability of a binary classifier.
...
%%%%%%%%%%%%%%%
\section{Classification Using K-Nearest Neighbor Algorithm}
...
What is Machine Learning?
Machine Learning (ML) studies statistical models which can identify patterns in the data and make predictions.
ML is closely related to statistics, but with an emphasis on prediction.
...
ML uses several techniques to calibrate models and improve prediction.
