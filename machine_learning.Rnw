  ...
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \begin{displaymath}
        (y - y_{fit})^T (y_{fit} - \bar{y}) = 0
      \end{displaymath}
      Therefore the \emph{Residual Sum of Squares} $RSS = \varepsilon^T \varepsilon$ and the \emph{Explained Sum of Squares} $ESS = (y_{fit} - \bar{y})^T (y_{fit} - \bar{y})$ follow independent \emph{chi-squared} distributions with $(n - k - 1)$ and $k$ degrees of freedom.
      \vskip1ex
      Then the \emph{F}-statistic, equal to the ratio of the \emph{ESS} divided by \emph{RSS}: 
      \begin{displaymath}
        F = \frac{ESS/k}{RSS/(n - k - 1)}
      \end{displaymath}
      Follows the \emph{F-distribution} with $(n - k - 1)$ and $k$ degrees of freedom (assuming that the \emph{residuals} are normally distributed).
      \vskip1ex
