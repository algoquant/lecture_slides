...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      The parameter \texttt{eigen\_max} specifies the number of eigenvalues used for calculating the \emph{regularized} inverse of the covariance matrix of returns.
      \vskip1ex
...
lmtest::dwtest(model)
    ...
    \column{0.5\textwidth}
    \vspace{-1em}
      \includegraphics[width=0.45\paperwidth]{figure/returns_regress-1}\\
      \vspace{-1em}
      <<echo=TRUE,eval=FALSE>>=
...
retscum <- cumsum(retsp)
# Plot the solved returns
for (symbol in symbolv) {
  plot.zoo(cbind(retscum[, symbol], solved[, symbol]),
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \end{displaymath}
      With two \emph{regression coefficients}: $\beta = (\alpha, \beta1)$, and a \emph{predictor matrix} $\mathbb{X}$ with two columns, with the first column equal to a unit vector.
      \vskip1ex
  ...
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \begin{displaymath}
        y = \mathbb{X} \beta + \varepsilon
      \end{displaymath}
      Where the \emph{regression coefficients} $\beta$ now contain the intercept $\alpha$: $\beta = (\alpha, \beta1, \ldots, \beta_k)$, and the \emph{predictor matrix} $\mathbb{X}$ has $k+1$ columns and $n$ rows.
      \vskip1ex
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      If any of the predictor matrix columns are close to being \emph{collinear}, then the squared predictor matrix becomes singular, and the covariance of their regression coefficients becomes very large.
      \vskip1ex
      The matrix $\mathbb{X}_{inv} = (\mathbb{X}^T \mathbb{X})^{-1} \mathbb{X}^T$ is the generalized inverse of the \emph{predictor matrix} $\mathbb{X}$.
    \column{0.5\textwidth}
      \vspace{-1em}
...
invcpp <- HighFreq::calc_inv(retsp, eigen_max=3)
...
%%%%%%%%%%%%%%%
\subsection{draft: Principal Component Regression}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      In \emph{Principal Component Regression} (\emph{PCR}), the \emph{predictor matrix} $\mathbb{X}$ is mulltiplied by a \emph{PCA rotation matrix} $\mathbb{W}$:
      \begin{displaymath}
        \mathbb{X}_{pca} = \mathbb{X} \mathbb{W}
      \end{displaymath}
      The matrix $\mathbb{W}$ is chosen so that the columns (principal components) of the \emph{PCA predictor matrix} $\mathbb{X}_{pca}$ form an orthogonal set of predictors.
      \vskip1ex
      The \emph{PCA predictor matrix} $\mathbb{X}_{pca}$ produces the same \emph{influence matrix} $\mathbb{H}$ as the original \emph{predictor matrix} $\mathbb{X}$, so \emph{Principal Component Regression} is equivalent to the original regression model.
      \vskip1ex
      \emph{Principal Component Regression} is used to:
      \begin{itemize}
        \item Remove \emph{collinearity} between predictors, by excluding principal components with very small eignevalues,
        \item Perform \emph{dimension reduction}, by including only principal components with significantly large eignevalues,
        \item Reduce model overfitting, by reducing the number of predictors and parameters.
      \end{itemize}
    \column{0.5\textwidth}
      \vspace{-1em}
        <<echo=TRUE,eval=TRUE>>=
...
%%%%%%%%%%%%%%%
\subsection{draft: Principal Component Regression With Shrinkage}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
...
%%%%%%%%%%%%%%%
\subsection{draft: Standard Errors of Principal Component Regression}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
...
%%%%%%%%%%%%%%%
\subsection{draft: Forecasting Using Principal Component Regression}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
...
forecastv <- predict(glmod, type="response")
all.equal(glmod$fitted.values, forecastv)
