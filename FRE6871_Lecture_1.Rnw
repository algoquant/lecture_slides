...
returns <- rutils::diff_it(log(EuStockMarkets[, 1]))
...
for (indeks in 1:NROW(sigmavs)) {
  curve(expr=dlnorm(x, sdlog=sigmavs[indeks]),
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.5\paperwidth]{figure/lognormv_dist.png}
      \vspace{-2em}
      <<echo=TRUE,eval=FALSE>>=
...
\begin{block}{Required}
  \begin{itemize}[]
    \item Study all the lecture slides in \texttt{FRE6871\_Lecture\1.pdf}, and run all the code in \texttt{FRE6871\_Lecture\1.R},
    \item Study the 
    \href{https://style.tidyverse.org/}{\color{blue}{\emph{RStudio Style Guide}}}.
  \end{itemize}
\end{block}
