...
\subtitle{Lecture\#1, Spring 2020}
...
The graduate assistant (GA) will be Gaurav Shah \href{mailto:gauravshah@nyu.edu}{gauravshah@nyu.edu}.\\
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      \texttt{substitute()} looks up the object names in either named \texttt{lists} (symbol-value pairs) or in \texttt{environments}, and evaluates the objects in them.
      \vskip1ex
