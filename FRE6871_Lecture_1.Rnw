...
\subtitle{Lecture\#1, Spring 2017}
...
\begin{block}{Homeworks and Tests}
  Grading will be based on several in-class tests.  There will be no final exam.\\
  The tests will require writing code, which should run directly when pasted into an \texttt{R} session, and produce the required output, without any modifications.\\
  ...
  Students will submit their test files through \emph{NYU Classes}.\\
  ...
  Homeworks will consist of reading assignments designed to help prepare for tests.
\end{block}
\pause

\begin{block}{Graduate Assistant}
  The graduate assistant (GA) will be Alex Sun \href{alex.sun@nyu.edu}{alex.sun@nyu.edu}\\
...
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
  \texttt{R} has two popular packages for plotting: \emph{graphics} and \emph{ggplot2},
  \begin{columns}[T]
    \column{0.5\textwidth}
\begin{block}{}
...
\end{block}
    \column{0.5\textwidth}
\begin{block}{}
      Plotting functions in package \emph{ggplot2}:
      \vskip1ex
      \texttt{legend()} adds a legend to a plot,
\end{block}
  \end{columns}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \begin{displaymath}
        N(x, 0, 1) = \frac{e^{-(x-\mu)^2/2\sigma^2}}{\sigma\sqrt{2 \pi}}
      \end{displaymath}
      The Standard Normal distribution $N(0, 1)$ is a special case of the Normal $N(\mu, \sigma)$ with $\mu=0$ and $\sigma=1$,
      \vskip1ex
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \end{displaymath}
      If the \emph{seed} value $x_{0}$ is in the interval $(0, 1)$ and if $r=4$, then the sequence $x_{n}$ is also contained in the interval $(0, 1)$, 
      \vskip1ex
      The function \texttt{curve()} plots a function defined by its name,
      <<echo=TRUE,eval=FALSE>>=
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vskip1ex
      For most \emph{seed} values $x_{0}$ and $r=4$, the \emph{logistic map} produces a pseudo-random sequence, but it's not uniformly distributed, 
      \vskip1ex
      The inverse cosine function \texttt{acos()} transforms a \emph{logistic map} sequence into a uniformly distributed sequence, 
      \begin{displaymath}
        u_{n} = \arccos(1 - 2 x_{n}) / \pi
      \end{displaymath}
      \vspace{-1em}
      <<echo=TRUE,eval=FALSE>>=
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      Consider a portfolio of credit assets (bonds or loans), with default probabilities over a single time period equal to $p_{i}$, 
      \vskip1ex
      Individual defaults can be simulated by comparing the $p_{i}$ with uniform random numbers $u_{i}$, with default occurring if $u_{i} < p_{i}$, 
    \column{0.5\textwidth}
      <<echo=TRUE,eval=FALSE>>=
...
%%%%%%%%%%%%%%%
\subsection{Simulating Asset Values and Defaults}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      Defaults can also be simulated using asset values and default thresholds, with default occurring if the asset value drops below the default threshold, 
      \vskip1ex
      The asset values are mathematical variables, which can have negative values, and they are not related to actual firm asset values, 
      \vskip1ex
      Define the default thresholds to be equal to $t_{i} = \Phi^{-1}(p_{i})$, where $p_{i}$ are the default probabilities, and $\Phi()$ is the cumulative Standard Normal distribution, 
      \vskip1ex
      Default occurs if $n_{i} < t_{i}$, where $n_{i}$ are standard normal random numbers representing the asset values,
      \vspace{-1em}
      <<echo=TRUE,eval=FALSE>>=
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      Under the \emph{Vasicek} single factor model, asset values are equal to the sum of a systematic factor, plus an idiosyncratic factor: 
      \begin{displaymath}
        x_{i} = sqrt(\rho) s + sqrt(1-\rho) z_{i}
      \end{displaymath}
      Where $\rho$ is the correlation between asset values, 
      \vskip1ex
      The asset values $x_{i}$, the systematic factor $s$, and the idiosyncratic factors $z_{i}$, all follow the Standard Normal distribution $N(0, 1)$, 
      \vskip1ex
      If all default probabilities are the same $p_{i}=p$, then the default threshold is $t=N^{-1}(p)$, and the conditional default probability $p(s)$, given $s$, is equal to:
      \begin{displaymath}
        p(s) = N(\frac{t - sqrt(\rho) s}{sqrt(1-\rho)})
      \end{displaymath}
      The cumulative probability for the fraction of $x$ defaults is equal to:
      \begin{displaymath}
...
\begin{block}{Recommended}
  \begin{itemize}[]
    \item Read about the \emph{Vasicek} single factor model in \texttt{BOE Credit Risk Models.pdf}
  \end{itemize}
\end{block}
