...
The graduate assistant (GA) will be Rishikesh Mahadevan \href{mailto:rm6204@nyu.edu}{rm6204@nyu.edu}.\\
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      \texttt{R} objects can have different attributes, such as: \texttt{namev}, \texttt{dimnames}, \texttt{dimensions}, \emph{class}, etc.
      \vskip1ex
      ...
      \vskip1ex
      A \texttt{vector} that is assigned an attribute other than \texttt{namev} is not treated as a \texttt{vector}.
      \vskip1ex
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      Vector elements can be assigned \texttt{namev} using a list of symbol-value pairs.
      \vskip1ex
      The function \texttt{names()} returns the \texttt{namev} attribute of an object.
      \vskip1ex
      The \texttt{namev} attribute of a vector can be modified by assigning to the \texttt{names()} function ("\texttt{names() <-}" assignment).
      \vskip1ex
      The function \texttt{unname()} removes the \texttt{namev} attribute.
      \vskip1ex
      The function \texttt{structure()} adds attributes (specified as \texttt{symbol=value} pairs) to an object, and returns it.
    \column{0.5\textwidth}
      \vspace{-1em}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \vspace{-1em}
      <<echo=TRUE,eval=FALSE>>=
for (indeks in vecv) {ex_pressions}
@
      \vspace{-1em}
...
vecv <- sapply(seq_along(vecv), function(x) (x^2))
      @
  \end{columns}
\end{block}
...
matrixp <- lapply(1:NCOL(matv), 
  function(x) vector1[x]*matv[, x])
do.call(cbind, matrixp)
library(microbenchmark)
summary(microbenchmark(
  trans=t(vector1*t(matv)),
  lapp={
    matrixp <- lapply(1:NCOL(matv), function(x) vector1[x]*matv[, x])
    do.call(cbind, matrixp)
  },
  times=10))[, c(1, 4, 5)]  # end microbenchmark summary
      @
...
# Multiply columns of matrix by vector
matv %*% vector1  # Single column matrix
drop(matv %*% vector1)  # vector
rowSums(t(vector1 * t(matv)))
# using rowSums() and t() is 10 times slower than %*%
library(microbenchmark)
summary(microbenchmark(
  inner=drop(matv %*% vector1),
  transp=rowSums(t(vector1 * t(matv))),
  times=10))[, c(1, 4, 5)]  # end microbenchmark summary
      @
  \end{columns}
\end{block}
...
bind_dots(1, 2, 3, input=10, argv=4)  # Dots bound
