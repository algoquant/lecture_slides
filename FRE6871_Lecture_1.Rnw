...
\subtitle{Lecture\#1, Summer 2015}
...
The TA will be Jaimin Doshi (\emph{\color{blue}jbd316@nyu.edu})\\
All homework and test files \emph{should be emailed to the TA}.
\end{block}
...
Letter grades for the course will be assigned based on the sum of homework and test scores, as a percentage of the sum of maximum scores.\\
\end{block}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.4\textwidth}
      ...
      \vskip1ex
      Splitting on the \texttt{"."} character requires surrounding it with brackets: \texttt{"[.]"},
      \vskip1ex
